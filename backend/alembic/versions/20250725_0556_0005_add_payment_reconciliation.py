...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('reconciliation_status', sa.Enum('pending', 'matched', 'discrepancy', 'resolved', name='reconciliationstatus', create_type=False),
                  nullable=False),
        sa.Column('discrepancy_type', sa.Enum('amount_mismatch', 'missing_payment', 'duplicate_payment', name='discrepancytype', create_type=False), nullable=True),
        sa.Column('discrepancy_details', sa.Text(), nullable=True),
        sa.Column('reconciliation_action', sa.Enum('auto_matched', 'manual_review', 'exception_handled', name='reconciliationaction', create_type=False),
                  nullable=True),
