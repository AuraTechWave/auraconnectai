...
class MenuVersioningService:
    ...
    def _apply_version_to_live_menu(self, version: MenuVersion, audit_entries: List[MenuAuditLog], user_id: int):
        ...
        """
        # Sync categories ---------------------------------------------------
        snapshot_categories = {c.original_category_id: c for c in version.category_versions}
    ...
    def _copy_version_data(self, source_version: MenuVersion, target_version: MenuVersion, audit_entries: List[MenuAuditLog], user_id: int):
        ...
        for item_ver in source_version.item_versions:
            new_item_ver = _clone_model(item_ver, {"menu_version_id": target_version.id})
        ...
        for group_ver in source_version.modifier_versions:
            new_group_ver = _clone_model(group_ver, {"menu_version_id": target_version.id})
            ...
            for mod_ver in group_ver.modifier_versions:
                new_mod_ver = _clone_model(mod_ver, {"modifier_group_version_id": new_group_ver.id})
                self.db.add(new_mod_ver)

        # Copy menu item ↔ modifier group associations --------------------------
        for item_ver in source_version.item_versions:
            for assoc in item_ver.modifier_versions:
                new_assoc = _clone_model(assoc, {
                    "menu_item_version_id": id_map_item_versions.get(assoc.menu_item_version_id),
                    "modifier_group_version_id": id_map_group_versions.get(assoc.modifier_group_version_id),
                })
        ...
        audit_entries.append(MenuAuditLog(
            ...
            entity_name=target_version.version_name or target_version.version_number,
            change_type=ChangeType.CREATE,
    ...
    def _generate_version_comparison(self, from_version: MenuVersion, to_version: MenuVersion, request: VersionComparisonRequest) -> MenuVersionComparison:
        ...
        def _entity_sets(entity_cls):
            return {
                "from": {e.original_category_id if hasattr(e, "original_category_id") else e.original_item_id if hasattr(e, "original_item_id") else e.original_group_id for e in getattr(from_version, entity_cls)},
                "to": {e.original_category_id if hasattr(e, "original_category_id") else e.original_item_id if hasattr(e, "original_item_id") else e.original_group_id for e in getattr(to_version, entity_cls)},
            }
        ...
        i_created, i_updated, i_deleted = _calc_stats(item_sets["from"], item_sets["to"])
    ...
    def _bulk_change_categories(self, request: BulkChangeRequest, audit_entries: List[MenuAuditLog], user_id: int) -> Dict[str, Any]:
        ...
        for category in categories:
            try:
                old_values = {k: getattr(category, k) for k in request.changes.keys() if hasattr(category, k)}
    ...
    def _bulk_change_modifiers(self, request: BulkChangeRequest, audit_entries: List[MenuAuditLog], user_id: int) -> Dict[str, Any]:
        ...
        for group in groups:
            try:
                old_values = {k: getattr(group, k) for k in request.changes.keys() if hasattr(group, k)}
        ...
        remaining_ids = [id_ for id_ in request.entity_ids if id_ not in {g.id for g in groups}]
        if remaining_ids:
            ...
            for modifier in modifiers:
                try:
                    old_values = {k: getattr(modifier, k) for k in request.changes.keys() if hasattr(modifier, k)}
