...
class MenuSyncMonitor:
    ...
    def get_sync_health(self, pos_integration_id: int) -> SyncHealthMetrics:
        ...
        sync_config = (
            ...
            .filter(MenuSyncConfig.pos_integration_id == pos_integration_id)
            .first()
        )
        ...
        success_rate = (
            (len(successful_jobs) / total_jobs * 100) if total_jobs > 0 else 100
        )
        ...
        if successful_jobs:
            last_successful_sync = max(
                job.completed_at for job in successful_jobs if job.completed_at
            )

        # Calculate average sync time
        avg_sync_time = None
        completed_jobs = [
            job for job in jobs_24h if job.started_at and job.completed_at
        ]
        if completed_jobs:
            sync_times = [
    ...
    def _determine_health_status(
        ...
        if success_rate < 50:
            ...
            recommendations.append(
                "Review recent sync failures and resolve underlying issues"
            )
        ...
        if last_successful_sync:
            hours_since_success = (
                datetime.utcnow() - last_successful_sync
            ).total_seconds() / 3600
            if hours_since_success > 48:
                ...
                recommendations.append(
                    "Investigate sync failures and restore connectivity"
                )
                ...
                recommendations.append(
                    "Check sync schedule and resolve any blocking issues"
                )
        ...
        if pending_conflicts > 50:
            ...
            recommendations.append(
                "Review and resolve pending conflicts to prevent sync degradation"
            )
            ...
            recommendations.append(
                "Consider auto-resolution strategies for common conflict types"
            )
    ...
    def get_sync_performance_metrics(
        self, pos_integration_id: int, period_hours: int = 24
    ) -> Dict[str, Any]:
        ...
        # Error analysis
        failed_jobs = [
            j for j in jobs if j.status == SyncStatus.ERROR and j.error_message
        ]
        error_analysis = {}
        ...
        if failed_jobs:
            ...
            for msg in error_messages:
                ...
                if "connection" in msg.lower() or "timeout" in msg.lower():
                    ...
                    error_types["authentication"] = (
                        error_types.get("authentication", 0) + 1
                    )
                elif "rate limit" in msg.lower():
                    error_types["rate_limiting"] = (
                        error_types.get("rate_limiting", 0) + 1
                    )
                else:
            ...
            error_analysis = {
                ...
                "most_recent_error": (
                    failed_jobs[-1].error_message if failed_jobs else None
                ),
            }
    ...
    def get_sync_trends(self, pos_integration_id: int, days: int = 7) -> Dict[str, Any]:
        ...
        stats = (
            ...
            .order_by(MenuSyncStatistics.period_start)
            .all()
        )
        ...
        if stats:
            summary = {
                "period_days": days,
                "avg_success_rate": sum(s.success_rate_percentage or 0 for s in stats)
                / len(stats),
                "avg_error_rate": sum(s.error_rate_percentage or 0 for s in stats)
                / len(stats),
        ...
        return {"summary": summary, "trend_data": trend_data, "data_points": len(stats)}
    ...
    def get_conflict_analysis(self, pos_integration_id: int) -> Dict[str, Any]:
        ...
        conflicts = (
            self.db.query(MenuSyncConflict)
            .join(MenuSyncJob)
            .filter(MenuSyncJob.pos_integration_id == pos_integration_id)
            .all()
        )
        ...
        by_entity_type = {}
        for conflict in conflicts:
            by_entity_type[conflict.entity_type] = (
                by_entity_type.get(conflict.entity_type, 0) + 1
            )

        # Analyze by conflict type
        by_conflict_type = {}
        for conflict in conflicts:
            by_conflict_type[conflict.conflict_type] = (
                by_conflict_type.get(conflict.conflict_type, 0) + 1
            )
        ...
        if resolved_conflicts:
            ...
            resolution_analysis = {
                ...
                "auto_resolved": len(
                    [c for c in resolved_conflicts if c.resolved_by == 0]
                ),
            }
    ...
    def get_integration_overview(self) -> List[Dict[str, Any]]:
        ...
        for integration in integrations:
            try:
                ...
                overview.append(
                    {
                        ...
                        "last_successful_sync": (
                            health_metrics.last_successful_sync.isoformat()
                            if health_metrics.last_successful_sync
                            else None
                        ),
                ...
                logger.error(
                    f"Error getting health metrics for integration {integration.id}: {str(e)}"
                )
                overview.append(
                    {
                        "pos_integration_id": integration.id,
    ...
    def _generate_recommendations(
        self,
        health_metrics: SyncHealthMetrics,
        ...
        if (
            performance_metrics.get("performance_metrics", {}).get(
                "avg_duration_seconds", 0
            )
            > 300
        ):  # 5 minutes
            recommendations.append(
                "Sync operations are taking longer than expected - consider optimizing batch sizes"
            )
        ...
        if conflict_stats.get("data_mismatch", 0) > 10:
            recommendations.append(
                "High number of data mismatch conflicts - review field mappings and transformation rules"
            )

        if conflict_stats.get("deleted_entity", 0) > 5:
            recommendations.append(
                "Multiple entity deletion conflicts - establish clear deletion policies"
            )
