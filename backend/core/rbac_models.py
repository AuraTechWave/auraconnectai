...
user_roles = Table(
    ...
    Column('tenant_id', Integer, nullable=True),  # Role can be scoped to specific tenant
...
class RBACUser(Base):
    ...
    accessible_tenant_ids = Column(JSONB, default=list)  # List of tenant IDs user can access
    ...
    def get_effective_permissions(self, tenant_id=None):
        ...
        for user_perm in self.direct_permissions:
            if user_perm.is_active and (tenant_id is None or user_perm.tenant_id == tenant_id):
...
class RBACPermission(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    key = Column(String(100), unique=True, index=True, nullable=False)  # e.g., "payroll:read"
...
SYSTEM_PERMISSIONS = [
    ...
    ("role:manage_permissions", "Manage Role Permissions", "Assign/remove permissions from roles"),
    ...
    ("staff:manage_schedule", "Manage Staff Schedule", "Create and modify staff schedules"),
