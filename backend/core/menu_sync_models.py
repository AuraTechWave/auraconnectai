...
class POSMenuMapping(Base, TimestampMixin):
    ...
    # POS Integration
    pos_integration_id = Column(
        Integer, ForeignKey("pos_integrations.id"), nullable=False
    )
    pos_vendor = Column(
    ...
    entity_type = Column(
        String(50), nullable=False, index=True
    )  # category, item, modifier_group, modifier
    ...
    sync_direction = Column(
        SQLEnum(SyncDirection), nullable=False, default=SyncDirection.BIDIRECTIONAL
    )
    conflict_resolution = Column(
        SQLEnum(ConflictResolution), nullable=False, default=ConflictResolution.MANUAL
    )
    ...
    )  # Hash of synced data for change detection
...
class MenuSyncJob(Base, TimestampMixin):
    ...
    entity_types = Column(
        JSON, nullable=True
    )  # Specific entity types to sync, null = all
...
class MenuSyncLog(Base, TimestampMixin):
    ...
    conflict_resolved_by = Column(
        Integer, nullable=True
    )  # User ID who resolved conflict
...
class MenuSyncConflict(Base, TimestampMixin):
    ...
    )  # data_mismatch, deleted_entity, etc.
...
class MenuSyncConfig(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    pos_integration_id = Column(
        Integer, ForeignKey("pos_integrations.id"), nullable=False, unique=True
    )

    # Global sync settings
    sync_enabled = Column(Boolean, nullable=False, default=True)
    default_sync_direction = Column(
        SQLEnum(SyncDirection), nullable=False, default=SyncDirection.BIDIRECTIONAL
    )
    default_conflict_resolution = Column(
        SQLEnum(ConflictResolution), nullable=False, default=ConflictResolution.MANUAL
    )
    ...
    version_name_template = Column(
        String(200), nullable=True
    )  # Template for auto-generated version names
    ...
    rate_limit_requests = Column(
        Integer, nullable=True
    )  # Max requests per minute to POS
...
class MenuSyncStatistics(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index(
            "ix_menu_sync_stats_period",
            "pos_integration_id",
            "period_type",
            "period_end",
        ),
        Index(
            "ix_menu_sync_stats_performance",
