...
class InventoryAlert(Base, TimestampMixin):
    ...
    )  # low_stock, expired, damaged, etc.
    priority = Column(
        SQLEnum(AlertPriority), nullable=False, default=AlertPriority.MEDIUM
    )
...
class InventoryCountItem(Base, TimestampMixin):
    ...
    inventory_count_id = Column(
        Integer, ForeignKey("inventory_counts.id"), nullable=False
    )
    inventory_id = Column(Integer, ForeignKey("inventory.id"), nullable=False)
    ...
    adjustment_id = Column(
        Integer, ForeignKey("inventory_adjustments.id"), nullable=True
    )
    notes = Column(Text, nullable=True)
...
class Inventory(Base, TimestampMixin):
    ...
    alert_threshold_percentage = Column(
        Float, nullable=True
    )  # Alternative to fixed threshold
