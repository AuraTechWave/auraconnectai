...
class MenuVersioningTriggers:
    ...
    def should_create_auto_version(
        self, change_type: str, entity_type: str, changes: Dict[str, Any]
    ) -> bool:
    ...
    def create_auto_version(
        self, db: Session, trigger_reason: str, user_id: int = 1
    ) -> Optional[int]:
    ...
    def _summarize_changes(self) -> str:
        ...
        if change_types:
            changes_str = ", ".join(
                [f"{count} {type}" for type, count in change_types.items()]
            )
            summary_parts.append(f"Changes: {changes_str}")

        if entity_types:
            entities_str = ", ".join(
                [f"{count} {type}" for type, count in entity_types.items()]
            )
    ...
    def _on_category_update(self, mapper, connection, target):
        ...
        has_critical_change = any(
            field in state.get("_sa_instance_state", {}).get("committed_state", {})
    ...
    def _on_modifier_update(self, mapper, connection, target):
        ...
        change_type = "modifier_change"
        if (
            hasattr(target, "_price_adjustment_changed")
            and target._price_adjustment_changed
        ):
            change_type = "price_change"
...
def create_manual_version_on_bulk_change(
    db: Session, operation: str, affected_count: int, user_id: int = 1
):
    """
    ...
    if menu_versioning_triggers.should_create_auto_version(
        "bulk_operation", "multiple", change_data
    ):
        version_id = menu_versioning_triggers.create_auto_version(
            db, f"Bulk {operation} operation affecting {affected_count} items", user_id
        )
        return version_id
...
def get_change_buffer_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "recent_changes": (
            menu_versioning_triggers.change_buffer[-5:]
            if menu_versioning_triggers.change_buffer
            else []
        ),
    }
