...
class EnhancedRBACService:
    ...
    def _perform_permission_check(
        ...
        if resource_id and self._has_resource_permission(
            user, permission_str, resource_id, tenant_id
        ):
            return True
    ...
    def _has_direct_permission(
        self, user: RBACUser, permission: str, tenant_id: int
    ) -> bool:
    ...
    def _has_resource_permission(
        self, user: RBACUser, permission: str, resource_id: int, tenant_id: int
    ) -> bool:
        """Check resource-specific permissions"""
    ...
    def _has_hierarchical_permission(
        self, user: RBACUser, resource: str, action: str, tenant_id: int
    ) -> bool:
        ...
        for higher_action, implied_actions in hierarchy.items():
            if action in implied_actions:
                ...
                if self._has_direct_permission(
                    user, higher_permission, tenant_id
                ) or self._has_role_permission(user, higher_permission, tenant_id):
                    return True
    ...
    def clear_user_cache(self, user_id: int):
        ...
        keys_to_remove = [
            key for key in self.permission_cache.keys() if key.startswith(f"{user_id}:")
        ]
        for key in keys_to_remove:
            del self.permission_cache[key]

    def get_user_permissions_summary(
        self, user: RBACUser, tenant_id: int
    ) -> Dict[str, Any]:
...
def require_permission(
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            tenant_id = None
            if tenant_id_param:
                tenant_id = kwargs.get(tenant_id_param) or getattr(
                    args[0] if args else None, tenant_id_param, None
                )

            resource_id = None
            if resource_id_param:
                resource_id = kwargs.get(resource_id_param) or getattr(
                    args[0] if args else None, resource_id_param, None
                )
            ...
            if not has_permission:
                ...
                raise HTTPException(
                    status_code=403, detail=error_message or default_message
                )
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not any(role.name == role_name for role in user_roles):
                raise HTTPException(
                    status_code=403, detail=f"Required role '{role_name}' not found"
                )
