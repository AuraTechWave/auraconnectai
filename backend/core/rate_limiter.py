...
class RateLimiter:
    ...
    def check_rate_limit(
        ...
        if limit == 0:  # No limit (admin bypass)
            return True, {"limit": "unlimited", "remaining": "unlimited", "reset": None}
    ...
    def _check_burst_allowance(
        self, identifier: str, endpoint: str, base_limit: int
    ) -> bool:
    ...
    def _block_identifier(self, identifier: str):
        ...
        try:
            ...
            logger.warning(
                f"Blocked identifier {identifier} for {self.config.VIOLATION_PENALTY_MINUTES} minutes"
            )
        except redis.RedisError as e:
    ...
    def _get_endpoint_limit(self, path: str, identifier_type: str) -> Dict[str, Any]:
        ...
        if config:
            if identifier_type == "admin":
                ...
                limit = config.get(
                    "authenticated", self.config.DEFAULT_AUTHENTICATED_LIMIT
                )
            else:
                limit = config.get("anonymous", self.config.DEFAULT_ANONYMOUS_LIMIT)

            return {"limit": limit, "window": config.get("window", 60)}
...
class RateLimitMonitor:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            today_key = (
                f"rate_limit:monitor:violations:{datetime.utcnow().strftime('%Y%m%d')}"
            )
            stats["total_violations_today"] = self.redis_client.llen(today_key)

            # Get blocked identifiers
            for key in self.redis_client.scan_iter(
                match="rate_limit:blocked:*", count=100
            ):
                identifier = key.split(":")[-1]
                ttl = self.redis_client.ttl(key)
                stats["blocked_identifiers"].append(
                    {"identifier": identifier, "expires_in": ttl}
                )
