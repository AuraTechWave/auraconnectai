...
from .rbac_models import (
    ...
    RBACSession, user_roles, role_permissions
)
...
class RBACService:
    ...
    def create_user(
        ...
        existing = self.db.query(RBACUser).filter(
    ...
    def create_permission(
        ...
        existing = self.db.query(RBACPermission).filter(RBACPermission.key == key).first()
        if existing:
        ...
        if not resource or not action:
            ...
            if len(parts) == 2:
                ...
                raise ValueError("Invalid permission key format. Expected 'resource:action'")
    ...
    def get_user_roles(
        ...
        query = self.db.query(RBACRole).join(user_roles).filter(
            and_(
                user_roles.c.user_id == user_id,
    ...
    def get_rbac_session(self, session_id: str) -> Optional[RBACSession]:
        ...
        return self.db.query(RBACSession).filter(
            and_(
    ...
    def refresh_session_cache(self, session_id: str) -> bool:
        ...
        if session.active_tenant_id is not None:
            permissions = self.get_user_permissions(session.user_id, session.active_tenant_id)
    ...
    def setup_default_roles_and_permissions(self) -> None:
        ...
        role_permissions_map = {
            'super_admin': [
                ...
                'role:read', 'role:write', 'role:delete', 'role:manage_permissions',
                'permission:read', 'permission:write',
                'staff:read', 'staff:write', 'staff:delete', 'staff:manage_schedule',
                'payroll:read', 'payroll:write', 'payroll:approve', 'payroll:export',
                'order:read', 'order:write', 'order:delete', 'order:manage_kitchen',
                'system:read', 'system:write', 'system:audit', 'system:backup'
            ],
            'admin': [
                'user:read', 'user:write', 'user:manage_roles',
                'role:read', 'staff:read', 'staff:write', 'staff:manage_schedule',
                'payroll:read', 'payroll:write', 'payroll:approve', 'payroll:export',
                'order:read', 'order:write', 'order:delete', 'order:manage_kitchen',
                'system:read'
            ],
            'payroll_manager': [
                'staff:read', 'payroll:read', 'payroll:write', 'payroll:approve', 'payroll:export'
            ],
            'payroll_clerk': [
                'staff:read', 'payroll:read', 'payroll:write'
            ],
            'staff_manager': [
                'staff:read', 'staff:write', 'staff:manage_schedule', 'order:read'
            ],
            'kitchen_manager': [
                'order:read', 'order:write', 'order:manage_kitchen', 'staff:read'
            ],
            'manager': [
                'staff:read', 'order:read', 'order:write', 'payroll:read'
            ],
            'server': [
                'order:read', 'order:write'
            ],
            'viewer': [
                'staff:read', 'order:read', 'payroll:read'
            ]
        }
            ...
            for permission_key in permission_keys:
                ...
                if permission:
                    try:
                        ...
                        logger.warning(f"Failed to assign {permission_key} to {role_name}: {e}")
