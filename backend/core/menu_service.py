...
class MenuService:
    ...
    def create_category(
        self, category_data: MenuCategoryCreate, user_id: int = None
    ) -> MenuCategory:
        ...
        category = MenuCategory(**category_data.dict(), created_by=user_id)
    ...
    def get_category_by_id(self, category_id: int) -> Optional[MenuCategory]:
        ...
        return (
            ...
            .filter(MenuCategory.id == category_id, MenuCategory.deleted_at.is_(None))
            .first()
        )

    def update_category(
        self, category_id: int, category_data: MenuCategoryUpdate, user_id: int = None
    ) -> MenuCategory:
        ...
        if (
            ...
            and category_data.parent_category_id != category_id
        ):
            parent = self.get_category_by_id(category_data.parent_category_id)
            if not parent:
                raise HTTPException(
    ...
    def delete_category(self, category_id: int, user_id: int = None) -> bool:
        ...
        items_count = (
            ...
            .filter(MenuItem.category_id == category_id, MenuItem.deleted_at.is_(None))
            .count()
        )
    ...
    def create_menu_item(
        self, item_data: MenuItemCreate, user_id: int = None
    ) -> MenuItem:
        ...
        if item_data.sku:
            existing_item = (
                self.db.query(MenuItem)
                .filter(MenuItem.sku == item_data.sku, MenuItem.deleted_at.is_(None))
                .first()
            )
            if existing_item:
                raise HTTPException(
        ...
        menu_item = MenuItem(**item_data.dict(), created_by=user_id)
    ...
    def get_menu_item_by_id(self, item_id: int) -> Optional[MenuItem]:
        ...
        return (
            self.db.query(MenuItem)
            .filter(MenuItem.id == item_id, MenuItem.deleted_at.is_(None))
            .first()
        )

    def update_menu_item(
        self, item_id: int, item_data: MenuItemUpdate, user_id: int = None
    ) -> MenuItem:
        """Update a menu item"""
    ...
    def create_modifier_group(
        self, group_data: ModifierGroupCreate, user_id: int = None
    ) -> ModifierGroup:
        """Create a new modifier group"""
        modifier_group = ModifierGroup(**group_data.dict(), created_by=user_id)
    ...
    def get_modifier_group_by_id(self, group_id: int) -> Optional[ModifierGroup]:
        ...
        return (
            ...
            .filter(ModifierGroup.id == group_id, ModifierGroup.deleted_at.is_(None))
            .first()
        )

    def update_modifier_group(
        self, group_id: int, group_data: ModifierGroupUpdate, user_id: int = None
    ) -> ModifierGroup:
    ...
    def delete_modifier_group(self, group_id: int, user_id: int = None) -> bool:
        ...
        modifiers_count = (
            ...
            .filter(
                Modifier.modifier_group_id == group_id, Modifier.deleted_at.is_(None)
            )
            .count()
        )
    ...
    # Modifier operations
    def create_modifier(
        self, modifier_data: ModifierCreate, user_id: int = None
    ) -> Modifier:
        ...
        modifier = Modifier(**modifier_data.dict(), created_by=user_id)
    ...
    def get_modifiers_by_group(
        self, group_id: int, active_only: bool = True
    ) -> List[Modifier]:
    ...
    def get_modifier_by_id(self, modifier_id: int) -> Optional[Modifier]:
        ...
        return (
            self.db.query(Modifier)
            .filter(Modifier.id == modifier_id, Modifier.deleted_at.is_(None))
            .first()
        )

    def update_modifier(
        self, modifier_id: int, modifier_data: ModifierUpdate, user_id: int = None
    ) -> Modifier:
        """Update a modifier"""
    ...
    def add_modifier_to_item(
        self, link_data: MenuItemModifierCreate, user_id: int = None
    ) -> MenuItemModifier:
        ...
        existing_link = (
            self.db.query(MenuItemModifier)
            .filter(
        ...
        menu_item_modifier = MenuItemModifier(**link_data.dict(), created_by=user_id)
    ...
    def get_item_modifiers(self, menu_item_id: int) -> List[MenuItemModifier]:
        ...
        return (
            ...
            .order_by(MenuItemModifier.display_order)
            .all()
        )

    def remove_modifier_from_item(
        self, menu_item_id: int, modifier_group_id: int, user_id: int = None
    ) -> bool:
    ...
    def add_inventory_to_item(
        self, link_data: MenuItemInventoryCreate, user_id: int = None
    ) -> MenuItemInventory:
        ...
        menu_item_inventory = MenuItemInventory(**link_data.dict(), created_by=user_id)
    ...
    def get_inventory(
        self, params: InventorySearchParams
    ...
    def bulk_update_items(
        self, item_ids: List[int], updates: MenuItemUpdate, user_id: int = None
    ) -> List[MenuItem]:
        """Bulk update menu items"""
        items = (
            self.db.query(MenuItem)
            .filter(MenuItem.id.in_(item_ids), MenuItem.deleted_at.is_(None))
            .all()
        )
    ...
    def get_menu_stats(self) -> Dict[str, Any]:
        ...
        total_categories = (
            self.db.query(MenuCategory)
            .filter(MenuCategory.deleted_at.is_(None), MenuCategory.is_active == True)
            .count()
        )

        total_items = (
            self.db.query(MenuItem)
            .filter(MenuItem.deleted_at.is_(None), MenuItem.is_active == True)
            .count()
        )
        ...
        total_modifiers = (
            self.db.query(Modifier)
            .filter(Modifier.deleted_at.is_(None), Modifier.is_active == True)
            .count()
        )
    ...
    def bulk_update_items(
        self, item_ids: List[int], updates: Dict[str, Any], user_id: int = None
    ) -> Dict[str, Any]:
        ...
        for item in items:
            try:
                ...
                if (
                    ...
                    and updates["is_available"] != item.is_available
                ):
        ...
        if updated_count > 0:
            ...
            version_id = create_manual_version_on_bulk_change(
                self.db, "bulk_update_items", updated_count, user_id or 1
            )
    ...
    def bulk_activate_items(
        self, item_ids: List[int], active: bool, user_id: int = None
    ) -> Dict[str, Any]:
    ...
    def bulk_update_prices(
        self, price_updates: List[Dict[str, Any]], user_id: int = None
    ) -> Dict[str, Any]:
        ...
        for update in price_updates:
            try:
                ...
                errors.append(
                    f"Error updating item {update.get('item_id', 'unknown')}: {str(e)}"
                )
        ...
        if updated_count > 0:
            ...
            version_id = create_manual_version_on_bulk_change(
                self.db, "bulk_price_update", updated_count, user_id or 1
            )
    ...
    def bulk_delete_items(
        self, item_ids: List[int], user_id: int = None
    ) -> Dict[str, Any]:
        ...
        version_id = create_manual_version_on_bulk_change(
            self.db, "bulk_delete_items", deleted_count, user_id or 1
        )
