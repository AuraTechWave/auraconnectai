        ...
        def wrapper(*args, **kwargs):
            ...
            if key_func:
                ...
                cache_key = cache_manager.generate_key(
                    cache_type, base_key, tenant_id=tenant_id, user_id=user_id
                )
...
def cache_menu(ttl: Optional[int] = None):
    ...
    return cache(cache_type="menu", ttl=ttl or CacheTTL.MENU_ITEMS, tenant_aware=True)


def cache_permissions(ttl: Optional[int] = None):
    """Cache decorator specifically for user permissions"""
    return cache(
        cache_type="permissions", ttl=ttl or CacheTTL.USER_PERMISSIONS, user_aware=True
    )
...
def cache_api_response(ttl: int = 60):
    ...
    return cache(cache_type="api", ttl=ttl, tenant_aware=True)


def cache_search_results(ttl: int = 180):
    """Cache decorator for search results"""
    return cache(cache_type="search", ttl=ttl, tenant_aware=True)


def cache_report(ttl: int = 900):
    """Cache decorator for report data"""
    return cache(cache_type="report", ttl=ttl, tenant_aware=True)
...
def cache_through(cache_type: str = "api", ttl: Optional[Union[int, CacheTTL]] = None):
    """
...
def invalidate_cache(cache_types: Union[str, List[str]], pattern: Optional[str] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            # Invalidate cache(s)
            types_to_invalidate = (
                cache_types if isinstance(cache_types, list) else [cache_types]
            )
            ...
            for cache_type in types_to_invalidate:
                if pattern:
                    ...
                    if tenant_id is not None:
                        ...
                        logger.warning(
                            f"No pattern specified for cache invalidation of {cache_type}"
                        )
...
def _refresh_cache(
    func: Callable,
    cache_type: str,
    ttl: Optional[Union[int, CacheTTL]],
    *args,
    **kwargs,
):
