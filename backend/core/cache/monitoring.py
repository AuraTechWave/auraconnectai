...
class CacheMonitor:
    ...
    def get_metrics(
        ...
        with self.metric_lock:
            ...
            return {
                ...
                'time_window': f"Last {last_minutes} minutes" if last_minutes else "All time",
                'metrics': {
    ...
    def get_top_keys(self, limit: int = 10, cache_type: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        return [
            {
                ...
                'access_count': count,
                'percentage': round((count / sum(key_counts.values()) * 100) if key_counts else 0, 2)
            }
            for key, count in top_keys
        ]
    
    def get_slow_operations(self, threshold_ms: float = 100, limit: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        if metrics['error_rate_percent'] > self.thresholds['error_rate_max']:
            ...
            health['recommendations'].append(
                "Check Redis connection and server health"
            )
