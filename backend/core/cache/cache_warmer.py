...
class CacheWarmer:
        ...
        try:
            ...
            if tenant_id:
                menu_items = (
                    ...
                    .filter_by(restaurant_id=tenant_id, is_active=True)
                    .all()
                )
                categories = (
                ...
                menu_items = (
                    self.db.query(MenuItem).filter_by(is_active=True).limit(1000).all()
                )
                categories = (
                    self.db.query(MenuCategory)
            ...
            for item in menu_items:
                cache_key = cache_manager.generate_key(
                    "menu", "item", item.id, tenant_id=item.restaurant_id
                )
                ...
                if cache_manager.set(
                    "menu", cache_key, item_data, CacheTTL.MENU_ITEMS.value
                ):
            ...
            for category in categories:
                cache_key = cache_manager.generate_key(
                    "menu", "category", category.id, tenant_id=category.restaurant_id
                )
                ...
                if cache_manager.set(
                    "menu", cache_key, category_data, CacheTTL.MENU_ITEMS.value
                ):
    ...
    def warm_permissions_cache(
        ...
        try:
            ...
            for user in users:
                ...
                cache_key = cache_manager.generate_key(
                    "permissions", "user", user_id=user.id
                )
                ...
                if cache_manager.set(
                    ...
                    cache_key,
                    permission_data,
                    CacheTTL.USER_PERMISSIONS.value,
                ):
    ...
    def warm_settings_cache(self, tenant_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if tenant_id:
                ...
                restaurants = (
                    self.db.query(Restaurant).filter_by(is_active=True).limit(100).all()
                )
            ...
            for restaurant in restaurants:
                ...
                if settings:
                    cache_key = cache_manager.generate_key(
                        "settings", "restaurant", tenant_id=restaurant.id
                    )
                    ...
                    settings_data = {
                        ...
                        "tax_rate": (
                            float(settings.tax_rate) if settings.tax_rate else 0
                        ),
                        "service_charge": (
                            float(settings.service_charge)
                    ...
                    if cache_manager.set(
                        ...
                        cache_key,
                        settings_data,
                        CacheTTL.RESTAURANT_SETTINGS.value,
                    ):
        ...
        try:
            ...
            for i in range(days):
                ...
                for metric in metrics:
                    cache_key = cache_manager.generate_key(
                        "analytics", metric, date.isoformat(), tenant_id=tenant_id
                    )
    ...
    def _calculate_metric(
        self, tenant_id: int, date: datetime.date, metric: str
    ...
    def warm_all(
        self, tenant_id: Optional[int] = None, parallel: bool = True
    ) -> Dict[str, Any]:
        """
        Warm all caches
