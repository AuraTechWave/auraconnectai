...
class StandardResponse(BaseModel, Generic[T]):
    ...
    def success(
        ...
        return cls(
            success=True, data=data, meta=response_meta, errors=[], message=message
        )
        ...
        for field, messages in errors.items():
            for message in messages:
                error_details.append(
                    ErrorDetail(code="VALIDATION_ERROR", message=message, field=field)
                )

        return cls(
            success=False, data=None, errors=error_details, message="Validation failed"
        )
...
class NotFoundResponse(StandardResponse[None]):
    ...
    def create(cls, resource: str, identifier: Any) -> "NotFoundResponse":
        ...
        return cls.error(
            message=f"{resource} with ID {identifier} not found", code="NOT_FOUND"
        )
...
class UnauthorizedResponse(StandardResponse[None]):
    ...
    def create(cls, message: str = "Unauthorized access") -> "UnauthorizedResponse":
        ...
        return cls.error(message=message, code="UNAUTHORIZED")
...
class ForbiddenResponse(StandardResponse[None]):
    ...
    def create(cls, message: str = "Access forbidden") -> "ForbiddenResponse":
        ...
        return cls.error(message=message, code="FORBIDDEN")
