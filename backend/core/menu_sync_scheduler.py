...
class MenuSyncScheduler:
    ...
    async def _schedule_auto_sync(self, config: MenuSyncConfig):
        ...
        if trigger:
            ...
            logger.info(
                f"Scheduled auto sync for POS integration {config.pos_integration_id}"
            )

    def _create_time_window_trigger(
        self, time_windows: List[Dict[str, str]]
    ...
    async def _execute_scheduled_sync(self, pos_integration_id: int):
        ...
        if pos_integration_id in self._running_jobs:
            logger.info(
                f"Sync already running for POS integration {pos_integration_id}, skipping"
            )
            return
        ...
        try:
            with next(get_db()) as db:
                ...
                if not config or not config.sync_enabled:
                    logger.warning(
                        f"Sync disabled or config not found for POS integration {pos_integration_id}"
                    )
                    return
                ...
                if not self._is_within_time_window(config.sync_time_windows):
                    logger.info(
                        f"Current time is outside sync windows for POS integration {pos_integration_id}"
                    )
                    return

                # Check for existing active jobs
                active_jobs = (
                    db.query(MenuSyncJob)
                    .filter(
                ...
                if active_jobs >= config.max_concurrent_jobs:
                    logger.info(
                        f"Max concurrent jobs reached for POS integration {pos_integration_id}"
                    )
                    return
                ...
                logger.info(
                    f"Started scheduled sync job {sync_job.job_id} for POS integration {pos_integration_id}"
                )

        except Exception as e:
            logger.error(
                f"Error executing scheduled sync for POS integration {pos_integration_id}: {str(e)}"
            )
    ...
    def _is_within_time_window(
        self, time_windows: Optional[List[Dict[str, str]]]
    ) -> bool:
    ...
    async def schedule_one_time_sync(
        self,
    ...
    async def _execute_one_time_sync(
        self,
        pos_integration_id: int,
    ...
    async def _cleanup_old_sync_logs(self):
        ...
        try:
            with next(get_db()) as db:
                ...
                deleted_logs = (
                    db.query(MenuSyncLog)
                    .filter(MenuSyncLog.created_at < cutoff_date)
                    .delete()
                )
                ...
                logger.info(
                    f"Cleaned up {deleted_logs} old sync logs and {deleted_jobs} old jobs"
                )
    ...
    async def _monitor_sync_health(self):
        ...
        try:
            with next(get_db()) as db:
                ...
                if failed_jobs > 5:
                    logger.warning(
                        f"High number of failed sync jobs in last hour: {failed_jobs}"
                    )
                ...
                if unresolved_conflicts > 10:
                    logger.warning(
                        f"High number of unresolved conflicts: {unresolved_conflicts}"
                    )
