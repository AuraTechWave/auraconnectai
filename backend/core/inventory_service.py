...
from .inventory_models import (
    Inventory, InventoryAlert, InventoryAdjustment, Vendor, PurchaseOrder,
    PurchaseOrderItem, InventoryUsageLog, InventoryCount, InventoryCountItem,
    AlertStatus, AlertPriority, AdjustmentType, VendorStatus
)
...
class InventoryService:
    ...
    def get_inventory_items(self, 
                           ...
                           limit: int = 50,
                           offset: int = 0) -> Tuple[List[Inventory], int]:
    ...
    def create_inventory_item(self, item_data: Dict[str, Any], user_id: int) -> Inventory:
    ...
    def update_inventory_item(self, inventory_id: int, update_data: Dict[str, Any], user_id: int) -> Inventory:
    ...
    def adjust_inventory_quantity(self,
                                 inventory_id: int,
    ...
    # Alert Management
    def check_and_create_alerts(self, inventory_item: Inventory) -> List[InventoryAlert]:
        ...
        if inventory_item.enable_low_stock_alerts and inventory_item.is_low_stock:
            existing_alert = self.db.query(InventoryAlert).filter(
                ...
                InventoryAlert.status.in_([AlertStatus.PENDING, AlertStatus.ACKNOWLEDGED])
            ).first()
            ...
            if not existing_alert:
                alert = self.create_alert(
                    ...
                    priority=AlertPriority.HIGH if inventory_item.quantity == 0 else AlertPriority.MEDIUM,
    ...
    def create_alert(self,
                    ...
                    expires_at: Optional[datetime] = None) -> InventoryAlert:
    ...
    def get_active_alerts(self, priority: Optional[AlertPriority] = None) -> List[InventoryAlert]:
        """Get active inventory alerts"""
        query = self.db.query(InventoryAlert).options(
            joinedload(InventoryAlert.inventory_item)
        ).filter(
    ...
    def acknowledge_alert(self, alert_id: int, user_id: int, notes: Optional[str] = None) -> InventoryAlert:
    ...
    def resolve_alert(self, alert_id: int, user_id: int, notes: Optional[str] = None) -> InventoryAlert:
        """Resolve an alert"""
        alert = self.db.query(InventoryAlert).filter(InventoryAlert.id == alert_id).first()
        if not alert:
            raise HTTPException(
    ...
    def record_usage(self,
                    ...
                    user_id: Optional[int] = None) -> InventoryUsageLog:
        ...
        usage_log = InventoryUsageLog(
            ...
            total_cost=quantity_used * inventory_item.cost_per_unit if inventory_item.cost_per_unit else None,
            location=location,
            station=station,
            created_by=user_id
        )
    ...
    def get_usage_analytics(self, 
        ...
        average_daily_usage = total_usage / max(1, (end_date - start_date).days) if start_date and end_date else 0
    ...
    # Purchase Orders
    def create_purchase_order(self, po_data: Dict[str, Any], user_id: int) -> PurchaseOrder:
        """Create new purchase order"""
        # Generate PO number if not provided
        if 'po_number' not in po_data:
            po_data['po_number'] = f"PO-{datetime.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
    ...
    def add_item_to_purchase_order(self, po_id: int, item_data: Dict[str, Any]) -> PurchaseOrderItem:
    ...
    def update_purchase_order_totals(self, po_id: int):
        ...
        total = self.db.query(func.sum(PurchaseOrderItem.total_cost)).filter(
    ...
    # Inventory Counting
    def create_inventory_count(self, count_data: Dict[str, Any], user_id: int) -> InventoryCount:
        """Create new inventory count"""
        if 'count_number' not in count_data:
            count_data['count_number'] = f"COUNT-{datetime.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
    ...
    def add_count_item(self, count_id: int, item_data: Dict[str, Any], user_id: int) -> InventoryCountItem:
    ...
    def get_inventory_dashboard_stats(self) -> Dict[str, Any]:
        ...
        total_items = self.db.query(Inventory).filter(
            Inventory.is_active == True,
            Inventory.deleted_at.is_(None)
        ).count()
        ...
        pending_alerts = self.db.query(InventoryAlert).filter(
            InventoryAlert.status == AlertStatus.PENDING,
            InventoryAlert.is_active == True
        ).count()
        
        total_value = self.db.query(
            func.sum(Inventory.quantity * Inventory.cost_per_unit)
        ).filter(
            Inventory.is_active == True,
            Inventory.deleted_at.is_(None),
        ...
        return {
            ...
            "stock_percentage": ((total_items - low_stock_items) / max(1, total_items)) * 100
        }