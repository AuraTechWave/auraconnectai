...
from .menu_sync_models import (
    MenuSyncConflict, ConflictResolution, SyncDirection, SyncStatus
)
...
class MenuSyncConflictResolver:
    ...
    def get_pending_conflicts(self, pos_integration_id: Optional[int] = None) -> List[MenuSyncConflict]:
        ...
        return query.order_by(
            MenuSyncConflict.priority.desc(),
            MenuSyncConflict.created_at.asc()
        ).all()
    
    def get_conflict_summary(self, pos_integration_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        by_entity_type = {}
        entity_type_results = base_query.filter(
            MenuSyncConflict.status == "unresolved"
        ).with_entities(
            MenuSyncConflict.entity_type,
            func.count(MenuSyncConflict.id)
        ).group_by(MenuSyncConflict.entity_type).all()
        ...
        # Group by severity
        by_severity = {}
        severity_results = base_query.filter(
            MenuSyncConflict.status == "unresolved"
        ).with_entities(
            MenuSyncConflict.severity,
            func.count(MenuSyncConflict.id)
        ).group_by(MenuSyncConflict.severity).all()
        ...
        by_conflict_type = {}
        conflict_type_results = base_query.filter(
            MenuSyncConflict.status == "unresolved"
        ).with_entities(
            MenuSyncConflict.conflict_type,
            func.count(MenuSyncConflict.id)
        ).group_by(MenuSyncConflict.conflict_type).all()
        
        for conflict_type, count in conflict_type_results:
            by_conflict_type[conflict_type] = count
        
        # Get oldest conflict
        oldest_conflict = base_query.filter(
            MenuSyncConflict.status == "unresolved"
        ).order_by(MenuSyncConflict.created_at.asc()).first()
    ...
    def resolve_conflict(self, conflict_id: int, resolution: MenuSyncConflictResolve, 
                        user_id: Optional[int] = None) -> bool:
        """Resolve a specific conflict using the provided resolution strategy"""
        
        conflict = self.db.query(MenuSyncConflict).filter(
            MenuSyncConflict.id == conflict_id
        ).first()
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"Conflict {conflict_id} resolved using {resolution.resolution_strategy}")
                return True
            else:
    ...
    def auto_resolve_conflicts(self, pos_integration_id: Optional[int] = None, 
                              max_conflicts: int = 100) -> Dict[str, int]:
        ...
        for conflict in conflicts:
            try:
                ...
                resolution = MenuSyncConflictResolve(
                    resolution_strategy=conflict.resolution_strategy or ConflictResolution.LATEST_WINS,
    ...
    def ignore_conflict(self, conflict_id: int, user_id: Optional[int] = None, 
    ...
    def reopen_conflict(self, conflict_id: int, user_id: Optional[int] = None, 
                       reason: Optional[str] = None) -> bool:
        ...
        conflict.resolution_notes = f"Reopened by user: {reason}" if reason else "Reopened by user"
        
        self.db.commit()
        return True
    
    def _resolve_aura_wins(self, conflict: MenuSyncConflict, resolution: MenuSyncConflictResolve, 
    ...
    def _resolve_pos_wins(self, conflict: MenuSyncConflict, resolution: MenuSyncConflictResolve, 
                         user_id: Optional[int]) -> bool:
    ...
    def _resolve_latest_wins(self, conflict: MenuSyncConflict, resolution: MenuSyncConflictResolve, 
                            user_id: Optional[int]) -> bool:
    ...
    def _handle_manual_resolution(self, conflict: MenuSyncConflict, resolution: MenuSyncConflictResolve, 
                                 user_id: Optional[int]) -> bool:
    ...
    def _apply_data_to_pos(self, conflict: MenuSyncConflict, data: Dict[str, Any]) -> bool:
    ...
    def _apply_data_to_aura(self, conflict: MenuSyncConflict, data: Dict[str, Any]) -> bool:
    ...
    def _update_category_from_pos_data(self, category: MenuCategory, pos_data: Dict[str, Any]):
    ...
    def _update_modifier_group_from_pos_data(self, modifier_group: ModifierGroup, pos_data: Dict[str, Any]):
    ...
    def _update_modifier_from_pos_data(self, modifier: Modifier, pos_data: Dict[str, Any]):
    ...
    def get_conflict_recommendations(self, conflict: MenuSyncConflict) -> List[str]:
        ...
        if conflict.severity == "high":
            recommendations.append("High priority conflict - immediate attention required")
        
        if "price" in conflict.conflicting_fields:
            recommendations.append("Price conflict detected - verify with management before resolving")
        ...
        if conflict.conflict_type == "deleted_entity":
            recommendations.append("Entity deletion conflict - confirm if item should be removed")
        ...
        if aura_timestamp and pos_timestamp:
            if aura_timestamp > pos_timestamp:
                recommendations.append("AuraConnect data is more recent - consider AURA_WINS strategy")
            else:
                recommendations.append("POS data is more recent - consider POS_WINS strategy")
