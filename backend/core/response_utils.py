...
class PaginationParams:
    ...
    def create_pagination_meta(self, total: int) -> PaginationMeta:
        ...
        total_pages = (
            (total + self.per_page - 1) // self.per_page if self.per_page > 0 else 0
        )
...
def create_error_response(
    ...
    response = StandardResponse.error(
        message=message, code=code, errors=errors, meta=meta
    )
...
) -> Optional[ValidationErrorResponse]:
    """
...
def format_validation_errors(errors: Dict[str, Any]) -> List[ErrorDetail]:
    ...
    for field, messages in errors.items():
        if isinstance(messages, list):
            ...
            error_details.append(
                ErrorDetail(code="VALIDATION_ERROR", message=str(messages), field=field)
            )
