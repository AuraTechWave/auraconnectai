...
class MenuVersion(Base, TimestampMixin):
    ...
    )  # e.g., "v1.0.0", "2024-01-15-001"
    version_name = Column(String(200), nullable=True)  # Human-readable name
    description = Column(Text, nullable=True)
    version_type = Column(
        SQLEnum(VersionType), nullable=False, default=VersionType.MANUAL
    )
    ...
    category_versions = relationship(
        "MenuCategoryVersion", back_populates="menu_version"
    )
    item_versions = relationship("MenuItemVersion", back_populates="menu_version")
    modifier_versions = relationship(
        "ModifierGroupVersion", back_populates="menu_version"
    )
...
class MenuCategoryVersion(Base, TimestampMixin):
    ...
    original_category_id = Column(
        Integer, ForeignKey("menu_categories.id"), nullable=False
    )
...
class MenuItemVersion(Base, TimestampMixin):
    ...
    modifier_versions = relationship(
        "MenuItemModifierVersion", back_populates="menu_item_version"
    )
...
class ModifierGroupVersion(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    menu_version_id = Column(Integer, ForeignKey("menu_versions.id"), nullable=False)
    original_group_id = Column(
        Integer, ForeignKey("modifier_groups.id"), nullable=False
    )
    ...
    modifier_versions = relationship(
        "ModifierVersion", back_populates="modifier_group_version"
    )
...
class ModifierVersion(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    modifier_group_version_id = Column(
        Integer, ForeignKey("modifier_group_versions.id"), nullable=False
    )
    ...
    # Relationships
    modifier_group_version = relationship(
        "ModifierGroupVersion", back_populates="modifier_versions"
    )
...
class MenuItemModifierVersion(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    menu_item_version_id = Column(
        Integer, ForeignKey("menu_item_versions.id"), nullable=False
    )
    modifier_group_version_id = Column(
        Integer, ForeignKey("modifier_group_versions.id"), nullable=False
    )
    original_association_id = Column(
        Integer, ForeignKey("menu_item_modifiers.id"), nullable=True
    )
    ...
    # Relationships
    menu_item_version = relationship(
        "MenuItemVersion", back_populates="modifier_versions"
    )
...
class MenuAuditLog(Base, TimestampMixin):
    ...
    )  # create_version, publish_version, rollback, etc.
    entity_type = Column(
        String(50), nullable=False
    )  # menu_item, category, modifier, etc.
    ...
    parent_log_id = Column(
        Integer, ForeignKey("menu_audit_logs.id"), nullable=True
    )  # For nested changes
