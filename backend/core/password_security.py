...
PREVENT_PASSWORD_REUSE = int(
    os.getenv("PREVENT_PASSWORD_REUSE", "5")
)  # Last N passwords
...
class EnhancedPasswordSecurity:
    ...
    def hash_password(
        self, password: str, algorithm: Optional[PasswordAlgorithm] = None
    ) -> str:
        """
    ...
    def validate_password(
        ...
        if len(password) > MAX_PASSWORD_LENGTH:
            errors.append(
                f"Password must be no longer than {MAX_PASSWORD_LENGTH} characters"
            )
    ...
    def cleanup_user_reset_tokens(self, user_id: int) -> None:
        ...
        tokens_to_remove = [
            token
            for token, reset_token in self._reset_tokens.items()
        ...
        if tokens_to_remove:
            logger.info(
                f"Cleaned up {len(tokens_to_remove)} reset tokens for user_id={user_id}"
            )
