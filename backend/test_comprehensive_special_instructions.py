...
Tests data preservation, bidirectional conversion, validation, and schema
functionality
"""
import sys
import os
...
from modules.orders.schemas.order_schemas import (  # noqa: E402
    SpecialInstructionBase, OrderItemUpdate, OrderItemOut
)
from modules.orders.enums.order_enums import (  # noqa: E402
    SpecialInstructionType
)
from modules.orders.services.order_service import (  # noqa: E402
    serialize_instructions_to_notes, parse_notes_to_instructions
)
from modules.orders.controllers.order_controller import (  # noqa: E402
...
def test_bidirectional_conversion():
    ...
    expected = ("[P1] [PREP] ALLERGEN: No nuts | "
                "[P2] [GRILL] PREPARATION: Extra crispy")
    assert formatted_notes == expected, (
        f"Expected: {expected}, Got: {formatted_notes}")
...
def test_enhanced_validation():
    ...
    result = asyncio.run(
        validate_special_instructions(invalid_items, mock_db))
    ...
    result = asyncio.run(
        validate_special_instructions(invalid_items_many, mock_db))
    assert not result["valid"]
    ...
    result = asyncio.run(
        validate_special_instructions(long_station_items, mock_db))
    assert not result["valid"]
...
def test_schema_functionality():
    ...
    try:
        order_item_out = OrderItemOut.from_orm_with_instructions(
            mock_orm_obj)
        assert order_item_out.special_instructions is not None
        assert len(order_item_out.special_instructions) == 1
        assert (order_item_out.special_instructions[0].instruction_type ==
                SpecialInstructionType.ALLERGEN)
...
if __name__ == "__main__":
    try:
        ...
        print("  ✅ Bidirectional conversion between structured and "
              "formatted notes")
        print("  ✅ Enhanced validation (description, station length, "
              "instruction count)")
