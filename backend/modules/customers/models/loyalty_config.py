...
class LoyaltyTierConfig(Base, TimestampMixin):
    ...
    benefits = Column(
        JSONB, nullable=True
    )  # e.g., {"point_multiplier": 1.5, "free_delivery": true}
...
class LoyaltyPointsConfig(Base, TimestampMixin):
    ...
    )  # order, referral, review, birthday, etc.
    ...
    conditions = Column(
        JSONB, nullable=True
    )  # e.g., {"min_order_amount": 25, "item_categories": ["entrees"]}
    ...
    # Tier Multipliers
    tier_multipliers = Column(
        JSONB, nullable=True
    )  # e.g., {"gold": 1.5, "platinum": 2.0}
...
class LoyaltyService:
    ...
    def get_tier_configs(self) -> List[LoyaltyTierConfig]:
        ...
        return (
            self.db.query(LoyaltyTierConfig)
            .filter(LoyaltyTierConfig.is_active == True)
            .order_by(LoyaltyTierConfig.tier_order)
            .all()
        )
    ...
    def get_points_configs(self) -> List[LoyaltyPointsConfig]:
        ...
        return (
            self.db.query(LoyaltyPointsConfig)
            .filter(LoyaltyPointsConfig.is_active == True)
            .all()
        )
    ...
    def _customer_meets_tier_requirements(
        self, customer, tier_config: LoyaltyTierConfig
    ) -> bool:
        ...
        if (
            tier_config.min_total_spent
            and customer.total_spent < tier_config.min_total_spent
        ):
            return False
    ...
    def calculate_points_earned(
        ...
        **kwargs,
        ...
        for config in points_configs:
            ...
            if (
                customer_tier
                and config.tier_multipliers
                and customer_tier in config.tier_multipliers
            ):
