...
class Customer(Base, TimestampMixin):
    ...
    tier = Column(
        SQLEnum(CustomerTier), nullable=False, default=CustomerTier.BRONZE, index=True
    )
    ...
    rewards = relationship(
        "CustomerReward", back_populates="customer", cascade="all, delete-orphan"
    )
...
class CustomerAddress(Base, TimestampMixin):
    ...
    def __repr__(self):
        return (
            f"<CustomerAddress(id={self.id}, label='{self.label}', city='{self.city}')>"
        )
...
class CustomerNotification(Base, TimestampMixin):
    ...
    notification_metadata = Column(
        JSONB, nullable=True
    )  # Additional data like order_id, promotion_id
...
class CustomerSegment(Base, TimestampMixin):
    ...
    criteria = Column(
        JSONB, nullable=True
    )  # e.g., {"min_orders": 5, "tier": ["gold", "platinum"]}

    # Status
    is_active = Column(Boolean, default=True)
    is_dynamic = Column(
        Boolean, default=False
    )  # Auto-update membership based on criteria
    ...
    customers = relationship(
        "Customer", secondary="customer_segment_members", back_populates="segments"
    )
...
class CustomerPreference(Base, TimestampMixin):
    ...
    )  # dietary, communication, ordering, etc.
