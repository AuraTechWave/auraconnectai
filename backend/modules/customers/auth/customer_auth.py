...
# JWT settings
CUSTOMER_JWT_SECRET = getattr(
    settings, "CUSTOMER_JWT_SECRET", secrets.token_urlsafe(32)
)
...
class CustomerTokenData:
    ...
    def __init__(
        self, customer_id: int, email: str, tier: str, tenant_id: Optional[int] = None
    ):
...
def create_customer_access_token(
    ...
    to_encode.update(
        {"exp": expire, "iat": datetime.utcnow(), "type": "customer_access"}
    )

    encoded_jwt = jwt.encode(
        to_encode, CUSTOMER_JWT_SECRET, algorithm=CUSTOMER_JWT_ALGORITHM
    )
    return encoded_jwt
...
def verify_customer_token(token: str) -> Optional[CustomerTokenData]:
    ...
    try:
        payload = jwt.decode(
            token, CUSTOMER_JWT_SECRET, algorithms=[CUSTOMER_JWT_ALGORITHM]
        )
        ...
        return CustomerTokenData(
            customer_id=customer_id, email=email, tier=tier, tenant_id=tenant_id
        )
...
class CustomerAuthService:
    ...
    def change_password(
        self, customer_id: int, old_password: str, new_password: str
    ) -> bool:
        ...
        # Verify old password
        if not customer.password_hash or not verify_customer_password(
            old_password, customer.password_hash
        ):
    ...
    def reset_password(self, reset_token: str, new_password: str) -> bool:
        ...
        try:
            payload = jwt.decode(
                reset_token, CUSTOMER_JWT_SECRET, algorithms=[CUSTOMER_JWT_ALGORITHM]
            )
...
def require_customer_tier(required_tier: str):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            customer_tier_level = tier_hierarchy.get(
                current_customer.tier.value.lower(), 0
            )
