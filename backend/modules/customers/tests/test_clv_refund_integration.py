...
class TestCLVRefundIntegration:
    ...
    def create_order(
        self, db: Session, customer_id: int, items_data: list, status: str = "completed"
    ):
        """Helper to create an order with items"""
        order = Order(
            customer_id=customer_id, status=status, created_at=datetime.utcnow()
        )
        db.add(order)
        db.commit()
    ...
    def test_clv_consistency_across_services(
        self, db: Session, test_customer: Customer, services: dict
    ):
        ...
        order1, total1 = self.create_order(
            db,
            test_customer.id,
            [{"quantity": 2, "price": 50.00}, {"quantity": 1, "price": 100.00}],
        )
        ...
        order2, total2 = self.create_order(
            db, test_customer.id, [{"quantity": 3, "price": 50.00}]
        )
        ...
        order3, total3 = self.create_order(
            db, test_customer.id, [{"quantity": 2, "price": 50.00}]
        )
    ...
    def test_clv_with_order_status_changes(
        ...
        order, total = self.create_order(
            db, test_customer.id, [{"quantity": 1, "price": 100.00}], status="pending"
        )
        ...
        assert (
            test_customer.lifetime_value == 0.0
        )  # Can't be negative, even with refunds
    ...
    def test_clv_calculation_performance(
        ...
        for i in range(50):
            order, _ = self.create_order(
                db, test_customer.id, [{"quantity": 1, "price": 10.00 + i}]
            )
    ...
    def test_concurrent_refund_handling(
        ...
        order, total = self.create_order(
            db, test_customer.id, [{"quantity": 10, "price": 100.00}]  # $1000 order
        )
