...
class CustomerSecurityService:
        ...
        if 'email' in sanitized and sanitized['email']:
            ...
            if len(email_parts) == 2:
                sanitized['email_masked'] = (
                    CustomerSecurityService.mask_sensitive_data(email_parts[0], visible_chars=2) +
                    '@' + email_parts[1]
                )
    ...
    def check_account_lockout(
        ...
        lockout_until = last_failed_attempt + timedelta(minutes=lockout_duration_minutes)
        ...
        return {
            ...
            "minutes_remaining": int((lockout_until - datetime.utcnow()).total_seconds() / 60) if is_locked else 0
        }
    
    @staticmethod
    def validate_data_retention_compliance(customer_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Check data retention compliance for customer data
        Helps with GDPR and other privacy regulations
        """
        last_activity = customer_data.get('last_login') or customer_data.get('updated_at')
