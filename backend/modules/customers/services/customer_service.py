...
class CustomerService:
    ...
    def create_customer(self, customer_data: CustomerCreate) -> Customer:
        ...
        try:
            ...
            if customer_data.password:
                ...
                password_validation = (
                    CustomerSecurityService.validate_password_strength(
                ...
                customer.password_hash = CustomerSecurityService.hash_password(
                    customer_data.password
                )
            ...
            if customer_data.referral_code:
                referrer = (
                    self.db.query(Customer)
                    .filter(Customer.referral_code == customer_data.referral_code)
                    .first()
                )
                if referrer:
    ...
    def update_customer(
        self, customer_id: int, update_data: CustomerUpdate
    ) -> Customer:
    ...
    def update_customer_status(
        self, customer_id: int, status_update: CustomerStatusUpdate
    ) -> Customer:
        ...
        logger.info(
            f"Updated customer {customer_id} status from {old_status} to {status_update.status}"
        )
        return customer

    def update_customer_tier(
        self, customer_id: int, tier_update: CustomerTierUpdate
    ) -> Customer:
        ...
        logger.info(
            f"Updated customer {customer_id} tier from {old_tier} to {tier_update.tier}"
        )
        return customer

    def search_customers(
        self, params: CustomerSearchParams
        ...
        count_query = self.db.query(func.count(Customer.id)).filter(
    ...
    def get_customer_analytics(self, customer_id: int) -> CustomerAnalytics:
        ...
        customer = (
            self.db.query(Customer)
            .options(joinedload(Customer.orders), joinedload(Customer.preferences))
        ...
        if (
            ...
            and customer.first_order_date
            and customer.last_order_date
        ):
        ...
        timing_data = (
            ...
            .filter(and_(Order.customer_id == customer_id, Order.deleted_at.is_(None)))
        ...
        for hour, dow, count in timing_data:
            preferred_order_times[str(int(hour))] = (
                preferred_order_times.get(str(int(hour)), 0) + count
            )
            day_names = [
                "Sunday",
                "Monday",
            ...
            preferred_order_days[day_name] = (
                preferred_order_days.get(day_name, 0) + count
            )
        ...
        lifetime_value = (
            ...
            and customer.lifetime_value is not None
            else customer.total_spent
        )
    ...
    def add_loyalty_points(
        self, customer_id: int, points: int, reason: str
    ) -> Customer:
    ...
    def redeem_loyalty_points(
        self, customer_id: int, points: int, reward_id: Optional[int] = None
    ) -> bool:
    ...
    def add_customer_address(
        self, customer_id: int, address_data: CustomerAddressCreate
    ) -> CustomerAddress:
        ...
        address = CustomerAddress(customer_id=customer_id, **address_data.model_dump())
    ...
    def update_customer_address(
        self, address_id: int, update_data: CustomerAddressUpdate
    ) -> CustomerAddress:
    ...
    def set_customer_preference(
        self, customer_id: int, preference_data: CustomerPreferenceCreate
    ) -> CustomerPreference:
        ...
        if preference:
            ...
            preference = CustomerPreference(
                customer_id=customer_id, **preference_data.model_dump()
            )
    ...
    def get_customer_preferences(
        self, customer_id: int, category: Optional[str] = None
    ...
    def _generate_referral_code(self) -> str:
        ...
        while True:
            ...
            existing = (
                self.db.query(Customer).filter(Customer.referral_code == code).first()
            )
            if not existing:
                return code
    ...
    def _notify_tier_change(
        self, customer: Customer, old_tier: CustomerTier, new_tier: CustomerTier
    ):
        """Create notification for tier change"""
