...
def encrypt_payment_data(payment_method_data: dict) -> dict:
    ...
    if 'card_token' in encrypted_data and encrypted_data['card_token']:
        encrypted_data['card_token'] = payment_security.store_payment_token(encrypted_data['card_token'])
    
    if 'wallet_id' in encrypted_data and encrypted_data['wallet_id']:
        encrypted_data['wallet_id'] = payment_security.store_payment_token(encrypted_data['wallet_id'])
...
def decrypt_payment_data(payment_method_data: dict) -> dict:
    ...
    if 'card_token' in decrypted_data and decrypted_data['card_token']:
        try:
            decrypted_data['card_token'] = payment_security.retrieve_payment_token(decrypted_data['card_token'])
        except ValueError:
    ...
    if 'wallet_id' in decrypted_data and decrypted_data['wallet_id']:
        try:
            decrypted_data['wallet_id'] = payment_security.retrieve_payment_token(decrypted_data['wallet_id'])
        except ValueError:
...
def log_payment_access(user_id: int, customer_id: int, action: str, payment_method_id: Optional[int] = None):
...
class PCIComplianceHelper:
    ...
    @staticmethod
    def validate_card_holder_data_access(user_permissions: list, required_permission: str) -> bool:
