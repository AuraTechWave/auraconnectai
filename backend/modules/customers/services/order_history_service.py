...
class OrderHistoryService:
    ...
    def get_customer_orders(
        ...
        query = self.db.query(Order).filter(
            and_(Order.customer_id == customer_id, Order.deleted_at.is_(None))
        )
    ...
    def get_order_summaries(
        ...
        orders = (
            self.db.query(Order)
            .options(joinedload(Order.order_items))  # Fix N+1: Eager load order_items
    ...
    def get_favorite_items(
        self, customer_id: int, limit: int = 10, min_orders: int = 2
        ...
        item_counts = (
            ...
            .having(func.count(OrderItem.id) >= min_orders)
        ...
        menu_items = (
            ...
            .options(joinedload(MenuItem.category))  # Eager load category relationship
    ...
    def get_order_analytics(
        self, customer_id: int, period_days: int = 365
    ) -> Dict[str, Any]:
        ...
        for order in orders:
            ...
            order_total = sum(item.price * item.quantity for item in order.order_items)
            ...
            monthly_spending[month_key] = monthly_spending.get(month_key, 0) + float(
                order_total
            )
        ...
        if total_orders > 1:
            ...
            order_frequency_days = (
                days_between / (total_orders - 1) if days_between > 0 else None
            )
        else:
            order_frequency_days = None
    ...
    def get_spending_trends(
        self, customer_id: int, period_months: int = 12
    ) -> Dict[str, Any]:
        ...
        monthly_data = (
            self.db.query(
                ...
                func.sum(
                    self.db.query(func.sum(OrderItem.price * OrderItem.quantity))
            ...
            trends.append(
                {
                    ...
                    "average_order_value": (
                        float(total_spent) / order_count if order_count > 0 else 0
                    ),
                }
            )
        ...
        if len(trends) >= 2:
            recent_avg = sum(t["total_spent"] for t in trends[-3:]) / min(
                3, len(trends)
            )
            older_avg = sum(t["total_spent"] for t in trends[:-3]) / max(
                1, len(trends) - 3
            )
        ...
        return {
            ...
            "highest_spending_month": (
                max(trends, key=lambda x: x["total_spent"]) if trends else None
            ),
            "most_active_month": (
                max(trends, key=lambda x: x["order_count"]) if trends else None
            ),
        }
    ...
    def update_customer_order_stats(self, customer_id: int) -> Customer:
        ...
        if order_stats.total_orders:
            ...
            new_total_spent = (
                float(order_stats.total_spent) if order_stats.total_spent else 0
            )
            ...
            if current_lifetime_value > current_total_spent:
                logger.warning(
                    f"Data inconsistency for customer {customer.id}: lifetime_value ({current_lifetime_value}) > total_spent ({current_total_spent}). Correcting..."
                )
            ...
            customer.lifetime_value = max(
                0, min(new_total_spent, new_total_spent - total_refunds)
            )
