...
async def set_customer_preference(
    ...
    try:
        customer_service = CustomerService(db)
        preference = customer_service.set_customer_preference(customer_id, preference_data)
        return preference
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
...
async def add_loyalty_points(
    ...
    try:
        ...
        logger.error(f"Error adding loyalty points for customer {customer_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def redeem_loyalty_points(
    ...
    try:
        ...
        logger.error(f"Error redeeming loyalty points for customer {customer_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_customer_favorite_items(
    ...
    try:
        order_history_service = OrderHistoryService(db)
        favorites = order_history_service.get_favorite_items(customer_id, limit, min_orders)
        return favorites
    except Exception as e:
        logger.error(f"Error getting favorite items for customer {customer_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
