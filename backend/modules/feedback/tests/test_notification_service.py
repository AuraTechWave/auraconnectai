...
class TestEmailBackend:
    ...
    async def test_send_email_success(self, email_backend):
        ...
        mock_smtp.assert_called_once_with(
            email_backend.smtp_host, email_backend.smtp_port
        )
        mock_smtp_instance.starttls.assert_called_once()
        mock_smtp_instance.login.assert_called_once_with(
            email_backend.smtp_username, email_backend.smtp_password
        )
...
class TestSMSBackend:
    ...
    async def test_send_sms_success(self, sms_backend):
        ...
        with patch("twilio.rest.Client", return_value=mock_client):
            result = await sms_backend.send_sms(
                to_number="+0987654321", message="Test SMS message"
            )

        # Verify Twilio client was called correctly
        mock_client.messages.create.assert_called_once_with(
            body="Test SMS message", from_=sms_backend.from_number, to="+0987654321"
        )
    ...
    async def test_send_sms_invalid_number(self, sms_backend):
        ...
        with patch("twilio.rest.Client", return_value=mock_client):
            result = await sms_backend.send_sms(
                to_number="invalid", message="Test SMS message"
            )
    ...
    async def test_send_sms_message_truncation(self, sms_backend):
        ...
        with patch("twilio.rest.Client", return_value=mock_client):
            result = await sms_backend.send_sms(
                to_number="+0987654321", message=long_message
            )
...
class TestNotificationService:
    ...
    async def test_send_review_invitation(self, notification_service, mock_db):
        ...
        mock_customer.notification_preferences = {"email": True, "sms": True}
        ...
        with (
            patch.object(
                notification_service.email_backend, "send_email", new_callable=AsyncMock
            ) as mock_email,
            patch.object(
                notification_service.sms_backend, "send_sms", new_callable=AsyncMock
            ) as mock_sms,
            patch.object(
                notification_service, "_queue_notification_task", new_callable=AsyncMock
            ) as mock_queue,
        ):
            ...
            result = await notification_service.send_review_invitation(
                customer_id=1, entity_type="product", entity_id=100
            )
    ...
    @pytest.mark.asyncio
    async def test_send_feedback_response_notification(
    ...
    async def test_queue_notification_task(self, notification_service, mock_db):
        ...
        with patch(
            "backend.modules.feedback.services.notification_service.background_processor",
            mock_processor,
        ):
            await notification_service._queue_notification_task(
                notification_type="test_notification", data={"test": "data"}
            )

        # Verify task was enqueued
        mock_processor.enqueue_notification.assert_called_once_with(
            notification_type="test_notification", test="data"
        )
    ...
    def test_render_template(self, notification_service):
        ...
        rendered = notification_service._render_template(
            template, context={"name": "John Doe"}
        )
...
class TestBackgroundTaskQueuing:
    ...
    @pytest.mark.asyncio
    async def test_notification_task_queued_with_correct_params(
        ...
        with patch(
            ...
            await notification_service._queue_notification_task(
                ...
                data={"customer_id": 123, "entity_type": "product", "entity_id": 456},
            )
    ...
    async def test_multiple_notification_types_queued(self, notification_service):
        ...
        with patch(
            ...
            await notification_service._queue_notification_task(
                notification_type="review_invitation", data={"customer_id": 1}
            )
    ...
    async def test_task_queue_error_handling(self, notification_service):
        ...
        with patch(
            ...
            try:
                await notification_service._queue_notification_task(
                    notification_type="test", data={"test": "data"}
                )
            except Exception:
