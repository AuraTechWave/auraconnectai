...
class TestLoadPerformance:
    ...
    def test_analytics_overview_load(
        self, client: TestClient, large_dataset, auth_headers_staff
    ):
        ...
        def make_request():
            ...
            response = client.get(
                "/feedback/analytics/overview", headers=auth_headers_staff
            )
            end_time = time.time()
    ...
    def test_review_insights_load(
        self, client: TestClient, multiple_reviews, auth_headers_staff
    ):
            ...
            for _ in range(5):  # 5 requests per entity
                for entity_type, entity_id in test_entities:
                    future = executor.submit(
                        make_insights_request, entity_type, entity_id
                    )
        ...
        # Assertions
        assert (
            len(successful_requests) >= len(results) * 0.9
        ), "At least 90% of requests should succeed"
        assert (
            max(response_times) < 3.0
        ), "No insights request should take longer than 3 seconds"
    ...
    def test_reviews_list_pagination_load(self, client: TestClient, large_dataset):
        ...
        assert all(
            r["item_count"] <= 20 for r in successful_requests
    ...
    @pytest.mark.asyncio
    async def test_sentiment_analysis_batch_performance(
        ...
        assert all(
            hasattr(r, "score") and hasattr(r, "confidence") for r in results
        ), "All results should have score and confidence"

        # Test performance metrics
        avg_time_per_item = processing_time / len(test_items)
        assert (
            avg_time_per_item < 0.1
        ), "Average processing time per item should be under 0.1 seconds"
    ...
    def test_aggregation_service_performance(
        self, aggregation_service: ReviewAggregationService, large_dataset
    ):
        ...
        test_entities = [("product", i) for i in range(1, 21)]  # 20 products
        ...
        assert (
            max(processing_times) < 5.0
        ), "No single aggregation should take longer than 5 seconds"
        assert (
            statistics.mean(processing_times) < 2.0
        ), "Average aggregation time should be under 2 seconds"
    ...
    def test_concurrent_read_write_operations(
        self, client: TestClient, auth_headers_customer, auth_headers_staff
    ):
        ...
        def create_review():
            ...
            response = client.post(
                "/reviews/", json=review_data, headers=auth_headers_customer
            )
            end_time = time.time()
        ...
        # Assertions
        create_success_rate = sum(1 for r in create_results if r["success"]) / len(
            create_results
        )
        read_success_rate = sum(1 for r in read_results if r["success"]) / len(
            read_results
        )
        analytics_success_rate = sum(
            1 for r in analytics_results if r["success"]
        ) / len(analytics_results)

        assert (
            create_success_rate >= 0.8
        ), "At least 80% of create operations should succeed"
        assert (
            read_success_rate >= 0.95
        ), "At least 95% of read operations should succeed"
        assert (
            analytics_success_rate >= 0.9
        ), "At least 90% of analytics operations should succeed"

        print(f"Concurrent Operations Performance:")
        print(
            f"  Create operations: {len(create_results)} ({create_success_rate:.1%} success)"
        )
        print(
            f"  Read operations: {len(read_results)} ({read_success_rate:.1%} success)"
        )
        print(
            f"  Analytics operations: {len(analytics_results)} ({analytics_success_rate:.1%} success)"
        )
    ...
    def test_memory_usage_under_load(self, client: TestClient, large_dataset):
        ...
        assert (
            memory_increase < 100
        ), f"Memory usage shouldn't increase by more than 100MB (increased by {memory_increase:.1f}MB)"
...
class TestStressTests:
    ...
    def test_maximum_concurrent_requests(self, client: TestClient):
        ...
        acceptable_performance = [
            i for i, rate in enumerate(success_rates) if rate >= 0.9
        ]

        if acceptable_performance:
            max_acceptable_load = max_workers_tested[max(acceptable_performance)]
            print(
                f"System can handle up to {max_acceptable_load} concurrent requests with 90%+ success rate"
            )
        else:
    ...
    def test_large_response_handling(self, client: TestClient, large_dataset):
        ...
        for page_size in page_sizes:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Page size {page_size}: {actual_items} items in {response_time:.3f}s"
                )

                # Assertions
                assert (
                    response_time < 10.0
                ), f"Large response ({page_size} items) should complete within 10 seconds"
                assert (
                    actual_items <= page_size
                ), "Response shouldn't exceed requested page size"
            else:
                print(
                    f"Page size {page_size}: Failed with status {response.status_code}"
                )
    ...
    def test_sustained_load_endurance(self, client: TestClient):
        ...
        assert (
            total_requests > 100
        ), "Should have made at least 100 requests in 60 seconds"
        assert (
            success_rate >= 0.95
