...
class TestDatabaseRelationships:
    ...
    def test_review_media_cascade_delete(self, db_session: Session, sample_review):
        ...
        # Verify media exists
        media_count = (
            db_session.query(ReviewMedia).filter_by(review_id=sample_review.id).count()
        )
    ...
    def test_review_votes_cascade_delete(self, db_session: Session, sample_review):
        ...
        # Verify votes exist
        votes_count = (
            db_session.query(ReviewVote).filter_by(review_id=sample_review.id).count()
        )
    ...
    def test_business_response_cascade_delete(self, db_session: Session, sample_review):
        ...
        response_count = (
            db_session.query(BusinessResponse)
            .filter_by(review_id=sample_review.id)
            .count()
        )
    ...
    def test_feedback_responses_cascade_delete(
        self, db_session: Session, sample_feedback
    ):
        ...
        response_count = (
            db_session.query(FeedbackResponse)
            .filter_by(feedback_id=sample_feedback.id)
            .count()
        )
    ...
    def test_computed_fields_consistency(self, db_session: Session, sample_review):
        ...
        assert (
            abs(sample_review.helpful_percentage - 66.67) < 0.01
