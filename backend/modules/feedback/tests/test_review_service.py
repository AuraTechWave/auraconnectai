...
class TestReviewService:
    ...
    def test_create_review_success(
        self, review_service: ReviewService, sample_review_data
    ):
    ...
    def test_create_review_without_auto_verify(
        self, review_service: ReviewService, sample_review_data
    ):
    ...
    def test_create_review_duplicate_prevention(
        self, review_service: ReviewService, sample_review_data, sample_review
    ):
    ...
    def test_update_review_success(self, review_service: ReviewService, sample_review):
        ...
        result = review_service.update_review(
            sample_review.id, update_data, customer_id=sample_review.customer_id
        )

        assert result.title == "Updated title"
        assert (
            result.content
            == "Updated content with more details about the product experience."
        )
        assert result.rating == 5.0
        assert (
            result.status == ReviewStatus.PENDING
        )  # Should reset to pending after content change

    def test_update_review_permission_denied(
        self, review_service: ReviewService, sample_review
    ):
        ...
        with pytest.raises(PermissionError):
            review_service.update_review(
                sample_review.id, update_data, customer_id=999  # Different customer
            )

    def test_moderate_review_success(
        self, review_service: ReviewService, sample_review
    ):
        ...
        result = review_service.moderate_review(
            sample_review.id, moderation_data, moderator_id=100
        )
    ...
    def test_delete_review_soft_delete(
        self, review_service: ReviewService, sample_review
    ):
        """Test soft delete of review"""
        result = review_service.delete_review(
            sample_review.id, customer_id=sample_review.customer_id, soft_delete=True
        )
    ...
    def test_vote_on_review_success(self, review_service: ReviewService, sample_review):
        ...
        result = review_service.vote_on_review(
            sample_review.id, customer_id=2, vote_data=vote_data  # Different customer
        )
    ...
    def test_vote_on_review_update_existing(
        self, review_service: ReviewService, sample_review, db_session
    ):
        """Test updating existing vote"""
        # Create initial vote
        initial_vote = ReviewVote(
            review_id=sample_review.id, customer_id=2, is_helpful=True
        )
        ...
        result = review_service.vote_on_review(
            sample_review.id, customer_id=2, vote_data=vote_data
        )
    ...
    def test_add_business_response_duplicate_prevention(
        self, review_service: ReviewService, sample_review, db_session
    ):
        ...
        response_data = BusinessResponseCreate(
            content="Duplicate response", responder_name="Staff"
        )
    ...
    def test_add_review_media_success(
        ...
        result = review_service.add_review_media(
            sample_review.id, media_data, customer_id=sample_review.customer_id
        )
    ...
    def test_add_review_media_permission_denied(
        self, review_service: ReviewService, sample_review
    ):
        """Test adding media with wrong customer"""
        media_data = [
            ReviewMediaCreate(
                media_type="image", file_path="/uploads/test.jpg", file_name="test.jpg"
            )
        ]

        with pytest.raises(PermissionError):
            review_service.add_review_media(
                sample_review.id, media_data, customer_id=999  # Wrong customer
            )

    def test_list_reviews_with_filters(
        self, review_service: ReviewService, multiple_reviews
    ):
    ...
    def test_get_review_aggregates_basic(
        self, review_service: ReviewService, multiple_reviews
    ):
    ...
    def test_get_review_aggregates_force_refresh(
        self, review_service: ReviewService, multiple_reviews
    ):
        """Test forced refresh of review aggregates"""
        # Get initial aggregates
        result1 = review_service.get_review_aggregates(
            "product", 101, force_refresh=False
        )

        # Force refresh
        result2 = review_service.get_review_aggregates(
            "product", 101, force_refresh=True
        )
    ...
    @pytest.mark.asyncio
    async def test_sentiment_analysis_scheduling(
        self, review_service: ReviewService, sample_review_data
    ):
        ...
        with pytest.MonkeyPatch().context() as m:
            ...
            m.setattr(
                review_service, "_schedule_sentiment_analysis", mock_schedule_sentiment
            )
    ...
    def test_review_verification_logic(
        self, review_service: ReviewService, sample_review_data
    ):
    ...
    def test_review_content_validation(
        self, review_service: ReviewService, sample_review_data
    ):
    ...
    def test_review_metadata_handling(
        self, review_service: ReviewService, sample_review_data
    ):
    ...
    def test_review_engagement_metrics(
        self, review_service: ReviewService, sample_review
    ):
...
class TestReviewServiceEdgeCases:
    ...
    def test_large_content_handling(
        ...
        long_content = (
            "This is a very detailed review. " * 160
        )  # Close to 5000 char limit
    ...
    def test_database_constraint_violations(
        self, review_service: ReviewService, sample_review_data, db_session
    ):
