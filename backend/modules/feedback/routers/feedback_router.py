...
async def get_feedback(
    ...
    try:
        ...
        if not current_staff:
            if not current_user or (result.customer_id and result.customer_id != current_user["id"]):
...
async def get_my_feedback_detail(
    ...
    try:
        ...
        if result.customer_id != current_user["id"]:
            raise HTTPException(status_code=403, detail="Cannot view another customer's feedback")
