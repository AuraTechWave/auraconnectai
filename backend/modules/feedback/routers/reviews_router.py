...
async def delete_review(
    review_id: int = Path(..., description="Review ID"),
    soft_delete: bool = Query(True, description="Whether to soft delete (hide) or hard delete"),
    db: Session = Depends(get_db),
    current_user: Dict = Depends(get_current_user)
):
...
async def list_reviews(
    ...
    rating_max: Optional[float] = Query(None, ge=1.0, le=5.0, description="Maximum rating"),
    verified_only: Optional[bool] = Query(None, description="Show only verified reviews"),
    with_media: Optional[bool] = Query(None, description="Show only reviews with media"),
