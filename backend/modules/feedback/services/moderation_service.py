...
class ContentModerationService:
    ...
    def moderate_review(
        self, review: Review, auto_moderate: bool = True
    ) -> ModerationResult:
        ...
        logger.info(
            f"Moderated review {review.id}: {moderation_result.action} (confidence: {moderation_result.confidence})"
        )

        return moderation_result

    def moderate_feedback(
        self, feedback: Feedback, auto_moderate: bool = True
    ) -> ModerationResult:
        ...
        # Log moderation action
        logger.info(
            f"Moderated feedback {feedback.id}: {moderation_result.action} (confidence: {moderation_result.confidence})"
        )
    ...
    def get_moderation_queue(
        ...
        if content_type == "review":
            ...
            query = query.order_by(desc(Feedback.priority), desc(Feedback.created_at))
                ...
                formatted_items.append(
                    {
                        ...
                        "sentiment": (
                            item.sentiment_score.value if item.sentiment_score else None
                        ),
    ...
    def get_moderation_stats(
        ...
        auto_moderation_rate = (
            (auto_approved / total_reviews * 100) if total_reviews > 0 else 0
        )

        # Pending items
        pending_reviews = (
            self.db.query(Review).filter(Review.status == ReviewStatus.PENDING).count()
        )
        ...
        return {
            ...
            "efficiency_metrics": {
                "total_items_processed": sum(review_stats.values())
                + sum(feedback_stats.values()),
                ...
                type(
                    ...
                    (),
                    {
                        ...
                        "review_type": type(
                            "MockType",
                            (),
                            {"value": context.get("review_type", "product")},
                        )(),
    ...
    def _determine_moderation_action(
        self, severity: float, reasons: List[str], context: Dict[str, Any] = None
    ) -> Tuple[str, float]:
    ...
    def _apply_auto_moderation_feedback(
        self, feedback: Feedback, result: ModerationResult
    ) -> None:
