...
class ReviewAggregationService:
    ...
    def calculate_aggregates_for_entity(
        self, entity_type: str, entity_id: int, force_recalculation: bool = False
    ) -> AggregationResult:
        """Calculate comprehensive aggregates for a specific entity"""

        # Check if we need to recalculate
        existing_aggregate = (
            self.db.query(ReviewAggregate)
            .filter(
                and_(
        ...
        if not force_recalculation and existing_aggregate:
            ...
            if existing_aggregate.last_calculated_at > datetime.utcnow() - timedelta(
                hours=1
            ):
        ...
        logger.info(
            f"Updated aggregates for {entity_type} {entity_id}: {basic_metrics['total_reviews']} reviews"
        )

        return self._format_aggregation_result(aggregate)

    def bulk_update_aggregates(
        self, entity_type: str, entity_ids: List[int] = None, batch_size: int = 50
    ) -> Dict[str, Any]:
    ...
    def get_top_rated_entities(
        self, entity_type: str, limit: int = 10, min_reviews: int = 5
    ) -> List[Dict[str, Any]]:
    ...
    def get_trending_entities(
        self, entity_type: str, limit: int = 10, days_back: int = 7
    ) -> List[Dict[str, Any]]:
        ...
        for row in result:
            ...
            if aggregate:
                ...
                if row.prev_week_reviews > 0:
                    growth_rate = (
                        (row.last_week_reviews - row.prev_week_reviews)
                        / row.prev_week_reviews
                    ) * 100
    ...
    def get_review_insights(self, entity_type: str, entity_id: int) -> Dict[str, Any]:
        ...
        for i in range(6):
            ...
            month_reviews = [
                r for r in reviews if month_start <= r.created_at < month_end
            ]
                ...
                monthly_trends.append(
                    {
                        ...
                        "average_rating": sum(r.rating for r in month_reviews)
                        / len(month_reviews),
                    }
                )
        ...
        positive_keywords = self._extract_keywords(
            [r.content for r in positive_reviews]
        )
        negative_keywords = self._extract_keywords(
            [r.content for r in negative_reviews]
        )

        # Verification insights
        verified_reviews = [r for r in reviews if r.is_verified_purchase]
        verification_rate = (
            len(verified_reviews) / total_reviews * 100 if total_reviews > 0 else 0
        )
        ...
        unverified_avg_rating = (
            sum(r.rating for r in reviews if not r.is_verified_purchase)
            / (total_reviews - len(verified_reviews))
            if (total_reviews - len(verified_reviews)) > 0
            else 0
        )
        ...
        return {
            ...
            "quality_metrics": {
                ...
                "average_content_length": sum(len(r.content) for r in reviews)
                / total_reviews,
                "helpful_reviews": sum(
                    1 for r in reviews if r.helpful_votes > r.not_helpful_votes
                ),
            },
        }

    async def update_aggregates_async(
        self, entity_type: str, entity_ids: List[int], batch_size: int = 20
    ) -> Dict[str, Any]:
        ...
        for i in range(0, len(entity_ids), batch_size):
            batch = entity_ids[i : i + batch_size]
            task = asyncio.create_task(self._update_batch_async(entity_type, batch))
            tasks.append(task)
    ...
    def _calculate_sentiment_metrics(self, reviews: List[Review]) -> Dict[str, Any]:
        ...
        if not reviews:
            return {"sentiment_distribution": {}, "positive_sentiment_percentage": 0.0}
        ...
        positive_sentiments = sentiment_counts.get(
            "positive", 0
        ) + sentiment_counts.get("very_positive", 0)
    ...
    def _calculate_quality_metrics(self, reviews: List[Review]) -> Dict[str, Any]:
        ...
        avg_content_length = sum(len(review.content) for review in reviews) / len(
            reviews
        )

        # Helpful review ratio
        helpful_reviews = sum(
            1 for review in reviews if review.helpful_votes > review.not_helpful_votes
        )
        helpful_ratio = helpful_reviews / len(reviews) * 100

        # Reviews with business responses
        business_response_count = sum(
            1 for review in reviews if review.has_business_response
        )
    ...
    def _calculate_trending_metrics(self, reviews: List[Review]) -> Dict[str, Any]:
        ...
        trending_score = (
            (volume_score * 0.4) + (recency_score * 0.4) + (momentum_score * 0.2)
        )
    ...
    def _get_entities_needing_update(self, entity_type: str) -> List[int]:
        ...
        result = self.db.execute(
            query, {"entity_type": entity_type, "cutoff_time": cutoff_time}
        )

        return [row.entity_id for row in result]

    def _update_batch_aggregates(
        ...
        for entity_id in entity_ids:
            try:
                self.calculate_aggregates_for_entity(
                    entity_type, entity_id, force_recalculation=True
                )
                updated_count += 1
            except Exception as e:
    ...
    async def _update_batch_async(
        ...
        return await asyncio.get_event_loop().run_in_executor(
            None, self._update_batch_aggregates, entity_type, entity_ids
        )
    ...
    def _format_aggregation_result(
        self, aggregate: ReviewAggregate
    ) -> AggregationResult:
