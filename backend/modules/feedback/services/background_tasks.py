...
class BackgroundTaskProcessor:
    ...
    async def _process_sentiment_analysis(self, data: Dict[str, Any]):
        ...
        try:
            if data.get("review_id"):
                ...
                feedback = db.query(Feedback).filter(Feedback.id == data["feedback_id"]).first()
                if feedback:
    ...
    async def _process_content_moderation(self, data: Dict[str, Any]):
        ...
        try:
            ...
            if data.get("review_id"):
                ...
                feedback = db.query(Feedback).filter(Feedback.id == data["feedback_id"]).first()
                if feedback:
                    result = await moderation_service.moderate_feedback_content(feedback)
    ...
    async def _process_review_aggregation(self, data: Dict[str, Any]):
        ...
        try:
            ...
            if entity_id:
                ...
                existing = db.query(ReviewAggregate).filter(
    ...
    async def _process_analytics_calculation(self, data: Dict[str, Any]):
        ...
        try:
            ...
            if calculation_type == "daily_summary":
                ...
                entity_id = data.get("entity_id")
                await analytics_service.calculate_sentiment_trends(entity_type, entity_id)
    ...
    async def enqueue_sentiment_analysis(self, review_id: Optional[int] = None, 
    ...
    async def enqueue_content_moderation(self, review_id: Optional[int] = None,
                                        feedback_id: Optional[int] = None):
