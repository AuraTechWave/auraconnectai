...
from modules.feedback.models.feedback_models import Review, Feedback, SentimentScore
...
class SentimentAnalysisService:
    ...
    def analyze_review_sentiment(
        self, review: Review, force_reanalysis: bool = False
    ) -> SentimentResult:
        ...
        if (
            ...
            and review.sentiment_score
            and review.sentiment_confidence
        ):
    ...
    def analyze_feedback_sentiment(
        self, feedback: Feedback, force_reanalysis: bool = False
    ) -> SentimentResult:
        ...
        if (
            ...
            and feedback.sentiment_confidence
        ):
            return SentimentResult(
    ...
    def _pattern_based_analysis(
        ...
        sentiment_patterns = [
            ...
            (
                r"\b(completely disappointed|total waste|never again|extremely poor)\b",
                -1.8,
            ),
            # Intensity modifiers
            (
                r"\b(very|extremely|really|super|incredibly|absolutely)\s+(\w+)",
                1.5,
            ),  # Multiplier
    ...
    def _combine_sentiment_results(
        ...
        if not results:
            return {"score": SentimentScore.NEUTRAL, "confidence": 0.0, "details": {}}
    ...
    def _apply_context_adjustments(
        ...
        if "rating" in context:
            ...
            if rating >= 4.0 and result["score"] in [
            ...
            elif rating <= 2.0 and result["score"] in [
    ...
    async def _analyze_text_sentiment_async(
        ...
        return await asyncio.get_event_loop().run_in_executor(
            None, self._analyze_text_sentiment, text, context
        )

    def get_sentiment_summary(self, items: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Generate insights
        insights = []
        positive_pct = sentiment_distribution.get(
            SentimentScore.POSITIVE.value, 0
        ) + sentiment_distribution.get(SentimentScore.VERY_POSITIVE.value, 0)
        negative_pct = sentiment_distribution.get(
            SentimentScore.NEGATIVE.value, 0
        ) + sentiment_distribution.get(SentimentScore.VERY_NEGATIVE.value, 0)

        if positive_pct > 70:
            insights.append(
                "Predominantly positive sentiment - customers are generally satisfied"
            )
        elif negative_pct > 30:
            insights.append(
                "Significant negative sentiment - attention needed for customer satisfaction"
            )

        if avg_confidence < 0.5:
            insights.append(
                "Low confidence in sentiment analysis - consider manual review"
            )
        ...
        return {
            ...
            "neutral_percentage": round(
                sentiment_distribution.get(SentimentScore.NEUTRAL.value, 0), 1
            ),
        }
