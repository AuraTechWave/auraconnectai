...
class SMSBackend:
    ...
    async def send_sms(self, to_phone: str, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            data = {"From": self.from_phone, "To": to_phone, "Body": message}
...
class PushNotificationBackend:
    ...
    async def send_push(
        ...
        try:
            ...
            return {"success": False, "error": str(e), "provider": "fcm"}
...
class NotificationService:
    ...
    async def send_feedback_notification(
        ...
        try:
            ...
            # Determine recipients based on event type
            recipients = self._get_feedback_notification_recipients(
                event_type, feedback
            )
    ...
    async def send_reminder_notifications(
        self, reminder_type: str = "review_invitation", max_reminders: int = 100
    ) -> Dict[str, Any]:
    ...
    async def process_notification_queue(self, batch_size: int = 50) -> Dict[str, Any]:
    ...
    async def _send_notification(self, request: NotificationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            for notification_type in request.notification_types:
                ...
                if handler:
                    ...
                    results[notification_type.value] = {
                        "success": False,
    ...
    async def _send_review_invitation_reminders(
        ...
        invitations = (
            self.db.query(ReviewInvitation)
            .filter(
                and_(
                    ...
                    or_(
                        ...
                        ReviewInvitation.last_reminder_sent
        ...
        for invitation in invitations:
            ...
            variables = {
                ...
                "review_url": f"{settings.FRONTEND_URL}/reviews/create?invitation={invitation.uuid}",
                "product_name": (
                    self._get_product_name(invitation.product_id)
    ...
    async def _send_feedback_follow_up_reminders(
        self, max_reminders: int
    ) -> Dict[str, Any]:
        ...
        feedback_items = (
            ...
            .filter(
                and_(
                    ...
                    Feedback.status.in_(
                        [FeedbackStatus.NEW, FeedbackStatus.IN_PROGRESS]
                    ),
                )
            )
            .limit(max_reminders)
    ...
    def _prepare_review_variables(
        self, review: Review, additional_data: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    def _prepare_feedback_variables(
        self, feedback: Feedback, additional_data: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    def _get_review_notification_recipients(
        self, event_type: str, review: Review
    ) -> List[Dict[str, Any]]:
    ...
    def _get_feedback_notification_recipients(
        self, event_type: str, feedback: Feedback
    ) -> List[Dict[str, Any]]:
    ...
    def _render_template(
        ...
        try:
            ...
            if template.template_id.startswith(
                "review_"
            ) or template.template_id.startswith("feedback_"):
                try:
    ...
    async def _send_email_notification(
        self, request: NotificationRequest, rendered_content: Dict[str, str]
    ) -> Dict[str, Any]:
    ...
    async def _send_sms_notification(
        ...
        try:
            ...
            result = await self.sms_backend.send_sms(
                to_phone=request.recipient_phone, message=rendered_content["content"]
            )
    ...
    async def _send_push_notification(
        ...
        try:
            ...
            logger.info(
                f"Push notification sent to user {request.recipient_id}: {result}"
            )
            return result
    ...
    async def _send_webhook_notification(
        ...
        try:
            ...
            logger.info(
                f"Sending webhook notification for template {request.template_id}"
            )
