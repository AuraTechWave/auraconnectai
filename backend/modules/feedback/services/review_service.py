...
class ReviewService:
    ...
    def create_review(
        self, review_data: ReviewCreate, auto_verify: bool = False
    ) -> ReviewResponse:
        """Create a new review"""
        ...
        try:
            ...
            if review_data.order_id and self._verify_purchase(
                review_data.customer_id, review_data.order_id, review_data.product_id
            ):
            ...
            logger.info(
                f"Created review {review.id} for customer {review_data.customer_id}"
            )
    ...
    def moderate_review(
        self, review_id: int, moderation_data: ReviewModeration, moderator_id: int
    ) -> ReviewResponse:
        ...
        if (
            old_status != ReviewStatus.APPROVED
    ...
    def _build_paginated_response(
        self, items, total: int, page: int, per_page: int
    ) -> PaginatedResponse:
    ...
    def vote_on_review(
        self, review_id: int, customer_id: int, vote_data: ReviewVoteCreate
    ) -> Dict[str, Any]:
    ...
    def add_business_response(
        self, review_id: int, response_data: BusinessResponseCreate
    ) -> Dict[str, Any]:
        ...
        existing_response = (
            self.db.query(BusinessResponse)
            .filter(BusinessResponse.review_id == review_id)
            .first()
        )
    ...
    def get_review_aggregates(
        self, entity_type: str, entity_id: int, force_refresh: bool = False
    ) -> Dict[str, Any]:
        ...
        if (
            not aggregate
            or force_refresh
            or (aggregate.last_calculated_at < datetime.utcnow() - timedelta(hours=1))
        ):
    ...
    def _verify_purchase(
        self, customer_id: int, order_id: int, product_id: Optional[int]
    ) -> bool:
    ...
    def _schedule_sentiment_analysis(self, review_id: int) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to schedule sentiment analysis for review {review_id}: {e}"
            )
    ...
    def _fallback_sentiment_analysis(self, review_id: int) -> None:
        ...
        try:
            ...
            if review:
                ...
                self.db.commit()
                logger.info(
                    f"Completed fallback sentiment analysis for review {review_id}"
                )
        except Exception as e:
            logger.error(
                f"Fallback sentiment analysis failed for review {review_id}: {e}"
            )
    ...
    def _update_review_aggregates(self, review: Review) -> None:
        ...
        try:
            if review.product_id:
                asyncio.create_task(
                    background_processor.enqueue_review_aggregation(
    ...
    def _calculate_review_aggregates(
        ...
        if total_reviews == 0:
            ...
            aggregate_data = self._calculate_aggregate_metrics(
                base_query, entity_type, entity_id, total_reviews
            )
    ...
    def _create_empty_aggregate_data(
        self, entity_type: str, entity_id: int
    ) -> Dict[str, Any]:
    ...
    def _calculate_aggregate_metrics(
        self, base_query, entity_type: str, entity_id: int, total_reviews: int
    ) -> Dict[str, Any]:
        ...
        positive_percentage = self._calculate_positive_sentiment_percentage(
            base_query, total_reviews
        )
    ...
    def _calculate_positive_sentiment_percentage(
        self, base_query, total_reviews: int
    ) -> float:
        """Calculate percentage of positive sentiment reviews"""
        positive_sentiments = base_query.filter(
            Review.sentiment_score.in_(
