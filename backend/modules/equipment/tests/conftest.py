...
@pytest.fixture
def created_equipment_api(client: TestClient, headers: dict, equipment_data: dict) -> dict:
...
@pytest.fixture
def created_maintenance_api(client: TestClient, headers: dict, created_equipment_api: dict, maintenance_data: dict) -> dict:
...
def assert_equipment_shape():
    ...
    def _assert(equipment: dict):
        ...
        assert equipment["status"] in ["operational", "needs_maintenance", "under_maintenance", "out_of_service", "retired"]
        ...
        optional_fields = [
            ...
            "location", "notes", "maintenance_interval_days",
        ...
        date_fields = ["purchase_date", "warranty_expiry", "last_maintenance_date", "next_due_date", "created_at", "updated_at"]
...
def assert_maintenance_shape():
    ...
    def _assert(record: dict):
        ...
        assert record["maintenance_type"] in ["preventive", "corrective", "emergency", "inspection", "calibration"]
        
        assert "status" in record
        assert record["status"] in ["scheduled", "in_progress", "completed", "overdue", "cancelled"]
        ...
        # Optional fields
        optional_fields = [
            "date_performed", "next_due_date", "performed_by",
            "parts_replaced", "issues_found", "resolution",
            "created_at", "updated_at"
        ]
        for field in optional_fields:
        ...
        # Date validation
        date_fields = ["scheduled_date", "date_performed", "next_due_date", "created_at", "updated_at"]
        for field in date_fields:
...
def assert_paginated_response():
    ...
    def _assert(response: dict):
        ...
        if response["total"] > 0:
            expected_pages = (response["total"] + response["size"] - 1) // response["size"]
