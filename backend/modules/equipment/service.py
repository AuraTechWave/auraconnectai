...
class EquipmentService:
    ...
    def create_equipment(self, equipment_data: EquipmentCreate, user_id: int) -> Equipment:
        ...
        if equipment.maintenance_interval_days:
            equipment.next_due_date = datetime.now() + timedelta(days=equipment.maintenance_interval_days)
    ...
    def update_equipment(self, equipment_id: int, update_data: EquipmentUpdate, user_id: int) -> Equipment:
        """Update equipment"""
        equipment = self.get_equipment(equipment_id)
        
        # Check for duplicate serial number if updating
        if update_data.serial_number and update_data.serial_number != equipment.serial_number:
            existing = self.db.query(Equipment).filter(
        ...
        if 'maintenance_interval_days' in update_dict and equipment.maintenance_interval_days:
    ...
    def search_equipment(self, params: EquipmentSearchParams) -> Tuple[List[Equipment], int]:
    ...
    def create_maintenance_record(self, record_data: MaintenanceRecordCreate, user_id: int) -> MaintenanceRecord:
    ...
    def update_maintenance_record(self, record_id: int, update_data: MaintenanceRecordUpdate) -> MaintenanceRecord:
    ...
    def complete_maintenance(self, record_id: int, completion_data: MaintenanceRecordComplete, user_id: int) -> MaintenanceRecord:
        ...
        if completion_data.next_due_date:
            ...
            next_due = record.date_performed + timedelta(days=equipment.maintenance_interval_days)
    ...
    def search_maintenance_records(self, params: MaintenanceSearchParams) -> Tuple[List[MaintenanceRecord], int]:
        ...
        if params.maintenance_type:
            query = query.filter(MaintenanceRecord.maintenance_type == params.maintenance_type)
        ...
        if params.performed_by:
            query = query.filter(MaintenanceRecord.performed_by.ilike(f"%{params.performed_by}%"))
    ...
    def get_maintenance_summary(self) -> MaintenanceSummary:
        ...
        total_equipment = self.db.query(Equipment).filter(Equipment.is_active == True).count()
        
        operational = self.db.query(Equipment).filter(
            Equipment.is_active == True,
            Equipment.status == EquipmentStatus.OPERATIONAL
        ).count()
        
        needs_maintenance = self.db.query(Equipment).filter(
            Equipment.is_active == True,
            Equipment.status == EquipmentStatus.NEEDS_MAINTENANCE
        ).count()
        
        under_maintenance = self.db.query(Equipment).filter(
            Equipment.is_active == True,
            Equipment.status == EquipmentStatus.UNDER_MAINTENANCE
        ).count()
        
        # Overdue maintenance
        overdue = self.db.query(Equipment).filter(
            Equipment.is_active == True,
        ...
        scheduled_week = self.db.query(MaintenanceRecord).filter(
        ...
        completed_month = self.db.query(MaintenanceRecord).filter(
            MaintenanceRecord.status == MaintenanceStatus.COMPLETED,
            MaintenanceRecord.date_performed >= month_start
        ).count()
        
        # Cost this month
        cost_result = self.db.query(func.sum(MaintenanceRecord.cost)).filter(
            MaintenanceRecord.status == MaintenanceStatus.COMPLETED,
            MaintenanceRecord.date_performed >= month_start
        ).scalar()
        ...
        # Average downtime
        downtime_result = self.db.query(func.avg(MaintenanceRecord.downtime_hours)).filter(
            MaintenanceRecord.status == MaintenanceStatus.COMPLETED,
    ...
    def check_and_update_overdue_maintenance(self):
        ...
        overdue_records = self.db.query(MaintenanceRecord).filter(
            MaintenanceRecord.status == MaintenanceStatus.SCHEDULED,
