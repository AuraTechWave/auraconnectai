...
async def get_equipment(
    ...
    downtime_hours = [
        ...
        if r.downtime_hours and r.downtime_hours > 0
    ]
...
async def create_maintenance_record(
    ...
    if (
        record_data.scheduled_date
        and record_data.scheduled_date < datetime.now().date()
    ):
        raise APIValidationError(
...
async def search_maintenance_records(
    ...
    try:
        ...
        raise APIValidationError(
            "Invalid date format. Use ISO format (YYYY-MM-DD)", {"error": str(e)}
        )
...
async def complete_maintenance_record(
    ...
    return service.complete_maintenance_record(
        record_id, completion_data, current_user.id
    )
...
# Bulk operations
@router.post(
    "/bulk/schedule-maintenance", response_model=BulkScheduleMaintenanceResponse
)
@handle_api_errors
