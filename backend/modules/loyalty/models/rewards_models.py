...
class RewardTemplate(Base, TimestampMixin):
    ...
    # Relationships
    reward_instances = relationship(
        "CustomerReward", back_populates="template", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "points_cost >= 0 OR points_cost IS NULL", name="points_cost_non_negative"
        ),
...
class CustomerReward(Base, TimestampMixin):
    ...
    template_id = Column(
        Integer, ForeignKey("reward_templates.id"), nullable=False, index=True
    )
    ...
    # Metadata
    issued_by = Column(
        Integer, ForeignKey("rbac_users.id"), nullable=True
    )  # Admin who manually issued
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            and self.status == RewardStatus.AVAILABLE
        )
...
class RewardRedemption(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    reward_id = Column(
        Integer, ForeignKey("customer_rewards_v2.id"), nullable=False, index=True
    )
    customer_id = Column(
        Integer, ForeignKey("customers.id"), nullable=False, index=True
    )
...
class LoyaltyPointsTransaction(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index(
            "ix_loyalty_points_transactions_customer_type",
            "customer_id",
            "transaction_type",
        ),
...
class RewardAnalytics(Base, TimestampMixin):
    ...
    # Performance Metrics
    redemption_rate = Column(
        Float, default=0.0
    )  # Percentage of issued rewards redeemed
    ...
    __table_args__ = (
        Index(
            "ix_reward_analytics_template_period",
            "template_id",
            "period_start",
            "period_end",
            unique=True,
