...
class RewardsEngine:
    ...
    def process_order_completion(self, order_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if points_earned > 0:
                ...
                self._award_points(customer, points_earned, f"Order #{order_id}", order_id=order_id)
            ...
            self.db.commit()
            logger.info(f"Order completion processed for customer {customer.id}: {points_earned} points, {len(results['rewards_triggered'])} rewards")
    ...
    def issue_reward_to_customer(
        ...
        reward = self._create_reward_instance(customer, template, issued_by, custom_data)
    ...
    def redeem_reward(
        ...
        reward = self.db.query(CustomerReward).filter(
            CustomerReward.code == reward_code
        ).first()
        ...
        if order.customer_id != reward.customer_id:
            return {"success": False, "error": "Reward does not belong to order customer"}
        ...
        try:
            ...
            redemption = RewardRedemption(
                ...
                final_order_amount=float(self._calculate_order_total(order)) - discount_amount,
            ...
            logger.info(f"Redeemed reward {reward.code} for ${discount_amount:.2f} discount")
    ...
    def get_customer_available_rewards(self, customer_id: int) -> List[CustomerReward]:
        ...
        ).order_by(CustomerReward.valid_until.asc()).all()
    ...
    def expire_old_rewards(self) -> int:
        ...
        expired_count = self.db.query(CustomerReward).filter(
            and_(
                CustomerReward.status == RewardStatus.AVAILABLE,
                CustomerReward.valid_until <= now
            )
    ...
    def run_automated_campaigns(self) -> Dict[str, Any]:
        ...
        campaigns = self.db.query(RewardCampaign).filter(
            and_(
        ...
        for campaign in campaigns:
            try:
                ...
                for customer in eligible_customers:
                    ...
                    existing_rewards = self.db.query(CustomerReward).filter(
                        and_(
                    ...
                    if existing_rewards < campaign.max_rewards_per_customer:
                        try:
                            ...
                            if (campaign.max_rewards_total and 
                                campaign.rewards_distributed + rewards_distributed >= campaign.max_rewards_total):
                                break
                                
                        except Exception as e:
                            logger.warning(f"Failed to issue campaign reward to customer {customer.id}: {str(e)}")
    ...
    def get_reward_analytics(
        ...
        redeemed_count = query.filter(CustomerReward.status == RewardStatus.REDEEMED).count()
        expired_count = query.filter(CustomerReward.status == RewardStatus.EXPIRED).count()
        
        # Calculate metrics
        redemption_rate = (redeemed_count / total_issued * 100) if total_issued > 0 else 0
        
        # Get total discount value
        total_discount = self.db.query(func.sum(RewardRedemption.discount_applied)).join(
            CustomerReward
        ).filter(CustomerReward.template_id == template_id).scalar() or 0
        ...
        return {
            ...
            "average_discount_per_redemption": float(total_discount / redeemed_count) if redeemed_count > 0 else 0
        }
    ...
    def _process_trigger_rewards(
        ...
        for template in templates:
            try:
                ...
                # Create reward
                reward = self._create_reward_instance(customer, template, trigger_data=trigger_data)
                template.total_issued += 1
                ...
                logger.warning(f"Failed to process trigger reward {template.name} for customer {customer.id}: {str(e)}")
    ...
    def _check_milestone_rewards(self, customer: Customer) -> List[Dict[str, Any]]:
        ...
        for template in templates:
            try:
                ...
                logger.warning(f"Failed to process milestone reward {template.name} for customer {customer.id}: {str(e)}")
        
        return milestone_rewards
    
    def _is_customer_eligible(self, customer: Customer, template: RewardTemplate) -> bool:
    ...
    def _check_trigger_conditions(self, template: RewardTemplate, trigger_data: Dict[str, Any]) -> bool:
    ...
    def _generate_reward_code(self) -> str:
        ...
        while True:
            ...
            existing = self.db.query(CustomerReward).filter(CustomerReward.code == code).first()
            if not existing:
                return code
    
    def _calculate_reward_discount(self, reward: CustomerReward, order: Order) -> Dict[str, Any]:
        ...
        if reward.reward_type == RewardType.FIXED_DISCOUNT:
            ...
            discount_amount = order_total * (float(reward.percentage) / 100) if reward.percentage else 0.0
    ...
    def _find_campaign_eligible_customers(self, campaign: RewardCampaign) -> List[Customer]:
        ...
        if campaign.target_tiers:
            tier_values = [getattr(CustomerTier, tier.upper()) for tier in campaign.target_tiers if hasattr(CustomerTier, tier.upper())]
        ...
        if campaign.target_criteria:
            ...
            if "inactive_days" in criteria:
                cutoff_date = datetime.utcnow() - timedelta(days=criteria["inactive_days"])
                query = query.filter(
                    or_(
