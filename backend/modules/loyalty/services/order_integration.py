...
class OrderLoyaltyIntegration:
    ...
    def process_order_completion(self, order_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error processing order loyalty for order {order_id}: {str(e)}"
            )
            return {
                "success": False,
                "error": str(e),
    ...
    def apply_reward_discount(self, order_id: int, reward_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.rewards_engine.redeem_reward(
                reward_code=reward_code, order_id=order_id
            )
            ...
            if result.get("success"):
                ...
                logger.info(
                    f"Applied reward discount of ${result['discount_amount']:.2f} to order {order_id}"
                )
    ...
    def get_applicable_rewards(
        self, customer_id: int, order_total: float
    ) -> Dict[str, Any]:
        """Get rewards that can be applied to an order"""
        try:
            # Get customer's available rewards
            available_rewards = self.rewards_engine.get_customer_available_rewards(
                customer_id
            )
            ...
            for reward in available_rewards:
                ...
                if (
                    ...
                    and order_total < template.min_order_amount
                ):
                    continue

                # Calculate potential discount
                discount_result = self.rewards_engine._calculate_reward_discount(
                    reward, None
                )
                    ...
                    applicable_rewards.append(
                        {
                            ...
                            "potential_discount": discount_result.get(
                                "discount_amount", 0
                            ),
            ...
            return {
                ...
                "total_potential_savings": sum(
                    r["potential_discount"] for r in applicable_rewards
                ),
            }

        except Exception as e:
            logger.error(
                f"Error getting applicable rewards for customer {customer_id}: {str(e)}"
            )
            return {"success": False, "error": str(e), "applicable_rewards": []}

    def calculate_points_preview(
        self, customer_id: int, order_total: float
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            points_to_earn = (
                self.rewards_engine.loyalty_service.calculate_points_earned(
                    action_type="order",
                    amount=order_total,
            ...
            potential_tier = (
                self.rewards_engine.loyalty_service.calculate_tier_for_customer_points(
                    new_lifetime_points
                )
            )
            ...
            if potential_tier != current_tier:
                tier_upgrade = {
                    ...
                    "tier_benefits": self.rewards_engine.loyalty_service.get_tier_benefits(
                        potential_tier
                    ),
                }
    ...
    def handle_order_cancellation(self, order_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Order cancellation processed: {points_reversed} points reversed, {rewards_revoked} rewards revoked"
            )
            ...
            self.db.rollback()
            logger.error(
                f"Error handling order cancellation for order {order_id}: {str(e)}"
            )
            return {"success": False, "error": str(e)}

    def handle_partial_refund(
        self, order_id: int, refund_amount: float
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            refund_percentage = (
                refund_amount / original_total if original_total > 0 else 0
            )
            ...
            if points_to_reverse > 0:
                ...
                customer.loyalty_points = max(
                    0, customer.loyalty_points - points_to_reverse
                )
                customer.lifetime_points = max(
                    0, customer.lifetime_points - points_to_reverse
                )
            ...
            if customer.lifetime_value is None:
                ...
                customer.lifetime_value = (
                    float(customer.total_spent) if customer.total_spent else 0.0
                )

            customer.lifetime_value = max(
                0, float(customer.lifetime_value) - refund_amount
            )

            self.db.commit()

            logger.info(
                f"Partial refund processed: {points_to_reverse} points adjusted"
            )
            ...
            self.db.rollback()
            logger.error(
                f"Error handling partial refund for order {order_id}: {str(e)}"
            )
            return {"success": False, "error": str(e)}
    ...
    def _send_reward_notifications(self, order_id: int, loyalty_result: Dict[str, Any]):
        ...
        try:
            ...
            if loyalty_result.get("points_earned", 0) > 0:
                logger.info(
                    f"Notification: Customer earned {loyalty_result['points_earned']} points from order {order_id}"
                )

            if loyalty_result.get("rewards_triggered"):
                for reward in loyalty_result["rewards_triggered"]:
                    logger.info(
                        f"Notification: Customer received reward '{reward['title']}' from order {order_id}"
                    )

            if loyalty_result.get("tier_upgrade"):
                tier_info = loyalty_result["tier_upgrade"]
                logger.info(
                    f"Notification: Customer upgraded from {tier_info['old_tier']} to {tier_info['new_tier']}"
                )
