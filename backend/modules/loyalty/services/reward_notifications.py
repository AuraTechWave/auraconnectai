...
class RewardNotificationService:
    ...
    def notify_reward_earned(
        ...
        try:
            ...
            if not customer:
                logger.error(f"Customer {customer_id} not found for reward notification")
                return False
            
            # Check customer's communication preferences
            if not self._should_send_notification(customer, NotificationType.REWARD_EARNED):
                logger.info(f"Skipping reward notification for customer {customer_id} due to preferences")
                return True
            ...
            channels = self._get_preferred_channels(customer, NotificationType.REWARD_EARNED)
            ...
            for channel in channels:
                try:
                    ...
                    logger.error(f"Failed to send reward notification via {channel}: {str(e)}")
                    success = False
    ...
    def notify_points_earned(
        ...
        try:
            ...
            if not self._should_send_notification(customer, NotificationType.POINTS_EARNED):
                return True
            ...
            content = self._build_points_earned_content(customer, points_earned, reason, order_id)
            
            channels = self._get_preferred_channels(customer, NotificationType.POINTS_EARNED)
            ...
            for channel in channels:
                try:
                    ...
                    logger.error(f"Failed to send points notification via {channel}: {str(e)}")
                    success = False
    ...
    def notify_tier_upgrade(
        ...
        try:
            ...
            if not self._should_send_notification(customer, NotificationType.TIER_UPGRADE):
                return True
            
            subject = f"🌟 Congratulations! You've been upgraded to {new_tier.title()} tier!"
            content = self._build_tier_upgrade_content(customer, old_tier, new_tier, tier_benefits)
            
            # Tier upgrades are important, send via all channels
            channels = [NotificationChannel.EMAIL, NotificationChannel.IN_APP]
            if customer.communication_preferences and customer.communication_preferences.get("sms", False):
            ...
            for channel in channels:
                try:
                    ...
                    logger.error(f"Failed to send tier upgrade notification via {channel}: {str(e)}")
                    success = False
    ...
    def notify_rewards_expiring(self, days_ahead: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            expiring_rewards = self.db.query(CustomerReward).filter(
            ...
            for customer_id, rewards in customer_rewards.items():
                try:
                    ...
                    if not self._should_send_notification(customer, NotificationType.REWARD_EXPIRING):
                        continue
                    
                    # Create notification for expiring rewards
                    subject = f"⏰ {len(rewards)} reward(s) expiring soon!"
                    content = self._build_expiring_rewards_content(customer, rewards, days_ahead)
                    
                    channels = self._get_preferred_channels(customer, NotificationType.REWARD_EXPIRING)
                    ...
                    logger.error(f"Failed to send expiring rewards notification to customer {customer_id}: {str(e)}")
                    failures += 1
            
            logger.info(f"Expiring rewards notifications: {notifications_sent} sent, {failures} failed")
    ...
    def notify_milestone_achieved(
        ...
        try:
            ...
            if not self._should_send_notification(customer, NotificationType.MILESTONE_ACHIEVED):
                return True
            
            subject = f"🏆 Milestone Achieved: {self._format_milestone_title(milestone_type, milestone_value)}"
            content = self._build_milestone_content(customer, milestone_type, milestone_value, rewards_triggered)
            
            channels = self._get_preferred_channels(customer, NotificationType.MILESTONE_ACHIEVED)
            ...
            for channel in channels:
                try:
                    ...
                    logger.error(f"Failed to send milestone notification via {channel}: {str(e)}")
                    success = False
    ...
    def send_birthday_rewards(self) -> Dict[str, Any]:
        ...
        try:
            ...
            birthday_customers = self.db.query(Customer).filter(
                Customer.date_of_birth.isnot(None),
                Customer.deleted_at.is_(None)
            ).all()
            
            birthday_customers = [
                c for c in birthday_customers 
                if c.date_of_birth and c.date_of_birth.month == today.month and c.date_of_birth.day == today.day
            ]
            ...
            birthday_template = self.db.query(RewardTemplate).filter(
                ...
                try:
                    ...
                    if birthday_template:
                        ...
                        try:
                            ...
                            self._send_birthday_notification_with_reward(customer, reward)
                            
                        except Exception as e:
                            # Still send birthday notification even if reward fails
                            logger.warning(f"Failed to issue birthday reward to customer {customer.id}: {str(e)}")
                    ...
                    logger.error(f"Failed to process birthday for customer {customer.id}: {str(e)}")
            
            logger.info(f"Birthday processing: {notifications_sent} notifications sent, {rewards_issued} rewards issued")
    ...
    def _should_send_notification(self, customer: Customer, notification_type: NotificationType) -> bool:
        """Check if customer wants to receive this type of notification"""
        if not customer.marketing_opt_in and notification_type != NotificationType.TIER_UPGRADE:
            return False
    ...
    def _get_preferred_channels(self, customer: Customer, notification_type: NotificationType) -> List[NotificationChannel]:
    ...
    def _send_notification(
        ...
        try:
            ...
            logger.info(f"Notification sent to customer {customer.id} via {channel.value}: {subject}")
    ...
    def _build_reward_earned_content(self, reward: CustomerReward, trigger_context: Optional[Dict[str, Any]]) -> str:
    ...
    def _build_points_earned_content(self, customer: Customer, points_earned: int, reason: str, order_id: Optional[int]) -> str:
    ...
    def _build_tier_upgrade_content(self, customer: Customer, old_tier: str, new_tier: str, tier_benefits: Optional[Dict[str, Any]]) -> str:
    ...
    def _build_expiring_rewards_content(self, customer: Customer, rewards: List[CustomerReward], days_ahead: int) -> str:
    ...
    def _build_milestone_content(self, customer: Customer, milestone_type: str, milestone_value: Any, rewards_triggered: Optional[List[Dict[str, Any]]]) -> str:
    ...
    def _get_tier_progress_text(self, customer: Customer) -> str:
        ...
        tier_points_map = {"bronze": 0, "silver": 1000, "gold": 2500, "platinum": 5000, "vip": 10000}
    ...
    def _send_birthday_notification(self, customer: Customer):
        ...
        content += "We hope you have a wonderful day filled with joy and celebration.\n\n"
    ...
    def _send_birthday_notification_with_reward(self, customer: Customer, reward: CustomerReward):
        """Send birthday notification with reward"""
        subject = f"🎂🎁 Happy Birthday, {customer.first_name}! You've got a gift!"
        content = f"Happy Birthday, {customer.first_name}!\n\n"
        content += "To celebrate your special day, we've got a birthday gift for you:\n\n"
