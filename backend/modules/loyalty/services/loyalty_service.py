...
class LoyaltyService:
    ...
    def add_points(
        self, transaction_data: PointsTransactionCreate, staff_id: Optional[int] = None
    ) -> LoyaltyPointsTransaction:
        """Add points to customer account"""
        # Validate customer
        customer = (
            self.db.query(Customer)
            .filter(Customer.id == transaction_data.customer_id)
            .first()
        )
        ...
        # Get current balance
        current_balance = self._get_customer_points_balance(
        ...
        if (
            ...
            and transaction_data.points_change > 0
        ):
        ...
        if new_balance < 0:
            raise APIValidationError(
                "Insufficient points balance",
                {
                    ...
                    "requested": abs(transaction_data.points_change),
                },
            )
    ...
    def adjust_points(
        self, adjustment: PointsAdjustment, staff_id: int
    ) -> LoyaltyPointsTransaction:
    ...
    def transfer_points(
        self, transfer: PointsTransfer, staff_id: int
    ) -> Tuple[LoyaltyPointsTransaction, LoyaltyPointsTransaction]:
        """Transfer points between customers"""
        # Validate both customers exist
        from_customer = (
            self.db.query(Customer)
            .filter(Customer.id == transfer.from_customer_id)
            .first()
        )
        to_customer = (
            self.db.query(Customer)
            .filter(Customer.id == transfer.to_customer_id)
            .first()
        )
    ...
    def create_reward_template(
        self, template_data: RewardTemplateCreate
    ) -> RewardTemplate:
    ...
    def update_reward_template(
        self, template_id: int, update_data: RewardTemplateUpdate
    ) -> RewardTemplate:
        """Update reward template"""
        template = (
            self.db.query(RewardTemplate)
            .filter(RewardTemplate.id == template_id)
            .first()
        )
    ...
    def get_available_templates(
        ...
        query = self.db.query(RewardTemplate).filter(RewardTemplate.is_active == True)
        ...
        templates = query.order_by(
            desc(RewardTemplate.priority), RewardTemplate.name
        ).all()
    ...
    def issue_reward(
        ...
        template = (
            self.db.query(RewardTemplate)
            .filter(RewardTemplate.id == template_id, RewardTemplate.is_active == True)
            .first()
        )
        if not template:
        ...
        if (
            template.max_uses_per_customer
            and existing_count >= template.max_uses_per_customer
        ):
            raise APIValidationError(
                "Customer has reached the maximum uses for this reward",
                {
                    "max_uses": template.max_uses_per_customer,
                    "current_uses": existing_count,
                },
            )
    ...
    def issue_manual_reward(
        self, issuance: ManualRewardIssuance, staff_id: int
    ) -> CustomerReward:
    ...
    def issue_bulk_rewards(
        self, bulk_issuance: BulkRewardIssuance, staff_id: int
    ) -> List[CustomerReward]:
        ...
        if bulk_issuance.customer_ids:
            ...
            customer_ids = self._find_customers_by_criteria(
                bulk_issuance.customer_criteria
            )
    ...
    def validate_reward(
        self, validation_request: RewardValidationRequest
    ) -> Dict[str, Any]:
        """Validate if a reward can be used"""
        reward = (
            self.db.query(CustomerReward)
            .filter(CustomerReward.code == validation_request.reward_code.upper())
            .first()
        )

        if not reward:
            return {"is_valid": False, "validation_errors": ["Invalid reward code"]}
        ...
        if (
            ...
            and validation_request.order_amount < template.min_order_amount
        ):
        ...
        discount_amount = self._calculate_discount(
            reward, validation_request.order_amount, validation_request.order_items
        )
    ...
    def redeem_reward(
        ...
        reward = (
            self.db.query(CustomerReward)
            .filter(CustomerReward.code == redemption_request.reward_code.upper())
            .with_for_update()
            .first()
        )
        ...
        redemption = RewardRedemption(
            ...
            final_order_amount=redemption_request.order_amount
    ...
    def search_customer_rewards(
        self, search_params: RewardSearchParams
        ...
        if search_params.customer_id:
            query = query.filter(
                CustomerReward.customer_id == search_params.customer_id
            )

        if search_params.reward_type:
            query = query.filter(
                CustomerReward.reward_type == search_params.reward_type
            )
        ...
        if search_params.valid_on_date:
            date_check = datetime.combine(
                search_params.valid_on_date, datetime.min.time()
            )
            query = query.filter(
        ...
        # Apply sorting
        sort_column = getattr(
            CustomerReward, search_params.sort_by, CustomerReward.created_at
        )
        ...
        # Apply pagination
        rewards = (
            query.offset((search_params.page - 1) * search_params.limit)
    ...
    def process_order_completion(
        self, order_data: OrderCompletionReward
    ) -> Dict[str, Any]:
        ...
        if points_earned > 0:
            ...
            points_transaction = PointsTransactionCreate(
                ...
                + timedelta(days=self.default_points_expiry_days),
            )
            self.add_points(points_transaction)
        ...
        for template_id in triggered_rewards:
            try:
                ...
                results["rewards_triggered"].append(
                    {"reward_id": reward.id, "title": reward.title, "code": reward.code}
                )
            except Exception as e:
    ...
    def _get_customer_points_balance(self, customer_id: int) -> int:
        ...
        result = (
            self.db.query(func.sum(LoyaltyPointsTransaction.points_change))
            .filter(
    ...
    def _get_lifetime_points_spent(self, customer_id: int) -> int:
        ...
        result = (
            self.db.query(func.sum(func.abs(LoyaltyPointsTransaction.points_change)))
            .filter(
                LoyaltyPointsTransaction.customer_id == customer_id,
                LoyaltyPointsTransaction.points_change < 0,
            )
    ...
    def _get_points_history(
        ...
        transactions = (
            self.db.query(LoyaltyPointsTransaction)
            .filter(
                LoyaltyPointsTransaction.customer_id == customer_id,
                LoyaltyPointsTransaction.created_at >= since_date,
            )
            .order_by(desc(LoyaltyPointsTransaction.created_at))
            .limit(50)
            .all()
        )
    ...
    def _get_customer_rewards_stats(self, customer_id: int) -> Dict[str, int]:
        ...
        earned = (
            self.db.query(func.count(CustomerReward.id))
    ...
    def _calculate_customer_tier(
        self, customer_id: int, current_balance: int
    ) -> Dict[str, Any]:
            ...
            if customer:
                ...
                if current_tier in tier_names:
                    ...
                    if current_idx < len(tier_configs) - 1:
                        ...
                        points_needed = (
                            next_cfg.min_lifetime_points or 0
                        ) - lifetime_points
                        result.update(
                            {
                                ...
                                "progress_percentage": min(
                                    ...
                                    (
                                        lifetime_points
                                        / (next_cfg.min_lifetime_points or 1)
                                    )
                                    * 100,
                                ),
                            }
        ...
        tiers = {
            ...
            "silver": {
                "min": 2000,
                "benefits": ["1.2x points earning", "Birthday bonus"],
            },
            "gold": {
                "min": 5000,
                "benefits": ["1.5x points earning", "Birthday bonus", "Free delivery"],
            },
            "platinum": {
                "min": 10000,
                "benefits": [
            ...
            result.update(
                {
                    ...
                    "next_tier": next_tier,
                    "progress_percentage": min(
                        100, (lifetime_points / tiers[next_tier]["min"]) * 100
                    ),
                }
            )
    ...
    def _check_tier_upgrade(self, customer_id: int, new_balance: int):
        ...
        if new_tier != old_tier:
            ...
            logger.info(
                f"Customer {customer_id} upgraded from {old_tier} to {new_tier}"
            )
    ...
    def _validate_reward_template(self, template_data: RewardTemplateCreate):
        ...
        if template_data.reward_type == RewardType.PERCENTAGE_DISCOUNT:
            if not template_data.percentage or template_data.percentage <= 0:
                raise APIValidationError(
                    "Percentage discount requires valid percentage"
                )
    ...
    def _find_customers_by_criteria(self, criteria: Dict[str, Any]) -> List[int]:
        """Find customers matching criteria"""
        # TODO: Implement customer search based on criteria
        # For now, return empty list
        return []

    def _check_reward_triggers(self, order_data: OrderCompletionReward) -> List[int]:
