...
from ..schemas.rewards_schemas import (
    ...
    CustomerReward, CustomerRewardSummary, RewardSearchParams, RewardSearchResponse,
    RewardCampaign, RewardCampaignCreate, RewardCampaignUpdate,
    RewardRedemptionRequest, RewardRedemptionResponse,
    ManualRewardIssuance, BulkRewardIssuance, BulkRewardIssuanceResponse,
    OrderCompletionReward, OrderCompletionResponse,
    RewardAnalytics, CustomerLoyaltyStats, LoyaltyPointsTransaction
)
...
async def list_reward_templates(
    ...
    templates = query.order_by(RewardTemplateModel.priority.desc(), RewardTemplateModel.created_at.desc())\
...
async def get_reward_template(
    ...
    template = db.query(RewardTemplateModel).filter(RewardTemplateModel.id == template_id).first()
    if not template:
...
# Customer Rewards
@router.get("/customer/{customer_id}/rewards", response_model=List[CustomerRewardSummary])
async def get_customer_rewards(
    ...
    try:
        ...
        if valid_only and not status:
            ...
            query = db.query(CustomerRewardModel).filter(CustomerRewardModel.customer_id == customer_id)
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve customer rewards")
...
async def issue_reward_manually(
    ...
    try:
        ...
        reward = rewards_engine.issue_reward_to_customer(
            ...
            custom_data={"message": issuance_data.custom_message} if issuance_data.custom_message else None
        )
...
async def issue_rewards_bulk(
    ...
    def process_bulk_issuance():
        ...
        for customer_id in bulk_issuance.customer_ids:
            try:
                reward = rewards_engine.issue_reward_to_customer(
                    ...
                    issued_by=current_user.id,
                    custom_data={"message": bulk_issuance.custom_message} if bulk_issuance.custom_message else None
                )
                successful += 1
        ...
        logger.info(f"Bulk reward issuance completed: {successful} successful, {failed} failed")
...
async def process_order_completion(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to process order completion")
...
@router.get("/customer/{customer_id}/loyalty-stats", response_model=CustomerLoyaltyStats)
async def get_customer_loyalty_stats(
    ...
    total_rewards = db.query(CustomerRewardModel).filter(CustomerRewardModel.customer_id == customer_id).count()
    redeemed_rewards = db.query(CustomerRewardModel).filter(
    ...
    total_discount = db.query(RewardRedemption).filter(
        RewardRedemption.customer_id == customer_id
    ).with_entities(db.func.sum(RewardRedemption.discount_applied)).scalar() or 0
    ...
    points_earned_this_month = db.query(LPTransaction).filter(
        LPTransaction.customer_id == customer_id,
        LPTransaction.transaction_type == "earned",
        LPTransaction.created_at >= month_start
    ).with_entities(db.func.sum(LPTransaction.points_change)).scalar() or 0
    
    points_redeemed_this_month = abs(db.query(LPTransaction).filter(
        LPTransaction.customer_id == customer_id,
