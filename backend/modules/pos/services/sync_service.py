...
class SyncService:
    ...
    async def coordinate_pos_platform_sync(self, tenant_id: Optional[int] = None, team_id: Optional[int] = None):
        if not self.is_sync_enabled(tenant_id, team_id):
            logger.info(f"POS platform sync skipped for tenant {tenant_id}, team {team_id} - sync disabled")
            return False
        
        logger.info(f"Coordinating POS platform sync for tenant {tenant_id}, team {team_id}")
        return True
