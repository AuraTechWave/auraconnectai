...
class SyncScheduler:
    ...
    async def trigger_scheduled_sync(self, tenant_id: Optional[int] = None, team_id: Optional[int] = None):
        if not self.is_sync_enabled(tenant_id, team_id):
            logger.info(f"Scheduled sync skipped for tenant {tenant_id}, team {team_id} - sync disabled")
            return False
        
        logger.info(f"Triggering scheduled sync for tenant {tenant_id}, team {team_id}")
        return True
