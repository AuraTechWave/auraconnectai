...
def setup_test_data(db: Session):
    ...
    # Recipes
    pizza_recipe = Recipe(
        id=1, menu_item_id=1, name="Pizza Recipe", status="active", created_by=1
    )
    pasta_recipe = Recipe(
        id=2, menu_item_id=2, name="Pasta Recipe", status="active", created_by=1
    )
    ...
    ingredients = [
        RecipeIngredient(
            recipe_id=1, inventory_id=1, quantity=0.3, unit="kg", created_by=1
        ),
        RecipeIngredient(
            recipe_id=1, inventory_id=2, quantity=0.2, unit="kg", created_by=1
        ),
        RecipeIngredient(
            recipe_id=1, inventory_id=3, quantity=0.15, unit="kg", created_by=1
        ),
        RecipeIngredient(
            recipe_id=2, inventory_id=1, quantity=0.2, unit="kg", created_by=1
        ),
        RecipeIngredient(
            recipe_id=2, inventory_id=2, quantity=0.1, unit="kg", created_by=1
        ),
    ]
...
class TestInventoryImpactAPI:
    ...
    @pytest.mark.api
    def test_preview_order_inventory_impact(
        self, client: TestClient, db: Session, auth_headers, setup_test_data
    ):
        """Test previewing inventory impact for an order."""
        # Create an order
        order = Order(
            id=1, order_number="TEST-001", status=OrderStatus.PENDING, total_amount=20.0
        )
        db.add(order)
        ...
        # Preview impact
        response = client.get(
            f"/inventory-impact/order/{order.id}/preview", headers=auth_headers
        )
    ...
    @pytest.mark.api
    def test_preview_items_inventory_impact(
        self, client: TestClient, auth_headers, setup_test_data
    ):
        ...
        response = client.post(
            "/inventory-impact/preview", json=items_data, headers=auth_headers
        )
    ...
    def test_partial_fulfillment_endpoint(
        ...
        fulfillment_data = [{"menu_item_id": 1, "fulfilled_quantity": 2}]
        ...
        flour_deduction = next(
            d for d in data["deducted_items"] if d["inventory_id"] == 1
        )
        assert flour_deduction["quantity_deducted"] == 0.6  # 0.3 * 2

    @pytest.mark.api
    def test_reverse_deduction_endpoint(
        self, client: TestClient, db: Session, auth_headers, setup_test_data
    ):
    ...
    @pytest.mark.api
    def test_insufficient_stock_preview(
        self, client: TestClient, db: Session, auth_headers, setup_test_data
    ):
        ...
        items_data = [{"menu_item_id": 1, "quantity": 1}]  # Pizza needs 0.15kg cheese
        ...
        cheese_impact = next(
            i for i in data["impact_preview"] if i["inventory_id"] == 3
        )
    ...
    def test_unauthorized_access(self, client: TestClient, setup_test_data):
        ...
        response = client.post(
            "/inventory-impact/order/1/reverse-deduction", json={"reason": "test"}
        )
        assert response.status_code == 401
    ...
    @pytest.mark.api
    def test_low_stock_detection_in_preview(
        self, client: TestClient, db: Session, auth_headers, setup_test_data
    ):
        ...
        items_data = [{"menu_item_id": 1, "quantity": 2}]  # Will use 0.6kg flour
    ...
    @pytest.mark.api
    def test_complex_order_modification_flow(
        self, client: TestClient, db: Session, auth_headers, setup_test_data
    ):
        ...
        response = client.get(
            f"/inventory-impact/order/10/preview", headers=auth_headers
        )
        assert response.status_code == 200
        ...
        flour_impact = next(
            i for i in updated_data["impact_preview"] if i["inventory_id"] == 1
        )
