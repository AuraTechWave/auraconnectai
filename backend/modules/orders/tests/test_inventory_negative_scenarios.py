...
class TestNegativeScenarios:
    ...
    async def test_circular_dependency_deadlock(self, db_engine):
        ...
        for i in range(10):
            order_data = {
                "order_num": i + 1,
                "menu_item_id": menu_item_ids[i % len(menu_item_ids)],  # Rotate through recipes
                "user": test_user
            }
    ...
    async def test_race_condition_double_deduction(self, db_engine):
        ...
        with db_engine.connect() as conn:
            session = Session(bind=conn)
            final_inventory = session.query(Inventory).filter(Inventory.id == inventory_id).first()
    ...
    async def test_transaction_rollback_on_partial_failure(self, db_engine):
        ...
        with db_engine.connect() as conn:
            ...
            sufficient_check = session.query(Inventory).filter(Inventory.id == sufficient_item_id).first()
