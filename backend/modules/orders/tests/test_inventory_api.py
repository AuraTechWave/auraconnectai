...
class TestInventoryAPI:
    ...
    def test_get_inventory_with_items(self, client, db_session):
        inventory1 = Inventory(
            item_name="Item 1", quantity=10.0, unit="kg", threshold=5.0
        )
        inventory2 = Inventory(
            item_name="Item 2", quantity=20.0, unit="pieces", threshold=10.0
        )
    ...
    def test_get_inventory_by_id_soft_deleted(
        self, client, db_session, sample_inventory
    ):
    ...
    def test_update_inventory_success(self, client, sample_inventory):
        update_data = {"quantity": 25.0, "threshold": 8.0}
    ...
    def test_get_low_stock_alerts(self, client, db_session):
        inventory1 = Inventory(
            item_name="Low Stock Item", quantity=3.0, unit="kg", threshold=5.0
        )
        inventory2 = Inventory(
            item_name="Normal Stock Item", quantity=15.0, unit="kg", threshold=5.0
        )
        db_session.add_all([inventory1, inventory2])
        db_session.commit()
