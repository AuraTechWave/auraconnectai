...
class TestConcurrentInventoryDeduction:
    ...
    def create_test_data(self, db_engine):
        ...
        def _create_data(session):
            ...
            # Create menu item
            menu_item = MenuItem(id=1, name="Test Product", price=10.0, is_active=True)
            session.add(menu_item)

            # Create recipe
            recipe = Recipe(
                id=1, menu_item_id=1, name="Test Recipe", status="active", created_by=1
            )
            session.add(recipe)
            session.flush()
            ...
            with db_engine.connect() as conn:
                ...
                try:
                    ...
                    result = await service.deduct_inventory_for_order(
                        order_items=[order_item], order_id=order_id, user_id=1
                    )

                    with lock:
                        results.append(
                            {"order_id": order_id, "success": True, "result": result}
                        )

                except HTTPException as e:
                    with lock:
                        errors.append({"order_id": order_id, "error": str(e.detail)})
    ...
    async def test_concurrent_order_and_reversal(self, db_engine, create_test_data):
        ...
        with db_engine.connect() as conn:
            ...
            order_item1 = OrderItem(order_id=1, menu_item_id=1, quantity=1, price=10.0)
            session.add(order_item1)
            session.commit()

            # Deduct inventory for first order
            await service.deduct_inventory_for_order(
                order_items=[order_item1], order_id=1, user_id=1
            )
            ...
            with db_engine.connect() as conn:
                ...
                try:
                    result = await service.reverse_inventory_deduction(
                        order_id=1, user_id=1, reason="Concurrent test"
                    )
        ...
        async def create_new_order():
            ...
            with db_engine.connect() as conn:
                ...
                try:
                    order2 = Order(
                        id=2, order_number="ORD-002", status=OrderStatus.PENDING
                    )
                    session.add(order2)

                    order_item2 = OrderItem(
                        order_id=2, menu_item_id=1, quantity=1, price=10.0
                    )
                    session.add(order_item2)
                    session.commit()

                    result = await service.deduct_inventory_for_order(
                        order_items=[order_item2], order_id=2, user_id=1
                    )
        ...
        # Run concurrently
        await asyncio.gather(reverse_first_order(), create_new_order())
        ...
        with db_engine.connect() as conn:
            ...
            adjustments = (
                ...
                .order_by(InventoryAdjustment.created_at)
                .all()
            )
    ...
    async def test_deadlock_prevention(self, db_engine, create_test_data):
        ...
        with db_engine.connect() as conn:
            ...
            for i in range(1, 3):
                menu_item = MenuItem(
                    id=i, name=f"Product {i}", price=20.0, is_active=True
                )
                session.add(menu_item)
