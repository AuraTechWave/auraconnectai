...
class TestDatabaseRetry:
    ...
    async def test_retry_on_deadlock_max_retries_exceeded(self):
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock):
            with pytest.raises(OperationalError):
                await retry_on_deadlock(mock_func, max_retries=2, initial_delay=0.01)
    ...
    async def test_retry_exponential_backoff(self):
        ...
        mock_func = AsyncMock(side_effect=[deadlock_error, deadlock_error, "success"])
    ...
    async def test_retry_with_jitter(self):
        ...
        with patch("asyncio.sleep", side_effect=mock_sleep):
            result = await retry_on_deadlock(mock_func, initial_delay=0.1, jitter=True)
...
class TestDatabaseRetryIntegration:
    ...
    async def test_concurrent_inventory_update_with_retry(self, db):
        ...
        tasks = [update_inventory(10.0), update_inventory(15.0), update_inventory(5.0)]
