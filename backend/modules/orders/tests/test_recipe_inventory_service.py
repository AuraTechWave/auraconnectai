...
class TestRecipeInventoryService:
    ...
    async def test_calculate_required_ingredients(self, db_session, sample_order, sample_recipes):
        """Test calculation of required ingredients including sub-recipes."""
        service = RecipeInventoryService(db_session)
        
        # Calculate required ingredients
        required = await service._calculate_required_ingredients(sample_order.order_items)
    ...
    async def test_successful_inventory_deduction(self, db_session, sample_order, sample_recipes, sample_inventory_items):
        ...
        adjustments = db_session.query(InventoryAdjustment).filter(
    ...
    async def test_insufficient_inventory(self, db_session, sample_order, sample_recipes, sample_inventory_items):
    ...
    async def test_low_stock_alerts(self, db_session, sample_order, sample_recipes, sample_inventory_items):
    ...
    async def test_inventory_reversal(self, db_session, sample_order, sample_recipes, sample_inventory_items):
        ...
        reversals = db_session.query(InventoryAdjustment).filter(
    ...
    async def test_partial_fulfillment(self, db_session, sample_recipes, sample_inventory_items):
        """Test handling partial order fulfillment."""
        service = RecipeInventoryService(db_session)
        
        # Partial fulfillment data (1 pizza instead of 2)
        partial_items = [
            {"menu_item_id": 1, "fulfilled_quantity": 1}
        ]
    ...
    async def test_inventory_impact_preview(self, db_session, sample_order, sample_recipes, sample_inventory_items):
