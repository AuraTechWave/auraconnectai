...
from ..schemas.order_split_schemas import (
    OrderSplitRequest, OrderItemSplitRequest, SplitType,
    PaymentSplitRequest, PaymentStatus, MergeSplitRequest
)
...
class TestOrderSplitValidation:
    ...
    def test_validate_split_request_success(self, mock_db, sample_order):
        ...
        # Setup
        mock_db.query.return_value.options.return_value.filter.return_value.first.return_value = sample_order
...
class TestSplitTracking:
    ...
    def test_update_split_payment(self, mock_db):
        ...
        # Test
        result = service.update_split_payment(
            1, PaymentStatus.PAID, "REF123", "card"
        )
...
class TestMergeOperations:
    ...
    def test_merge_split_orders(self, mock_db):
        ...
        mock_db.query.return_value.filter.return_value.all.return_value = [split1, split2]
...
class TestTrackingFeatures:
    ...
    def test_get_split_tracking(self, mock_db, sample_order):
        ...
        mock_db.query.return_value.options.return_value.filter.return_value.all.return_value = [split]
        mock_db.query.return_value.filter.return_value.first.return_value = None  # No payment
