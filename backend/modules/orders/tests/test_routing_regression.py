...
def mock_kds_routing(monkeypatch):
    ...
    mock_service = Mock()
    mock_service.route_order_to_stations = Mock(return_value=[
        Mock(station_id=1, item_id=1),
        Mock(station_id=2, item_id=2)
    ])
...
class TestOrderCreationRegression:
    ...
    async def test_order_reaches_kds_when_no_rules_match(
        ...
        from modules.orders.models.routing_models import RoutingRuleCondition, RuleConditionOperator
    ...
    async def test_order_creation_continues_if_routing_fails(
        ...
        with patch('modules.orders.services.routing_rule_service.RoutingRuleService.evaluate_order_routing') as mock_eval:
...
class TestPriorityConflicts:
    ...
    def test_first_rule_wins_behavior(self, db_session):
        ...
        from modules.orders.models.routing_models import (
            RoutingRuleCondition, RoutingRuleAction, RuleConditionOperator
        )
