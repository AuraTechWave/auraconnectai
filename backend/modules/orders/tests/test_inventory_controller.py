...
class TestInventoryController:
    ...
    @pytest.mark.asyncio
    @patch('backend.modules.orders.controllers.inventory_controller.'
           'get_inventory_service')
    async def test_get_inventory_by_id_delegates_to_service(
            self, mock_service, db_session):
    ...
    @pytest.mark.asyncio
    @patch('backend.modules.orders.controllers.inventory_controller.'
           'check_low_stock_service')
    async def test_check_low_stock_delegates_to_service(
            self, mock_service, db_session):
        mock_alerts = [{"item_name": "Test Item",
    ...
    async def test_list_inventory_delegates_to_service(
            self, mock_service, db_session):
        ...
        assert len(result) == 2
        assert all(isinstance(item, InventoryOut)
                   for item in result)

    @pytest.mark.asyncio
    @patch('backend.modules.orders.controllers.inventory_controller.'
           'update_inventory_service')
    async def test_update_inventory_delegates_to_service(
            self, mock_service, db_session):
