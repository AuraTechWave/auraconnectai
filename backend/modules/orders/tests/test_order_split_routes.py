...
from ..schemas.order_split_schemas import (
    SplitType, PaymentStatus, SplitValidationResponse,
    OrderSplitResponse, SplitOrderSummary
)
...
class TestOrderSplitValidation:
    ...
    def test_validate_split_success(
        self, mock_service_class, mock_get_db, mock_get_user,
        client, auth_headers, mock_current_user
    ):
        ...
        mock_validation = SplitValidationResponse(
            ...
            reason=None,
            splittable_items=[
                {"item_id": 1, "quantity": 2, "unit_price": 25.00}
            ],
            warnings=[],
            estimated_totals={
        ...
        response = client.post(
            "/api/v1/orders/1/split/validate",
            json=split_data,
...
class TestTableSplits:
    ...
    def test_get_table_splits(
        ...
        # Test
        response = client.get(
            "/api/v1/orders/splits/by-table/5",
            headers=auth_headers
        )
...
class TestTrackingEndpoints:
    ...
    def test_get_split_tracking(
        ...
        # Test
        response = client.get(
            "/api/v1/orders/1/splits/tracking",
            headers=auth_headers
        )
