...
class TestOrderTrackingService:
    ...
    @pytest.mark.asyncio
    async def test_create_tracking_event_with_location(
        self, tracking_service, test_order
    ):
        """Test creating a tracking event with location data"""
        location_data = {"latitude": 40.7128, "longitude": -74.0060, "accuracy": 10.5}
    ...
    def test_create_customer_tracking(self, tracking_service, test_order, db):
        ...
        tracking2 = tracking_service.create_customer_tracking(
            order_id=another_order.id, customer_id=another_order.customer_id
        )
    ...
    def test_get_order_tracking_by_code(self, tracking_service, test_order, db):
        ...
        # Create tracking
        tracking = tracking_service.create_customer_tracking(
            order_id=test_order.id, customer_id=test_order.customer_id
        )
        ...
        # Get tracking info
        tracking_info = tracking_service.get_order_tracking_by_code(
    ...
    async def test_send_event_notifications(self, tracking_service, test_order, db):
        ...
        with patch.object(
            tracking_service.notification_adapter, "send_to_user"
        ) as mock_send:
        ...
        notification = (
            ...
            .filter(OrderNotification.order_id == test_order.id)
            .first()
        )
    ...
    def test_get_active_orders_for_customer(self, tracking_service, db):
        ...
        active_order1 = OrderFactory(
            customer_id=customer.id, status=OrderStatus.IN_PROGRESS
        )
        active_order2 = OrderFactory(
            customer_id=customer.id, status=OrderStatus.IN_KITCHEN
        )
        completed_order = OrderFactory(
            customer_id=customer.id, status=OrderStatus.COMPLETED
        )
        cancelled_order = OrderFactory(
            customer_id=customer.id, status=OrderStatus.CANCELLED
        )
        db.commit()
        ...
        active_orders = tracking_service.get_active_orders_for_customer(
            customer_id=customer.id, include_completed=False
        )
        ...
        all_orders = tracking_service.get_active_orders_for_customer(
            customer_id=customer.id, include_completed=True
        )
    ...
    async def test_websocket_integration(self, tracking_service, test_order, db):
        ...
        # Create tracking
        tracking = tracking_service.create_customer_tracking(order_id=test_order.id)
...
class TestOrderTrackingAPI:
    ...
    def test_get_order_by_tracking_code(self, client, test_customer_order, db):
        ...
        # Get tracking info
        response = client.get(f"/api/customer/tracking/track/{tracking.tracking_code}")
    ...
    def test_update_notification_preferences(self, client, test_customer_order, db):
        ...
        # Enable tracking
        tracking_service = OrderTrackingService(db)
        tracking = tracking_service.create_customer_tracking(order_id=order.id)
...
class TestPushNotificationService:
    ...
    async def test_send_order_push_notification(self):
        ...
        # Mock configuration
        config = {"fcm_server_key": "test-key", "environment": "development"}
