...
from modules.orders.models.order_models import Order, OrderItem, OrderStatus, OrderSplit, SplitPayment
from modules.orders.schemas.order_split_schemas import (
    TicketSplitRequest, DeliverySplitRequest, PaymentSplitRequest,
    PaymentSplitDetail, MergeSplitsRequest
)
...
class TestPaymentSplitMath:
    ...
    def test_payment_split_total_must_equal_order_total(self, split_service, db_session, sample_order_with_items):
    ...
    def test_payment_split_with_exact_amounts(self, split_service, db_session, sample_order_with_items):
        ...
        # Verify totals
        total_amount = sum(Decimal(str(s["final_amount"])) for s in result["split_orders"])
        assert total_amount == order.final_amount
        
        # Check split payments
        split_payments = db_session.query(SplitPayment).filter(
            SplitPayment.parent_order_id == order.id
        ).all()
    ...
    def test_payment_split_with_rounding(self, split_service, db_session):
        ...
        request = PaymentSplitRequest(
            splits=[
                PaymentSplitDetail(
                    name="Person 1",
                    amount=Decimal("36.11")  # 108.33 / 3 = 36.11
                ),
                PaymentSplitDetail(
                    name="Person 2",
                    amount=Decimal("36.11")
                ),
                PaymentSplitDetail(
                    name="Person 3",
                    amount=Decimal("36.11")
    ...
    def test_payment_split_updates_statuses(self, split_service, db_session, sample_order_with_items):
...
class TestMergeSplits:
    ...
    def test_merge_restores_correct_totals(self, split_service, db_session, sample_order_with_items):
    ...
    def test_cannot_merge_paid_splits(self, split_service, db_session, sample_order_with_items):
        ...
        merge_request = MergeSplitsRequest(
            split_order_ids=split_ids,
            merge_reason="Trying to merge paid order"
        )
...
class TestSplitStatusTransitions:
    ...
    def test_split_creates_audit_records(self, split_service, db_session, sample_order_with_items):
        ...
        splits = db_session.query(OrderSplit).filter(
    ...
    def test_payment_tracking_accuracy(self, split_service, db_session, sample_order_with_items):
        ...
        payments = db_session.query(SplitPayment).filter(
            SplitPayment.parent_order_id == order.id
        ).all()
...
class TestEdgeCases:
    ...
    def test_split_order_with_zero_items(self, split_service, db_session):
        ...
        request = TicketSplitRequest(
            splits=[{"name": "Empty split", "item_ids": []}]
        )
