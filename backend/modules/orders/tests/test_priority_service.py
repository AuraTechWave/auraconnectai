...
def sample_priority_rule():
    ...
    rule = PriorityRule(
        ...
        is_active=True,
        score_config={"type": "linear", "base_score": 10, "multiplier": 2},
        min_score=0,
        max_score=100,
        default_weight=1.0,
    )
...
class TestPriorityCalculation:
    ...
    def test_calculate_weighted_priority(
        ...
        mock_profile_query.filter.return_value.first.return_value = (
            sample_queue_config.profile
        )
...
class TestQueueRebalancing:
    ...
    def test_rebalance_not_needed(
        ...
        sample_queue_config.last_rebalance_time = datetime.utcnow() - timedelta(
            minutes=1
        )
        ...
        with patch.object(
            priority_service, "_calculate_fairness_index", return_value=0.9
        ):
    ...
    def test_rebalance_queue_success(
        ...
        with patch.object(
            priority_service, "_calculate_fairness_index", side_effect=[0.3, 0.8]
        ):
...
class TestPriorityAdjustments:
    ...
    def test_adjust_priority_boost(
        ...
        existing_score = OrderPriorityScore(
            queue_item_id=1, total_score=30.0, base_score=30.0, boost_score=0.0
        )
...
class TestPriorityComponents:
    ...
    def test_calculate_wait_time_priority(self, priority_service):
        ...
        queue_item = QueueItem(queued_at=datetime.utcnow() - timedelta(minutes=15))
    ...
    def test_calculate_order_value_priority(self, priority_service, sample_order):
        ...
        rule = PriorityRule(
            score_config={"type": "linear", "min_value": 0, "max_value": 100},
            min_score=0,
            max_score=100,
        )
    ...
    def test_calculate_vip_priority(self, priority_service, db_session, sample_order):
        ...
        vip_customer = Customer(id=1, is_vip=True, loyalty_tier="gold", order_count=50)
    ...
    def test_calculate_delivery_time_priority(self, priority_service):
        ...
        order = Order(promised_delivery_time=datetime.utcnow() + timedelta(minutes=30))

        rule = PriorityRule(
            score_config={"type": "inverse_linear", "max_minutes": 120},
            min_score=0,
            max_score=100,
        )
...
class TestQueueSequencing:
    ...
    def test_get_queue_priority_sequence(self, priority_service, db_session):
        ...
        for i in range(3):
            items.append(
                (
                    ...
                    OrderPriorityScore(
                        total_score=float(30 - i * 10), is_boosted=False
                    ),
        ...
        mock_join = Mock()
        mock_join.join.return_value.join.return_value.filter.return_value.order_by.return_value.all.return_value = (
            items
        )
...
class TestEnumUsage:
    ...
    def test_enum_comparisons_in_queries(self, priority_service, db_session):
        ...
        with patch.object(
            priority_service, "_calculate_fairness_index"
        ) as mock_fairness:
