...
class TestPricingRuleEdgeCases:
    ...
    def overlapping_percentage_rules(self):
        ...
        rule1.conditions = {"min_order_amount": 40.00, "discount_percentage": 15}
        ...
        rule2.conditions = {"min_order_amount": 45.00, "discount_percentage": 20}
        ...
        rule3.conditions = {"min_order_amount": 30.00, "discount_percentage": 10}
    ...
    def mixed_type_rules(self):
        ...
        rule1.excluded_rule_ids = []
        rule1.conditions = {"min_order_amount": 25.00, "discount_amount": 5.00}
        rule1.valid_from = datetime.utcnow() - timedelta(days=1)
        rule1.valid_until = datetime.utcnow() + timedelta(days=30)
        rule1.is_valid = Mock(return_value=True)
        rules.append(rule1)
        ...
        rule3.excluded_rule_ids = []
        rule3.conditions = {"discount_percentage": 15}
        rule3.valid_from = datetime.utcnow() - timedelta(days=1)
        rule3.valid_until = datetime.utcnow() + timedelta(days=30)
        rule3.is_valid = Mock(return_value=True)
        rules.append(rule3)
    ...
    def time_sensitive_rules(self):
        ...
        rule2.stackable = False
        rule2.excluded_rule_ids = []
        rule2.conditions = {"discount_percentage": 30, "valid_days": ["SAT", "SUN"]}
    ...
    async def test_highest_discount_conflict_resolution(
        ...
        final_results = await pricing_service._resolve_conflicts(
            evaluation_results, sample_order
        )
    ...
    async def test_priority_based_conflict_resolution(
        ...
        sample_order.restaurant.default_conflict_resolution = (
    ...
    @pytest.mark.asyncio
    async def test_excluded_rules_enforcement(
    ...
    @pytest.mark.asyncio
    async def test_time_boundary_edge_cases(
        self, pricing_service, mock_db, sample_order
    ):
        ...
        applicable_rules = await pricing_service._get_applicable_rules(
            mock_db, sample_order
        )
    ...
    @pytest.mark.asyncio
    async def test_zero_discount_edge_case(
        self, pricing_service, mock_db, sample_order
    ):
    ...
    @pytest.mark.asyncio
    async def test_negative_discount_protection(
        self, pricing_service, mock_db, sample_order
    ):
    ...
    @pytest.mark.asyncio
    async def test_circular_exclusion_protection(
        self, pricing_service, mock_db, sample_order
    ):
    ...
    async def test_complex_multi_rule_scenario(
        ...
        total_discount = sum(r.discount_amount for r in final_results)
        expected_total = (
            Decimal("10.00") + Decimal("3.00") + Decimal("2.00") + Decimal("12.50")
        )
        assert total_discount == expected_total

        # Verify the non-stackable rule with highest discount was selected
        percentage_rules = [
            r for r in final_results if r.rule_name in ["20% Off", "15% Off"]
        ]
