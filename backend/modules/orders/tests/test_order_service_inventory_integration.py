...
def setup_order_test_data(db: Session):
    ...
    # Inventory
    ingredients = [
        Inventory(id=1, item_name="Base", quantity=100.0, unit="unit", threshold=20.0, is_active=True),
        Inventory(id=2, item_name="Topping A", quantity=50.0, unit="unit", threshold=10.0, is_active=True),
        Inventory(id=3, item_name="Topping B", quantity=30.0, unit="unit", threshold=5.0, is_active=True),
    ]
    ...
    recipes = [
        ...
        Recipe(id=3, menu_item_id=3, name="Combo Recipe", status="active", created_by=1),
    ]
    db.add_all(recipes)
    db.flush()
    ...
    recipe_ingredients = [
        ...
        RecipeIngredient(recipe_id=1, inventory_id=1, quantity=1.0, unit="unit", created_by=1),
        RecipeIngredient(recipe_id=1, inventory_id=2, quantity=0.5, unit="unit", created_by=1),
        
        # Product B uses Base + Topping B
        RecipeIngredient(recipe_id=2, inventory_id=1, quantity=1.0, unit="unit", created_by=1),
        RecipeIngredient(recipe_id=2, inventory_id=3, quantity=0.3, unit="unit", created_by=1),
        
        # Combo uses less base directly
        RecipeIngredient(recipe_id=3, inventory_id=1, quantity=0.5, unit="unit", created_by=1),
    ]
    db.add_all(recipe_ingredients)
    
    # Combo includes sub-recipes (Product A + Product B)
    sub_recipes = [
        RecipeSubRecipe(parent_recipe_id=3, sub_recipe_id=1, quantity=1.0, created_by=1),
        RecipeSubRecipe(parent_recipe_id=3, sub_recipe_id=2, quantity=1.0, created_by=1),
    ]
    db.add_all(sub_recipes)
    
    db.commit()
    
    return {"user": user, "ingredients": ingredients, "items": items, "recipes": recipes}
...
class TestOrderServiceInventoryIntegration:
    ...
    async def test_order_status_change_triggers_deduction(self, mock_get_config, mock_config, db: Session, setup_order_test_data):
        ...
        initial_topping_a = db.query(Inventory).filter(Inventory.id == 2).first().quantity
    ...
    @patch('modules.orders.services.order_service.get_inventory_config')
    async def test_order_cancellation_reverses_inventory(self, mock_get_config, mock_config, db: Session, setup_order_test_data):
        ...
        # Process order
        await update_order_service(2, OrderUpdate(status=OrderStatus.IN_PROGRESS), db, user.id)
        
        # Get quantities after deduction
        after_deduction_base = db.query(Inventory).filter(Inventory.id == 1).first().quantity
        after_deduction_topping_b = db.query(Inventory).filter(Inventory.id == 3).first().quantity
        
        # Cancel order
        await update_order_service(2, OrderUpdate(status=OrderStatus.CANCELLED), db, user.id)
        ...
        reversals = db.query(InventoryAdjustment).filter(
            InventoryAdjustment.adjustment_type == AdjustmentType.RETURN
        ).all()
        assert len(reversals) == 2
    
    @patch('modules.orders.services.order_service.get_inventory_config')
    async def test_deduct_on_completion_mode(self, mock_get_config, db: Session, setup_order_test_data):
        ...
        for status in [OrderStatus.IN_PROGRESS, OrderStatus.READY, OrderStatus.SERVED]:
            ...
            db.refresh(db.query(Inventory).filter(Inventory.id == 1).first())
            assert db.query(Inventory).filter(Inventory.id == 1).first().quantity == initial_base
        
        # Complete order - NOW it should deduct
        await update_order_service(3, OrderUpdate(status=OrderStatus.COMPLETED), db, user.id)
    ...
    @patch('modules.orders.services.order_service.get_inventory_config')
    async def test_sub_recipe_deduction(self, mock_get_config, mock_config, db: Session, setup_order_test_data):
        ...
        initial_base = db.query(Inventory).filter(Inventory.id == 1).first().quantity
        initial_topping_a = db.query(Inventory).filter(Inventory.id == 2).first().quantity
        initial_topping_b = db.query(Inventory).filter(Inventory.id == 3).first().quantity
        
        # Process order
        await update_order_service(4, OrderUpdate(status=OrderStatus.IN_PROGRESS), db, user.id)
        ...
        assert db.query(Inventory).filter(Inventory.id == 1).first().quantity == initial_base - 5.0
        assert db.query(Inventory).filter(Inventory.id == 2).first().quantity == initial_topping_a - 1.0
        assert db.query(Inventory).filter(Inventory.id == 3).first().quantity == initial_topping_b - 0.6
    
    @patch('modules.orders.services.order_service.get_inventory_config')
    async def test_insufficient_inventory_blocks_order(self, mock_get_config, mock_config, db: Session, setup_order_test_data):
        ...
        with pytest.raises(Exception) as exc_info:
            await update_order_service(5, OrderUpdate(status=OrderStatus.IN_PROGRESS), db, user.id)
    ...
    async def test_low_stock_warning_logged(self, mock_logger, mock_get_config, mock_config, db: Session, setup_order_test_data):
        ...
        # Process order
        await update_order_service(6, OrderUpdate(status=OrderStatus.IN_PROGRESS), db, user.id)
    ...
    async def test_order_workflow_complete_flow(self, db: Session, setup_order_test_data):
        ...
        adjustments = db.query(InventoryAdjustment).filter(
            InventoryAdjustment.reference_id == "7"
        ).all()
