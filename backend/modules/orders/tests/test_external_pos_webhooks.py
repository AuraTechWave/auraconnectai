...
class TestWebhookAuthentication:
    ...
    async def test_verify_square_webhook_valid(self, mock_db, square_provider):
        ...
        signature = base64.b64encode(
            hmac.new(
                b"test_square_secret", string_to_sign.encode(), hashlib.sha256
            ).digest()
        ).decode()
    ...
    async def test_verify_stripe_webhook_valid(self, mock_db, stripe_provider):
        ...
        signature = hmac.new(
            b"whsec_test_secret", signed_payload.encode(), hashlib.sha256
        ).hexdigest()
        ...
        with patch(
            "backend.modules.orders.services.webhook_auth_service.datetime"
        ) as mock_datetime:
    ...
    def test_generate_test_signature(self, mock_db, stripe_provider):
        ...
        headers = auth_service.generate_test_signature(
            provider_code="stripe", body=test_body, timestamp=1234567890
        )
...
class TestWebhookEndpoints:
    ...
    def test_receive_webhook_unknown_provider(self, client):
        ...
        response = client.post(
            "/api/webhooks/external-pos/unknown_provider/events", json={"test": "data"}
        )
    ...
    async def test_receive_webhook_success(self, client):
        ...
        with patch(
            "backend.modules.orders.routers.external_pos_webhook_router.get_db"
        ) as mock_get_db:
            mock_db = Mock()
            mock_provider = Mock(id=1, provider_code="square", is_active=True)
            ...
            with patch(
                "backend.modules.orders.routers.external_pos_webhook_router.WebhookAuthService"
            ) as mock_auth:
    ...
    def test_get_provider_status(self, client, mock_auth):
        ...
        with patch(
            ...
            mock_db = Mock()
            mock_provider = Mock(
                provider_code="square", provider_name="Square", is_active=True
            )
            mock_db.query().filter().first.return_value = mock_provider
            ...
            "data": {
                "object": {
                    "payment": {
                        ...
                        "status": "COMPLETED",
                        "card_details": {
                            "card": {"last_4": "1234", "card_brand": "VISA"}
                        },
                    }
                }
            }
