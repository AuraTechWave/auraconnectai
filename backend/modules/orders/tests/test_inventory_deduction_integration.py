...
def setup_restaurant_data(db: Session):
    ...
    # Create category
    category = Category(
        id=1, name="Pizzas", description="Our delicious pizzas", is_active=True
    )
    db.add(category)
    ...
    margherita_ingredients = [
        RecipeIngredient(
            recipe_id=1, inventory_id=1, quantity=1, unit="piece", created_by=1
        ),  # Dough
        RecipeIngredient(
            recipe_id=1, inventory_id=2, quantity=0.15, unit="liter", created_by=1
        ),  # Sauce
        RecipeIngredient(
            recipe_id=1, inventory_id=3, quantity=0.2, unit="kg", created_by=1
        ),  # Cheese
        RecipeIngredient(
            recipe_id=1, inventory_id=5, quantity=0.01, unit="kg", created_by=1
        ),  # Basil
    ]
    ...
    pepperoni_ingredients = [
        RecipeIngredient(
            recipe_id=2, inventory_id=1, quantity=1, unit="piece", created_by=1
        ),  # Dough
        RecipeIngredient(
            recipe_id=2, inventory_id=2, quantity=0.15, unit="liter", created_by=1
        ),  # Sauce
        RecipeIngredient(
            recipe_id=2, inventory_id=3, quantity=0.25, unit="kg", created_by=1
        ),  # More cheese
        RecipeIngredient(
            recipe_id=2, inventory_id=4, quantity=0.1, unit="kg", created_by=1
        ),  # Pepperoni
    ]
    ...
    garlic_bread_ingredients = [
        RecipeIngredient(
            recipe_id=3, inventory_id=1, quantity=0.5, unit="piece", created_by=1
        ),  # Half dough
        RecipeIngredient(
            recipe_id=3, inventory_id=6, quantity=0.05, unit="liter", created_by=1
        ),  # Olive oil
        RecipeIngredient(
            recipe_id=3, inventory_id=5, quantity=0.005, unit="kg", created_by=1
        ),  # Basil
    ]

    db.add_all(
        margherita_ingredients + pepperoni_ingredients + garlic_bread_ingredients
    )
    db.commit()
...
class TestOrderInventoryIntegration:
    ...
    @pytest.mark.slow
    async def test_place_order_verify_ingredient_deduction(
        self, db: Session, setup_restaurant_data, test_user
    ):
        ...
        initial_basil = (
            db.query(Inventory).filter(Inventory.id == 5).first().quantity
        )  # 5
        ...
        order_items = [
            OrderItem(
                order_id=1, menu_item_id=1, quantity=2, price=12.99  # Margherita Pizza
            )
        ]
        ...
        result = await update_order_service(
            order_id=1, order_update=order_update, db=db, user_id=test_user.id
        )
    ...
    async def test_cancel_order_ensure_inventory_rollback(
        self, db: Session, setup_restaurant_data, test_user
    ):
        ...
        order_item = OrderItem(
            order_id=2, menu_item_id=2, quantity=1, price=14.99  # Pepperoni Pizza
        )
        db.add(order_item)
        db.commit()
        ...
        initial_pepperoni = (
            db.query(Inventory).filter(Inventory.id == 4).first().quantity
        )

        # Move to IN_PROGRESS (deducts inventory)
        order_update = OrderUpdate(status=OrderStatus.IN_PROGRESS)
        await update_order_service(
            order_id=2, order_update=order_update, db=db, user_id=test_user.id
        )
        ...
        after_deduction_dough = (
            db.query(Inventory).filter(Inventory.id == 1).first().quantity
        )
        after_deduction_pepperoni = (
            db.query(Inventory).filter(Inventory.id == 4).first().quantity
        )
        ...
        await update_order_service(
            order_id=2, order_update=cancel_update, db=db, user_id=test_user.id
        )
        ...
        assert final_pepperoni == pytest.approx(
            initial_pepperoni, rel=1e-3
        )  # Should be restored
    ...
    @pytest.mark.slow
    async def test_multiple_items_shared_ingredients(
        self, db: Session, setup_restaurant_data, test_user
    ):
        ...
        order_items = [
            OrderItem(
                order_id=3, menu_item_id=1, quantity=1, price=12.99
            ),  # Margherita
            OrderItem(order_id=3, menu_item_id=2, quantity=1, price=14.99),  # Pepperoni
            OrderItem(
                order_id=3, menu_item_id=3, quantity=1, price=5.99
            ),  # Garlic Bread
        ]
        db.add_all(order_items)
        db.commit()
        ...
        order_update = OrderUpdate(status=OrderStatus.IN_PROGRESS)
        await update_order_service(
            order_id=3, order_update=order_update, db=db, user_id=test_user.id
        )
    ...
    async def test_insufficient_inventory_handling(
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_order_service(
                order_id=4, order_update=order_update, db=db, user_id=test_user.id
            )
    ...
    async def test_low_stock_alert_generation(
        self, db: Session, setup_restaurant_data, test_user
    ):
        ...
        order_update = OrderUpdate(status=OrderStatus.IN_PROGRESS)
        result = await update_order_service(
            order_id=5, order_update=order_update, db=db, user_id=test_user.id
        )
        ...
        basil_impact = next(
            item for item in preview["impact_preview"] if item["inventory_id"] == 5
        )
        assert basil_impact["will_be_low_stock"] is True

    async def test_order_completion_deduction_mode(
        self, db: Session, setup_restaurant_data, test_user, monkeypatch
    ):
        ...
        monkeypatch.setattr(
            "modules.orders.services.order_service.get_inventory_config",
            lambda: MockConfig(),
        )
        ...
        order_item = OrderItem(
            order_id=6, menu_item_id=3, quantity=1, price=5.99  # Garlic Bread
        )
        db.add(order_item)
        db.commit()
        ...
        order_update = OrderUpdate(status=OrderStatus.IN_PROGRESS)
        await update_order_service(
            order_id=6, order_update=order_update, db=db, user_id=test_user.id
        )
        ...
        await update_order_service(
            order_id=6, order_update=complete_update, db=db, user_id=test_user.id
        )
    ...
    @pytest.mark.slow
    async def test_complex_order_flow_with_modifications(
        self, db: Session, setup_restaurant_data, test_user
    ):
        ...
        order_items = [
            OrderItem(
                order_id=7, menu_item_id=1, quantity=2, price=12.99
            )  # 2 Margherita
        ]
        db.add_all(order_items)
        db.commit()
    ...
    async def test_concurrent_orders_shared_inventory(
        self, db: Session, setup_restaurant_data, test_user
    ):
        ...
        for order_data in orders_data:
            try:
                ...
                order_update = OrderUpdate(status=OrderStatus.IN_PROGRESS)
                await update_order_service(
                    order_data["id"], order_update, db, test_user.id
                )
