...
class TestInventoryErrorHandling:
    ...
    def service(self, db: Session):
        ...
        service.notification_service.send_role_notification = AsyncMock(
            return_value=True
        )
    ...
    async def test_missing_recipe_error(self, db: Session, service):
        ...
        review = (
            ...
            .filter(ManualReviewQueue.order_id == order.id)
            .first()
        )
    ...
    async def test_insufficient_inventory_error(self, db: Session, service):
        ...
        adjustments = (
            ...
            .filter(InventoryAdjustment.reference_id == str(order.id))
            .all()
        )
        assert len(adjustments) == 0

    @pytest.mark.asyncio
    async def test_insufficient_inventory_high_value_order_alert(
        ...
        inventory = InventoryFactory(
            item_name="Truffle Oil", quantity=1.0, unit="bottle"
        )
        menu_item = MenuItemFactory(name="Truffle Pasta", price=150.0)  # High price
        recipe = RecipeFactory(menu_item=menu_item)
        ingredient = RecipeIngredientFactory(
            recipe=recipe, inventory_item=inventory, quantity=0.5, unit="bottle"
        )
    ...
    async def test_recipe_circular_dependency_error(self, db: Session, service):
        ...
        # A uses inventory
        RecipeIngredientFactory(recipe=recipe_a, inventory_item=inventory, quantity=1.0)
    ...
    async def test_partial_deduction_allowed(self, db: Session, service):
        ...
        RecipeIngredientFactory(
            recipe=bread_recipe, inventory_item=flour, quantity=2.0, unit="kg"
        )
    ...
    async def test_low_stock_notification(self, db: Session, service):
        ...
        user = UserFactory()
        inventory = InventoryFactory(
            item_name="Sugar", quantity=15.0, threshold=10.0, unit="kg"
        )
...
class TestManualReviewIntegration:
    ...
    async def test_manual_review_workflow(self, db: Session):
        ...
        review_service.notification_service.send_role_notification = AsyncMock(
            return_value=True
        )
        ...
        review = await review_service.assign_review(
            review_id=review.id, assignee_id=manager.id
        )
    ...
    async def test_review_statistics(self, db: Session):
        ...
        for i in range(5):
            ...
            review = await review_service.create_review_request(
                order_id=order.id,
                reason=(
                    ReviewReason.MISSING_RECIPE
                    if i < 3
                    else ReviewReason.INSUFFICIENT_STOCK
                ),
                priority=i,
            )
