...
class TestConflictResolutionIntegration:
    ...
    def test_get_conflicts(self, client, auth_headers, sample_conflict):
        ...
        with patch('backend.core.auth.get_current_user', return_value=Mock(id=1, role="admin")):
            response = client.get(
    ...
    def test_resolve_conflict_local_wins(self, client, auth_headers, sample_conflict, db_session):
        ...
        sync_status = db_session.query(OrderSyncStatus).filter(
            OrderSyncStatus.order_id == sample_conflict.order_id
        ).first()
        assert sync_status.sync_status == SyncStatus.RETRY
    
    def test_resolve_conflict_remote_wins(self, client, auth_headers, sample_conflict, db_session):
    ...
    def test_resolve_already_resolved_conflict(self, client, auth_headers, sample_conflict, db_session):
...
class TestSchedulerLifecycleIntegration:
    ...
    async def test_scheduler_error_handling(self, scheduler):
        ...
        with patch('backend.modules.orders.tasks.sync_tasks.OrderSyncService') as mock_service:
...
class TestManualSyncIntegration:
    ...
    def test_manual_sync_all_orders(self, client):
        ...
        with patch('backend.core.auth.get_current_user', return_value=Mock(id=1, role="admin")):
            with patch('backend.modules.orders.tasks.sync_tasks.order_sync_scheduler.trigger_manual_sync', 
                      return_value=True):
                response = client.post(
    ...
    async def test_manual_sync_specific_orders(self, client):
        ...
        with patch('backend.core.auth.get_current_user', return_value=Mock(id=1, role="admin")):
            with patch('backend.modules.orders.services.sync_service.OrderSyncService') as mock_service:
                instance = Mock()
