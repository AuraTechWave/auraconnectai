...
class TestInventoryDeductionAPI:
    ...
    def setup_test_data(self, client: TestClient, headers: dict, db_session: Session):
        ...
        response = client.post("/api/v1/menu/items", json=menu_item_data, headers=headers)
        assert response.status_code == status.HTTP_201_CREATED
        ...
        response = client.post("/api/v1/menu/recipes", json=recipe_data, headers=headers)
        assert response.status_code == status.HTTP_201_CREATED
        ...
        order_data = {
            "customer_id": 1,
            "items": [
                {"menu_item_id": menu_item_id, "quantity": 2, "price": 12.50}
            ],
            "total_amount": 25.00
        }
    ...
    def test_check_inventory_availability(
        ...
        response = client.get(
            f"/api/v1/orders/{order_id}/inventory-availability",
            headers=headers
        )
        ...
        # Check flour impact
        flour_impact = next(
            item for item in result["impact_preview"]
    ...
    def test_partial_fulfillment(
        ...
        partial_data = {
            "fulfilled_items": [
                {"menu_item_id": menu_item_id, "fulfilled_quantity": 1}
            ]
        }
