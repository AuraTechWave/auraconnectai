...
class TestPaymentReconciliationService:
    ...
    @pytest.mark.asyncio
    async def test_create_payment_reconciliation_success(self, db_session, sample_order):
    ...
    @pytest.mark.asyncio
    async def test_create_payment_reconciliation_duplicate_reference(self, db_session, sample_order):
        reconciliation_data = PaymentReconciliationCreate(
            order_id=sample_order.id,
            external_payment_reference="PAY_123456",
            amount_expected=Decimal("25.50"),
    ...
    @pytest.mark.asyncio
    async def test_get_payment_reconciliation_by_id_success(self, db_session, sample_order):
        reconciliation_data = PaymentReconciliationCreate(
            order_id=sample_order.id,
            external_payment_reference="PAY_123456",
            amount_expected=Decimal("25.50"),
    ...
    async def test_get_payment_reconciliation_by_id_not_found(self, db_session):
        ...
        assert exc_info.value.status_code == 404
        assert "Payment reconciliation with id 999 not found" in exc_info.value.detail

    @pytest.mark.asyncio
    async def test_update_payment_reconciliation_success(self, db_session, sample_order):
        reconciliation_data = PaymentReconciliationCreate(
            order_id=sample_order.id,
            external_payment_reference="PAY_123456",
            amount_expected=Decimal("25.50"),
        ...
        result = await update_payment_reconciliation(db_session, created.id, update_data)
    ...
    @pytest.mark.asyncio
    async def test_get_payment_reconciliations_with_filters(self, db_session, sample_order):
        ...
        assert results[0].reconciliation_status == ReconciliationStatus.DISCREPANCY
    ...
    async def test_perform_payment_reconciliation(self, db_session):
        ...
        item1 = OrderItem(order_id=order1.id, menu_item_id=101, quantity=2, price=Decimal("12.50"))
        item2 = OrderItem(order_id=order2.id, menu_item_id=102, quantity=1, price=Decimal("15.00"))
    ...
    async def test_resolve_payment_discrepancy(self, db_session, sample_order):
        ...
        result = await resolve_payment_discrepancy(db_session, created.id, resolution_data)
        
        assert result.reconciliation_status == ReconciliationStatus.RESOLVED
        assert result.reconciliation_action == ReconciliationAction.EXCEPTION_HANDLED
        assert result.resolution_notes == "Customer provided additional payment"
        assert result.resolved_by == 1
