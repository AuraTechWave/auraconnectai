...
class ManualReviewService:
    ...
    async def create_review_request(
        ...
        try:
            ...
            existing = self.db.query(ManualReviewQueue).filter(
                ManualReviewQueue.order_id == order_id,
                ManualReviewQueue.status.in_([ReviewStatus.PENDING, ReviewStatus.IN_REVIEW])
            ).first()
    ...
    async def log_deduction_attempt(
        ...
        try:
            ...
            attempt_count = self.db.query(InventoryAdjustmentAttempt).filter(
                InventoryAdjustmentAttempt.order_id == order_id
            ).count()
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error logging deduction attempt: {str(e)}"
            )
    ...
    async def get_pending_reviews(
        ...
        return {
            ...
            "high_priority_count": self.db.query(ManualReviewQueue).filter(
                ManualReviewQueue.status == ReviewStatus.PENDING,
    ...
    async def _notify_managers_of_review(self, review: ManualReviewQueue, order: Order):
        ...
        try:
            ...
            self.logger.logger.error(
                f"Failed to send review notification: {str(e)}"
            )
