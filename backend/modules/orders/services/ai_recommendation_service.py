...
from ..schemas.dynamic_pricing_schemas import (
    DynamicPricingRequest, DynamicPricingResponse, PriceAdjustment, PricingContext
)
...
class RecommendationService:
    ...
    async def calculate_dynamic_price(
        ...
        try:
            cache_key = f"{request.menu_item_id}_{request.quantity}_{hash(str(request.context))}"
            ...
            if request.context:
                if request.context.time_of_day:
                    time_adjustment = self._calculate_time_adjustment(request.context.time_of_day)
                    if time_adjustment != 0:
                        adjustment_amount = request.base_price * Decimal(str(time_adjustment))
                        ...
                        adjustments.append(PriceAdjustment(
                            ...
                            reason=f"Time-based adjustment for {request.context.time_of_day}"
                        ))
                
                if request.context.demand_level:
                    demand_adjustment = self._calculate_demand_adjustment(request.context.demand_level)
                    if demand_adjustment != 0:
                        adjustment_amount = request.base_price * Decimal(str(demand_adjustment))
                        calculated_price += adjustment_amount
                        adjustments.append(PriceAdjustment(
                            adjustment_type="demand",
                            adjustment_amount=adjustment_amount,
                            adjustment_percentage=demand_adjustment * 100,
                            reason=f"Demand-based adjustment for {request.context.demand_level} demand"
                        ))
                
                if request.context.inventory_level is not None:
                    inventory_adjustment = self._calculate_inventory_adjustment(request.context.inventory_level)
                    if inventory_adjustment != 0:
                        adjustment_amount = request.base_price * Decimal(str(inventory_adjustment))
                        calculated_price += adjustment_amount
                        adjustments.append(PriceAdjustment(
                            adjustment_type="inventory",
                            adjustment_amount=adjustment_amount,
                            adjustment_percentage=inventory_adjustment * 100,
                            reason=f"Inventory-based adjustment for {request.context.inventory_level}% stock level"
                        ))
            
            calculated_price = max(calculated_price, request.base_price * Decimal('0.5'))
            calculated_price = min(calculated_price, request.base_price * Decimal('2.0'))
