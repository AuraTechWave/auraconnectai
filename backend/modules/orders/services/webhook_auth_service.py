...
class WebhookAuthService:
    ...
    async def verify_webhook_request(
        ...
        provider = (
            self.db.query(ExternalPOSProvider)
            .filter(
        ...
        try:
            ...
            if provider.auth_type == AuthenticationType.HMAC_SHA256:
                return await self._verify_hmac_sha256(
                    provider, headers, body, request_signature
                )
            elif provider.auth_type == AuthenticationType.HMAC_SHA512:
                return await self._verify_hmac_sha512(
                    provider, headers, body, request_signature
                )
            ...
            logger.error(
                f"Error verifying webhook for {provider_code}: {str(e)}, headers: {masked_hdrs}"
            )
    ...
    async def _verify_square_signature(
        self, secret: str, headers: Dict[str, str], body: bytes, signature: str
    ) -> Tuple[bool, Optional[str], Optional[Dict[str, Any]]]:
        ...
        return (
            ...
            {"signature_algorithm": "square-hmac-sha256", "provider": "square"},
        )
    ...
    async def _verify_hmac_sha512(
        ...
        expected_signature = hmac.new(secret.encode(), body, hashlib.sha512).hexdigest()
    ...
    async def _verify_api_key(
        self, provider: ExternalPOSProvider, headers: Dict[str, str]
    ) -> Tuple[bool, Optional[str], Optional[Dict[str, Any]]]:
    ...
    def generate_test_signature(
        self, provider_code: str, body: Dict[str, Any], timestamp: Optional[int] = None
        ...
        if provider.auth_type == AuthenticationType.HMAC_SHA256:
            ...
            if provider.provider_code == POSProviderEnum.STRIPE:
                ...
                signature_header = auth_config.get(
                    "signature_header", "X-Webhook-Signature"
                )
                return {signature_header: f"sha256={signature}"}

        elif provider.auth_type == AuthenticationType.API_KEY:
            key_header = auth_config.get("api_key_header", "X-API-Key")
            return {key_header: auth_config.get("api_key", "")}

        elif provider.auth_type == AuthenticationType.BEARER_TOKEN:
            return {"Authorization": f"Bearer {auth_config.get('bearer_token', '')}"}
