...
class OrderCalculationService:
    ...
    def calculate_order_totals(
        ...
        try:
            ...
            logger.info(f"Calculated order totals for order {order.id}: "
            ...
            logger.error(f"Error calculating order totals for order {order.id}: {str(e)}")
            raise
    ...
    def get_order_summary(self, order: Order) -> Dict[str, Any]:
        ...
        try:
            ...
            if order.total_amount and order.total_amount > 0:
                savings_percentage = (float(order.discount_amount or 0) / float(order.total_amount)) * 100
            ...
            summary = {
                ...
                } if order.referral_code_used else None
            }
    ...
    def validate_order_amounts(self, order: Order) -> Dict[str, Any]:
        ...
        try:
            ...
            if abs(calculated["final_amount"] - float(order.final_amount or 0)) > tolerance:
            ...
            logger.error(f"Error validating order amounts for order {order.id}: {str(e)}")
            return {
                "is_valid": False,
                "error": str(e),
