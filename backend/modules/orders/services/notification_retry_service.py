...
from ..models.notification_config_models import (
    NotificationChannelConfig, NotificationRetryQueue,
    NotificationRetryStrategy, NotificationChannelStatus
)
...
class NotificationRetryService:
    ...
    async def queue_for_retry(
        ...
        existing = self.db.query(NotificationRetryQueue).filter(
            and_(
        ...
        if existing:
            ...
            if next_retry and existing.retry_count <= channel_config.max_retry_attempts:
                ...
                existing.abandoned_reason = f"Max retries ({channel_config.max_retry_attempts}) exceeded"
    ...
    async def process_retry_queue(self, batch_size: int = 100):
        ...
        """
        # Get pending retries
        pending_retries = self.db.query(NotificationRetryQueue).filter(
            and_(
    ...
    async def _process_channel_retries(
        self,
        channel_name: str,
        retries: List[NotificationRetryQueue]
    ):
        """Process retries for a specific channel"""
        # Get channel config
        config = self.db.query(NotificationChannelConfig).filter(
            NotificationChannelConfig.channel_name == channel_name
        ).first()
        
        if not config or not config.is_enabled:
            logger.warning(f"Channel {channel_name} not found or disabled, abandoning retries")
        ...
        for retry in retries:
            try:
                ...
                if success:
                    ...
                    notification = self.db.query(OrderNotification).filter(
                        OrderNotification.id == retry.notification_id
                    ).first()
    ...
    async def _send_with_adapter(
        ...
        try:
            ...
            # For now, just log
            logger.info(f"Retrying notification to {recipient} via {config.channel_name}")
            return True
        except Exception as e:
    ...
    def _check_rate_limit(self, config: NotificationChannelConfig) -> bool:
        ...
        if not any([config.rate_limit_per_minute, config.rate_limit_per_hour, config.rate_limit_per_day]):
            return True
    ...
    async def cleanup_old_retries(self, days_to_keep: int = 7):
        ...
        deleted = self.db.query(NotificationRetryQueue).filter(
            or_(
...
class NotificationHealthChecker:
    ...
    async def check_all_channels(self):
        ...
        channels = self.db.query(NotificationChannelConfig).filter(
    ...
    async def check_channel_health(self, channel: NotificationChannelConfig):
        ...
        try:
            ...
            if is_available:
                ...
                if (channel.auto_disable_after_failures and 
                    channel.consecutive_failures >= channel.auto_disable_after_failures):
                    channel.is_enabled = False
                    channel.status = NotificationChannelStatus.FAILED
                    logger.error(f"Auto-disabled channel {channel.channel_name} after {channel.consecutive_failures} failures")
