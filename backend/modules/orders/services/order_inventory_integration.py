...
class OrderInventoryIntegrationService:
        ...
        try:
            ...
            try:
                ...
                if reverse_inventory and old_status == OrderStatus.COMPLETED:
                    reversal_result = (
                        await self.recipe_inventory_service.reverse_inventory_deduction(
                ...
                return {
                    ...
                    "inventory_reversed": reverse_inventory
    ...
    async def handle_partial_fulfillment(
        self, order_id: int, fulfilled_items: List[Dict[str, Any]], user_id: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            deduction_result = (
                await self.recipe_inventory_service.handle_partial_fulfillment(
                    order_items=fulfilled_items, order_id=order_id, user_id=user_id
                )
            )
            ...
            self.db.rollback()
            logger.error(
                f"Failed to handle partial fulfillment for order {order_id}: {str(e)}"
            )
            raise

    async def validate_inventory_availability(self, order_id: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            order_items = (
                self.db.query(OrderItem)
                .filter(OrderItem.order_id == order_id, OrderItem.is_cancelled == False)
                .all()
            )
            ...
            impact_preview = (
                await self.recipe_inventory_service.get_inventory_impact_preview(
                    order_items=order_items
                )
            )
