...
class RedisWebSocketManager:
    ...
    async def disconnect(self, websocket: WebSocket):
        ...
        await self._publish_connection_event("disconnect", order_id, session_id, user_id)
    ...
    async def check_heartbeats(self):
        ...
        for ws in stale_connections:
            logger.warning(f"Disconnecting stale connection: {self.connection_metadata.get(ws)}")
            await self.disconnect(ws)
            try:
    ...
    async def _process_broadcast_message(self, data: str):
        ...
        try:
            ...
            if message["type"] == "order_message":
                ...
                await self._send_to_local_order(
                    message["order_id"],
                    message["message"]
                )
    ...
    async def _check_connection_rate_limit(self, client_ip: str) -> bool:
        ...
        if client_ip in self.connection_rate_limits:
            ...
            if (now - last_connection).total_seconds() < 12:  # 5 per minute = 1 per 12 seconds
                return False
        ...
        self.connection_rate_limits = {
            ip: time for ip, time in self.connection_rate_limits.items()
            if time > cutoff
        }
    ...
    async def _check_message_rate_limit(self, websocket: WebSocket) -> bool:
        ...
        self.message_rate_limits[session_id] = [
            ts for ts in self.message_rate_limits[session_id]
            if ts > cutoff
        ]
