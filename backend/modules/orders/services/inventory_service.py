...
async def get_inventory_by_id(db: Session, inventory_id: int):
    ...
    if not inventory:
        raise HTTPException(
            status_code=404, detail=f"Inventory item with id {inventory_id} not found"
        )
    ...
    try:
        for order_item in order_items:
            mappings = (
                ...
                .filter(MenuItemInventory.menu_item_id == order_item.menu_item_id)
                .all()
            )
        ...
        db.commit()
        return {"success": True, "low_stock_alerts": low_stock_items}
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(
            status_code=500, detail="Error processing inventory deduction"
        )
...
async def check_low_stock(db: Session) -> List[Dict]:
    low_stock = (
        db.query(Inventory)
        .filter(
            Inventory.quantity <= Inventory.threshold, Inventory.deleted_at.is_(None)
        )
        .all()
    )
