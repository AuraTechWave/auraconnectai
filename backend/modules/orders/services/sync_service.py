...
class OrderSyncService:
    ...
    async def sync_pending_orders(self) -> SyncBatch:
        ...
        try:
            ...
            logger.info(f"Starting sync batch {batch.batch_id} with {len(pending_orders)} orders")
    ...
    async def _sync_order_batch(
        ...
        results = await asyncio.gather(
            *[bounded_sync(task) for task in tasks],
    ...
    def _handle_sync_error(
        ...
        if sync_status.attempt_count >= self.max_retry_attempts:
            ...
            sync_status.sync_status = SyncStatus.RETRY
            sync_status.next_retry_at = datetime.utcnow() + timedelta(minutes=retry_delay)
            logger.warning(
    ...
    def _get_pending_orders(self) -> List[Order]:
        ...
        orders = self.db.query(Order).outerjoin(OrderSyncStatus).filter(
            or_(
    ...
    def _get_retry_orders(self) -> List[Order]:
        ...
        orders = self.db.query(Order).join(OrderSyncStatus).filter(
    ...
    def _get_or_create_sync_status(self, order: Order) -> OrderSyncStatus:
        ...
        sync_status = self.db.query(OrderSyncStatus).filter(
    ...
    def _serialize_order(self, order: Order) -> Dict[str, Any]:
        ...
        for item in order.order_items:
            order_data["items"].append({
                ...
                "amount": float(item.amount) if hasattr(item, 'amount') else float(item.price * item.quantity),
                "instructions": item.instructions if hasattr(item, 'instructions') else None
            })
    ...
    async def get_sync_status(self) -> Dict[str, Any]:
        ...
        status_counts = self.db.query(
            OrderSyncStatus.sync_status,
            func.count(OrderSyncStatus.id)
        ).group_by(OrderSyncStatus.sync_status).all()
        
        # Get unsynced orders count
        unsynced_count = self.db.query(Order).filter(
            Order.is_synced == False
        ).count()
        
        # Get last batch info
        last_batch = self.db.query(SyncBatch).order_by(
        ...
        return {
            ...
            "last_batch": {
                ...
                "started_at": last_batch.started_at.isoformat() if last_batch else None,
                "completed_at": last_batch.completed_at.isoformat() if last_batch and last_batch.completed_at else None,
                "successful_syncs": last_batch.successful_syncs if last_batch else 0,
