...
from ..models.queue_models import OrderQueue, QueueItem, QueueStatus, QueueItemStatus
...
class PriorityService:
    ...
    def calculate_order_priority(
        self, order_id: int, queue_id: int, profile_override: Optional[int] = None
    ) -> OrderPriorityScore:
        """
        ...
        queue_item = (
            ...
            .filter(QueueItem.order_id == order_id, QueueItem.queue_id == queue_id)
            .first()
        )
        ...
        for profile_rule in profile.profile_rules:
            ...
            try:
                ...
                # Apply thresholds
                if (
                    profile_rule.min_threshold
                    and component_score < profile_rule.min_threshold
                ):
                    component_score = profile_rule.fallback_score or 0.0
                if (
                    profile_rule.max_threshold
                    and component_score > profile_rule.max_threshold
                ):
                    component_score = profile_rule.max_threshold

                # Weight the score
                weight = (
                    profile_rule.weight_override
                    or profile_rule.weight
                    or rule.default_weight
                )
        ...
        if profile.normalize_scores and total_weight > 0:
            total_score = self._normalize_score(
                total_score, profile.normalization_method
            )
        ...
        priority_score = (
            self.db.query(OrderPriorityScore)
    ...
    def _get_base_value(
        ...
        # Calculate customer loyalty
            ...
            if customer:
                loyalty = (
                    ...
                    .filter(LoyaltyProgram.customer_id == customer.id)
                    .first()
                )
                if loyalty:
        ...
        # Check if peak hours
            ...
            peak_hours = config.peak_hours or [
                11,
                12,
                13,
                18,
        ...
        # Check special needs
            ...
            return sum(
                ...
                if keyword.lower() in special_notes.lower()
            )
    ...
    def _custom_score(
        self, value: float, config: Dict[str, Any], function_code: Optional[str]
    ) -> float:
    ...
    def _apply_queue_boosts(
        self, config: QueuePriorityConfig, order: Order, queue_item: QueueItem
    ) -> float:
    ...
    def _calculate_suggested_sequence(self, queue_id: int, score: float) -> int:
        ...
        queue_items = (
            ...
            .order_by(OrderPriorityScore.total_score.desc())
            .all()
        )
        ...
        for i, item in enumerate(queue_items):
            item_score = (
                self.db.query(OrderPriorityScore)
                .filter(OrderPriorityScore.queue_item_id == item.id)
                .first()
            )
    ...
    def _create_default_score(
        self, queue_item_id: int, config_id: Optional[int]
    ) -> OrderPriorityScore:
    ...
    def adjust_priority_manually(
        ...
        current_score = (
            self.db.query(OrderPriorityScore)
            .filter(OrderPriorityScore.queue_item_id == queue_item_id)
            .first()
        )

        if not current_score:
            raise NotFoundError(
                f"No priority score found for queue item {queue_item_id}"
            )
        ...
        if duration_seconds:
            current_score.boost_expires_at = datetime.utcnow() + timedelta(
    ...
    def _get_queue_position(self, queue_item_id: int) -> int:
        ...
        queue_item = (
            self.db.query(QueueItem).filter(QueueItem.id == queue_item_id).first()
        )
        if not queue_item:
            return 0
        ...
        position = (
            ...
            .filter(
                ...
                OrderPriorityScore.total_score
                > self.db.query(OrderPriorityScore.total_score)
                .filter(OrderPriorityScore.queue_item_id == queue_item_id)
                .scalar(),
            )
            .count()
    ...
    def rebalance_queue(
        self, queue_id: int, force: bool = False
    ) -> QueueRebalanceResponse:
            ...
            sequence.append(
                {
                    ...
                    "priority_score": (
                        priority_score.total_score if priority_score else 0.0
                    ),
                    "priority_tier": (
                        priority_score.priority_tier if priority_score else "medium"
                    ),
                    "wait_time_minutes": (
                        datetime.utcnow() - (item.queued_at or item.created_at)
                    ).total_seconds()
                    / 60,
                }
            )
