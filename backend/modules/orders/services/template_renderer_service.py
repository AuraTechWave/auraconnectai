...
class TemplateRendererService:
    ...
    def render_for_channel(
        ...
        """
        # Add default context
        context = {**context, "now": datetime.utcnow(), "channel": channel.value}
    ...
    def _render_email(
        self, template: OrderTrackingTemplate, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _render_push(
        ...
        if template.push_image_url:
            result["image_url"] = self._render_template(
                template.push_image_url, context
            )

        if template.push_action_url:
            result["action_url"] = self._render_template(
                template.push_action_url, context
            )
    ...
    def _render_sms(
        ...
        result = {"message": message, "length": len(message)}
    ...
    def validate_template(
        self, template_str: str, available_variables: Dict[str, str]
...
<body>
    ...
    NotificationChannel.SMS: {
        "order_ready": {
            ...
            "channel_settings": {"sender_id": "RESTAURANT", "type": "transactional"},
        }
    },
}
