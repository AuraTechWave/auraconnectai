...
async def create_payment_reconciliation(
    ...
    if not order:
        raise HTTPException(
            status_code=404, detail=f"Order {reconciliation_data.order_id} not found"
        )

    if hasattr(order, "status") and order.status in ["CANCELLED", "REFUNDED"]:
        raise HTTPException(
            status_code=400, detail=f"Cannot reconcile {order.status} orders"
        )
    ...
    amount_diff = abs(
        reconciliation_data.amount_expected - reconciliation_data.amount_received
    )
    if amount_diff > Decimal("0.01"):  # More than 1 cent difference
        reconciliation_data.reconciliation_status = ReconciliationStatus.DISCREPANCY
        reconciliation_data.discrepancy_type = DiscrepancyType.AMOUNT_MISMATCH
    ...
    reconciliation = PaymentReconciliation(
        ...
        discrepancy_type=(
            reconciliation_data.discrepancy_type.value
            if reconciliation_data.discrepancy_type
            else None
        ),  # noqa: E501
...
async def get_payment_reconciliation_by_id(
    db: Session, reconciliation_id: int
) -> PaymentReconciliationOut:
    reconciliation = (
        db.query(PaymentReconciliation)
        .filter(PaymentReconciliation.id == reconciliation_id)
        .first()
    )
    ...
    if not reconciliation:
        raise HTTPException(
            status_code=404,
            detail=f"Payment reconciliation with id {reconciliation_id} " f"not found",
        )
...
async def update_payment_reconciliation(
    db: Session, reconciliation_id: int, update_data: PaymentReconciliationUpdate
) -> PaymentReconciliationOut:
    reconciliation = (
        db.query(PaymentReconciliation)
        .filter(PaymentReconciliation.id == reconciliation_id)
...
async def get_payment_reconciliations(
    db: Session, filters: ReconciliationFilter
...
async def perform_payment_reconciliation(
    db: Session, request: ReconciliationRequest
) -> ReconciliationResponse:
    matched_count = 0
    discrepancy_count = 0
    reconciliations = []
...
async def resolve_payment_discrepancy(
    db: Session, reconciliation_id: int, resolution_data: ResolutionRequest
) -> PaymentReconciliationOut:
...
def calculate_match_score(
    order: Order, payment: Dict[str, Any], order_total: float
) -> float:
...
async def get_reconciliation_metrics(db: Session) -> Dict[str, Any]:
    ...
    success_rate = (
        (matched_count + resolved_count) / total_reconciled * 100
    ...
    discrepancy_types = (
        ...
        .filter(PaymentReconciliation.discrepancy_type.isnot(None))
        .group_by(PaymentReconciliation.discrepancy_type)
        .order_by(func.count(PaymentReconciliation.id).desc())
        .limit(5)
        .all()
    )
...
async def auto_reconcile_pending(db: Session) -> Dict[str, Any]:
    ...
    for reconciliation in pending_reconciliations:
        provider = MockPOSProvider({})
        payments = await provider.get_payments(order_ids=[reconciliation.order_id])

        order = db.query(Order).filter(Order.id == reconciliation.order_id).first()
...
async def handle_payment_webhook(
    db: Session, payment_data: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        order = db.query(Order).filter(Order.id == int(order_reference)).first()

        if not order:
            raise HTTPException(
                status_code=404, detail=f"Order {order_reference} not found"
            )

        existing = (
            db.query(PaymentReconciliation)
            .filter(
                PaymentReconciliation.external_payment_reference == payment_reference
            )
            .first()
        )
        ...
        return {
            ...
            "message": f"Reconciliation created for payment " f"{payment_reference}",
        }

    except Exception as e:
        logger.error(f"Error handling payment webhook: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Error processing payment webhook: {str(e)}"
        )
