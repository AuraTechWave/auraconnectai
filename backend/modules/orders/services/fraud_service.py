...
from ..schemas.order_schemas import FraudCheckRequest, FraudCheckResponse, FraudAlertCreate
...
async def _check_timing_patterns(db: Session, order: Order) -> Dict:
    ...
    if recent_orders > 3:
        flags.append(f"Multiple orders from same staff in 10 minutes: {recent_orders}")
        score += 10.0
...
async def _check_suspicious_patterns(db: Session, order: Order) -> Dict:
    ...
    excessive_notes = sum(1 for item in order.order_items if item.notes and len(item.notes) > 100)
