...
class NotificationMetrics:
    ...
    retry_queue_length = Gauge(
        "notification_retry_queue_length", "Current length of retry queue", ["channel"]
    )
    ...
    healthy_channels = Gauge(
        "notification_channels_healthy", "Number of healthy notification channels"
    )

    total_channels = Gauge(
        "notification_channels_total", "Total number of notification channels"
    )
    ...
    notification_system_info = Info(
        "notification_system", "Notification system information"
    )

    def __init__(self):
        # Set system info
        self.notification_system_info.info(
            {"version": "1.0.0", "retry_strategy": "exponential_backoff"}
        )

    # Notification metrics methods
    def record_notification_sent(
        self, channel: str, event_type: str, status: str = "success"
    ):
        """Record a notification being sent"""
        self.notifications_sent.labels(
            channel=channel, event_type=event_type, status=status
        ).inc()
    ...
    def record_notification_retry(self, channel: str, retry_count: int):
        ...
        self.notification_retries.labels(
            channel=channel, retry_count=str(retry_count)
        ).inc()

    def record_notification_failure(self, channel: str, error_type: str):
        """Record a notification failure"""
        self.notification_failures.labels(channel=channel, error_type=error_type).inc()
    ...
    def record_retry_batch_processed(
        self, count: int, duration: Optional[float] = None
    ):
    ...
    def record_websocket_message(self, server_id: str, message_type: str):
        ...
        self.websocket_messages.labels(
            server_id=server_id, message_type=message_type
        ).inc()
    ...
    # Worker metrics
    def increment_worker_errors(
        self, worker_type: str, error_type: Optional[str] = None
    ):
        """Increment worker error count"""
        self.worker_errors.labels(
            worker_type=worker_type, error_type=error_type or "unknown"
        ).inc()

    # Stats aggregation
    def record_notification_stats(
        self, channel: str, sent: int, delivered: int, failed: int
    ):
