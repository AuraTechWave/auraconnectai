...
class PushNotificationService(NotificationAdapter):
    ...
    async def send_to_user(
        ...
        logger.info(f"Would send push notification to user {user_id}: {message.subject}")
    ...
    async def is_available(self) -> bool:
        ...
        return bool(self.config.get('fcm_server_key') or self.config.get('apns_cert_path'))
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('success') == 1:
                    logger.info(f"FCM notification sent successfully to {token[:10]}...")
                    return True
                else:
                    logger.error(f"FCM error: {result.get('results', [{}])[0].get('error')}")
            else:
