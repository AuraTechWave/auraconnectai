...
from ..models.queue_models import (
    ...
    QueueSequenceRule, QueueMetrics, QueueDisplay,
...
from ..schemas.queue_schemas import (
    ...
    HoldItemRequest, BatchStatusUpdateRequest, QueueMetricsRequest,
    SequenceRuleCreate, SequenceRuleUpdate
)
...
class QueueService:
    ...
    def create_queue(self, queue_data: QueueCreate) -> OrderQueue:
        ...
        try:
            ...
            existing = self.db.query(OrderQueue).filter(
                OrderQueue.name == queue_data.name
            ).first()
            if existing:
                raise HTTPException(
    ...
    def get_queue(self, queue_id: int) -> OrderQueue:
        """Get queue by ID"""
        queue = self.db.query(OrderQueue).filter(
            OrderQueue.id == queue_id
        ).first()
    ...
    def add_to_queue(self, item_data: QueueItemCreate, user_id: Optional[int] = None) -> QueueItem:
        ...
        try:
            ...
            existing_item = self.db.query(QueueItem).filter(
                QueueItem.order_id == item_data.order_id,
                QueueItem.status.notin_([QueueItemStatus.COMPLETED, QueueItemStatus.CANCELLED])
            ).first()
            if existing_item:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            queue_item = QueueItem(
                ...
                display_name=item_data.display_name or self._generate_display_name(order),
                ...
                estimated_ready_time=item_data.estimated_ready_time or self._calculate_estimated_time(queue, order)
            )
            ...
            self._add_status_history(queue_item, None, QueueItemStatus.QUEUED, user_id, "Added to queue")
            
            self.db.commit()
            self.db.refresh(queue_item)
            
            logger.info(f"Added order {item_data.order_id} to queue {queue.name} at position {queue_item.sequence_number}")
            return queue_item
    ...
    def update_queue_item(
        ...
        if "status" in update_dict:
            ...
            if new_status == QueueItemStatus.IN_PREPARATION:
                ...
                if item.started_at:
                    item.prep_time_actual = int((item.ready_at - item.started_at).total_seconds() / 60)
            elif new_status == QueueItemStatus.COMPLETED:
                item.completed_at = datetime.utcnow()
                if item.queued_at:
                    item.wait_time_actual = int((item.completed_at - item.queued_at).total_seconds() / 60)
    ...
    def get_queue_items(
        ...
        query = query.order_by(
            ...
            QueueItem.sequence_number.asc()
        )
        
        return query.offset(offset).limit(limit).all()
    
    # Queue Operations
    def move_item(self, move_request: MoveItemRequest, user_id: Optional[int] = None) -> QueueItem:
        ...
        if new_position < old_position:
            ...
            affected_items = self.db.query(QueueItem).filter(
        ...
        if move_request.reason:
            self._add_status_history(
                item, item.status, item.status, user_id,
        ...
        logger.info(f"Moved queue item {item.id} from position {old_position} to {new_position}")
        return item
    ...
    def transfer_item(
        ...
        logger.info(f"Transferred item {item.id} from queue {old_queue_id} to {target_queue.id}")
        return item
    ...
    def expedite_item(
        ...
        if expedite_request.move_to_front:
            ...
            first_position = self.db.query(func.min(QueueItem.sequence_number)).filter(
                QueueItem.queue_id == item.queue_id,
                QueueItem.status.notin_([QueueItemStatus.COMPLETED, QueueItemStatus.CANCELLED])
            ).scalar() or 1
        ...
        logger.info(f"Expedited item {item.id} with priority boost {expedite_request.priority_boost}")
        return item
    
    def hold_item(self, hold_request: HoldItemRequest, user_id: Optional[int] = None) -> QueueItem:
        """Put an item on hold"""
        ...
        if hold_request.hold_until:
            ...
            item.hold_until = datetime.utcnow() + timedelta(minutes=hold_request.hold_minutes)
        
        # Add status history
        self._add_status_history(
            item, old_status, QueueItemStatus.ON_HOLD, user_id,
    ...
    def release_hold(self, item_id: int, user_id: Optional[int] = None) -> QueueItem:
        ...
        # Add status history
        self._add_status_history(
            item, QueueItemStatus.ON_HOLD, QueueItemStatus.QUEUED, user_id,
            "Released from hold"
        )
    ...
    def batch_update_status(
        ...
        items = self.db.query(QueueItem).filter(
            QueueItem.id.in_(batch_request.item_ids)
        ).all()
        ...
        for item in items:
            ...
            # Add history
            self._add_status_history(
                item, old_status, batch_request.new_status, user_id,
        ...
        logger.info(f"Batch updated {len(items)} items to status {batch_request.new_status}")
    ...
    def get_queue_status_summary(self, queue_id: int) -> Dict[str, Any]:
        ...
        # Get next ready time
        next_ready = self.db.query(func.min(QueueItem.estimated_ready_time)).filter(
            QueueItem.queue_id == queue_id,
            QueueItem.status == QueueItemStatus.IN_PREPARATION
        ).scalar()
        
        # Get staff count
        staff_count = self.db.query(func.count(func.distinct(QueueItem.assigned_to_id))).filter(
            QueueItem.queue_id == queue_id,
            QueueItem.status == QueueItemStatus.IN_PREPARATION,
        ...
        return {
            ...
            "capacity_percentage": (queue.current_size / queue.max_capacity * 100) if queue.max_capacity else 0
        }
    ...
    def update_sequence_rule(
        ...
        rule = self.db.query(QueueSequenceRule).filter(
            QueueSequenceRule.id == rule_id
        ).first()
    ...
    def _get_next_sequence_number(self, queue_id: int) -> int:
        ...
        max_seq = self.db.query(func.max(QueueItem.sequence_number)).filter(
            QueueItem.queue_id == queue_id
        ).scalar() or 0
    ...
    def _apply_sequence_rules(
        ...
        rules = self.db.query(QueueSequenceRule).filter(
            QueueSequenceRule.queue_id == queue.id,
            QueueSequenceRule.is_active == True
        ).order_by(QueueSequenceRule.priority.desc()).all()
    ...
    def _evaluate_rule_conditions(self, conditions: Dict[str, Any], order: Order) -> bool:
    ...
    def _generate_display_name(self, order: Order) -> str:
        ...
        item_names = [item.menu_item.name if hasattr(item, 'menu_item') else f"Item {item.id}" 
                      for item in items]
    ...
    def _calculate_estimated_time(self, queue: OrderQueue, order: Order) -> datetime:
        ...
        active_items = self.db.query(func.count(QueueItem.id)).filter(
    ...
    def _validate_status_transition(self, old_status: QueueItemStatus, new_status: QueueItemStatus):
    ...
    def _aggregate_metrics(self, metrics: List[QueueMetrics], granularity: str) -> Dict[str, Any]:
        """Aggregate metrics based on granularity"""
        ...
        avg_wait = sum(m.avg_wait_time for m in metrics) / len(metrics) if metrics else 0
        ...
        return {
            ...
            "volume": {
                ...
                "completion_rate": (total_completed / total_queued * 100) if total_queued else 0
            },
            "timing": {
                "avg_wait_time": round(avg_wait, 1),
