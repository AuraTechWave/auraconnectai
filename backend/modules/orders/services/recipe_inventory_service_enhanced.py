...
class RecipeInventoryServiceEnhanced:
    ...
    async def deduct_inventory_for_order(
        ...
        try:
            ...
            try:
                required_ingredients = await self._calculate_required_ingredients(
                    order_items
                )
            ...
            if "items_without_recipes" in required_ingredients:
                items_without_recipes = required_ingredients.pop(
                    "items_without_recipes"
                )
                if items_without_recipes and not allow_partial:
                    raise MissingRecipeError(
                        menu_items=items_without_recipes, order_id=order_id
                    )
            ...
            if inventory_not_found:
                ...
                if not allow_partial:
                    error = InventoryNotFoundError(
                        inventory_ids=inventory_not_found, order_id=order_id
                    )
                    if create_review_on_failure:
                        await self.manual_review_service.create_review_request(
                            order_id=order_id,
            ...
            if insufficient_stock_items:
                ...
                if not allow_partial:
                    ...
                    error = InsufficientInventoryError(
                        items=insufficient_stock_items, order_id=order_id
                    )
                    if create_review_on_failure:
                        await self.manual_review_service.create_review_request(
                            order_id=order_id,
            ...
            for inventory_id, required_data in required_ingredients.items():
                ...
                # Check for low stock
                if (
                    inventory_item.threshold
                    and inventory_item.quantity <= inventory_item.threshold
                ):
                    ...
                    await self.notification_service.send_role_notification(
                        ...
                        priority=(
                            "high"
                            if inventory_item.quantity < inventory_item.threshold * 0.5
                            else "normal"
                        ),
                    )
            ...
            self.logger.log_deduction_error(
                order_id=order_id, error=e, error_type="unexpected"
            )
    ...
    async def reverse_inventory_deduction(
        ...
        try:
                ...
                synced_adjustments = (
                    ...
                    .filter(
                        and_(
                            InventoryAdjustment.reference_type == "order",
                            InventoryAdjustment.reference_id == str(order_id),
                            InventoryAdjustment.metadata.op("->")(
                                "synced_to_external"
                            ).astext
                            == "true",
                        )
            ...
            for adjustment in adjustments:
                inventory_item = (
                    self.db.query(Inventory)
                    .filter(Inventory.id == adjustment.inventory_id)
                    .with_for_update()
                    .first()
                )  # Lock row for update
            ...
            self.logger.log_reversal_success(
                order_id=order_id, reversed_items=reversed_items
            )
            ...
            self.db.rollback()
            self.logger.log_deduction_error(
                order_id=order_id, error=e, error_type="reversal_error"
            )
            raise HTTPException(
                status_code=500, detail=f"Error reversing inventory deduction: {str(e)}"
            )
    ...
    async def _calculate_required_ingredients(
        ...
        recipes = (
            ...
            .filter(Recipe.menu_item_id.in_(menu_item_ids), Recipe.deleted_at.is_(None))
            .all()
        )
                ...
                items_without_recipes.append(
                    {
                        ...
                        "menu_item_name": getattr(
                            order_item.menu_item, "name", "Unknown"
                        ),
                    }
                )
                continue
    ...
    async def _add_recipe_ingredients(
        ...
        for ingredient in ingredients:
            ...
            if order_item_id:
                required_ingredients[ingredient.inventory_id]["order_items"].append(
                    order_item_id
                )
    ...
    async def handle_partial_fulfillment(
        self, order_items: List[Dict], order_id: int, user_id: int
    ) -> Dict:
        """
    ...
    async def get_inventory_impact_preview(self, order_items: List[OrderItem]) -> Dict:
        """
        ...
        try:
            ...
            if items_without_recipes:
                warnings.append(
                    f"{len(items_without_recipes)} menu items lack recipe configuration"
                )
                ...
                impact_preview.append(
                    {
                        ...
                        "will_be_low_stock": inventory_item.threshold
                        and new_quantity <= inventory_item.threshold,
