...
class AutoCancellationScheduler:
    ...
    async def run_auto_cancellation(
        ...
        """
        try:
            logger.info(f"Starting auto-cancellation process for tenant {tenant_id}, team {team_id}")
            ...
            if result["cancelled_count"] > 0:
                logger.info(
                    f"Auto-cancellation completed: {result['cancelled_count']} orders cancelled. "
