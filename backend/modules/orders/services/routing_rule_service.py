...
from ..models.routing_models import (
    ...
    RoutingRuleLog, RouteOverride, StaffRoutingCapability,
    TeamRoutingConfig, TeamMember, RouteTargetType,
    RuleConditionOperator, RuleStatus
)
from ..models.order_models import Order, OrderItem
from ..schemas.routing_schemas import (
    RoutingRuleCreate, RoutingRuleUpdate, RouteEvaluationRequest,
    RouteEvaluationResult, RouteOverrideCreate, RoutingRuleTestRequest,
    RoutingRuleTestResult, StaffCapabilityCreate, TeamRoutingConfigCreate,
    TeamMemberCreate
)
...
class RoutingRuleService:
    ...
    # Rule Management
    def create_routing_rule(self, rule_data: RoutingRuleCreate, created_by_id: int) -> OrderRoutingRule:
        ...
        try:
            ...
            logger.info(f"Created routing rule '{rule.name}' with {len(rule_data.conditions)} conditions and {len(rule_data.actions)} actions")
            return rule
    ...
    def update_routing_rule(self, rule_id: int, update_data: RoutingRuleUpdate, updated_by_id: int) -> OrderRoutingRule:
        """Update an existing routing rule"""
        rule = self.db.query(OrderRoutingRule).filter(OrderRoutingRule.id == rule_id).first()
        if not rule:
            raise HTTPException(
    ...
    def get_routing_rule(self, rule_id: int) -> OrderRoutingRule:
        ...
        rule = self.db.query(OrderRoutingRule).options(
            joinedload(OrderRoutingRule.conditions),
            joinedload(OrderRoutingRule.actions)
        ).filter(OrderRoutingRule.id == rule_id).first()
    ...
    def list_routing_rules(
        ...
        return query.order_by(desc(OrderRoutingRule.priority)).offset(offset).limit(limit).all()
    ...
    # Rule Evaluation
    def evaluate_order_routing(self, request: RouteEvaluationRequest) -> RouteEvaluationResult:
        ...
        routing_decision = self._determine_routing_decision(matched_rules, order, request.test_mode)
    ...
    def _evaluate_rule(self, rule: OrderRoutingRule, order: Order) -> Tuple[bool, Dict[str, Any]]:
    ...
    def _evaluate_condition(self, condition: RoutingRuleCondition, order: Order) -> bool:
    ...
    def _extract_order_fields(self, path_parts: List[str], order: Order) -> Any:
        ...
        if field == "type":
            ...
            return order.status.value if hasattr(order.status, 'value') else order.status
        elif field == "priority":
            return order.priority.value if hasattr(order, 'priority') and order.priority else "normal"
    ...
    def _extract_customer_fields(self, path_parts: List[str], order: Order) -> Any:
        ...
        if field == "vip_status":
            ...
            return self.db.query(Order).filter(Order.customer_id == order.customer_id).count()
        else:
    ...
    def _extract_item_fields(self, path_parts: List[str], order: Order) -> Any:
        ...
        if field == "categories":
            ...
            for item in order.order_items:
                menu_item = self.db.query(MenuItem).filter(MenuItem.id == item.menu_item_id).first()
                if menu_item and hasattr(menu_item, 'contains_alcohol') and menu_item.contains_alcohol:
                    return True
            return False
        else:
    ...
    def _apply_operator(self, field_value: Any, operator: RuleConditionOperator, expected_value: Any) -> bool:
    ...
    def _get_applicable_rules(self, include_inactive: bool = False) -> List[OrderRoutingRule]:
        ...
        if not include_inactive:
            query = query.filter(OrderRoutingRule.status.in_([RuleStatus.ACTIVE, RuleStatus.TESTING]))
    ...
    def _determine_routing_decision(
        ...
        sorted_matches = sorted(matched_rules, key=lambda x: x["priority"], reverse=True)
        ...
        if len(sorted_matches) > 1:
            ...
            if len(conflicts) > 1:
                ...
                decision_base = self._build_routing_decision_from_rule(conflicts[0]["rule_id"])
        ...
        rule = self.db.query(OrderRoutingRule).filter(
            OrderRoutingRule.id == highest_priority_match["rule_id"]
        ).first()
        ...
        for action in sorted(rule.actions, key=lambda x: x.execution_order):
            if action.action_type == "route":
                ...
                if "target_config" in action.action_config:
                    decision["target_config"].update(action.action_config["target_config"])
                executed_actions.append({"type": "route", "config": action.action_config})
            elif action.action_type == "priority":
                decision["priority_adjustment"] = action.action_config.get("adjustment", 0)
                executed_actions.append({"type": "priority", "adjustment": decision["priority_adjustment"]})
            elif action.action_type == "split":
                decision["split_config"] = action.action_config
                executed_actions.append({"type": "split", "config": action.action_config})
                ...
                executed_actions.append({"type": "notify", "config": action.action_config})
    ...
    def _route_to_team(self, order: Order, team_id: int) -> None:
        ...
        team = self.db.query(TeamRoutingConfig).filter(
            TeamRoutingConfig.id == team_id
        ).first()
        ...
        logger.info(f"Routed order {order.id} to team member {selected_member.staff_id}")
    
    def _check_route_override(self, order_id: int) -> Optional[RouteOverride]:
        """Check if there's an active override for this order"""
        override = self.db.query(RouteOverride).filter(
    ...
    def create_route_override(
        ...
        existing = self.db.query(RouteOverride).filter(
            RouteOverride.order_id == override_data.order_id
        ).first()
        if existing:
    ...
    # Testing
    def test_routing_rules(self, test_request: RoutingRuleTestRequest) -> RoutingRuleTestResult:
    ...
    def create_staff_capability(self, capability_data: StaffCapabilityCreate) -> StaffRoutingCapability:
        """Create a staff routing capability"""
        # Check if staff exists
        staff = self.db.query(StaffMember).filter(
            StaffMember.id == capability_data.staff_id
        ).first()
        if not staff:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        logger.info(f"Created capability for staff {capability_data.staff_id}: {capability_data.capability_type}={capability_data.capability_value}")
        return capability
    
    def get_staff_capabilities(self, staff_id: int) -> List[StaffRoutingCapability]:
        """Get all capabilities for a staff member"""
        return self.db.query(StaffRoutingCapability).filter(
            StaffRoutingCapability.staff_id == staff_id
        ).all()
    ...
    def detect_rule_conflicts(self, rule_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """Detect potential conflicts between routing rules"""
        conflicts = []
        
        # Get all active rules
        rules = self.db.query(OrderRoutingRule).filter(
            OrderRoutingRule.status == RuleStatus.ACTIVE
        ).all()
    ...
    def _check_rule_conflict(self, rule1: OrderRoutingRule, rule2: OrderRoutingRule) -> Optional[Dict[str, Any]]:
        ...
        if rule1.target_type == rule2.target_type and rule1.target_id == rule2.target_id:
    ...
    def _conditions_could_overlap(
        ...
        for op1, op2 in opposite_pairs:
            if (cond1.operator == op1 and cond2.operator == op2 and cond1.value == cond2.value) or \
               (cond1.operator == op2 and cond2.operator == op1 and cond1.value == cond2.value):
                return False
    ...
    def add_team_member(self, member_data: TeamMemberCreate) -> TeamMember:
        ...
        team = self.db.query(TeamRoutingConfig).filter(
            TeamRoutingConfig.id == member_data.team_id
        ).first()
        if not team:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        staff = self.db.query(StaffMember).filter(
            StaffMember.id == member_data.staff_id
        ).first()
        if not staff:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
