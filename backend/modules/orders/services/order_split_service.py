...
class OrderSplitService:
    ...
    def validate_split_request(
        self, order_id: int, split_request: OrderSplitRequest
    ) -> SplitValidationResponse:
        """
    ...
    def split_order(
        self, order_id: int, split_request: OrderSplitRequest, current_user_id: int
    ) -> OrderSplitResponse:
        """
    ...
    def _split_by_ticket(
        self, parent_order: Order, split_request: OrderSplitRequest, user_id: int
    ) -> Dict[str, Any]:
        ...
        for group_name, items in item_groups.items():
            ...
            new_order = Order(
                ...
                external_id=(
                    f"{parent_order.external_id}_split_{len(split_order_ids) + 1}"
                    if parent_order.external_id
                    else None
                ),
            )
            ...
            # Calculate totals
            tax_rate = (
                float(parent_order.tax_amount / parent_order.subtotal)
                if parent_order.subtotal
                else 0
            )
            ...
            order_split = OrderSplit(
                ...
                or f"Kitchen ticket split - {group_name}",
                split_by=user_id,
                split_metadata={"group_name": group_name, "item_count": len(items)},
            )
        ...
        return {"split_order_ids": split_order_ids, "details": split_details}
    ...
    def _split_by_delivery(
        ...
        new_order = Order(
            ...
            external_id=(
                f"{parent_order.external_id}_delivery"
                if parent_order.external_id
                else None
            ),
        )
        ...
        order_split = OrderSplit(
            ...
            split_metadata={
                ...
                "scheduled_time": (
                    split_request.scheduled_time.isoformat()
                    if split_request.scheduled_time
                    else None
                ),
            },
        )
    ...
    def _split_by_payment(
        ...
        for customer_id, items in customer_groups.items():
            ...
            order_split = OrderSplit(
                ...
                split_by=user_id,
                split_metadata={"customer_id": customer_id, "item_count": len(items)},
            )
            self.db.add(order_split)
    ...
    def get_split_summary(self, order_id: int) -> SplitOrderSummary:
        ...
        splits = (
            self.db.query(OrderSplit)
            .filter(OrderSplit.parent_order_id == order_id)
            .all()
        )
        ...
        payment_splits = (
            ...
            .filter(SplitPayment.parent_order_id == order_id)
            .all()
        )

        # Convert to response models
        split_details = [OrderSplitDetail.model_validate(split) for split in splits]
    ...
    def update_split_payment(
        ...
        """
        payment = (
            self.db.query(SplitPayment).filter(SplitPayment.id == payment_id).first()
        )
    ...
    def merge_split_orders(
        self, merge_request: MergeSplitRequest, current_user_id: int
    ) -> OrderSplitResponse:
        """
        ...
        try:
            ...
            splits = (
                self.db.query(OrderSplit)
                .filter(OrderSplit.split_order_id.in_(merge_request.split_order_ids))
                .all()
            )
            ...
            split_orders = (
                self.db.query(Order)
                .filter(Order.id.in_(merge_request.split_order_ids))
                .all()
            )
    ...
    def _group_items_for_ticket_split(
        self, parent_order: Order, split_items: List[OrderItemSplitRequest]
    ...
    def split_order_for_payment(
        self, order_id: int, payment_request: PaymentSplitRequest, current_user_id: int
    ) -> OrderSplitResponse:
        """
        ...
        try:
            ...
            total_split_amount = sum(
                Decimal(str(split.get("amount", 0))) for split in payment_request.splits
            )
    ...
    def get_split_tracking(self, order_id: int) -> Dict[str, Any]:
        ...
        splits = (
            self.db.query(OrderSplit)
            .options(joinedload(OrderSplit.split_order))
            .filter(OrderSplit.parent_order_id == order_id)
            .all()
        )
        ...
        for split in splits:
            ...
            payment = (
                self.db.query(SplitPayment)
                .filter(SplitPayment.split_order_id == split.split_order_id)
                .first()
            )
            ...
            if payment:
                ...
                if payment.payment_status == PaymentStatus.PAID.value:
                    ...
                    split_tracking["payment_summary"][
                        "pending_amount"
                    ] += payment.amount
    ...
    def update_split_status(
        ...
        split_record = (
            self.db.query(OrderSplit)
            .filter(OrderSplit.split_order_id == split_order_id)
            .first()
        )
