...
async def get_print_queue(db: Session) -> PrintQueueStatus:
    ...
    pending_items = len([item for item in queue_items if item.status == PrintStatus.PENDING])
    printing_items = len([item for item in queue_items if item.status == PrintStatus.PRINTING])
    failed_items = len([item for item in queue_items if item.status == PrintStatus.FAILED])
...
async def retry_failed_tickets(db: Session, max_retries: int = 3) -> List[PrintTicketResponse]:
...
async def _route_to_station(order, ticket_type: TicketType, db: Session) -> Optional[int]:
    station_mapping = {
