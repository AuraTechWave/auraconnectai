...
async def get_print_queue(db: Session) -> PrintQueueStatus:
    ...
    pending_items = len(
        [item for item in queue_items if item.status == PrintStatus.PENDING]
    )
    printing_items = len(
        [item for item in queue_items if item.status == PrintStatus.PRINTING]
    )
    failed_items = len(
        [item for item in queue_items if item.status == PrintStatus.FAILED]
    )
...
async def update_print_ticket_status(
    ...
    if not ticket:
        raise HTTPException(
            status_code=404, detail=f"Print ticket with id {ticket_id} not found"
        )
...
async def _route_to_station(
    ...
    stations = db.query(PrintStation).filter(PrintStation.is_active).all()
...
async def _get_queue_items(db: Session) -> List[PrintQueueItem]:
    tickets = (
        ...
        .order_by(PrintTicket.priority.desc(), PrintTicket.created_at.asc())
        .all()
    )
    return [
        PrintQueueItem(
