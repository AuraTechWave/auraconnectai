...
class OrderTrackingService:
    ...
    def _generate_tracking_code(self) -> str:
        ...
        while True:
            ...
            existing = (
                self.db.query(CustomerOrderTracking)
                .filter(CustomerOrderTracking.tracking_code == code)
                .first()
            )
            if not existing:
                return code
    ...
    def get_order_tracking_by_code(self, tracking_code: str) -> Optional[Dict]:
        ...
        tracking = (
            self.db.query(CustomerOrderTracking)
            .filter(CustomerOrderTracking.tracking_code == tracking_code)
            .first()
        )
        ...
        events = (
            self.db.query(OrderTrackingEvent)
        ...
        return {
            ...
            "estimated_completion_time": (
                events[0].estimated_completion_time if events else None
            ),
            "events": [
                {
                    "id": event.id,
    ...
    def get_order_tracking_by_token(self, access_token: str) -> Optional[Dict]:
        ...
        tracking = (
            self.db.query(CustomerOrderTracking)
            .filter(CustomerOrderTracking.access_token == access_token)
            .first()
        )
    ...
    async def _send_event_notifications(self, event: OrderTrackingEvent, order: Order):
        """Send notifications for a tracking event"""
        # Get customer tracking preferences
        tracking = (
            self.db.query(CustomerOrderTracking)
            .filter(CustomerOrderTracking.order_id == order.id)
            .first()
        )
        ...
        templates = (
            self.db.query(OrderTrackingTemplate)
            .filter(
                and_(
            ...
            .order_by(OrderTrackingTemplate.priority.desc())
            .all()
        )
        ...
        if not templates:
            # Use default message
            message = (
                f"Order #{order.id}: {event.description or event.event_type.value}"
            )
        else:
            ...
            message = template.message_template.format(
                ...
                estimated_time=(
                    event.estimated_completion_time.strftime("%I:%M %p")
            ...
            if email_config:
                try:
                    sent = await self.notification_adapter.send_to_user(
                        user_id=tracking.customer_id or 0, message=notification_msg
                    )
        ...
        if (
            tracking.enable_sms
            and tracking.notification_phone
            and event.event_type
            in [TrackingEventType.ORDER_READY, TrackingEventType.ORDER_DELIVERED]
        ):
    ...
    async def _broadcast_websocket_update(
        self, order_id: int, event: OrderTrackingEvent
    ):
        ...
        update = {
            ...
            "event": {
                ...
                "estimated_completion_time": (
                    event.estimated_completion_time.isoformat()
                    if event.estimated_completion_time
                    else None
                ),
                "location": (
    ...
    def register_websocket(self, websocket: WebSocket, session_id: str, order_id: int):
    ...
    def unregister_websocket(self, websocket: WebSocket, session_id: str):
        ...
        if session_id in self._websocket_connections:
            ...
            if not connections:
                ...
                tracking = (
                    self.db.query(CustomerOrderTracking)
                    .filter(CustomerOrderTracking.websocket_session_id == session_id)
                    .first()
                )
                if tracking:
                    tracking.websocket_connected = False
    ...
    def get_active_orders_for_customer(
        self, customer_id: int, include_completed: bool = False
    ) -> List[Dict]:
        """
        ...
        orders = (
            self.db.query(Order)
            .join(CustomerOrderTracking, Order.id == CustomerOrderTracking.order_id)
            .filter(
                and_(
            ...
            result.append(
                {
                    ...
                    "estimated_completion_time": (
                        latest_event.estimated_completion_time if latest_event else None
                    ),
                    "latest_event": (
                        {
    ...
    async def send_delivery_location_update(
        ...
        return await self.create_tracking_event(
            ...
            metadata={"update_type": "driver_location", "driver_id": driver_id},
        )
