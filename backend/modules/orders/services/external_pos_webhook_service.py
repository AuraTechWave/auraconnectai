...
class ExternalPOSWebhookService:
    ...
    async def process_webhook_event(self, webhook_event_id: int) -> bool:
        ...
        """
        webhook_event = (
            self.db.query(ExternalPOSWebhookEvent)
            .filter(ExternalPOSWebhookEvent.id == webhook_event_id)
            .first()
        )
        ...
        try:
            ...
            logger.error(
                f"Error processing webhook {webhook_event_id}: {str(e)}", exc_info=True
            )
    ...
    async def _process_payment_event(
        self, webhook_event: ExternalPOSWebhookEvent
    ) -> bool:
        ...
        if order:
            ...
            self._log_webhook_event(
                ...
                {
                    "order_id": order.id,
                    "payment_amount": str(payment_update.payment_amount),
                },
            )
        else:
    ...
    def _extract_payment_data(
        self, provider_code: str, body: Dict[str, Any]
    ...
    def _extract_square_payment_data(
        ...
        return {
            ...
            / settings.WEBHOOK_CENTS_TO_DOLLARS,  # Square uses cents
            "currency": amount_money.get("currency", "USD"),
            "card_last_four": payment.get("card_details", {})
            .get("card", {})
            .get("last_4"),
            "card_brand": payment.get("card_details", {})
            .get("card", {})
            .get("card_brand"),
            "customer_email": payment.get("buyer_email_address"),
            "tip_amount": payment.get("tip_money", {}).get("amount", 0)
            / settings.WEBHOOK_CENTS_TO_DOLLARS,
        }

    def _extract_stripe_payment_data(
        self, body: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "method": self._map_stripe_payment_method(
                payment_intent.get("payment_method_types", [])
            ),
            "amount": payment_intent.get("amount", 0)
            / settings.WEBHOOK_CENTS_TO_DOLLARS,  # Stripe uses cents
            "currency": payment_intent.get("currency", "usd").upper(),
            "customer_email": payment_intent.get("receipt_email"),
            "tip_amount": metadata.get("tip_amount", 0),
        }

    def _extract_toast_payment_data(
        self, body: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
    ...
    def _extract_generic_payment_data(
        self, body: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        ...
        return {
            "transaction_id": (
                data.get("transaction_id") or data.get("payment_id") or data.get("id")
            ),
    ...
    async def _match_order(
        self, payment_update: ExternalPOSPaymentUpdate
    ) -> Optional[Order]:
        ...
        if payment_update.external_order_id:
            order = (
                self.db.query(Order)
                .filter(Order.external_id == payment_update.external_order_id)
                .first()
            )
        ...
        time_window_start = payment_update.created_at - timedelta(
            minutes=settings.WEBHOOK_DUPLICATE_WINDOW_MINUTES
        )
        time_window_end = payment_update.created_at + timedelta(
            minutes=settings.WEBHOOK_ORDER_MATCH_WINDOW_MINUTES
        )
    ...
    async def _create_payment_reconciliation(
        self, order: Order, payment_update: ExternalPOSPaymentUpdate
    ):
        ...
        existing = (
            ...
            .filter(
                ...
                == payment_update.external_transaction_id,
            )
            .first()
        )
    ...
    async def _is_duplicate_event(self, webhook_event: ExternalPOSWebhookEvent) -> bool:
        ...
        duplicate_window = datetime.utcnow() - timedelta(
            minutes=settings.WEBHOOK_DUPLICATE_WINDOW_MINUTES
        )
    ...
    async def retry_failed_webhooks(
        self, limit: int = settings.WEBHOOK_RECENT_EVENTS_LIMIT
    ) -> int:
        ...
        failed_events = (
            ...
            .filter(
                ...
                ExternalPOSWebhookEvent.processing_attempts
                < self.max_processing_attempts,
            )
            .order_by(ExternalPOSWebhookEvent.created_at.asc())
            .limit(limit)
            .all()
        )
