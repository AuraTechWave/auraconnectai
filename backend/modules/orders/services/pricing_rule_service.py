...
class PricingRuleService:
    ...
    async def evaluate_rules_for_order(
        self, db: AsyncSession, order: Order, debug: bool = False
            ...
            for rule in rules:
                ...
                if result.applicable:
                    ...
                    pricing_metrics_collector.record_rule_skipped(
                        order.restaurant_id, result.skip_reason or "unknown"
                    )
    ...
    async def _evaluate_rule(
        self, db: AsyncSession, rule: PricingRule, order: Order
    ) -> RuleEvaluationResult:
        ...
        try:
            ...
            if "time" in rule.conditions:
                time_valid = await self._check_time_conditions(rule.conditions["time"])
            ...
            result.discount_amount = await self._calculate_discount(rule, order)
    ...
    async def _check_time_conditions(self, time_conditions: Dict[str, Any]) -> bool:
    ...
    async def _check_item_conditions(
        self, db: AsyncSession, item_conditions: Dict[str, Any], order: Order
    ) -> bool:
    ...
    async def _check_customer_conditions(
        self, db: AsyncSession, customer_conditions: Dict[str, Any], order: Order
    ) -> bool:
    ...
    async def _check_order_conditions(
        self, order_conditions: Dict[str, Any], order: Order
    ) -> bool:
    ...
    async def _calculate_discount(self, rule: PricingRule, order: Order) -> Decimal:
    ...
    async def _resolve_conflicts(
        self, results: List[RuleEvaluationResult], order: Order
        ...
        if non_stackable:
            if (
                order.restaurant.default_conflict_resolution
    ...
    async def _apply_rule(
        self, db: AsyncSession, result: RuleEvaluationResult, order: Order
    ...
    async def _update_metrics(
        self, db: AsyncSession, applications: List[PricingRuleApplication]
    ):
        ...
        # Group by rule
        rule_metrics = defaultdict(
            lambda: {"count": 0, "discount": Decimal("0"), "customers": set()}
        )
        ...
        for rule_id, metrics in rule_metrics.items():
            ...
            if not metric:
                metric = PricingRuleMetrics(rule_id=rule_id, date=datetime.utcnow())
                db.add(metric)
            ...
            metric.conflicts_skipped += self.metrics_collector.get(
                "conflicts_skipped", 0
            )
        ...
        try:
            ...
            self.audit_logger.log_action(
                ...
                details={
                    ...
                    "discount_value": (
                        float(rule.discount_value) if rule.discount_value else None
                    ),
