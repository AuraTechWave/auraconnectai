...
class ManualReviewQueue(Base):
    ...
    assigned_user = relationship("User", foreign_keys=[assigned_to], backref="assigned_reviews")
    reviewer = relationship("User", foreign_keys=[reviewed_by], backref="completed_reviews")
...
class InventoryAdjustmentAttempt(Base):
    ...
    # What was attempted
    attempted_deductions = Column(JSON, nullable=True)  # List of attempted inventory changes
    menu_items_affected = Column(JSON, nullable=True)  # Menu items that couldn't be processed
