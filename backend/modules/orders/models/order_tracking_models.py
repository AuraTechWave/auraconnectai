...
class OrderNotification(Base, TimestampMixin):
    ...
    tracking_event_id = Column(
        Integer, ForeignKey("order_tracking_events.id"), nullable=True, index=True
    )
    customer_id = Column(Integer, ForeignKey("customers.id"), nullable=True, index=True)
    ...
    )  # e.g., Firebase message ID, SendGrid ID
