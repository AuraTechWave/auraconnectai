...
from sqlalchemy import (
    ...
    Numeric, Text, Boolean, Enum, Index
)
...
class ExternalPOSWebhookEvent(Base, TimestampMixin):
    ...
    # Event details
    event_type = Column(String(100), nullable=False, index=True)  # payment.completed, payment.failed, etc.
...
class ExternalPOSPaymentUpdate(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    webhook_event_id = Column(Integer, ForeignKey("external_pos_webhook_events.id"), nullable=False, index=True)
    ...
    # Payment details
    payment_status = Column(String(50), nullable=False)  # completed, failed, pending, refunded
    ...
    webhook_event = relationship("ExternalPOSWebhookEvent", back_populates="payment_updates")
...
class ExternalPOSWebhookLog(Base, TimestampMixin):
    ...
    log_type = Column(String(50), nullable=False)  # validation, processing, mapping, etc.
