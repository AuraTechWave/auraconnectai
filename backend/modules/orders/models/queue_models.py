...
class OrderQueue(Base, TimestampMixin):
    ...
    queue_items = relationship(
        "QueueItem", back_populates="queue", cascade="all, delete-orphan"
    )
    queue_metrics = relationship("QueueMetrics", back_populates="queue")

    # Indexes
    __table_args__ = (Index("idx_queue_type_status", "queue_type", "status"),)
...
class QueueMetrics(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "queue_id", "metric_date", "hour_of_day", name="uq_queue_metrics_period"
        ),
