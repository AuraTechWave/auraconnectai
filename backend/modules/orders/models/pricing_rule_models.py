...
class PricingRule(Base, TimestampMixin):
    ...
    status = Column(
        SQLEnum(RuleStatus), nullable=False, default=RuleStatus.ACTIVE, index=True
    )
    ...
    excluded_rule_ids = Column(
        JSONB, default=[]
    )  # Rules that can't be combined with this
    conflict_resolution = Column(
        SQLEnum(ConflictResolution), default=ConflictResolution.HIGHEST_DISCOUNT
    )
