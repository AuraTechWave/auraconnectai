...
class OrderSyncStatus(Base, TimestampMixin):
    ...
    # Sync tracking
    sync_status = Column(
        SQLEnum(SyncStatus), nullable=False, default=SyncStatus.PENDING, index=True
    )
    sync_direction = Column(
        SQLEnum(SyncDirection), nullable=False, default=SyncDirection.LOCAL_TO_REMOTE
    )
...
class SyncConfiguration(Base, TimestampMixin):
    ...
    def get_config(cls, db_session, key: str, default=None):
        ...
        config = (
            ...
            .filter(cls.config_key == key, cls.is_active == True)
            .first()
        )
...
class SyncConflict(Base, TimestampMixin):
    ...
    )  # data_mismatch, version_conflict, etc.
