...
from sqlalchemy import (
    ...
    Enum, Text, JSON, Float, Index, UniqueConstraint, CheckConstraint
)
...
class OrderRoutingRule(Base, TimestampMixin):
    ...
    # Rule configuration
    priority = Column(Integer, nullable=False, default=0, index=True)  # Higher priority evaluated first
    status = Column(Enum(RuleStatus), nullable=False, default=RuleStatus.ACTIVE, index=True)
    ...
    schedule_config = Column(JSONB, nullable=True)  # {"days": ["monday", "tuesday"], "hours": {"start": "09:00", "end": "17:00"}}
    ...
    # Relationships
    conditions = relationship("RoutingRuleCondition", back_populates="rule", cascade="all, delete-orphan")
    actions = relationship("RoutingRuleAction", back_populates="rule", cascade="all, delete-orphan")
...
class RoutingRuleCondition(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    rule_id = Column(Integer, ForeignKey("order_routing_rules.id", ondelete="CASCADE"), nullable=False, index=True)
    
    # Condition configuration
    field_path = Column(String(200), nullable=False)  # e.g., "order.type", "item.category", "customer.vip_status"
...
class RoutingRuleAction(Base, TimestampMixin):
    ...
    action_type = Column(String(50), nullable=False)  # "route", "notify", "tag", "priority", "split"
    action_config = Column(JSONB, nullable=False)  # Action-specific configuration
    execution_order = Column(Integer, default=0)  # Order of execution for multiple actions
    
    # Conditional execution
    condition_expression = Column(Text, nullable=True)  # Optional additional condition for this action
...
class RoutingRuleLog(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    rule_id = Column(Integer, ForeignKey("order_routing_rules.id"), nullable=False, index=True)
    ...
    conditions_evaluated = Column(JSONB, nullable=True)  # Which conditions passed/failed
...
class RouteOverride(Base, TimestampMixin):
    ...
    override_type = Column(String(50), nullable=False)  # "manual", "system", "emergency"
    target_type = Column(Enum(RouteTargetType), nullable=False)
...
class StaffRoutingCapability(Base, TimestampMixin):
    ...
    capability_type = Column(String(50), nullable=False)  # "category", "station", "skill", "certification"
    capability_value = Column(String(100), nullable=False)  # "grill", "sushi_chef", "alcohol_service"
    ...
    # Time restrictions
    available_schedule = Column(JSONB, nullable=True)  # Schedule when this capability is active
    
    __table_args__ = (
        UniqueConstraint('staff_id', 'capability_type', 'capability_value', name='uq_staff_capability'),
...
class TeamRoutingConfig(Base, TimestampMixin):
    ...
    routing_strategy = Column(String(50), nullable=False, default="round_robin")  # "round_robin", "least_loaded", "skill_based"
    ...
    # Team specialization
    specializations = Column(JSONB, nullable=True)  # ["lunch_rush", "catering", "delivery"]
    ...
    __table_args__ = (
        Index('idx_team_routing_active', 'is_active'),
    )
...
class TeamMember(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    team_id = Column(Integer, ForeignKey("team_routing_configs.id", ondelete="CASCADE"), nullable=False, index=True)
    staff_id = Column(Integer, ForeignKey("staff_members.id"), nullable=False, index=True)
