...
from ..services.order_service import (
    ...
    get_order_by_id as get_order_service,
    get_orders_service,
    ...
    get_order_audit_events_service,
    count_order_audit_events_service,
...
async def update_order(
    order_id: int, order_data: OrderUpdate, db: Session, user_id: int
):
...
async def validate_order_rules(
    rule_request: MultiItemRuleRequest, db: Session
) -> RuleValidationResult:
    return await validate_multi_item_rules(
        rule_request.order_items, rule_request.rule_types, db
    )


async def validate_special_instructions(
    order_items: List[OrderItemUpdate], db: Session
) -> dict:
    validation_results = []
        ...
        if item.special_instructions:
                ...
                validation_results.append(
                    {
                        ...
                        "error": (
                            f"Too many instructions (max 10), "
            ...
            for instruction in item.special_instructions:
                ...
                if instruction.priority and (
                    instruction.priority < 1 or instruction.priority > 5
                ):
                    validation_results.append(
                        {
                            "item_id": item.menu_item_id,
                            "error": (
                                f"Priority must be between 1-5, "
                                f"got {instruction.priority}"
                            ),
                        }
                    )

                if instruction.target_station and len(instruction.target_station) > 50:
                    validation_results.append(
                        {
                            "item_id": item.menu_item_id,
                            "error": (
                                f"Station name too long (max 50 chars), "
                                f"got {len(instruction.target_station)}"
                            ),
                        }
                    )
    return {"valid": len(validation_results) == 0, "errors": validation_results}


async def create_order_with_validation(
    order_data: dict, db: Session, skip_fraud_check: bool = False
):
    return await create_order_with_fraud_check(
        db, order_data, perform_fraud_validation=not skip_fraud_check
    )
...
async def add_order_tags(order_id: int, tag_request: OrderTagRequest, db: Session):
...
async def update_order_category(
    order_id: int, category_request: OrderCategoryRequest, db: Session
):
...
async def list_tags(db: Session, limit: int = 100, offset: int = 0) -> List[TagOut]:
...
async def list_archived_orders(
    ...
    orders = await get_archived_orders_service(
        db, staff_id=staff_id, table_no=table_no, limit=limit, offset=offset
    )
    return [OrderOut.model_validate(order) for order in orders]


async def update_order_priority(
    order_id: int, priority_data: OrderPriorityUpdate, db: Session
):
...
async def get_order_audit_trail(
    ...
    for event in events_data:
        try:
            ...
            logger.warning(f"Skipping malformed audit record " f"{event.id}: {str(e)}")
            continue
...
async def create_auto_cancellation_config_controller(config_data: dict, db: Session):
...
async def create_default_configs_controller(
    ...
    from ..services.order_service import create_default_auto_cancellation_configs

    return await create_default_auto_cancellation_configs(db, tenant_id, updated_by)
