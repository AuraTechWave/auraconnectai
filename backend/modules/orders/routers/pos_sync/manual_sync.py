...
@router.post(
    "/sync", response_model=POSSyncResponse, status_code=status.HTTP_202_ACCEPTED
)
...
async def _sync_specific_orders(
    ...
    valid_orders = (
        db.query(Order).filter(Order.id.in_(order_ids), Order.is_deleted == False).all()
    )
    ...
    background_tasks.add_task(
        process_sync_batch, order_ids=orders_to_sync, terminal_id=terminal_id
    )
...
async def _sync_all_pending_orders(
    ...
    query = (
        ...
        .filter(
            ...
            OrderSyncStatus.sync_status.in_(
                [SyncStatus.PENDING, SyncStatus.FAILED, SyncStatus.RETRY]
            ),
        )
    )
    ...
    if include_recent:
        ...
        recent_cutoff = datetime.utcnow() - timedelta(
            hours=settings.POS_SYNC_RECENT_HOURS
        )
        query = query.filter(
            db.or_(
    ...
    if not pending_sync_statuses:
        ...
        orders_without_sync = (
            ...
            .filter(Order.is_deleted == False, OrderSyncStatus.id.is_(None))
            .all()
        )
    ...
    if order_ids:
        ...
        return POSSyncResponse(
            ...
            details={"sync_type": "scheduled_batch", "include_recent": include_recent},
        )
    else:
        return POSSyncResponse(
            status="completed",
