...
@router.get(
    "/sync/status", response_model=SyncStatusResponse, status_code=status.HTTP_200_OK
)
async def get_pos_sync_status(
    ...
    for status in SyncStatus:
        count = (
            ...
            .filter(OrderSyncStatus.sync_status == status)
            .count()
        )
    ...
    last_batch = db.query(SyncBatch).order_by(SyncBatch.started_at.desc()).first()
    ...
    if last_batch:
        last_batch_info = {
            ...
            "completed_at": (
                last_batch.completed_at.isoformat() if last_batch.completed_at else None
            ),
