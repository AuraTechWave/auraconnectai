...
async def get_webhook_health(
    ...
    for provider in providers:
        ...
        last_24h = datetime.utcnow() - timedelta(
            hours=settings.WEBHOOK_HEALTH_CHECK_HOURS
        )
        recent_events = (
        ...
        failed_count = sum(
            ...
            if e.processing_status == WebhookProcessingStatus.FAILED
        )
        success_count = sum(
            1
            for e in recent_events
            if e.processing_status == WebhookProcessingStatus.PROCESSED
        )
        ...
        provider_status.append(
            {
                ...
                "success_rate": (
                    (success_count / len(recent_events) * 100) if recent_events else 100
                ),
                "status": (
                    "healthy"
                    if failed_count == 0
                        ...
                        "degraded"
                        if failed_count < settings.WEBHOOK_HEALTH_DEGRADED_THRESHOLD
                        else "unhealthy"
                    )
                ),
            }
    ...
    recent_errors = (
        ...
        .filter(
            ...
            ExternalPOSWebhookEvent.created_at
    ...
    return WebhookHealthStatus(
        overall_status=(
            "healthy"
            if all(p["status"] == "healthy" for p in provider_status)
            else "degraded"
        ),
...
async def get_webhook_statistics(
    time_period_hours: int = Query(
        settings.WEBHOOK_STATS_DEFAULT_HOURS, ge=1, le=settings.WEBHOOK_STATS_MAX_HOURS
    ),
    ...
    for provider in providers:
        ...
        status_counts = (
            ...
            .group_by(ExternalPOSWebhookEvent.processing_status)
            .all()
        )
        ...
        avg_processing_time = (
            db.query(
                func.avg(
                    func.extract(
                        "epoch",
                        ExternalPOSWebhookEvent.processed_at
                        - ExternalPOSWebhookEvent.created_at,
                    )
        ...
        statistics.append(
            WebhookStatistics(
                ...
                success_rate=(
                    status_dict.get(WebhookProcessingStatus.PROCESSED, 0)
                    / total_events
                    * 100
                    if total_events > 0
                    else 0
