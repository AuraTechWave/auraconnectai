...
async def receive_webhook_event(
    ...
    if not provider:
        logger.warning(
            f"Webhook received for unknown/inactive provider: {provider_code}"
        )
        raise HTTPException(
            status_code=404, detail=f"Provider {provider_code} not found or inactive"
        )
    ...
    try:
        ...
        is_valid, error_msg, verification_details = (
            await auth_service.verify_webhook_request(
        ...
        if not is_valid:
            ...
            raise HTTPException(
                status_code=401, detail=f"Webhook authentication failed: {error_msg}"
            )

        # Process webhook asynchronously
        background_tasks.add_task(
            process_webhook_async, webhook_event_id=webhook_event.id
        )
        ...
        logger.error(
            ...
            extra={"event_id": event_id, "provider": provider_code},
            exc_info=True,
        )
...
async def get_provider_webhook_status(
    provider_code: str, db: Session = Depends(get_db)
) -> Dict[str, Any]:
    """Get webhook configuration status for a provider"""

    provider = (
        db.query(ExternalPOSProvider)
        .filter(ExternalPOSProvider.provider_code == provider_code)
        .first()
    )
    ...
    recent_webhooks = (
        ...
        .filter(ExternalPOSWebhookEvent.provider_id == provider.id)
        .order_by(ExternalPOSWebhookEvent.created_at.desc())
        .limit(settings.WEBHOOK_RECENT_EVENTS_LIMIT)
        .all()
    )
...
async def test_webhook_endpoint(
    ...
    test_headers = auth_service.generate_test_signature(
        provider_code=provider_code, body=test_data
    )

    if not test_headers:
        return {"error": "Could not generate test signature", "provider": provider_code}
...
def _extract_event_type(provider_code: str, body: Dict[str, Any]) -> str:
    ...
    if provider_code == POSProviderEnum.SQUARE:
        ...
        if provider_code in PROVIDER_EVENT_MAPPING:
            for provider_event, generic_event in PROVIDER_EVENT_MAPPING[
                provider_code
            ].items():
