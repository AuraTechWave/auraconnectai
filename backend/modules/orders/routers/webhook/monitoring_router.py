...
async def get_webhook_event_detail(
    ...
    event = db.query(ExternalPOSWebhookEvent).filter(
        ExternalPOSWebhookEvent.id == event_id
    ).first()
    ...
    payment_updates = db.query(ExternalPOSPaymentUpdate).filter(
        ExternalPOSPaymentUpdate.webhook_event_id == event_id
    ).all()
...
async def get_webhook_event_logs(
    ...
    logs = db.query(ExternalPOSWebhookLog).filter(
        ExternalPOSWebhookLog.webhook_event_id == event_id
    ).order_by(
...
async def retry_failed_webhooks(
    limit: int = Query(settings.WEBHOOK_RECENT_EVENTS_LIMIT, ge=1, le=settings.WEBHOOK_RETRY_API_LIMIT),
    db: Session = Depends(get_db),
    current_user: StaffMember = Depends(get_current_user)
):
...
async def trigger_scheduler(
    ...
    if success:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to trigger webhook retry scheduler"
        )