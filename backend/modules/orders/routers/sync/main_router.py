...
from modules.orders.models.sync_models import SyncBatch, OrderSyncStatus, SyncStatus
from modules.orders.tasks.sync_tasks import order_sync_scheduler
from modules.orders.schemas.sync_schemas import SyncBatchResponse, ManualSyncRequest
    ...
    if request.order_ids:
        ...
        for order_id in request.order_ids:
            try:
                ...
                results[order_id] = {"success": success, "error": error}
                ...
                logger.error(
                    f"Unexpected error syncing order {order_id}: {e}", exc_info=True
                )
                results[order_id] = {
                    "success": False,
...
async def retry_failed_syncs(
    ...
    updated = (
        ...
        .filter(OrderSyncStatus.sync_status == SyncStatus.FAILED)
        .limit(limit)
        .update({"sync_status": SyncStatus.RETRY, "next_retry_at": datetime.utcnow()})
    )
