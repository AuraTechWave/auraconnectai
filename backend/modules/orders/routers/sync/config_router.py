...
async def update_sync_configuration(
    ...
    if current_user.role not in ["admin", "manager"]:
        raise HTTPException(
            status_code=403, detail="Only administrators can modify sync configuration"
        )
    ...
    if config.sync_enabled is not None:
        sync_config = (
            ...
            .filter(SyncConfiguration.config_key == "sync_enabled")
            .first()
        )
    ...
    if config.conflict_resolution_mode is not None:
        if config.conflict_resolution_mode not in ["auto", "manual"]:
            raise HTTPException(
                status_code=400, detail="Invalid conflict resolution mode"
            )

        sync_config = (
            db.query(SyncConfiguration)
            .filter(SyncConfiguration.config_key == "conflict_resolution_mode")
            .first()
        )
