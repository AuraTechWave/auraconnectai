...
rule_stacking_summary = Summary(
    'pricing_rule_stacking',
    'Summary of rule stacking occurrences',
    ['restaurant_id']
)
...
class PricingRuleMetricsCollector:
    ...
    def record_rules_per_order(self, restaurant_id: int, rule_count: int):
        ...
        rules_per_order_histogram.labels(
            restaurant_id=str(restaurant_id)
        ).observe(rule_count)
    ...
    def record_stacking(self, restaurant_id: int, stack_count: int):
        ...
        rule_stacking_summary.labels(
            restaurant_id=str(restaurant_id)
        ).observe(stack_count)
    
    def update_active_rules_gauge(self, restaurant_id: int, rule_counts: Dict[str, int]):
