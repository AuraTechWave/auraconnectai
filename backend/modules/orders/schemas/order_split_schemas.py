...
class OrderSplitRequest(BaseModel):
    ...
    items: List[OrderItemSplitRequest] = Field(..., min_items=1, description="Items to split")
    split_reason: Optional[str] = Field(None, description="Reason for splitting")
    customer_id: Optional[int] = Field(None, description="Customer ID for the split order")
    table_no: Optional[int] = Field(None, description="Table number for split order")
    delivery_address: Optional[Dict[str, Any]] = Field(None, description="Delivery address if splitting for delivery")
    scheduled_time: Optional[datetime] = Field(None, description="Scheduled time for split order")
...
class PaymentSplitRequest(BaseModel):
    ...
    splits: List[Dict[str, Any]] = Field(..., min_items=2, description="Payment split details")
    ...
    def validate_splits(cls, v):
        ...
        for split in v:
            ...
            if 'customer_id' not in split and 'payment_method' not in split:
                raise ValueError("Each split must have either customer_id or payment_method")
        return v
...
class MergeSplitRequest(BaseModel):
    ...
    split_order_ids: List[int] = Field(..., min_items=2, description="Split order IDs to merge")
...
class BulkSplitRequest(BaseModel):
    ...
    split_type: SplitType
    split_strategy: str = Field(..., description="Strategy: 'by_station', 'by_customer', 'by_course'")
    split_configs: List[Dict[str, Any]] = Field(..., description="Configuration for each split")
