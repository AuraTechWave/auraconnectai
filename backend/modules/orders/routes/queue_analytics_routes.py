...
async def get_queue_dashboard(
    ...
    for queue in queues:
        ...
        # Update summary
        dashboard["summary"]["total_items_processed"] += queue_data["metrics"]["items_completed"]
    
    # Calculate overall metrics
    if dashboard["queues"]:
        total_wait_time = sum(q["metrics"]["avg_wait_time"] * q["metrics"]["items_completed"] 
                             for q in dashboard["queues"])
        total_items = dashboard["summary"]["total_items_processed"]
        
        if total_items > 0:
            dashboard["summary"]["avg_wait_time"] = round(total_wait_time / total_items, 1)
        
        # On-time percentage
        total_on_time = sum(q["metrics"]["on_time_percentage"] * q["metrics"]["items_completed"] 
                           for q in dashboard["queues"])
        if total_items > 0:
            dashboard["summary"]["on_time_percentage"] = round(total_on_time / total_items, 1)
...
async def get_queue_performance(
    ...
    metrics = db.query(QueueMetrics).filter(
        ...
        QueueMetrics.metric_date >= start_date
    ).all()
...
async def get_staff_utilization(
    ...
    for stat in staff_stats:
        ...
        staff = db.query(StaffMember).filter(
            StaffMember.id == stat.assigned_to_id
        ).first()
        ...
        if staff:
            utilization.append({
                ...
                "avg_prep_time": round(stat.avg_prep_time, 1) if stat.avg_prep_time else 0,
                "efficiency_score": round(stat.efficiency_score, 1) if stat.efficiency_score else 0
            })
    ...
    return {
        ...
        "summary": {
            ...
            "avg_efficiency": round(
                sum(u["efficiency_score"] for u in utilization) / len(utilization), 1
...
async def get_peak_analysis(
    ...
    for hour, days in heat_map.items():
        ...
        peak_hours.append({
            ...
            "peak_days": [dow for dow, data in days.items() if data["volume"] > avg_volume * 1.2]
        })
...
# Helper functions
async def _get_queue_analytics(db: Session, queue: OrderQueue, start: datetime, end: datetime) -> Dict[str, Any]:
    """Get analytics for a single queue"""
    # Current status
    current_items = db.query(
        QueueItem.status,
    ...
    # Period metrics
    period_items = db.query(QueueItem).filter(
        QueueItem.queue_id == queue.id,
    ...
    metrics = {
        ...
        "items_cancelled": len([i for i in period_items if i.status == QueueItemStatus.CANCELLED]),
        "avg_wait_time": 0,
        "max_wait_time": 0,
        "on_time_percentage": 0
    }
    ...
    if completed_items:
        ...
        # On-time calculation
        on_time = len([i for i in completed_items 
                      if i.completed_at and i.estimated_ready_time 
...
async def _get_queue_trends(db: Session, queue_ids: Optional[List[int]], start: datetime, end: datetime) -> Dict[str, Any]:
...
def _calculate_wait_time_distribution(metrics: List[QueueMetrics]) -> List[Dict[str, Any]]:
...
def _calculate_hourly_performance(metrics: List[QueueMetrics]) -> List[Dict[str, Any]]:
    ...
    for hour, data in hourly.items():
        performance.append({
            ...
            "completion_rate": round(data["completed"] / data["volume"] * 100, 1) if data["volume"] > 0 else 0,
...
def _calculate_daily_trends(metrics: List[QueueMetrics]) -> List[Dict[str, Any]]:
    ...
    for date, data in daily.items():
        trends.append({
            ...
            "avg_wait_time": round(data["wait_time"] / data["completed"], 1) if data["completed"] > 0 else 0,
            "on_time_percentage": round(data["on_time"] / data["completed"], 1) if data["completed"] > 0 else 0
        })
    
    return sorted(trends, key=lambda x: x["date"])


def _identify_bottlenecks(db: Session, queue_id: int, start_date: datetime) -> List[Dict[str, Any]]:
    ...
    ).group_by('hour').having(func.count(QueueItem.id) > 5).all()
