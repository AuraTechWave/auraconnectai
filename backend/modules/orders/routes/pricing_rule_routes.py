...
from ..schemas.pricing_rule_schemas import (
    CreatePricingRuleRequest, UpdatePricingRuleRequest,
    PricingRuleResponse, PricingRuleDebugInfo,
    PricingRuleApplicationResponse, PricingRuleMetricsResponse,
    ValidatePricingRuleRequest, ValidatePricingRuleResponse
)
...
async def list_pricing_rules(
    ...
    return [
        PricingRuleResponse.from_orm(rule, is_valid=rule.is_valid())
        for rule in rules
    ]
...
@router.get("/applications/history", response_model=List[PricingRuleApplicationResponse])
...
async def get_pricing_rule_metrics(
    ...
    return PricingRuleMetricsResponse(
        ...
        average_discount=float(metrics.total_discount or 0) / max(metrics.total_applications or 1, 1),
