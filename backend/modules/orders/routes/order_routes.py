...
async def get_orders(
    ...
    staff_id: Optional[int] = Query(None, description="Filter by staff member ID"),
    table_no: Optional[int] = Query(None, description="Filter by table number"),
...
async def update_existing_order(
    order_id: int, order_data: OrderUpdate, user_id: int, db: Session = Depends(get_db)
):
...
async def validate_rules(
    rule_request: MultiItemRuleRequest, db: Session = Depends(get_db)
):
    """
...
async def validate_special_instructions_endpoint(
    order_items: List[OrderItemUpdate], db: Session = Depends(get_db)
):
    """
...
async def resolve_fraud_alert(alert_id: int, db: Session = Depends(get_db)):
    """
...
async def delay_order(
    order_id: int, delay_data: DelayFulfillmentRequest, db: Session = Depends(get_db)
):
    """
...
async def add_tags_to_order(
    order_id: int, tag_request: OrderTagRequest, db: Session = Depends(get_db)
):
    """
...
async def remove_tag_from_order(
    order_id: int, tag_id: int, db: Session = Depends(get_db)
):
    """
...
async def set_order_category(
    order_id: int, category_request: OrderCategoryRequest, db: Session = Depends(get_db)
):
    """
...
async def archive_order_endpoint(order_id: int, db: Session = Depends(get_db)):
    """
...
async def restore_order_endpoint(order_id: int, db: Session = Depends(get_db)):
    """
...
async def get_archived_orders_endpoint(
    ...
    - **offset**: Number of orders to skip for pagination
    """
    return await list_archived_orders(db, staff_id, table_no, limit, offset)
...
async def get_kitchen_queue(
    ...
    orders = await get_orders_service(
        db=db, status=None, min_priority=priority_filter, include_items=True, limit=50
    )
...
async def get_priority_distribution(
    ...
    query = db.query(Order.priority, func.count(Order.id).label("count")).group_by(
        Order.priority
    )
    ...
    return {
        (
            row.priority.value if hasattr(row.priority, "value") else str(row.priority)
        ): row.count
        for row in results
    }
...
async def get_order_audit_history(
    order_id: int,
    limit: int = Query(
        100, ge=1, le=1000, description="Number of audit events to return"
    ),
...
async def update_notes(
    order_id: int, notes_update: CustomerNotesUpdate, db: Session = Depends(get_db)
):
    """
...
async def upload_attachment(
    ...
    """
    result = await upload_order_attachment(order_id, file, db, description, is_public)
...
async def get_attachments(order_id: int, db: Session = Depends(get_db)):
    """
...
@router.get(
    "/auto-cancellation/configs", response_model=List[AutoCancellationConfigOut]
)
async def get_auto_cancellation_configs(
    ...
    """
    from ..controllers.order_controller import get_auto_cancellation_configs_controller

    configs = await get_auto_cancellation_configs_controller(
        db, tenant_id, team_id, status
    )
    return [AutoCancellationConfigOut.model_validate(config) for config in configs]


@router.post("/auto-cancellation/configs", response_model=AutoCancellationConfigOut)
async def create_auto_cancellation_config(
    config_data: AutoCancellationConfigCreate, db: Session = Depends(get_db)
):
    """
    ...
    config = await create_auto_cancellation_config_controller(config_data.dict(), db)
    return AutoCancellationConfigOut.model_validate(config)


@router.post("/auto-cancellation/trigger", response_model=StaleCancellationResponse)
async def trigger_stale_order_cancellation(
    tenant_id: Optional[int] = Query(None, description="Filter by tenant ID"),
    team_id: Optional[int] = Query(None, description="Filter by team ID"),
    system_user_id: int = Query(1, description="System user ID for audit logging"),
    db: Session = Depends(get_db),
):
    """
...
@router.post(
    "/auto-cancellation/scheduled-run", response_model=StaleCancellationResponse
)
async def scheduled_auto_cancellation(
    ...
    - **system_user_id**: User ID to use for audit logging (defaults to 1)
    """
    from ..services.auto_cancellation_scheduler import AutoCancellationScheduler

    scheduler = AutoCancellationScheduler(db)
    return await scheduler.run_auto_cancellation(tenant_id, team_id, system_user_id)
...
async def create_default_auto_cancellation_configs(
    ...
    from ..controllers.order_controller import create_default_configs_controller

    configs = await create_default_configs_controller(db, tenant_id, updated_by)
    return {
        "message": f"Created {len(configs)} default configurations",
        "configs": [AutoCancellationConfigOut.model_validate(c) for c in configs],
    }
...
async def get_auto_cancellation_metrics(
    ...
    query = (
        ...
        .filter(AuditLog.action == "auto_cancellation", AuditLog.module == "orders")
