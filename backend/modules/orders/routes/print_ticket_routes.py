...
@router.post("/orders/{order_id}/print-ticket", response_model=PrintTicketResponse)
async def generate_print_ticket(
    order_id: int, request: PrintTicketRequest, db: Session = Depends(get_db)
):
...
@router.put("/print-tickets/{ticket_id}/status", response_model=PrintTicketResponse)
...
@router.post("/print-tickets/retry-failed", response_model=List[PrintTicketResponse])
async def retry_failed_tickets(max_retries: int = 3, db: Session = Depends(get_db)):
