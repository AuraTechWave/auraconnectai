...
@router.post(
    "/{order_id}/calculate-discounts", response_model=DiscountCalculationResponse
)
async def calculate_order_discounts(
    ...
    try:
        ...
        result = service.calculate_order_discounts(
            order=order, coupon_codes=coupon_codes, promotion_ids=promotion_ids
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to calculate discounts: {str(e)}"
        )
...
async def apply_discounts_to_order(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to apply discounts: {str(e)}"
        )
...
async def get_applicable_promotions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get applicable promotions: {str(e)}"
        )
...
async def validate_promotion_for_order(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to validate promotion: {str(e)}"
        )
...
async def get_order_promotion_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get promotion summary: {str(e)}"
        )
...
async def complete_order_with_promotions(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to complete order: {str(e)}"
        )
    ...
    try:
        ...
        for order_id in order_ids:
            try:
                ...
                discount_result = service.calculate_order_discounts(
                    order=order, promotion_ids=[promotion_id]
                )
                ...
                results["total_discount_applied"] += application_result[
                    "total_discount"
                ]

            except Exception as e:
                results["failed_orders"].append({"order_id": order_id, "error": str(e)})
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to bulk apply promotion: {str(e)}"
        )
...
async def get_order_promotion_analytics(
    ...
    try:
        ...
        for usage in usages:
            ...
            if promo_id not in promotion_stats:
                promotion_stats[promo_id] = {
                    ...
                    "promotion_name": (
                        usage.promotion.name if usage.promotion else "Unknown"
                    ),
                    "usage_count": 0,
                    "total_discount": 0.0,
                    "total_revenue": 0.0,
        ...
        return {
            ...
            "summary": {
                ...
                "average_discount_per_order": (
                    round(total_discount / unique_orders, 2) if unique_orders > 0 else 0
                ),
            },
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get promotion analytics: {str(e)}"
        )
...
async def auto_apply_best_promotions(
    ...
    try:
        service = OrderPromotionService(db)
        results = {"processed_orders": [], "total_savings": 0.0}
        ...
        for order_id in order_ids:
            try:
                ...
                promotion_ids = [
                    p["promotion_id"] for p in best_promotions if p["auto_apply"]
                ]

                if promotion_ids:
                    discount_result = service.calculate_order_discounts(
                        order=order, promotion_ids=promotion_ids
                    )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to auto-apply promotions: {str(e)}"
        )
