...
async def create_webhook_config(
    config_data: WebhookConfigurationCreate, db: Session = Depends(get_db)
):
...
async def list_webhook_configs(
    is_active: Optional[bool] = Query(None), db: Session = Depends(get_db)
):
...
async def get_webhook_config(config_id: int, db: Session = Depends(get_db)):
...
async def delete_webhook_config(config_id: int, db: Session = Depends(get_db)):
...
async def test_webhook_config(config_id: int, db: Session = Depends(get_db)):
...
async def receive_incoming_webhook(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to process incoming webhook"
        )
