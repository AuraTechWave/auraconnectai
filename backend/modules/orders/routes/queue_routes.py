...
from ..schemas.queue_schemas import (
    ...
    MoveItemRequest, BulkMoveRequest, TransferItemRequest,
    ExpediteItemRequest, HoldItemRequest, BatchStatusUpdateRequest,
    QueueMetricsRequest, QueueMetricsResponse, QueueStatusSummary,
    SequenceRuleCreate, SequenceRuleUpdate, SequenceRuleResponse,
    DisplayConfigCreate, DisplayConfigResponse,
    QueueUpdateMessage, QueueSubscriptionRequest
)
...
async def get_queue_items(
    ...
    """
    service = QueueService(db)
    return service.get_queue_items(
        queue_id, status, include_completed, limit, offset
    )
        ...
        await manager.send_to_queue(
            str(queue_id),
            {
                ...
                "data": {
                    ...
                    "items": [QueueItemResponse.from_orm(item).dict() for item in queue_items]
                },
                "timestamp": datetime.utcnow().isoformat()
            }
        )
...
async def get_queue_metrics(
    ...
    return QueueMetricsResponse(
        queue_id=metrics_request.queue_id or 0,
