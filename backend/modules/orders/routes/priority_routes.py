...
from ..schemas.priority_schemas import (
    ...
    PriorityProfileDetailResponse, PriorityProfileListResponse,
    QueuePriorityConfigCreate, QueuePriorityConfigUpdate,
    QueuePriorityConfigResponse, QueuePriorityConfigListResponse,
...
@router.post("/rules", response_model=PriorityRuleResponse, status_code=status.HTTP_201_CREATED)
...
def delete_priority_rule(
    ...
    in_use = db.query(PriorityProfileRule).filter(
...
@router.post("/profiles", response_model=PriorityProfileDetailResponse, 
...
def create_priority_profile(
    ...
    existing = db.query(PriorityProfile).filter(
        PriorityProfile.name == profile.name
    ).first()
    if existing:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
...
def get_priority_profile(
    ...
    profile = db.query(PriorityProfile).options(
...
def update_priority_profile(
    ...
    if update.name and update.name != profile.name:
        existing = db.query(PriorityProfile).filter(
            PriorityProfile.name == update.name,
            PriorityProfile.id != profile_id
        ).first()
        if existing:
            raise HTTPException(
...
def delete_priority_profile(
    ...
    in_use = db.query(QueuePriorityConfig).filter(
        QueuePriorityConfig.profile_id == profile_id
    ).first()
    if in_use:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
...
def list_queue_priority_configs(
    ...
    configs = db.query(QueuePriorityConfig).options(
...
@router.post("/queue-configs", response_model=QueuePriorityConfigResponse,
             status_code=status.HTTP_201_CREATED)
def create_queue_priority_config(
    ...
    existing = db.query(QueuePriorityConfig).filter(
        QueuePriorityConfig.queue_id == config.queue_id
    ).first()
    if existing:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
...
def get_queue_priority_config(
    ...
    config = db.query(QueuePriorityConfig).options(
        joinedload(QueuePriorityConfig.queue),
        joinedload(QueuePriorityConfig.profile)
    ).filter(QueuePriorityConfig.id == config_id).first()
...
def update_queue_priority_config(
    ...
    config = db.query(QueuePriorityConfig).filter(
        QueuePriorityConfig.id == config_id
    ).first()
    if not config:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if update.profile_id:
        profile = db.query(PriorityProfile).filter(
            PriorityProfile.id == update.profile_id
        ).first()
        if not profile:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
def calculate_bulk_priority(
    ...
    for queue_item in queue_items:
        ...
        try:
            ...
            old_score = db.query(OrderPriorityScore).filter(
            ...
            if request.apply_boost:
                ...
                config = db.query(QueuePriorityConfig).filter(
                    QueuePriorityConfig.queue_id == request.queue_id
                ).first()
                if config:
                    new_score = service._apply_boost(
                        new_score, config, "bulk_boost"
                    )
...
def get_priority_metrics(
    ...
    if query.queue_id:
        metrics_query = metrics_query.filter(
            PriorityMetrics.queue_id == query.queue_id
        )
    ...
    for metric in metrics:
        queue = db.query(OrderQueue).filter(
            OrderQueue.id == metric.queue_id
        ).first()
        
        results.append(PriorityMetricsResponse(
