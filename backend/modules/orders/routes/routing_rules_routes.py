...
from ..schemas.routing_schemas import (
    ...
    RouteEvaluationRequest, RouteEvaluationResult,
    RouteOverrideCreate, RouteOverrideResponse,
    StaffCapabilityCreate, StaffCapabilityResponse,
    TeamRoutingConfigCreate, TeamRoutingConfigResponse,
    TeamMemberCreate, TeamMemberResponse,
    RoutingRuleTestRequest, RoutingRuleTestResult,
    RoutingLogQuery, RoutingLogResponse,
...
async def list_routing_rules(
    ...
    target_type: Optional[RouteTargetType] = Query(None, description="Filter by target type"),
...
@router.get("/overrides/order/{order_id}", response_model=Optional[RouteOverrideResponse])
@handle_api_errors
async def get_order_override(
    ...
    override = db.query(RouteOverride).filter(
...
@router.get("/staff/{staff_id}/capabilities", response_model=List[StaffCapabilityResponse])
@handle_api_errors
...
async def update_capability_availability(
    ...
    capability = db.query(StaffRoutingCapability).filter(
        StaffRoutingCapability.id == capability_id
    ).first()
...
async def query_routing_logs(
    ...
    return query.order_by(RoutingRuleLog.created_at.desc())\
...
async def check_rule_conflicts(
    rule_id: Optional[int] = Query(None, description="Check conflicts for specific rule"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
...
async def get_rule_performance_analytics(
    ...
    rule_stats = db.query(
        ...
        func.sum(func.cast(RoutingRuleLog.matched, type_=db.Integer)).label('match_count'),
        func.avg(RoutingRuleLog.evaluation_time_ms).label('avg_eval_time'),
        func.sum(func.cast(RoutingRuleLog.error_occurred, type_=db.Integer)).label('error_count')
    ).join(
        OrderRoutingRule, OrderRoutingRule.id == RoutingRuleLog.rule_id
    ).filter(
        RoutingRuleLog.created_at >= cutoff_date
    ).group_by(
        RoutingRuleLog.rule_id, OrderRoutingRule.name
    ).all()
    
    # Format results
    results = []
    for stat in rule_stats:
        match_rate = (stat.match_count / stat.total_evaluations * 100) if stat.total_evaluations > 0 else 0
        error_rate = (stat.error_count / stat.total_evaluations * 100) if stat.total_evaluations > 0 else 0
        ...
        results.append({
            ...
            "avg_evaluation_time_ms": round(stat.avg_eval_time, 2) if stat.avg_eval_time else 0,
    ...
    return {
        ...
        "summary": {
            ...
            "avg_match_rate": round(sum(r['match_rate'] for r in results) / len(results), 2) if results else 0
        }
    }