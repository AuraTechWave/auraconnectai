    ...
    try:
        ...
        for config in configs:
            try:
                ...
                logger.error(f"Failed to rebalance queue {config.queue_id}: {str(e)}")
...
def _calculate_metrics_for_queue(
    ...
    for item in queue_items:
        if item.queued_at and item.completed_at:
            ...
            if order and order.customer_id:
                ...
                if customer and customer.vip_status:
                    ...
                    if (
                        ...
                        and item.completed_at <= order.estimated_delivery_time
                    ):
            ...
            if (
                order
                and order.estimated_delivery_time
                and item.completed_at <= order.estimated_delivery_time
            ):
                on_time_count += 1
    ...
    fairness_index = _calculate_fairness_index(
        [ps.total_score for ps in priority_scores]
    )
    ...
    priority_override_count = (
        db.query(PriorityAdjustmentLog)
        .filter(
            and_(
                PriorityAdjustmentLog.queue_item_id.in_(
    ...
    calculation_times = [
        ps.calculation_time_ms
        for ps in priority_scores
        if ps.calculation_time_ms is not None
    ]
    avg_calculation_time_ms = (
        sum(calculation_times) / len(calculation_times) if calculation_times else 0
    )
    ...
    if queue_items:
        total_revenue = sum(
            float(
                db.query(Order).filter(Order.id == item.order_id).first().total_amount
                or 0
            )
            for item in queue_items
        )
    ...
    return {
        ...
        "on_time_delivery_rate": (
            (on_time_count / len(queue_items)) * 100 if queue_items else 0
        ),
        "vip_satisfaction_score": (
            (vip_orders_on_time / vip_orders_total) * 100 if vip_orders_total > 0 else 0
        ),
    ...
    try:
        ...
        for queue_item in queue_items:
            try:
                ...
                service.calculate_order_priority(queue_item.order_id, queue_id)
...
def cleanup_old_metrics(days_to_keep: int = 30):
    ...
    try:
        ...
        deleted_count = (
            db.query(PriorityMetrics)
            .filter(PriorityMetrics.metric_date < cutoff_date)
            .delete()
        )
        ...
        deleted_logs = (
            db.query(PriorityAdjustmentLog)
            .filter(PriorityAdjustmentLog.adjusted_at < cutoff_date)
            .delete()
        )
...
class PriorityMonitor:
    ...
    async def _collect_metrics(self):
        ...
        try:
            ...
            configs = (
                db.query(QueuePriorityConfig)
                .join(OrderQueue, QueuePriorityConfig.queue_id == OrderQueue.id)
                .filter(OrderQueue.status == QueueStatus.ACTIVE)
                .all()
            )
