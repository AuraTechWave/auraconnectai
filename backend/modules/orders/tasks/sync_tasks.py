...
class OrderSyncScheduler:
        ...
        try:
            ...
            self.scheduler.add_job(
                ...
                trigger=IntervalTrigger(
                    minutes=settings.SYNC_HEALTH_CHECK_INTERVAL_MINUTES
                ),
    ...
    def stop(self):
        ...
        try:
            ...
            logger.error(
                f"Unexpected error stopping sync scheduler: {e}", exc_info=True
            )
    ...
    async def _cleanup_task(self):
        ...
        try:
            ...
            deleted_logs = (
                db.query(SyncLog).filter(SyncLog.started_at < cutoff_date).delete()
            )

            # Delete old batches without logs
            deleted_batches = (
                db.query(SyncBatch)
                .filter(SyncBatch.started_at < cutoff_date, ~SyncBatch.sync_logs.any())
                .delete()
            )
    ...
    def update_sync_interval(self, minutes: int) -> bool:
        ...
        try:
            ...
            try:
                config = (
                    db.query(SyncConfiguration)
                    .filter(SyncConfiguration.config_key == "sync_interval_minutes")
                    .first()
                )
                ...
                if config:
                    ...
                    config = SyncConfiguration(
                        config_key="sync_interval_minutes", config_value=minutes
                    )
                    db.add(config)
            ...
            logger.critical(
                f"Unexpected error updating sync interval: {e}", exc_info=True
            )
            return False
