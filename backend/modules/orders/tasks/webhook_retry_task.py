        ...
        try:
            ...
            self.scheduler.add_job(
                ...
                trigger=IntervalTrigger(
                    minutes=settings.WEBHOOK_RETRY_SCHEDULER_INTERVAL_MINUTES
                ),
        ...
        try:
            ...
            retry_webhooks = (
                ...
                .filter(
                    ...
                    < settings.WEBHOOK_MAX_RETRY_ATTEMPTS,
                )
                .order_by(ExternalPOSWebhookEvent.created_at.asc())
                .limit(settings.WEBHOOK_RETRY_BATCH_SIZE)
                .all()
            )
            ...
            for webhook in retry_webhooks:
                try:
                    ...
                    logger.error(
                        f"Error retrying webhook {webhook.id}: {str(e)}", exc_info=True
                    )
                    failure_count += 1
    ...
    async def _cleanup_old_webhooks_task(self):
        ...
        try:
            ...
            deleted_count = (
                ...
                .limit(settings.WEBHOOK_CLEANUP_MAX_DELETE_BATCH)
