...
class QueueMonitor:
                ...
                try:
                    ...
                    for queue in queues:
                        ...
                        if not existing:
                            ...
                            metrics = self._calculate_queue_metrics(
                                db, queue, current_hour
                            )
                            db.add(metrics)
            ...
            try:
                ...
                try:
                    ...
                    for item in delayed_items:
                        ...
                        delay_minutes = int(
                            (
                                datetime.utcnow() - item.estimated_ready_time
                            ).total_seconds()
                            / 60
                        )
    ...
    def _calculate_queue_metrics(
        self, db: Session, queue: OrderQueue, metric_date: datetime
    ) -> QueueMetrics:
        ...
        items_completed = len(
            [i for i in items if i.status == QueueItemStatus.COMPLETED]
        )
        items_cancelled = len(
            [i for i in items if i.status == QueueItemStatus.CANCELLED]
        )
        items_delayed = len(
            [i for i in items if i.delay_minutes and i.delay_minutes > 0]
        )
        ...
        queue_sizes = (
            ...
            .group_by(func.date_trunc("minute", QueueItem.queued_at))
            .all()
        )

        avg_queue_size = (
            sum(size[0] for size in queue_sizes) / len(queue_sizes)
            if queue_sizes
            else 0
        )
        ...
        return QueueMetrics(
            ...
            on_time_percentage=(
                (on_time_count / items_completed * 100) if items_completed > 0 else 0
            ),
