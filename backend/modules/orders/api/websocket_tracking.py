...
class ConnectionManager:
    ...
    def disconnect(self, websocket: WebSocket):
        ...
        logger.info(
            f"WebSocket disconnected for order {order_id}, session {session_id}"
        )
...
async def websocket_order_tracking(
    ...
    tracking = await get_order_tracking_auth(
        tracking_code=tracking_code, access_token=access_token, db=db
    )
        ...
        while True:
            try:
                ...
                if message.get("type") == "ping":
                    ...
                    # Send current status
                    order_info = tracking_service.get_order_tracking_by_code(
