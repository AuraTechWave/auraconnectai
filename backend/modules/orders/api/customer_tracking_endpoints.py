...
async def enable_order_tracking(
    ...
    existing = db.query(CustomerOrderTracking).filter(
    ...
    background_tasks.add_task(
        ...
        triggered_by_id=current_customer.get("customer_id") if current_customer else None
    )
...
async def get_my_active_orders(
    include_completed: bool = Query(False, description="Include recently completed orders"),
    db: Session = Depends(get_db),
...
async def register_push_token(
    ...
    trackings = db.query(CustomerOrderTracking).join(
        Order,
...
async def test_notification(
    ...
    return {
        "message": f"Test {channel.value} notification sent",
        "event_id": event.id
    }