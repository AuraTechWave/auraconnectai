...
from ..models.reservation_models import (
    Waitlist, WaitlistStatus, Reservation, ReservationStatus,
    ReservationSettings, NotificationMethod
)
...
class TestWaitlistService:
    ...
    async def test_confirm_waitlist_availability(
        self, service, reservation_service, test_customer, reservation_settings, db_session
    ):
    ...
    async def test_confirm_expired_notification(
        ...
        with pytest.raises(ValueError, match="notification has expired"):
            await service.confirm_waitlist_availability(
                entry.id,
                test_customer.id
            )
    ...
    def test_cancel_converted_entry(
    ...
    def test_get_customer_waitlist_entries(
        self, service, test_customer, db_session
    ):
