...
class TestReservationService:
    ...
    async def test_confirm_reservation_success(
        ...
        confirmation_data = ReservationConfirmation(
            confirmed=True,
            special_requests_update="Allergic to nuts"
        )
    ...
    def test_get_customer_reservations(
        ...
        # Get with pagination
        page1, _ = service.get_customer_reservations(
            test_customer.id,
            skip=0,
            limit=2
        )
        ...
        page2, _ = service.get_customer_reservations(
            test_customer.id,
            skip=2,
            limit=2
        )
    ...
    async def test_staff_update_reservation(
        ...
        updated = await service.staff_update_reservation(
            reservation.id,
            staff_update
        )
    ...
    def test_get_daily_reservations(
        self, service, test_customer, db_session
    ):
