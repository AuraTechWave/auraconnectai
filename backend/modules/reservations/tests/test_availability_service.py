...
class TestAvailabilityService:
    ...
    async def test_check_availability_success(
        ...
        available, reason = await service.check_availability(
            tomorrow, time(19, 0), party_size=4, duration_minutes=90
        )
    ...
    async def test_check_availability_special_date_closed(
        ...
        special_date = SpecialDate(date=tomorrow, name="Christmas", is_closed=True)
    ...
    async def test_check_availability_special_date_restrictions(
        ...
        special_date = SpecialDate(
            date=tomorrow, name="Valentine's Day", min_party_size=2, max_party_size=4
        )
        db_session.add(special_date)
        db_session.commit()
        ...
        # Party too large
        available, reason = await service.check_availability(
            tomorrow, time(19, 0), party_size=6, duration_minutes=90
        )
        ...
        # Valid party size
        available, reason = await service.check_availability(
            tomorrow, time(19, 0), party_size=2, duration_minutes=90
        )
    ...
    @pytest.mark.asyncio
    async def test_get_available_tables(self, service, test_tables, db_session):
        ...
        available = await service.get_available_tables(
            tomorrow, time(19, 0), duration_minutes=90, party_size=2
        )
    ...
    @pytest.mark.asyncio
    async def test_get_available_tables_combinable(self, service, test_tables):
    ...
    @pytest.mark.asyncio
    async def test_assign_tables_single(self, service, test_tables):
    ...
    @pytest.mark.asyncio
    async def test_assign_tables_combination(self, service, test_tables):
    ...
    def test_get_time_slots(self, service, reservation_settings):
        ...
        slots = service.get_time_slots(next_monday, party_size=2, duration_minutes=90)
    ...
    def test_get_time_slots_special_date(
        ...
        slots = service.get_time_slots(tomorrow, party_size=2, duration_minutes=90)

        # Should use special hours
        assert slots[0]["time"] == time(17, 0)

    def test_get_effective_capacity(self, service, reservation_settings, db_session):
        ...
        special_date = SpecialDate(
            date=tomorrow, name="Private Event", capacity_modifier=0.5  # 50% capacity
        )
        db_session.add(special_date)
        db_session.commit()
    ...
    def test_get_overlapping_capacity(self, service, db_session):
        ...
        capacity = service._get_overlapping_capacity(
            tomorrow, time(19, 0), duration_minutes=90
        )
        assert capacity == 10  # 4 + 6

        # Check capacity at 20:00 (overlaps with res2 and res3)
        capacity = service._get_overlapping_capacity(
            tomorrow, time(20, 0), duration_minutes=90
        )
        assert capacity == 8  # 6 + 2

        # Check with exclusion
        capacity = service._get_overlapping_capacity(
            tomorrow, time(19, 0), duration_minutes=90, exclude_reservation_id=res2.id
        )
        assert capacity == 4  # Only res1

    def test_get_peak_times(self, service, db_session):
