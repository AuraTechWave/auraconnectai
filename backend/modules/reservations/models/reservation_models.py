...
class Reservation(Base):
    ...
    waitlist_entry = relationship(
        "Waitlist", back_populates="reservation", uselist=False
    )
...
class Waitlist(Base):
    ...
    reservation = relationship(
        "Reservation", back_populates="waitlist_entry", uselist=False
    )
...
class TableConfiguration(Base):
    ...
    combine_with = Column(
        JSON, default=list
    )  # List of table numbers it can combine with
    ...
    features = Column(
        JSON, default=list
    )  # ["window", "booth", "high-chair", "wheelchair"]
