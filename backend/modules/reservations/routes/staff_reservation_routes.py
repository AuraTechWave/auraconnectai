...
async def mark_as_seated(
    ...
    update_data = StaffReservationUpdate(
        status=ReservationStatus.SEATED, table_numbers=table_number
    )

    return await update_reservation_status(
        reservation_id, update_data, current_user, db
    )
...
async def notify_waitlist_customer(
    ...
    try:
        await service.notify_waitlist_availability(
            entry, entry.requested_date, time_obj, settings.waitlist_notification_window
        )
