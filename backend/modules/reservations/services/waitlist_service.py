...
class WaitlistService:
    ...
    async def add_to_waitlist(
        self, customer_id: int, waitlist_data: WaitlistCreate
    ) -> Waitlist:
        ...
        expires_at = datetime.utcnow() + timedelta(
            hours=settings.waitlist_auto_expire_hours
        )
        ...
        logger.info(
            f"Added customer {customer_id} to waitlist with position {position}"
        )

        return waitlist_entry

    def _calculate_waitlist_position(
        self, requested_date: date, requested_time: time, party_size: int
    ) -> int:
    ...
    async def check_waitlist_for_availability(
        self, target_date: date, target_time: time, available_capacity: int
    ):
        ...
        time_window_start = (
            datetime.combine(target_date, target_time) - timedelta(hours=1)
        ).time()
        time_window_end = (
            datetime.combine(target_date, target_time) + timedelta(hours=1)
        ).time()
        ...
        for entry in matching_entries:
            ...
            await self.notify_waitlist_availability(
                entry, target_date, target_time, settings.waitlist_notification_window
            )
    ...
    async def confirm_waitlist_availability(
        ...
        if waitlist_entry.status != WaitlistStatus.NOTIFIED:
            raise ValueError(
                "This waitlist entry has not been notified of availability"
            )
        ...
        try:
            ...
            reservation = await reservation_service.create_reservation(
                customer_id, reservation_data, skip_availability_check=True
            )
            ...
            logger.info(
                f"Converted waitlist entry {waitlist_id} to reservation {reservation.id}"
            )
    ...
    def cancel_waitlist_entry(self, waitlist_id: int, customer_id: int) -> Waitlist:
        ...
        self._recalculate_positions(
            waitlist_entry.requested_date, waitlist_entry.requested_time_start
        )
    ...
    def _recalculate_positions(self, date: date, time: time):
        ...
        active_entries = (
            ...
            .order_by(Waitlist.priority.desc(), Waitlist.created_at)
            .all()
        )
    ...
    def get_customer_waitlist_entries(
        self, customer_id: int, active_only: bool = True, skip: int = 0, limit: int = 20
        ...
        entries = (
            query.order_by(Waitlist.requested_date.desc(), Waitlist.created_at.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_waitlist_by_date(
        self, date: date, status: Optional[WaitlistStatus] = None
    ) -> List[Waitlist]:
        """Get all waitlist entries for a date (staff view)"""
        query = self.db.query(Waitlist).filter(Waitlist.requested_date == date)

        if status:
            query = query.filter_by(status=status)

        return query.order_by(Waitlist.priority.desc(), Waitlist.position).all()
