...
class ReservationService:
    ...
    def get_settings(self, force_refresh: bool = False) -> ReservationSettings:
        ...
        if (
            ...
            and datetime.utcnow() - self._settings_cache_time < timedelta(minutes=5)
        ):
    ...
    async def create_reservation(
        ...
        if not skip_availability_check:
            available, reason = await self.availability_service.check_availability(
                ...
                or settings.default_reservation_duration,
            )
        ...
        if settings.send_reminders:
            await self.notification_service.schedule_reminder(
                reservation, settings.reminder_hours_before
            )
    ...
    def _validate_booking_rules(
        self, reservation_data: ReservationCreate, settings: ReservationSettings
    ):
        ...
        if minutes_in_advance < settings.min_advance_minutes:
            raise ValueError(
                f"Reservations must be made at least {settings.min_advance_minutes} minutes in advance"
            )
        ...
        if (
            reservation_data.reservation_time < open_time
            or reservation_data.reservation_time >= close_time
        ):
            raise ValueError(
                f"Restaurant hours on {day_name} are {open_time} to {close_time}"
            )

        # Check special dates
        special_date = (
            self.db.query(SpecialDate)
            .filter_by(date=reservation_data.reservation_date)
            .first()
        )
        ...
        if special_date:
            if special_date.is_closed:
                raise ValueError(
                    f"Restaurant is closed on {special_date.name or 'this date'}"
                )

            if special_date.special_hours:
                open_time = datetime.strptime(
                    special_date.special_hours["open"], "%H:%M"
                ).time()
                close_time = datetime.strptime(
                    special_date.special_hours["close"], "%H:%M"
                ).time()
                ...
                if (
                    ...
                    raise ValueError(
                        f"Special hours on {special_date.name}: {open_time} to {close_time}"
                    )

    async def update_reservation(
        self, reservation_id: int, customer_id: int, update_data: ReservationUpdate
    ) -> Reservation:
        ...
        needs_availability_check = any(
            [
                ...
                update_data.party_size
                and update_data.party_size != reservation.party_size,
            ]
        )
        ...
        if needs_availability_check:
            ...
            reservation.table_numbers = ", ".join(
                [t.table_number for t in assigned_tables]
            )
    ...
    async def cancel_reservation(
        ...
        await self.notification_service.send_cancellation_notification(
            reservation, cancellation_data.reason
        )
    ...
    async def confirm_reservation(
        ...
        self._create_audit_log(
            ...
            user_type="customer",
            metadata={
                "special_requests_updated": confirmation_data.special_requests_update
                is not None
            },
        )
        ...
        # Send confirmation notification
        await self.notification_service.send_confirmation_status_notification(
            reservation
        )
    ...
    def get_reservation(
        self, reservation_id: int, customer_id: Optional[int] = None
    ...
    async def _check_waitlist_for_opening(
        self, date: date, time: time, party_size: int
    ):
    ...
    async def staff_update_reservation(
        self, reservation_id: int, update_data: StaffReservationUpdate, staff_id: int
    ) -> Reservation:
        ...
        if update_data.status:
            ...
            if update_data.status == ReservationStatus.SEATED:
                ...
                event = ReservationCompletedEvent(
                    ...
                    duration_minutes=(
                        int(
                            (datetime.utcnow() - reservation.seated_at).total_seconds()
                            / 60
                        )
        ...
        if update_data.table_numbers:
            ...
            field_changes["table_numbers"] = {
                "old": old_tables,
                "new": update_data.table_numbers,
            }
        ...
        if update_data.notes:
            ...
            field_changes["special_requests"] = {
                "old": old_requests,
                "new": reservation.special_requests,
            }
        ...
        # Create audit log
        action = (
            AuditAction.MANUAL_OVERRIDE if update_data.notes else AuditAction.UPDATED
        )
        if update_data.status == ReservationStatus.SEATED:
    ...
    def get_daily_reservations(
        self, date: date, status: Optional[ReservationStatus] = None
    ) -> List[Reservation]:
        """Get all reservations for a specific date (staff view)"""
        query = self.db.query(Reservation).filter(Reservation.reservation_date == date)
    ...
    async def process_unconfirmed_reservations(self):
        ...
        confirmation_deadline = datetime.utcnow() + timedelta(
            hours=settings.confirmation_required_hours
        )
