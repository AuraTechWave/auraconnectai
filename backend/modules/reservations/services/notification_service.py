...
class ReservationNotificationService:
    ...
    async def send_booking_confirmation(self, reservation: Reservation):
        ...
        if not customer:
            logger.error(
                f"Customer {reservation.customer_id} not found for reservation {reservation.id}"
            )
            return
        ...
        if reservation.notification_method in [
...
See you soon!
    ...
    async def send_cancellation_notification(
        self, reservation: Reservation, reason: Optional[str]
    ):
...
Your reservation {data['confirmation_code']} for {data['reservation_date']} at {data['reservation_time']} has been cancelled.
    ...
    async def send_confirmation_status_notification(self, reservation: Reservation):
        ...
        logger.info(
            f"Sent confirmation status notification for reservation {reservation.id}"
        )
    ...
    async def send_waitlist_confirmation(self, waitlist_entry: Waitlist):
        ...
        customer = (
            self.db.query(Customer).filter_by(id=waitlist_entry.customer_id).first()
        )
        if not customer:
            return
    ...
    async def send_waitlist_availability_notification(
        ...
        if waitlist_entry.notification_method in [
            NotificationMethod.SMS,
            NotificationMethod.BOTH,
        ]:
            if customer.phone:
                await self._send_waitlist_availability_sms(
                    customer.phone, notification_data
                )
        ...
        if waitlist_entry.notification_method in [
            ...
            await self._send_waitlist_availability_email(
                customer.email, notification_data
            )

        logger.info(
            f"Sent waitlist availability notification for entry {waitlist_entry.id}"
        )
...
Reply YES within {data['response_minutes']} min to confirm.
    ...
    async def schedule_reminder(self, reservation: Reservation, hours_before: int):
        ...
        logger.info(
            f"Scheduled reminder for reservation {reservation.id} at {reminder_time}"
        )
        ...
        for reminder in due_reminders:
            try:
                ...
                reservation = (
                    ...
                    .filter_by(id=reminder.reservation_id)
                    .first()
                )
