...
from sqlalchemy import Column, Integer, String, Boolean, DateTime, ForeignKey, DECIMAL, JSON, Enum as SQLEnum, UniqueConstraint, CheckConstraint
...
class Table(Base, TimestampMixin):
    ...
    current_session = relationship("TableSession", uselist=False, back_populates="table", 
    ...
    __table_args__ = (
        UniqueConstraint('restaurant_id', 'table_number', name='uix_table_restaurant_number'),
...
class TableCombination(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint('session_id', 'table_id', name='uix_combination_session_table'),
    )
...
class TableReservation(Base, TimestampMixin):
    ...
    # Table preferences
    table_preferences = Column(JSON, default={})  # e.g., {"by_window": true, "quiet": true}
