...
class TableStateService:
    ...
    async def get_table_availability(
        ...
        query = select(Table).where(
            and_(Table.restaurant_id == restaurant_id, Table.is_active == True)
        )
        ...
        if guest_count:
            query = query.where(
                and_(
                    Table.min_capacity <= guest_count, Table.max_capacity >= guest_count
                )
            )
    ...
    async def _is_table_available(
        ...
        if datetime_from <= datetime.utcnow() <= datetime_to:
            session_query = select(TableSession).where(
                and_(TableSession.table_id == table_id, TableSession.end_time.is_(None))
            )
    ...
    async def start_table_session(
        ...
        if session_data.combined_table_ids:
            combined_tables = await self._validate_combined_tables(
                db,
                restaurant_id,
                session_data.combined_table_ids,
                session_data.table_id,
            )
        ...
        if combined_tables:
            ...
            primary_combination = TableCombination(
                session_id=session.id, table_id=session_data.table_id, is_primary=True
            )
            ...
            for table_id in combined_tables:
                combination = TableCombination(
                    session_id=session.id, table_id=table_id, is_primary=False
                )
                db.add(combination)
        ...
        for table_id in combined_tables:
            ...
            await self._update_table_status(
                ...
                user_id,
                session_id=session.id,
                reason="Part of combined seating",
            )
    ...
    async def _validate_combined_tables(
        ...
        for table_id in table_ids:
            ...
            if not table.is_combinable:
                raise BusinessLogicError(
                    f"Table {table.table_number} cannot be combined"
                )
    ...
    async def end_table_session(
        self, db: AsyncSession, restaurant_id: int, session_id: int, user_id: int
    ) -> TableSession:
        """End a table session"""
        ...
        table_ids.extend(
            [c.table_id for c in session.combined_tables if not c.is_primary]
        )
    ...
    async def update_table_status(
        ...
        await self._update_table_status(
            db, table, status_update.status, user_id, reason=status_update.reason
        )
    ...
    async def bulk_update_table_status(
        ...
        for table_id in bulk_update.table_ids:
            ...
            await self._update_table_status(
                db, table, bulk_update.status, user_id, reason=bulk_update.reason
            )
    ...
    async def _update_table_status(
        ...
        last_log_query = (
            ...
            .order_by(TableStateLog.changed_at.desc())
            .limit(1)
        )
    ...
    def _validate_status_transition(
        ...
        allowed_transitions = {
            ...
            TableStatus.CLEANING: [TableStatus.AVAILABLE, TableStatus.MAINTENANCE],
    ...
    async def get_floor_status(
        self, db: AsyncSession, restaurant_id: int, floor_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """Get current status of all tables on floor(s)"""

        query = (
            select(Table)
            .where(and_(Table.restaurant_id == restaurant_id, Table.is_active == True))
            .options(
        ...
        for table in tables:
            ...
            table_data = {
                ...
                "visual": {"shape": table.shape, "color": table.color},
            }
    ...
    async def get_table_analytics(
        ...
        query = (
            select(
                ...
                func.avg(
                    func.extract(
                        "epoch", TableSession.end_time - TableSession.start_time
                    )
                    / 60
        ...
        # Get table info
        table_query = select(Table).where(Table.restaurant_id == restaurant_id)
        if table_id:
        ...
        for row in analytics_data:
            ...
            occupancy_rate = (
                (occupied_minutes / total_minutes) * 100 if total_minutes > 0 else 0
            )
            ...
            analytics.append(
                {
                    ...
                    "avg_guests_per_session": round(
                        (
                            (row.total_guests or 0) / row.session_count
