...
class LayoutService:
    ...
    async def create_floor(
        self, db: AsyncSession, restaurant_id: int, floor_data: FloorCreate
    ) -> Floor:
        """Create a new floor"""
        ...
        existing = await db.execute(
            select(Floor).where(
                and_(
                    Floor.restaurant_id == restaurant_id, Floor.name == floor_data.name
                )
            )
        )
        if existing.scalar():
            raise BusinessLogicError(
                f"Floor with name '{floor_data.name}' already exists"
            )
    ...
    async def get_floors(
        self, db: AsyncSession, restaurant_id: int, include_inactive: bool = False
    ) -> List[Floor]:
        ...
        for floor in floors:
            ...
            floor.occupied_tables = len(
                [t for t in floor.tables if t.status == TableStatus.OCCUPIED]
            )

        return floors

    async def get_floor(
        self, db: AsyncSession, restaurant_id: int, floor_id: int
    ) -> Floor:
        """Get floor by ID"""
    ...
    async def update_floor(
        ...
        if update_data.name and update_data.name != floor.name:
            ...
            if existing.scalar():
                raise BusinessLogicError(
                    f"Floor with name '{update_data.name}' already exists"
                )
    ...
    async def delete_floor(
        self, db: AsyncSession, restaurant_id: int, floor_id: int, force: bool = False
    ) -> None:
        """Delete a floor"""
        ...
        table_count = await db.execute(
            select(func.count()).select_from(Table).where(Table.floor_id == floor_id)
        )
    ...
    async def create_table(
        self, db: AsyncSession, restaurant_id: int, table_data: TableCreate
    ) -> Table:
        """Create a new table"""
        ...
        table = Table(restaurant_id=restaurant_id, **table_data.dict())
    ...
    async def bulk_create_tables(
        self, db: AsyncSession, restaurant_id: int, bulk_data: BulkTableCreate
    ) -> List[Table]:
        ...
        for table_data in bulk_data.tables:
            ...
            table = Table(restaurant_id=restaurant_id, **table_dict)
            db.add(table)
            tables.append(table)
    ...
    async def get_tables(
        ...
        query = (
            ...
            .options(selectinload(Table.floor), selectinload(Table.current_session))
        )
    ...
    async def get_table(
        self, db: AsyncSession, restaurant_id: int, table_id: int
    ) -> Table:
        """Get table by ID"""
    ...
    async def bulk_update_tables(
        self, db: AsyncSession, restaurant_id: int, bulk_data: BulkTableUpdate
    ) -> List[Table]:
    ...
    async def create_layout(
        self, db: AsyncSession, restaurant_id: int, layout_data: TableLayoutCreate
    ) -> TableLayout:
        ...
        layout = TableLayout(restaurant_id=restaurant_id, **layout_data.dict())
    ...
    async def get_layouts(
        ...
        query = select(TableLayout).where(TableLayout.restaurant_id == restaurant_id)

        if not include_inactive:
            query = query.where(TableLayout.is_active == True)
    ...
    async def get_layout(
        self, db: AsyncSession, restaurant_id: int, layout_id: int
    ) -> TableLayout:
        """Get layout by ID"""
    ...
    async def export_layout(
        ...
        if format == "json":
            export_data = {
                ...
                "floors": {},
                "summary": {"total_floors": len(floors), "total_tables": len(tables)},
            }
            ...
            try:
                ...
                for table_data in floor_data.get("tables", []):
                    ...
                    if table and merge:
                        ...
                        # Create new
                        table_dict = {
                            k: v for k, v in table_data.items() if k != "features"
                        }
                ...
                errors.append({"floor": floor_info.get("name"), "error": str(e)})
    ...
    async def generate_qr_codes(
        ...
        return {"success": True, "updated_count": updated_count, "qr_codes": qr_codes}
    ...
    async def validate_layout(
        ...
        for table in tables:
            ...
            if floor:
                if (
                    table.position_x + table.width > floor.width
                    or table.position_y + table.height > floor.height
                ):
                    issues.append(
                        {
    ...
    def _tables_overlap(self, table1: Table, table2: Table) -> bool:
        ...
        return not (
            ...
            or table2.position_x + table2.width <= table1.position_x
            or table1.position_y + table1.height <= table2.position_y
            or table2.position_y + table2.height <= table1.position_y
        )

    async def _get_floor(
        self, db: AsyncSession, floor_id: int, restaurant_id: int
    ) -> Floor:
        """Get floor with validation"""

        result = await db.execute(
            select(Floor).where(
                and_(Floor.id == floor_id, Floor.restaurant_id == restaurant_id)
            )
        )
        floor = result.scalar_one_or_none()
    ...
    async def _get_layout(
        self, db: AsyncSession, layout_id: int, restaurant_id: int
    ) -> TableLayout:
    ...
    async def _unset_default_floors(self, db: AsyncSession, restaurant_id: int):
        """Unset all default floors"""

        await db.execute(
            select(Floor)
            .where(and_(Floor.restaurant_id == restaurant_id, Floor.is_default == True))
            .execution_options(synchronize_session="fetch")
        )

        result = await db.execute(
            select(Floor).where(
                and_(Floor.restaurant_id == restaurant_id, Floor.is_default == True)
            )
        )
    ...
    async def _unset_default_layouts(self, db: AsyncSession, restaurant_id: int):
    ...
    async def _deactivate_layouts(self, db: AsyncSession, restaurant_id: int):
