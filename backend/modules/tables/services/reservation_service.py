...
from ..models.table_models import (
    Table, TableReservation, TableSession,
...
class ReservationService:
    ...
    async def update_reservation(
        ...
        if reservation.status in [ReservationStatus.COMPLETED, ReservationStatus.CANCELLED]:
            raise BusinessLogicError(
                f"Cannot modify {reservation.status} reservation"
            )
        
        # Validate new time if changed
        if update_data.reservation_date and update_data.reservation_date != reservation.reservation_date:
            self._validate_reservation_time(update_data.reservation_date)
        
        # Handle table change
        if update_data.table_id is not None and update_data.table_id != reservation.table_id:
    ...
    async def cancel_reservation(
        ...
        if reservation.status in [ReservationStatus.COMPLETED, ReservationStatus.CANCELLED]:
            raise BusinessLogicError(
                f"Cannot cancel {reservation.status} reservation"
            )
    ...
    async def seat_reservation(
        ...
        if reservation.status not in [ReservationStatus.PENDING, ReservationStatus.CONFIRMED]:
            raise BusinessLogicError(
                f"Cannot seat {reservation.status} reservation"
            )
        
        # Check if it's close to reservation time (allow 30 min early)
        time_until = (reservation.reservation_date - datetime.utcnow()).total_seconds() / 60
        if time_until > 30:
            raise BusinessLogicError(
    ...
    async def send_reminders(
        ...
        for reservation in reservations:
            try:
                ...
                logger.error(f"Failed to send reminder for {reservation.reservation_code}: {e}")
    ...
    async def _auto_assign_table(
        ...
        for table_info in available_tables:
            ...
            if preferences.get("by_window") and features["is_by_window"]:
                score += 8
            if preferences.get("wheelchair_accessible") and features["is_wheelchair_accessible"]:
                score += 10
    ...
    async def _send_reservation_confirmation(self, reservation: TableReservation):
        ...
        logger.info(f"Sending confirmation for reservation {reservation.reservation_code}")
    ...
    async def _send_reservation_cancellation(self, reservation: TableReservation):
        ...
        # TODO: Implement actual notification
        logger.info(f"Sending cancellation for reservation {reservation.reservation_code}")
