...
class Restaurant(Base, TimestampMixin):
    ...
    status = Column(
        SQLEnum(RestaurantStatus), nullable=False, default=RestaurantStatus.PENDING
    )
    ...
    floors = relationship(
        "Floor", back_populates="restaurant", cascade="all, delete-orphan"
    )
...
class Location(Base, TimestampMixin):
    ...
    location_type = Column(
        SQLEnum(LocationType), nullable=False, default=LocationType.RESTAURANT
    )
...
class Floor(Base, TimestampMixin):
    ...
    service_charge_percent = Column(
        DECIMAL(5, 2)
    )  # Optional floor-specific service charge
