...
async def list_settings(
    ...
    filters = SettingFilters(
        ...
        scope=scope,
        restaurant_id=(
            current_user.restaurant_id if scope != SettingScope.SYSTEM else None
        ),
...
async def list_setting_definitions(
    ...
    definitions = query.order_by(
        SettingDefinition.category, SettingDefinition.sort_order, SettingDefinition.key
    ).all()
...
async def check_feature_flag(
    ...
    is_enabled, reason = service.is_feature_enabled(
        key=key, restaurant_id=current_user.restaurant_id, user_id=current_user.id
    )

    return FeatureFlagStatus(key=key, is_enabled=is_enabled, reason=reason)
...
async def create_api_key(
    ...
    key_model, api_key = service.create_api_key(
        key_data, current_user.restaurant_id, current_user.id
    )

    response = APIKeyCreateResponse(**key_model.__dict__, api_key=api_key)
...
async def list_api_keys(
    ...
    query = db.query(APIKey).filter(APIKey.restaurant_id == current_user.restaurant_id)
...
async def revoke_api_key(
    ...
    key = (
        db.query(APIKey)
        .filter(APIKey.id == key_id, APIKey.restaurant_id == current_user.restaurant_id)
        .first()
    )
...
async def get_setting_history(
    ...
    query = db.query(SettingHistory).filter(SettingHistory.changed_at >= from_date)
