...
class KitchenStation(Base):
    ...
    prep_time_multiplier = Column(
        Float, default=1.0
    )  # Adjust prep times for this station
    ...
    # Indexes
    __table_args__ = (Index("idx_station_type_status", "station_type", "status"),)
...
class KitchenDisplay(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "station_id", "display_number", name="uq_station_display_number"
        ),
    )
...
class MenuItemStation(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "menu_item_id", "station_id", "sequence", name="uq_menu_item_station_seq"
        ),
    )
...
class KDSOrderItem(Base):
    ...
    def is_late(self):
        ...
        if self.target_time and self.status not in [
...
class StationRoutingRule(Base):
    ...
    target_station_id = Column(
        Integer, ForeignKey("kitchen_stations.id"), nullable=False
    )
