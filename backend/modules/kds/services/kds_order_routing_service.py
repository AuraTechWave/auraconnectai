...
from ..models.kds_models import (
    KitchenStation, StationAssignment, MenuItemStation, 
    KDSOrderItem, StationType, StationStatus, DisplayStatus
)
from ..schemas.kds_schemas import (
    StationAssignmentCreate, MenuItemStationCreate,
    OrderItemDisplay
)
...
class KDSOrderRoutingService:
    ...
    def create_station_assignment(self, assignment_data: StationAssignmentCreate) -> StationAssignment:
        """Create a new station assignment rule"""
        # Validate station exists
        station = self.db.query(KitchenStation).filter_by(id=assignment_data.station_id).first()
        if not station:
        ...
        logger.info(f"Created assignment for station {station.name}: "
                   f"category={assignment.category_name}, tag={assignment.tag_name}")
        return assignment
    
    def get_station_assignments(self, station_id: Optional[int] = None) -> List[StationAssignment]:
    ...
    def delete_station_assignment(self, assignment_id: int):
        ...
        assignment = self.db.query(StationAssignment).filter_by(id=assignment_id).first()
        if not assignment:
    ...
    def assign_menu_item_to_station(self, assignment_data: MenuItemStationCreate) -> MenuItemStation:
        ...
        existing = self.db.query(MenuItemStation).filter(
        ...
        logger.info(f"Assigned menu item {assignment.menu_item_id} to station {station.name}")
        return assignment
    
    def get_menu_item_stations(self, menu_item_id: int) -> List[MenuItemStation]:
        """Get all station assignments for a menu item"""
        return self.db.query(MenuItemStation).filter(
    ...
    def route_order_to_stations(self, order_id: int) -> List[KDSOrderItem]:
        ...
        for order_item in order.order_items:
            ...
            existing = self.db.query(KDSOrderItem).filter_by(
        ...
        self.db.commit()
        logger.info(f"Routed {len(routed_items)} items from order {order_id} to stations")
        return routed_items
    
    def _route_order_item(self, order_item: OrderItem, order: Order) -> List[KDSOrderItem]:
    ...
    def _find_stations_for_item(self, menu_item: MenuItem, order_item: OrderItem) -> List[Tuple[KitchenStation, int, Optional[str]]]:
        """Find appropriate stations for a menu item"""
        stations = []
        
        # First, check direct menu item to station mappings
        item_stations = self.db.query(MenuItemStation).filter(
            MenuItemStation.menu_item_id == menu_item.id
        ).order_by(MenuItemStation.sequence).all()
        
        for item_station in item_stations:
            station = self.db.query(KitchenStation).filter_by(id=item_station.station_id).first()
            if station and station.status == StationStatus.ACTIVE:
                stations.append((station, item_station.prep_time_minutes, item_station.station_notes))
        
        # If no direct mapping, use category/tag based routing
        if not stations:
            # Get menu item category and tags
            category_name = menu_item.category.name if hasattr(menu_item, 'category') else None
            ...
            for assignment in assignments:
                station = self.db.query(KitchenStation).filter_by(id=assignment.station_id).first()
                if station and station.status == StationStatus.ACTIVE:
                    prep_time = assignment.prep_time_override or self._default_prep_time(menu_item)
    ...
    def _find_matching_assignments(self, category_name: Optional[str], tags: List[str]) -> List[StationAssignment]:
        ...
        if conditions:
            assignments = self.db.query(StationAssignment).filter(
                or_(*conditions)
    ...
    def _fallback_routing(self, menu_item: MenuItem) -> List[Tuple[KitchenStation, int, Optional[str]]]:
        ...
        for station_type, keywords in keyword_mapping.items():
            if any(keyword in item_name_lower for keyword in keywords):
                type_stations = self.db.query(KitchenStation).filter(
                    KitchenStation.station_type == station_type,
                    KitchenStation.status == StationStatus.ACTIVE
                ).order_by(KitchenStation.priority.desc()).all()
        ...
        if not stations:
            expo_station = self.db.query(KitchenStation).filter(
        ...
        if order_item.special_instructions:
            if isinstance(order_item.special_instructions, dict):
                ...
                if "additions" in order_item.special_instructions:
                    modifiers.extend([f"Add {item}" for item in order_item.special_instructions["additions"]])
                if "removals" in order_item.special_instructions:
                    modifiers.extend([f"No {item}" for item in order_item.special_instructions["removals"]])
    ...
    def _extract_special_instructions(self, order_item: OrderItem) -> Optional[str]:
        ...
        if order_item.special_instructions and isinstance(order_item.special_instructions, dict):
    ...
    def _calculate_priority(self, order: Order) -> int:
        ...
        # Time-based priority
        if order.created_at:
            minutes_waiting = (datetime.utcnow() - order.created_at).total_seconds() / 60
    ...
    def _calculate_target_time(self, prep_time: int, station: KitchenStation) -> datetime:
