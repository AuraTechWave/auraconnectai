...
from ..models.kds_models import (
    KitchenStation, KitchenDisplay, KDSOrderItem,
    StationType, StationStatus, DisplayStatus
)
from ..schemas.kds_schemas import (
    StationCreate, StationUpdate, KitchenDisplayCreate,
    KitchenDisplayUpdate, OrderItemDisplay, StationSummary
)
...
class KDSService:
    ...
    def create_station(self, station_data: StationCreate) -> KitchenStation:
        ...
        logger.info(f"Created kitchen station: {station.name} ({station.station_type.value})")
        return station
    ...
    def update_station(self, station_id: int, update_data: StationUpdate) -> KitchenStation:
    ...
    def create_display(self, display_data: KitchenDisplayCreate) -> KitchenDisplay:
        ...
        existing = self.db.query(KitchenDisplay).filter(
            KitchenDisplay.station_id == display_data.station_id,
            KitchenDisplay.display_number == display_data.display_number
        ).first()
        
        if existing:
            raise ValueError(f"Display number {display_data.display_number} already exists for this station")
    ...
    def update_display(self, display_id: int, update_data: KitchenDisplayUpdate) -> KitchenDisplay:
    ...
    def get_station_items(
        ...
        if status_filter:
            ...
            query = query.filter(
                ~KDSOrderItem.status.in_([DisplayStatus.COMPLETED, DisplayStatus.CANCELLED])
            )
    ...
    def _check_order_completion(self, order_id: int):
        ...
        for order_item in order_items:
            items = self.db.query(KDSOrderItem).filter_by(
                order_item_id=order_item.id
            ).all()
            kds_items.extend(items)
        
        # Check if all are completed
        if kds_items and all(item.status == DisplayStatus.COMPLETED for item in kds_items):
            # Update order status
    ...
    def get_station_summary(self, station_id: int) -> StationSummary:
        ...
        status_counts = self.db.query(
            KDSOrderItem.status,
            func.count(KDSOrderItem.id)
        ).filter(
            KDSOrderItem.station_id == station_id
        ).group_by(KDSOrderItem.status).all()
        
        status_dict = {status: count for status, count in status_counts}
        
        # Calculate average wait time for completed items today
        today_start = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
        completed_today = self.db.query(KDSOrderItem).filter(
            KDSOrderItem.station_id == station_id,
        ...
        # Count late items
        late_items = self.db.query(func.count(KDSOrderItem.id)).filter(
            KDSOrderItem.station_id == station_id,
        ...
        # Get last activity
        last_item = self.db.query(KDSOrderItem).filter(
            KDSOrderItem.station_id == station_id
        ).order_by(KDSOrderItem.received_at.desc()).first()
    ...
    def get_all_station_summaries(self) -> List[StationSummary]:
        ...
        for station in stations:
            try:
                ...
                logger.error(f"Error getting summary for station {station.id}: {str(e)}")
    ...
    def update_item_timings(self):
        ...
        for station in stations:
            ...
            active_items = self.db.query(KDSOrderItem).filter(
                ...
                KDSOrderItem.status.in_([DisplayStatus.PENDING, DisplayStatus.IN_PROGRESS])
            ).order_by(KDSOrderItem.priority.desc(), KDSOrderItem.received_at).all()
            ...
            for item in active_items:
                if not item.target_time:
                    ...
                    item.target_time = base_time + timedelta(minutes=cumulative_time + prep_time)
    ...
    def _estimate_prep_time(self, item: KDSOrderItem, station: KitchenStation) -> int:
        ...
        if item.quantity > 1:
            adjusted_time += (item.quantity - 1) * 2  # 2 extra minutes per additional item
