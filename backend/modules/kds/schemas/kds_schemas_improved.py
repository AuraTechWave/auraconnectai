...
WebSocketMessageType = Literal[
    "new_item", "update_item", "remove_item", "station_update", 
    "heartbeat", "error", "reconnect"
]
...
class StationCreate(BaseModel):
    ...
    color_code: Optional[str] = Field(
        None,
        description="Hex color code for station"
    )
    priority: int = Field(
        50,
        ge=0,
    ...
    prep_time_multiplier: float = Field(
        ...
        ge=0.1,
        le=5.0,
        description="Multiplier for prep time estimates"
    )
    warning_time_minutes: int = Field(
        5,
        ge=1,
        le=30,
        description="Minutes before item is considered late"
    )
    critical_time_minutes: int = Field(
        10,
        ge=1,
        le=60,
        description="Minutes before item is critical"
    )
    features: List[str] = Field(
        default_factory=list,
        max_items=20,
        description="Station features/capabilities"
    )
    printer_id: Optional[str] = Field(
        None,
        max_length=50,
        description="Associated printer ID"
    )
...
class KitchenDisplayCreate(BaseModel):
    ...
    display_number: int = Field(
        ...
        ge=1,
        le=10,
        description="Display number within station"
    )
    name: str = Field(
        ...,
        min_length=1,
    ...
    ip_address: Optional[str] = Field(
        None,
        description="IP address for network displays"
    )
    ...
    auto_clear_completed: bool = Field(
        True,
        description="Auto-clear completed items"
    )
    auto_clear_delay_seconds: int = Field(
        30,
        ge=10,
        le=300,
        description="Delay before auto-clearing"
    )
...
class StationAssignmentCreate(BaseModel):
    ...
    is_primary: bool = Field(
        True,
        description="Primary station for this assignment"
    )
    prep_time_override: Optional[int] = Field(
        None,
        ge=1,
        le=120,
        description="Override prep time in minutes"
    )
    conditions: Dict[str, Any] = Field(
        default_factory=dict,
        description="Additional routing conditions"
    )
...
class MenuItemStationCreate(BaseModel):
    ...
    is_primary: bool = Field(
        True,
        description="Primary station for this item"
    )
    sequence: int = Field(
        0,
        ge=0,
        le=10,
        description="Preparation sequence order"
    )
    ...
    station_notes: Optional[str] = Field(
        None,
        max_length=500,
        description="Special instructions for this station"
    )
...
class OrderItemDisplay(BaseModel):
    ...
    course_number: int = Field(
        ...
        ge=1,
        le=10,
        description="Course number for sequencing"
    )
    priority: int = Field(
        0,
        ge=0,
    ...
    fire_time: Optional[datetime] = Field(
        None,
...
class KDSOrderItemResponse(BaseModel):
    ...
    def check_if_late(cls, v, values):
        ...
        if target_time and status not in [DisplayStatus.COMPLETED, DisplayStatus.CANCELLED]:
...
class KDSWebSocketMessage(BaseModel):
    ...
    def validate_data(cls, v, values):
        ...
        if msg_type in ['new_item', 'update_item']:
            ...
            if 'item_id' not in v:
                raise ValueError("Message type 'remove_item' requires 'item_id' in data")
        elif msg_type == 'station_update':
            if 'station' not in v and 'action' not in v:
                raise ValueError("Message type 'station_update' requires 'station' or 'action' in data")
        elif msg_type == 'error':
            if 'message' not in v:
...
class BulkStationStatusUpdate(BaseModel):
    ...
    station_ids: List[int] = Field(
        ...
        min_items=1,
        max_items=50,
        description="List of station IDs to update"
    )
    status: StationStatus
