...
@router.post(
    "/stations", response_model=StationResponse, status_code=status.HTTP_201_CREATED
)
@handle_api_errors
async def create_station(
    ...
    await ws_manager.broadcast_station_update(
        station.id,
        {"action": "created", "station": StationResponse.from_orm(station).dict()},
    )
...
async def list_stations(
    ...
    station_type: Optional[StationType] = Query(
    ...
    for station in paginated_stations:
        ...
        try:
            ...
            response.active_items_count = sum(
                1 for item in items if item.status == DisplayStatus.IN_PROGRESS
            )
            response.pending_items_count = sum(
                1 for item in items if item.status == DisplayStatus.PENDING
            )
...
async def update_station(
    ...
    await ws_manager.broadcast_station_update(
        station_id,
        {"action": "updated", "station": StationResponse.from_orm(station).dict()},
    )
...
async def delete_station(
    ...
    if not force:
        items = service.get_station_items(
            station_id, [DisplayStatus.PENDING, DisplayStatus.IN_PROGRESS]
        )
        if items:
            raise ConflictError(
                "Cannot delete station with active items", {"active_items": len(items)}
            )
    ...
    # Broadcast station update
    await ws_manager.broadcast_station_update(
        station_id, {"action": "deleted", "station_id": station_id}
    )
...
@router.post(
    "/displays",
    response_model=KitchenDisplayResponse,
...
@handle_api_errors
async def update_display_heartbeat(display_id: int, db: Session = Depends(get_db)):
    """
...
@router.post(
    "/assignments",
    response_model=StationAssignmentResponse,
    status_code=status.HTTP_201_CREATED,
)
@handle_api_errors
async def create_station_assignment(
    ...
    if existing:
        raise ConflictError("Assignment already exists", {"existing_id": existing.id})
...
@router.post(
    "/menu-items/stations",
    response_model=MenuItemStationResponse,
    status_code=status.HTTP_201_CREATED,
)
@handle_api_errors
async def assign_menu_item_to_station(
    ...
    existing = routing_service.get_menu_item_station_assignment(
        assignment_data.menu_item_id, assignment_data.station_id
    )
    if existing:
        raise ConflictError(
            "Menu item already assigned to this station", {"existing_id": existing.id}
        )
...
@router.get(
    "/menu-items/{menu_item_id}/stations", response_model=List[MenuItemStationResponse]
)
@handle_api_errors
...
async def acknowledge_item(
    ...
    if item.status != DisplayStatus.PENDING:
        raise ConflictError(
            "Can only acknowledge pending items", {"current_status": item.status}
        )
    ...
    await ws_manager.broadcast_item_update(
        item.station_id,
        item_id,
        {"action": "acknowledged", "item": KDSOrderItemResponse.from_orm(item).dict()},
    )
...
async def start_item(
    ...
    await ws_manager.broadcast_item_update(
        item.station_id,
        item_id,
        {"action": "started", "item": KDSOrderItemResponse.from_orm(item).dict()},
    )
...
async def complete_item(
    ...
    await ws_manager.broadcast_item_update(
        item.station_id,
        item_id,
        {"action": "completed", "item": KDSOrderItemResponse.from_orm(item).dict()},
    )
...
async def recall_item(
    ...
    reason: Optional[str] = Query(
        None, max_length=500, description="Reason for recall"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    if item.status != DisplayStatus.COMPLETED:
        raise ConflictError(
            "Can only recall completed items", {"current_status": item.status}
        )
...
async def route_order_to_stations(
    ...
    try:
        ...
        for station_id, items in station_items.items():
            await ws_manager.broadcast_new_items(
                station_id,
                {
                    "items": [
                        KDSOrderItemResponse.from_orm(item).dict() for item in items
...
@router.post(
    "/stations/bulk/update-status", response_model=BulkStationStatusUpdateResponse
)
@handle_api_errors
async def bulk_update_station_status(
    ...
    for station_id in station_ids:
        try:
            ...
            # Broadcast update
            await ws_manager.broadcast_station_update(
                station_id, {"action": "status_updated", "status": status}
            )
