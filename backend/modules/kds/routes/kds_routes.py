...
async def recall_item(
    ...
    try:
        ...
        await ws_manager.broadcast_item_update(
            item.station_id,
            item_id,
            {"action": "recalled", "item": KDSOrderItemResponse.from_orm(item).dict()},
        )
...
async def route_order_to_stations(
    ...
    try:
        ...
        for item in routed_items:
            await ws_manager.broadcast_new_item(
                item.station_id, {"item": KDSOrderItemResponse.from_orm(item).dict()}
            )
