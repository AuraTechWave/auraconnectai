...
class TestKDSAPI:
    ...
    def test_list_stations(self, client: TestClient, db: Session, auth_headers: dict):
        ...
        stations = [
            KitchenStation(name="Grill 1", station_type=StationType.GRILL, status=StationStatus.ACTIVE),
            KitchenStation(name="Salad 1", station_type=StationType.SALAD, status=StationStatus.ACTIVE),
            KitchenStation(name="Old Station", station_type=StationType.FRY, status=StationStatus.INACTIVE)
        ]
        db.add_all(stations)
        db.commit()
        ...
        response = client.get(
            "/api/v1/kds/stations?include_inactive=true",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
        ...
        # Filter by type
        response = client.get(
            "/api/v1/kds/stations?station_type=grill",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_update_station(self, client: TestClient, db: Session, auth_headers: dict):
        ...
        response = client.put(
            f"/api/v1/kds/stations/{station.id}",
            json=update_data,
            headers=auth_headers
        )
    ...
    def test_create_station_assignment(self, client: TestClient, db: Session, auth_headers: dict):
    ...
    def test_get_station_items(self, client: TestClient, db: Session, auth_headers: dict):
        ...
        # Get station items
        response = client.get(
            f"/api/v1/kds/stations/{station.id}/items",
            headers=auth_headers
        )
    ...
    def test_item_lifecycle_endpoints(self, client: TestClient, db: Session, auth_headers: dict):
        ...
        # Test acknowledge
        response = client.post(
            f"/api/v1/kds/items/{kds_item.id}/acknowledge",
            headers=auth_headers
        )
        assert response.status_code == 200
        ...
        # Test start
        response = client.post(
            f"/api/v1/kds/items/{kds_item.id}/start",
            headers=auth_headers
        )
        assert response.status_code == 200
        ...
        # Test complete
        response = client.post(
            f"/api/v1/kds/items/{kds_item.id}/complete",
            headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_station_summary(self, client: TestClient, db: Session, auth_headers: dict):
        ...
        # Get summary
        response = client.get(
            f"/api/v1/kds/stations/{station.id}/summary",
            headers=auth_headers
        )
    ...
    def test_route_order_endpoint(self, client: TestClient, db: Session, auth_headers: dict):
        ...
        # Route order
        response = client.post(
            f"/api/v1/kds/orders/{order.id}/route",
            headers=auth_headers
        )
