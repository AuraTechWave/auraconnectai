...
class TestKDSService:
    ...
    def test_get_stations_by_type(self, db: Session):
        ...
        grill1 = service.create_station(StationCreate(
            name="Grill 1",
            station_type=StationType.GRILL
        ))
        grill2 = service.create_station(StationCreate(
            name="Grill 2",
            station_type=StationType.GRILL
        ))
        salad = service.create_station(StationCreate(
            name="Salad Station",
    ...
    def test_station_staff_assignment(self, db: Session):
        ...
        # Create a station
        station = service.create_station(StationCreate(
            name="Pizza Station",
            station_type=StationType.PIZZA
        ))
    ...
    def test_order_item_lifecycle(self, db: Session):
        ...
        station = service.create_station(StationCreate(
            name="Saute Station",
            station_type=StationType.SAUTE
        ))
    ...
    def test_get_station_items(self, db: Session):
        ...
        station = service.create_station(StationCreate(
            ...
            station_type=StationType.DESSERT
        ))
        
        # Create multiple KDS items with different statuses
        pending_item = self._create_kds_item(db, station.id, DisplayStatus.PENDING, "Chocolate Cake")
        in_progress_item = self._create_kds_item(db, station.id, DisplayStatus.IN_PROGRESS, "Ice Cream")
        completed_item = self._create_kds_item(db, station.id, DisplayStatus.COMPLETED, "Apple Pie")
    ...
    def test_station_summary(self, db: Session):
        ...
        station = service.create_station(StationCreate(
            ...
            station_type=StationType.SANDWICH
        ))
        ...
        self._create_kds_item(db, station.id, DisplayStatus.IN_PROGRESS, "Grilled Cheese")
        
        # Create a completed item
        completed_item = self._create_kds_item(db, station.id, DisplayStatus.COMPLETED, "Turkey Sub")
    ...
    def test_update_item_timings(self, db: Session):
        ...
        item1 = self._create_kds_item(db, station.id, DisplayStatus.PENDING, "Prep Item 1", target_time=None)
        item2 = self._create_kds_item(db, station.id, DisplayStatus.PENDING, "Prep Item 2", target_time=None)
    ...
    def _create_kds_item(self, db: Session, station_id: int, status: DisplayStatus, 
                         display_name: str, target_time: datetime = None) -> KDSOrderItem:
    ...
    def test_error_handling(self, db: Session):
        ...
        completed_item = self._create_kds_item(db, station.id, DisplayStatus.COMPLETED, "Test Item")
