...
from modules.kds.models.kds_models import (
    KitchenStation, StationType, StationStatus, DisplayStatus,
    KDSOrderItem
)
from modules.kds.schemas.kds_schemas import (
    StationCreate, StationUpdate, KitchenDisplayCreate,
...
class TestKDSRoutes:
    ...
    def auth_headers(self, mock_user):
        ...
        with patch("modules.kds.routes.kds_routes.get_current_user", return_value=mock_user):
    ...
    def test_create_station_success(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSService") as mock_service:
                ...
                # Act
                response = client.post("/api/v1/kds/stations", json=station_data, headers=auth_headers)
    ...
    def test_update_station_success(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSService") as mock_service:
                ...
                # Act
                response = client.put("/api/v1/kds/stations/1", json=update_data, headers=auth_headers)
    ...
    def test_update_station_not_found(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSService") as mock_service:
                mock_service.return_value.update_station.side_effect = ValueError("Station not found")
                
                # Act
                response = client.put("/api/v1/kds/stations/999", json=update_data, headers=auth_headers)
    ...
    def test_list_stations_with_filters(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSService") as mock_service:
                mock_service.return_value.get_stations_by_type.return_value = [mock_stations[0]]
    ...
    def test_create_display_success(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSService") as mock_service:
                ...
                # Act
                response = client.post("/api/v1/kds/displays", json=display_data, headers=auth_headers)
    ...
    def test_acknowledge_item_success(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSService") as mock_service:
                ...
                # Act
                response = client.post("/api/v1/kds/items/1/acknowledge", headers=auth_headers)
    ...
    def test_acknowledge_item_not_found(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSService") as mock_service:
                mock_service.return_value.acknowledge_item.side_effect = ValueError("Item not found")
                
                # Act
                response = client.post("/api/v1/kds/items/999/acknowledge", headers=auth_headers)
    ...
    def test_complete_item_already_completed(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSService") as mock_service:
                mock_service.return_value.complete_item.side_effect = ValueError("Item already completed")
                
                # Act
                response = client.post("/api/v1/kds/items/1/complete", headers=auth_headers)
    ...
    def test_create_station_assignment_success(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSOrderRoutingService") as mock_service:
                ...
                mock_service.return_value.create_station_assignment.return_value = mock_assignment
                
                # Act
                response = client.post("/api/v1/kds/assignments", json=assignment_data, headers=auth_headers)
    ...
    def test_create_station_assignment_missing_fields(self, client, auth_headers):
        ...
        # Assert
        assert response.status_code == 422
        assert "Either category_name or tag_name must be provided" in str(response.json()["detail"])
    ...
    def test_concurrent_item_update_handling(self, client, mock_db, auth_headers):
        ...
        with patch("modules.kds.routes.kds_routes.get_db", return_value=mock_db):
            with patch("modules.kds.routes.kds_routes.KDSService") as mock_service:
                ...
                mock_service.return_value.start_item.side_effect = ValueError("Item status already changed")
                
                # Act
                response = client.post("/api/v1/kds/items/1/start", headers=auth_headers)
    ...
    def test_large_limit_validation(self, client, auth_headers):
        ...
        response = client.get("/api/v1/kds/stations/1/items?limit=1000", headers=auth_headers)
