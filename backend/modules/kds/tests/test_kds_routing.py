...
from modules.kds.models.kds_models import (
    KitchenStation, StationAssignment, MenuItemStation, KDSOrderItem,
...
class TestKDSOrderRouting:
    ...
    def setup_test_data(self, db: Session):
        ...
        grill_station = kds_service.create_station(StationCreate(
            name="Grill Station",
            station_type=StationType.GRILL,
            priority=10
        ))
        salad_station = kds_service.create_station(StationCreate(
            name="Salad Station",
            station_type=StationType.SALAD,
            priority=5
        ))
        dessert_station = kds_service.create_station(StationCreate(
            name="Dessert Station",
            station_type=StationType.DESSERT,
            priority=3
        ))
        expo_station = kds_service.create_station(StationCreate(
            name="Expo Station",
    ...
    def test_route_order_with_direct_assignments(self, db: Session, setup_test_data):
        ...
        burger_kds = next((item for item in routed_items 
                          if item.order_item_id == burger_item.id), None)
        ...
        # Check salad routing
        salad_kds = next((item for item in routed_items 
                         if item.order_item_id == salad_item.id), None)
    ...
    def test_fallback_routing(self, db: Session, setup_test_data):
        ...
        assert "No specific station found" in (routed_items[0].special_instructions or "")
    ...
    def test_multi_station_routing(self, db: Session, setup_test_data):
        ...
        routing_service.assign_menu_item_to_station(
            MenuItemStationCreate(
                menu_item_id=combo_item.id,
                station_id=data['stations']['salad'].id,  # Using salad station for seafood
                is_primary=False,
                sequence=1,
                prep_time_minutes=15,
        ...
        # Check sequencing
        grill_item = next((item for item in routed_items 
                          if item.station_id == data['stations']['grill'].id), None)
        salad_item = next((item for item in routed_items 
                          if item.station_id == data['stations']['salad'].id), None)
