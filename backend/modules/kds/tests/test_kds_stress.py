...
class TestKDSStress:
    ...
    def test_high_volume_order_routing(self, session_factory):
        ...
        with session_factory() as session:
            ...
            stations = []
            for i, station_type in enumerate([StationType.GRILL, StationType.FRY, 
                                             StationType.SALAD, StationType.DESSERT]):
                station = KitchenStation(
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(create_and_route_order, i)
    ...
    def test_station_summary_under_load(self, session_factory):
        ...
        with session_factory() as session:
            ...
            for i in range(100):
                ...
                kds_item = KDSOrderItem(
                    ...
                    quantity=1,
                    status=status,
                    received_at=datetime.utcnow() - timedelta(minutes=random.randint(1, 60))
                )
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(get_station_summary)
                for _ in range(20)
            ]
    ...
    async def test_websocket_broadcast_performance(self, session_factory):
        ...
        for _ in range(100):
            ...
            task = manager.broadcast_new_item(station_id, {
                "item": {
    ...
    def test_race_condition_prevention(self, session_factory):
        ...
        with session_factory() as session:
            ...
            staff1 = StaffMember(name="Chef 1", email="chef1@test.com", 
                                phone="1111111111", role="chef", is_active=True)
            staff2 = StaffMember(name="Chef 2", email="chef2@test.com", 
                                phone="2222222222", role="chef", is_active=True)
        ...
        assert success_count == 1, "Race condition: multiple chefs started the same item"
