...
from ..schemas.payroll_tax_schemas import (
    ...
    PayrollTaxCalculationRequest, TaxRuleValidationRequest,
...
class PayrollTaxService:
    ...
    def update_employee_payment_taxes(
        ...
        payment = self.db.query(EmployeePayment).filter(
            EmployeePayment.id == employee_payment_id
        ).first()
    ...
    def bulk_recalculate_taxes(
        ...
        query = self.db.query(EmployeePayment).join(
            EmployeePayment.payroll_policy
        ).filter(
