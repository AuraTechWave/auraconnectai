...
class PayrollConfigurationService:
    ...
    def get_staff_pay_policy_from_db(
        self, staff_id: int, location: str, effective_date: datetime = None
    ) -> Optional[StaffPayPolicy]:
        """
        ...
        policy = (
            ...
            .order_by(StaffPayPolicy.effective_date.desc())
            .first()
        )

        return policy

    def get_benefit_proration_factor(
        self, location: str = "default", tenant_id: Optional[int] = None
    ) -> Decimal:
        """
        ...
        config = (
            ...
            .filter(
                and_(
                    PayrollConfiguration.config_type
                    == PayrollConfigurationType.BENEFIT_PRORATION,
            ...
            .order_by(PayrollConfiguration.effective_date.desc())
            .first()
        )
    ...
    def get_overtime_rules(
        self, jurisdiction: str, tenant_id: Optional[int] = None
    ) -> List[OvertimeRule]:
        """
        ...
        rules = (
            ...
            .order_by(OvertimeRule.precedence.desc())
            .all()
        )

        return rules

    def get_tax_approximation_breakdown(
        self, jurisdiction: str, tenant_id: Optional[int] = None
    ) -> Dict[str, Decimal]:
        """
        ...
        """
        rule = (
            self.db.query(TaxApproximationRule)
            .filter(
                and_(
            ...
            .order_by(TaxApproximationRule.effective_date.desc())
            .first()
        )
    ...
    def get_role_based_pay_rate(
        self, role_name: str, location: str, experience_level: str = "default"
    ) -> Optional[RoleBasedPayRate]:
        """
        ...
        """
        rate = (
            self.db.query(RoleBasedPayRate)
            .filter(
                and_(
            ...
            .order_by(RoleBasedPayRate.effective_date.desc())
            .first()
        )

        return rate

    def create_job_tracking(
        self,
        job_type: str,
    ...
    def seed_default_configurations(
        ...
        proration_config = PayrollConfiguration(
            ...
            config_value={
                "factor": "0.46",
                "description": "Monthly benefit to biweekly proration",
            },
