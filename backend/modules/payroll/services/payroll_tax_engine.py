...
class PayrollTaxEngine:
    ...
    def calculate_payroll_taxes(
        self, request: PayrollTaxCalculationRequest
    ) -> PayrollTaxCalculationResponse:
        """
    ...
    def _get_applicable_tax_rules(
        self, location: str, pay_date: date, tenant_id: Optional[int] = None
    ) -> List[TaxRule]:
        """
    ...
    def _apply_tax_rule(
        self, rule: TaxRule, request: PayrollTaxCalculationRequest
    ...
    def _calculate_taxable_amount(self, rule: TaxRule, gross_pay: Decimal) -> Decimal:
        """
    ...
    def save_tax_applications(
        self, employee_payment_id: int, tax_applications: List[TaxApplicationDetail]
    ...
    def get_jurisdiction_summary(
        ...
        for rule in rules:
            rule_info = {
                ...
                "max_taxable_amount": (
                    float(rule.max_taxable_amount) if rule.max_taxable_amount else None
                ),
            }
