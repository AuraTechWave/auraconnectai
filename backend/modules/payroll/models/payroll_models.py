...
class TaxRule(Base, TimestampMixin):
    ...
    tax_type = Column(Enum(TaxType, values_callable=lambda obj: [e.value for e in obj], create_type=False), nullable=False)
...
class PayrollPolicy(Base, TimestampMixin):
    ...
    location = Column(String(100), nullable=False, index=True)
    pay_frequency = Column(Enum(PayFrequency, values_callable=lambda obj: [e.value for e in obj], create_type=False), nullable=False)
...
class EmployeePayment(Base, TimestampMixin):
    ...
    payment_status = Column(
        Enum(PaymentStatus, values_callable=lambda obj: [e.value for e in obj], create_type=False),
        default=PaymentStatus.PENDING,
        nullable=False
    )
    payment_method = Column(Enum(PaymentMethod, values_callable=lambda obj: [e.value for e in obj], create_type=False), nullable=True)
