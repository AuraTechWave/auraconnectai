...
from modules.payroll.enums import PaymentStatus, PayFrequency, TaxType, PaymentMethod
...
class TaxRule(Base, TimestampMixin):
    ...
    tax_applications = relationship(
        "EmployeePaymentTaxApplication", back_populates="tax_rule"
    )
...
class EmployeePayment(Base, TimestampMixin):
    ...
    tax_applications = relationship(
        "EmployeePaymentTaxApplication", back_populates="employee_payment"
    )
...
class EmployeePaymentTaxApplication(Base, TimestampMixin):
    ...
    employee_payment_id = Column(
        Integer, ForeignKey("employee_payments.id"), nullable=False
    )
    ...
    employee_payment = relationship(
        "EmployeePayment", back_populates="tax_applications"
    )
    tax_rule = relationship("TaxRule", back_populates="tax_applications")

    __table_args__ = (
        UniqueConstraint(
            "employee_payment_id", "tax_rule_id", name="uq_payment_tax_rule"
        ),
...
if os.getenv("AURACONNECT_ENABLE_TIP_RECORDS", "1") == "1":  # pragma: no cover - optional feature toggle
    class TipRecord(Base, TimestampMixin):
        ...
        payroll_period_id = Column(
            Integer, ForeignKey("payroll_periods.id"), nullable=True
        )
