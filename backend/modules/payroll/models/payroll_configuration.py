...
class PayrollConfiguration(Base, TimestampMixin):
    ...
    __table_args__ = {"comment": "Configurable payroll business logic settings"}
...
class PayrollJobTracking(Base, TimestampMixin):
    ...
    __table_args__ = {"comment": "Tracking for batch payroll processing jobs"}
...
class StaffPayPolicy(Base, TimestampMixin):
    ...
    double_time_multiplier = Column(
        Numeric(5, 4), default=Decimal("2.0"), nullable=False
    )

    # Overtime rules - configurable thresholds
    daily_overtime_threshold = Column(Numeric(6, 2), nullable=True)
    weekly_overtime_threshold = Column(
        Numeric(6, 2), default=Decimal("40.0"), nullable=False
    )

    # Benefit deductions (monthly amounts)
    health_insurance_monthly = Column(
        Numeric(8, 2), default=Decimal("0.00"), nullable=False
    )
    dental_insurance_monthly = Column(
        Numeric(8, 2), default=Decimal("0.00"), nullable=False
    )
    retirement_contribution_monthly = Column(
        Numeric(8, 2), default=Decimal("0.00"), nullable=False
    )
    ...
    __table_args__ = {"comment": "Staff-specific pay policies and configurations"}
...
class OvertimeRule(Base, TimestampMixin):
    ...
    __table_args__ = {"comment": "Configurable overtime rules by jurisdiction"}
...
class TaxApproximationRule(Base, TimestampMixin):
    ...
    unemployment_percentage = Column(
        Numeric(5, 4), default=Decimal("0.0"), nullable=False
    )
...
class RoleBasedPayRate(Base, TimestampMixin):
    ...
    __table_args__ = {"comment": "Role-based default pay rates by location"}
