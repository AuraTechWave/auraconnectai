...
class PayrollBusinessRuleError(PayrollException):
    ...
    def __init__(
        self, message: str, rule: str, details: Optional[List[ErrorDetail]] = None
    ):
        super().__init__(
            message=message,
...
class WebhookError(PayrollException):
    ...
    def __init__(
        self, message: str, webhook_url: Optional[str] = None, status_code: int = 500
    ):
        if webhook_url:
...
class WebhookDeliveryError(WebhookError):
    ...
    def __init__(
        ...
        if response_body:
            details.append(
                ErrorDetail(field="response", message=response_body[:200])
            )  # Truncate long responses

        super().__init__(
            message=(
                f"Failed to deliver webhook: HTTP {status_code}"
                if status_code
                else "Failed to deliver webhook"
            ),
...
class AuditLogError(PayrollException):
    ...
    def __init__(self, message: str, operation: Optional[str] = None):
        ...
        super().__init__(
            message=message, code=PayrollErrorCodes.AUDIT_LOG_ERROR, status_code=500
        )
...
class AuditExportError(AuditLogError):
    ...
    def __init__(self, message: str, export_format: Optional[str] = None):
        ...
        super().__init__(message=message, operation="export")
