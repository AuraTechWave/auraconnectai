...
class WebhookSubscriptionRequest(BaseModel):
    ...
    retry_policy: Optional[Dict[str, Any]] = Field(
        None,
        description="Optional retry policy configuration"
    )
    ...
    def validate_headers(cls, v):
        if v:
            ...
            reserved_headers = {'x-webhook-signature', 'x-webhook-timestamp', 'x-webhook-event'}
...
class WebhookTestResponse(BaseModel):
    ...
    subscription_id: int = Field(
        ...,
        description="Tested subscription ID"
    )
    webhook_url: str = Field(
        ...,
    ...
    event_type: str = Field(
        ...,
        description="Event type that was simulated"
    )
    status: str = Field(
        ...,
        description="Test status (queued, sent, failed)"
    )
...
class WebhookRetryPolicy(BaseModel):
    ...
    backoff_multiplier: float = Field(
        ...
        ge=1.0,
        le=10.0,
        description="Backoff multiplier for subsequent retries"
    )
...
class WebhookSecurityConfig(BaseModel):
    ...
    timestamp_tolerance_seconds: int = Field(
        ...
        ge=60,
        le=3600,
        description="Timestamp validation tolerance"
    )
    ip_whitelist: Optional[List[str]] = Field(
        None,
        description="Optional IP whitelist for webhook endpoints"
    )
