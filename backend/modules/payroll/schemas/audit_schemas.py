...
class AuditLogFilter(BaseModel):
    ...
    entity_ids: Optional[List[int]] = Field(
        None,
        description="Filter by specific entity IDs"
    )
...
class AuditLogResponse(BaseModel):
    ...
    filters_applied: Optional[Dict[str, Any]] = Field(
        None,
        description="Active filters summary"
    )
...
class AuditSummaryResponse(BaseModel):
    ...
    group_by: str = Field(
        ...,
        description="Grouping field used"
    )
...
class AuditExportRequest(BaseModel):
    ...
    include_old_values: bool = Field(
        True,
        description="Include old values for update events"
    )
    include_new_values: bool = Field(
        True,
        description="Include new values for update events"
    )
...
class AuditRetentionPolicy(BaseModel):
    ...
    delete_archived_after_days: Optional[int] = Field(
        ...
        ge=365,
        le=2555,
        description="Days to keep archived logs"
    )
    excluded_event_types: List[AuditEventType] = Field(
        default_factory=list,
        description="Event types to exclude from deletion"
    )
    compliance_mode: bool = Field(
        True,
        description="Enable compliance mode (stricter retention)"
    )
