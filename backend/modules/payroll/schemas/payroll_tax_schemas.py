...
class PayrollTaxCalculationRequest(BaseModel):
    ...
    location: str = Field(..., description="Location/jurisdiction for tax rules")
    gross_pay: Decimal = Field(..., ge=0, description="Gross pay amount for the period")
    pay_date: date = Field(..., description="Pay date for determining applicable tax rules")
    tenant_id: Optional[int] = Field(None, description="Tenant ID for multi-tenant setup")
...
class TaxApplicationDetail(BaseModel):
    ...
    tax_type: TaxType = Field(..., description="Type of tax (federal, state, local, etc.)")
    location: str = Field(..., description="Jurisdiction location")
    taxable_amount: Decimal = Field(..., ge=0, description="Amount subject to this tax")
    calculated_tax: Decimal = Field(..., ge=0, description="Calculated tax amount")
    effective_rate: Decimal = Field(..., ge=0, description="Effective tax rate applied")
...
class PayrollTaxCalculationResponse(BaseModel):
    ...
    gross_pay: Decimal = Field(..., ge=0, description="Original gross pay amount")
    total_taxes: Decimal = Field(..., ge=0, description="Total calculated taxes")
    net_pay: Decimal = Field(..., ge=0, description="Net pay after tax deductions")
    tax_breakdown: TaxBreakdown = Field(..., description="Breakdown by tax category")
...
class TaxRuleValidationResponse(BaseModel):
    ...
    location: str
    total_rules: int = Field(..., description="Total applicable tax rules")
    jurisdiction_summary: dict = Field(..., description="Rules grouped by jurisdiction")
...
class PayrollTaxServiceRequest(BaseModel):
    ...
    employee_payment_id: Optional[int] = Field(None, description="Existing payment record ID")
    ...
    gross_pay: Decimal = Field(..., ge=0, description="Calculated gross pay")
...
class PayrollTaxServiceResponse(BaseModel):
    ...
    employee_payment_id: Optional[int] = Field(None, description="Payment record ID")
    tax_calculation: PayrollTaxCalculationResponse = Field(..., description="Tax calculation results")
    applications_saved: bool = Field(default=False, description="Whether tax applications were persisted")
