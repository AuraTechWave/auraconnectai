...
class PayrollTaxCalculationResponse(BaseModel):
    ...
    tax_applications: List[TaxApplicationDetail] = Field(
        default_factory=list, description="Detailed tax rule applications"
    )
...
class TaxRuleValidationResponse(BaseModel):
    ...
    missing_jurisdictions: List[str] = Field(
        default_factory=list, description="Expected jurisdictions without rules"
    )
    potential_issues: List[str] = Field(
        default_factory=list, description="Potential configuration issues"
    )
...
class PayrollTaxServiceResponse(BaseModel):
    ...
    applications_saved: bool = Field(
        default=False, description="Whether tax applications were persisted"
    )
...
class TaxRuleResponse(BaseModel):
    ...
    fixed_amount: Optional[Decimal] = Field(
        None, description="Fixed tax amount if applicable"
    )
    ...
    is_active: bool = Field(
        default=True, description="Whether rule is currently active"
    )
