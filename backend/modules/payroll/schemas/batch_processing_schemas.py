...
class CalculationOptions(BaseModel):
    ...
    include_bonuses: bool = Field(
        True,
        description="Include bonus calculations in payroll"
    )
    include_commissions: bool = Field(
        True,
        description="Include commission calculations in payroll"
    )
    include_overtime: bool = Field(
        True,
        description="Calculate overtime pay"
    )
    include_deductions: bool = Field(
        True,
        description="Apply all deductions"
    )
    force_recalculate: bool = Field(
        False,
        description="Force recalculation even if payroll exists"
    )
    use_cached_tax_rates: bool = Field(
        True,
        description="Use cached tax rates for performance"
    )
...
class BatchPayrollResponse(BaseModel):
    ...
    tracking_url: Optional[str] = Field(
        None,
        description="URL to track job progress"
    )
...
class EmployeePayrollResult(BaseModel):
    ...
    success: bool = Field(
        ...,
        description="Whether payroll calculation succeeded"
    )
    ...
    payment_id: Optional[int] = Field(
        None,
        description="Generated payment record ID"
    )
    error_message: Optional[str] = Field(
        None,
...
class BatchJobDetail(BaseModel):
    ...
    employee_results: Optional[List[EmployeePayrollResult]] = Field(
        None,
        description="Individual employee results (if requested)"
    )
...
class BatchJobHistoryItem(BaseModel):
    ...
    total_gross: Optional[Decimal] = Field(
        None,
        description="Total gross pay calculated"
    )
    total_net: Optional[Decimal] = Field(
        None,
        description="Total net pay calculated"
    )
...
class BatchJobHistoryResponse(BaseModel):
    ...
    jobs: List[BatchJobHistoryItem] = Field(
        ...,
...
class BatchCancellationRequest(BaseModel):
    ...
    notify: bool = Field(
        True,
        description="Send notifications about cancellation"
    )
