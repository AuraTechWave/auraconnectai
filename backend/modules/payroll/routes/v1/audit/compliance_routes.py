...
async def get_compliance_report(
    ...
    try:
        ...
        if report_type in ["sensitive", "all"]:
            ...
            ).group_by(PayrollAuditLog.event_type).all()
            
            report_data["sensitive_operations"] = {
                "total_sensitive_events": sum(event.count for event in sensitive_events),
                "events_by_type": [
        ...
        report_data["recommendations"] = generate_compliance_recommendations(report_data)
