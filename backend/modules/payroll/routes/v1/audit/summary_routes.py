...
async def get_audit_summary(
    ...
    try:
        # Build base query
        query = db.query(func.count(PayrollAuditLog.id).label("count")).filter(
        ...
        summary_data = [
            {
                ...
                "count": result.count,
                "percentage": (
                    (result.count / total_events * 100) if total_events > 0 else 0
                ),
            }
        ...
        top_users = (
            top_users_query.group_by(PayrollAuditLog.user_email)
            .order_by(func.count(PayrollAuditLog.id).desc())
            .limit(10)
            .all()
        )
        ...
        raise DatabaseError(
            message="Failed to generate audit summary", operation="aggregate"
        ) from e
