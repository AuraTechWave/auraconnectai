    ...
    try:
        ...
        if effective_date:
            query = query.filter(
                ...
                (TaxRule.expiry_date.is_(None)) | (TaxRule.expiry_date >= effective_date)
            )
...
async def get_effective_tax_rates(
    ...
    try:
        ...
        return {
            ...
            "effective_rates": {
                "federal": str(next((r.rate for r in rules if r.tax_type == TaxRuleType.FEDERAL), Decimal('0'))),
                "state": str(next((r.rate for r in rules if r.tax_type == TaxRuleType.STATE), Decimal('0'))),
                "local": str(next((r.rate for r in rules if r.tax_type == TaxRuleType.LOCAL), Decimal('0'))),
                "social_security": str(next((r.rate for r in rules if r.tax_type == TaxRuleType.SOCIAL_SECURITY), Decimal('0'))),
                "medicare": str(next((r.rate for r in rules if r.tax_type == TaxRuleType.MEDICARE), Decimal('0')))
            },
            "estimated_taxes": {
