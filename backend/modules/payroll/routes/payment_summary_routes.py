...
async def get_payment_summary_by_period(
    ...
    try:
        ...
        summary_data = db.query(
            func.count(func.distinct(EmployeePayment.employee_id)).label(
                "employee_count"
            ),
...
async def get_tax_summary(
    ...
    end_date: date = Query(..., description="Period end date"),
    group_by: str = Query(
        "tax_type", description="Group by: tax_type, location, or month"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(require_payroll_access),
):
    ...
    try:
        ...
        # Base aggregation
        tax_totals = {
            "federal": db.query(func.sum(EmployeePayment.federal_tax_amount))
            .filter(
                EmployeePayment.pay_period_start >= start_date,
                EmployeePayment.pay_period_end <= end_date,
            )
            .scalar()
            or 0,
            "state": db.query(func.sum(EmployeePayment.state_tax_amount))
            .filter(
                EmployeePayment.pay_period_start >= start_date,
                EmployeePayment.pay_period_end <= end_date,
            )
            .scalar()
            or 0,
            "local": db.query(func.sum(EmployeePayment.local_tax_amount))
            .filter(
                EmployeePayment.pay_period_start >= start_date,
                EmployeePayment.pay_period_end <= end_date,
            )
            .scalar()
            or 0,
            "social_security": db.query(
                func.sum(EmployeePayment.social_security_amount)
            )
            .filter(
                EmployeePayment.pay_period_start >= start_date,
                EmployeePayment.pay_period_end <= end_date,
            ...
            .scalar()
            or 0,
            "medicare": db.query(func.sum(EmployeePayment.medicare_amount))
            .filter(
                EmployeePayment.pay_period_start >= start_date,
                EmployeePayment.pay_period_end <= end_date,
            )
            .scalar()
            or 0,
            "unemployment": db.query(func.sum(EmployeePayment.unemployment_amount))
            .filter(
                EmployeePayment.pay_period_start >= start_date,
                EmployeePayment.pay_period_end <= end_date,
            )
