...
async def create_overtime_rule(
    ...
    try:
        ...
        existing = db.query(OvertimeRule).filter(
            OvertimeRule.location == rule_data.location
        ).first()
        ...
        # Validate thresholds
        if rule_data.daily_double_time_threshold and rule_data.daily_double_time_threshold <= rule_data.daily_overtime_threshold:
    ...
    try:
        ...
        for field, value in rule_update.items():
            if hasattr(rule, field) and field not in ["id", "created_at"]:
                if field in ["daily_overtime_threshold", "weekly_overtime_threshold",
                           "daily_double_time_threshold", "weekly_double_time_threshold"]:
                    setattr(rule, field, Decimal(str(value)))
                else:
                    setattr(rule, field, value)
        
        # Validate thresholds after update
        if (rule.daily_double_time_threshold and 
            rule.daily_double_time_threshold <= rule.daily_overtime_threshold):
            raise PayrollValidationError(
                "Daily double time threshold must be greater than overtime threshold"
            )
