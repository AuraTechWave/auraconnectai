...
async def process_payment_export(
    db: Session, job_id: str, export_request: PaymentExportRequest, user_id: int
):
    """
...
async def export_payment_data(
    ...
    try:
        ...
        active_exports = (
            ...
            .filter(
                ...
                PayrollJobTracking.status.in_(
                    [PayrollJobStatus.PENDING, PayrollJobStatus.PROCESSING]
                ),
                PayrollJobTracking.created_at
        ...
        background_tasks.add_task(
            process_payment_export, db, job_id, export_request, current_user.id
        )
...
async def download_export(
    ...
    if (
        job.completed_at
        and (datetime.utcnow() - job.completed_at).total_seconds() > 86400
    ):
        raise HTTPException(
            status_code=410,
            detail=ErrorResponse(
