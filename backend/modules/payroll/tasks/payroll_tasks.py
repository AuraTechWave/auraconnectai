...
@celery_app.task(
    bind=True,
    base=DatabaseTask,
    name="payroll.process_batch_payroll",
    max_retries=3
)
...
@celery_app.task(
    bind=True,
    base=DatabaseTask,
    name="payroll.export_audit_logs",
    max_retries=2
)
def export_audit_logs(
    ...
    try:
        ...
        send_export_notification.apply_async(
            args=[user_id, export_id, download_url],
            countdown=5
        )
...
def send_webhook(event_type: str, payload: Dict[str, Any]):
    ...
    try:
        ...
        subscriptions = db.query(PayrollWebhookSubscription).filter(
        ...
        for sub in subscriptions:
            try:
                ...
                if status_code and 200 <= status_code < 300:
                    ...
                    logger.warning(f"Failed to send webhook to {sub.webhook_url}: {status_code}")
                    sub.failure_count += 1
...
def cleanup_old_jobs():
    ...
    try:
        ...
        deleted = db.query(PayrollJobTracking).filter(
...
@celery_app.task(
    name="payroll.retry_failed_webhooks"
)
...
@celery_app.task(
    name="payroll.generate_audit_summary"
)
