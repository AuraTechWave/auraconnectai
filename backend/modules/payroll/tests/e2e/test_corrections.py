...
class TestPayrollCorrections:
    ...
    async def test_overpayment_recovery(self, mock_db, sample_employees):
        ...
        with patch.object(payroll_service, 'apply_overpayment_recovery') as mock_recovery:
            mock_recovery.return_value = {
                "gross_pay": current_gross,
                "recovery_deduction": recovery_amount,
                "adjusted_net_pay": current_gross - recovery_amount - Decimal("575.00")  # After taxes
            }
        ...
        assert payment.net_pay < (current_gross - Decimal("575.00"))  # Less than normal net
