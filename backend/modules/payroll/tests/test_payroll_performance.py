...
class TestPayrollPerformance:
    ...
    def generate_employees(self):
        ...
        def _generate(count: int):
            ...
            for i in range(count):
                ...
                emp.department = random.choice(
                    ["Engineering", "Sales", "Support", "HR", "Finance"]
                )
                emp.location = random.choice(
    ...
    async def test_batch_processing_100_employees(self, mock_db, generate_employees):
        ...
        with patch.object(
            batch_service.payroll_service, "calculate_payroll"
        ) as mock_calc:
            mock_calc.return_value = Mock(
                employee_id=1, gross_pay=Decimal("2500.00"), net_pay=Decimal("1875.00")
            )
    ...
    async def test_batch_processing_1000_employees(self, mock_db, generate_employees):
        ...
        async def progress_callback(current, total, name):
            progress_updates.append(
                {"timestamp": time.time(), "current": current, "total": total}
            )
        ...
        with patch.object(
            batch_service.payroll_service, "calculate_payroll", mock_calculation
        ):
            # Measure performance
            start_time = time.time()
        ...
        if len(progress_updates) > 1:
            ...
            for i in range(1, len(progress_updates)):
                time_diff = (
                    progress_updates[i]["timestamp"]
                    - progress_updates[i - 1]["timestamp"]
                )
                emp_diff = (
                    progress_updates[i]["current"] - progress_updates[i - 1]["current"]
                )
                if time_diff > 0:
        ...
        def mock_query_filter(model):
            ...
            def filter_impl(**kwargs):
                if "department" in str(kwargs):
                    ...
                    dept_employees = [
                        e for e in all_employees if e.department in str(kwargs)
                    ]
    ...
    async def test_memory_efficiency(self, mock_db, generate_employees):
        ...
        std_dev = (
            statistics.stdev(mem_per_emp_values) if len(mem_per_emp_values) > 1 else 0
        )
    ...
    async def test_error_recovery_performance(self, mock_db, generate_employees):
        ...
        async def mock_calculation_with_errors(*args, **kwargs):
            ...
            return Mock(
                employee_id=kwargs.get("employee_id", 1), gross_pay=Decimal("2500.00")
            )

        with patch.object(
            batch_service.payroll_service,
            "calculate_payroll",
    ...
    async def test_export_performance(self, mock_db):
        ...
        for i in range(1000):
            ...
            payment.employee = Mock(
                full_name=f"Employee {i + 1}", employee_code=f"EMP{str(i + 1).zfill(6)}"
            )
