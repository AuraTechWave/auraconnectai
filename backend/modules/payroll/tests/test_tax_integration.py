...
from ....tax.models.tax_models import (
    TaxBracket,
    TaxRate,
    TaxExemption,
    TaxFilingStatus
...
class TestTaxIntegration:
    ...
    async def test_calculate_state_tax_california(
        ...
        mock_query.first.return_value = Mock(
            standard_deduction=Decimal("5363.00"),
            personal_exemption=Decimal("154.00")
        )
    ...
    @pytest.mark.asyncio
    async def test_calculate_fica_with_wage_base_limit(self, tax_engine, sample_employee):
    ...
    async def test_tax_calculation_married_filing_jointly(
        ...
        mock_query.first.return_value = Mock(
            standard_deduction=Decimal("27700.00"),
            personal_exemption=Decimal("0.00")
        )
