...
class TestPayrollTaxService:
    ...
    def tax_service(self, mock_db):
        ...
        with patch(
            "modules.payroll.services.payroll_tax_service.PayrollTaxEngine"
    ...
    def test_calculate_and_save_taxes_without_payment_id(
        ...
        tax_service.tax_engine.calculate_payroll_taxes.return_value = (
            sample_tax_calculation
        )
    ...
    def test_update_employee_payment_taxes_success(
        ...
        expected_total_deductions = (
            ...
            + Decimal("0.00")  # taxes
            + Decimal("310.00")
            + Decimal("72.50")  # payroll taxes
            + Decimal("100.00")
            + Decimal("200.00")
            + Decimal("50.00")  # other deductions
        )
    ...
    def test_update_employee_payment_taxes_not_found(self, tax_service, mock_db):
    ...
    def test_validate_tax_setup_complete(self, tax_service):
        ...
        request = TaxRuleValidationRequest(
            location="California", pay_date=date(2025, 7, 15), tenant_id=1
        )
    ...
    def test_validate_tax_setup_missing_jurisdictions(self, tax_service):
        ...
        tax_service.tax_engine.get_jurisdiction_summary.return_value = (
            incomplete_summary
        )

        # Create validation request
        request = TaxRuleValidationRequest(location="Texas", pay_date=date(2025, 7, 15))
    ...
    def test_get_effective_tax_rates_zero_gross_pay(self, tax_service):
        ...
        rates = tax_service.get_effective_tax_rates(
            location="California", gross_pay=Decimal("0.00"), pay_date=date(2025, 7, 15)
        )
