...
class TestDatabaseIntegration:
    ...
    def test_create_employee_payment(self, db_session):
        ...
        saved_payment = (
            db_session.query(EmployeePayment).filter_by(id=payment.id).first()
        )
    ...
    def test_payroll_configuration_crud(self, db_session):
        ...
        deactivated = (
            db_session.query(PayrollConfiguration).filter_by(id=config.id).first()
        )
    ...
    def test_staff_pay_policy_with_history(self, db_session):
        ...
        current_policy = (
            db_session.query(StaffPayPolicy)
            .filter(
    ...
    def test_payment_query_performance(self, db_session):
        ...
        payments = (
            db_session.query(EmployeePayment)
            .filter(
    ...
    def test_transaction_rollback(self, db_session):
        ...
        try:
            ...
            invalid_payment = EmployeePayment(
                employee_id=None, gross_pay=Decimal("1000.00")  # Required field
            )
        ...
        rolled_back = db_session.query(EmployeePayment).filter_by(id=payment_id).first()
