...
class TestForeignKeyConstraints:
    ...
    def test_tax_application_foreign_keys(self, db_session: Session):
        ...
        role = Role(name="server", permissions="basic")
        staff = StaffMember(name="Jane Doe", email="jane@example.com", role_id=1, status=StaffStatus.ACTIVE)
        policy = PayrollPolicy(
            policy_name="Test Policy", location="Test", pay_frequency=PayFrequency.BIWEEKLY,
            pay_period_start_day=1, minimum_wage=Decimal('15.00')
        )
        payment = EmployeePayment(
            staff_id=1, payroll_policy_id=1, pay_period_start=datetime.now(),
            pay_period_end=datetime.now() + timedelta(days=14), pay_date=datetime.now(),
            regular_rate=Decimal('15.00'), gross_pay=Decimal('600.00'),
            total_deductions=Decimal('120.00'), net_pay=Decimal('480.00')
        )
        tax_rule = TaxRule(
            rule_name="Federal Tax", location="US", tax_type=TaxType.FEDERAL,
            rate_percent=Decimal('0.12'), effective_date=datetime.now()
        )
...
class TestEnumValidation:
    ...
    def test_payment_status_enum_validation(self, db_session: Session):
        ...
        for status in PaymentStatus:
            payment = EmployeePayment(
                staff_id=1, payroll_policy_id=1,
                pay_period_start=datetime.now(),
                pay_period_end=datetime.now() + timedelta(days=14),
                pay_date=datetime.now(),
                regular_rate=Decimal('15.00'),
...
class TestDataValidationConstraints:
    ...
    def test_positive_hours_constraint(self, db_session: Session):
        ...
        # Create required entities
        role = Role(name="server", permissions="basic")
        staff = StaffMember(name="Test Staff", email="test@example.com", role_id=1, status=StaffStatus.ACTIVE)
        policy = PayrollPolicy(
            policy_name="Test Policy", location="Test", pay_frequency=PayFrequency.BIWEEKLY,
            pay_period_start_day=1, minimum_wage=Decimal('15.00')
        )
...
class TestTaxRuleApplicability:
    ...
    def test_tax_rule_effective_date_filtering(self, db_session: Session):
        ...
        applicable_rules = db_session.query(TaxRule).filter(
    ...
    def test_tax_rule_location_jurisdiction(self, db_session: Session):
        ...
        ca_rules = db_session.query(TaxRule).filter(
            TaxRule.location.in_(["California", "US"])
        ).all()
