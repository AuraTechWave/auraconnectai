...
class TestPaymentExportService:
    ...
    @pytest.mark.asyncio
    async def test_export_payments_excel_success(
        self, service, mock_db, sample_payments
    ):
    ...
    @pytest.mark.asyncio
    async def test_export_payments_json_success(
        self, service, mock_db, sample_payments
    ):
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            result = await service.export_payments(
                start_date=date(2024, 1, 1), end_date=date(2024, 1, 31), format="json"
            )
    ...
    @pytest.mark.asyncio
    async def test_export_payments_with_employee_filter(
        self, service, mock_db, sample_payments
    ):
    ...
    async def test_export_payments_empty_results(self, service, mock_db):
        ...
        # Execute
        result = await service.export_payments(
            start_date=date(2024, 1, 1), end_date=date(2024, 1, 31), format="csv"
        )
    ...
    @pytest.mark.asyncio
    async def test_export_bank_file_wire_format(
        self, service, mock_db, sample_payments
    ):
        ...
        # Execute
        result = await service.export_bank_file(
            payment_ids=[p.id for p in sample_payments], bank_format="wire"
        )
    ...
    async def test_generate_payslips_pdf(self, service, mock_db, sample_payments):
        ...
        with patch("reportlab.pdfgen.canvas.Canvas") as mock_canvas:
            ...
            result = await service.generate_payslips(
                payment_ids=[1], format="pdf", delivery_method="email"
            )
    ...
    async def test_export_tax_report(self, service, mock_db, sample_payments):
        ...
        result = await service.export_tax_report(
            year=2024, quarter=1, report_type="quarterly_941"
        )
    ...
    async def test_batch_export_multiple_formats(
        ...
        results = await service.batch_export(
            start_date=date(2024, 1, 1), end_date=date(2024, 1, 31), formats=formats
        )
