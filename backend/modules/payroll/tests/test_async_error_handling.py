...
class TestAsyncErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_batch_processing_with_database_failure(
        self, mock_db, mock_celery_task
    ):
        ...
        with patch(
            "backend.modules.payroll.tasks.payroll_tasks.get_db", return_value=mock_db
        ):
            with patch.object(mock_celery_task, "retry") as mock_retry:
                ...
                with pytest.raises(Retry):
                    await process_batch_payroll_task.apply_async(
                        args=[job_id], task_id=mock_celery_task.request.id
                    )
    ...
    async def test_batch_processing_partial_failure_recovery(self, mock_db):
        ...
        with patch(
            "backend.modules.payroll.tasks.payroll_tasks.BatchPayrollService"
        ) as mock_service:
        ...
        with patch("requests.post") as mock_post:
            ...
            with patch.object(mock_celery_task, "request") as mock_request:
                ...
                result = await send_webhook_notification_task(
                    webhook_url=webhook_url, payload=payload, _task=mock_celery_task
                )
    ...
    async def test_webhook_max_retries_exceeded(self, mock_celery_task):
        ...
        with patch(
            "backend.modules.payroll.tasks.payroll_tasks.log_webhook_failure"
        ) as mock_log:
            mock_log.assert_called_with(
                webhook_url=webhook_url, error="Permanent failure", attempts=4
            )
    ...
    async def test_export_task_with_memory_limit(self):
        ...
        with patch(
            "backend.modules.payroll.tasks.payroll_tasks.PaymentExportService"
        ) as mock_service:
            ...
            with patch.object(
                mock_export_service, "export_payments_chunked"
            ) as mock_chunked:
                ...
                result = await export_audit_logs_task.apply_async(args=[export_params])
    ...
    async def test_concurrent_job_locking(self, mock_db):
        ...
        with pytest.raises(BatchProcessingError) as exc_info:
            await process_batch_payroll_task.apply_async(args=[job_id])
    ...
    async def test_task_timeout_handling(self, mock_celery_task):
        ...
        with patch(
            "backend.modules.payroll.tasks.payroll_tasks.cleanup_failed_job"
        ) as mock_cleanup:
    ...
    async def test_dead_letter_queue_handling(self):
        ...
        with patch(
            "backend.modules.payroll.tasks.payroll_tasks.move_to_dlq"
        ) as mock_dlq:
        ...
        with patch(
            "backend.modules.payroll.tasks.payroll_tasks.send_admin_alert"
        ) as mock_alert:
            mock_alert.assert_called_with(
                subject="Payroll task moved to DLQ", task_info=failed_task
            )
    ...
    async def test_circuit_breaker_pattern(self):
        ...
        with patch(
            "backend.modules.payroll.tasks.payroll_tasks.CircuitBreaker"
        ) as mock_cb:
            ...
            for i in range(4):
                try:
                    await send_webhook_notification_task(
                        webhook_url=webhook_url, payload={"attempt": i}
                    )
    ...
    async def test_idempotent_task_execution(self, mock_db):
        ...
        with patch(
            "backend.modules.payroll.tasks.payroll_tasks.check_idempotency"
        ) as mock_check:
