...
from ..schemas.webhook_schemas import WebhookSubscriptionRequest, WebhookEventType
...
class TestWebhookSubscription:
    ...
    async def test_create_subscription(self, mock_db, mock_user, webhook_request):
        ...
        response = await create_webhook_subscription(
            subscription=webhook_request, db=mock_db, current_user=mock_user
        )
    ...
    @pytest.mark.asyncio
    async def test_duplicate_subscription_error(
        self, mock_db, mock_user, webhook_request
    ):
...
class TestWebhookSecurity:
    ...
    async def test_webhook_url_validation(self, mock_db, mock_user):
        ...
        with pytest.raises(Exception) as exc_info:
            await create_webhook_subscription(
                subscription=insecure_request, db=mock_db, current_user=mock_user
            )
