...
class TestRefundEnums:
    ...
    def test_refund_category_mapping(self):
        ...
        # Order issues
        assert (
            get_refund_category(RefundReason.ORDER_CANCELLED)
            == RefundCategory.ORDER_ISSUE
        )
        assert (
            get_refund_category(RefundReason.ORDER_MISTAKE)
            == RefundCategory.ORDER_ISSUE
        )
        assert (
            get_refund_category(RefundReason.WRONG_ITEMS) == RefundCategory.ORDER_ISSUE
        )
        assert (
            get_refund_category(RefundReason.MISSING_ITEMS)
            == RefundCategory.ORDER_ISSUE
        )

        # Quality issues
        assert (
            get_refund_category(RefundReason.FOOD_QUALITY)
            == RefundCategory.QUALITY_ISSUE
        )
        assert (
            get_refund_category(RefundReason.COLD_FOOD) == RefundCategory.QUALITY_ISSUE
        )
        assert (
            get_refund_category(RefundReason.INCORRECT_PREPARATION)
            == RefundCategory.QUALITY_ISSUE
        )

        # Service issues
        assert (
            get_refund_category(RefundReason.LONG_WAIT) == RefundCategory.SERVICE_ISSUE
        )
        assert (
            get_refund_category(RefundReason.POOR_SERVICE)
            == RefundCategory.SERVICE_ISSUE
        )

        # Payment issues
        assert (
            get_refund_category(RefundReason.DUPLICATE_CHARGE)
            == RefundCategory.PAYMENT_ISSUE
        )
        assert (
            get_refund_category(RefundReason.OVERCHARGE) == RefundCategory.PAYMENT_ISSUE
        )
        assert (
            get_refund_category(RefundReason.PRICE_DISPUTE)
            == RefundCategory.PAYMENT_ISSUE
        )

        # Other
        assert (
            get_refund_category(RefundReason.CUSTOMER_REQUEST) == RefundCategory.OTHER
        )
