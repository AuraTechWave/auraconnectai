...
class BillSplit(Base, TimestampMixin):
    ...
    # Split configuration
    split_method = Column(
        SQLEnum(SplitMethod), nullable=False, default=SplitMethod.EQUAL
    )
    status = Column(
        SQLEnum(SplitStatus), nullable=False, default=SplitStatus.PENDING, index=True
    )
    ...
    )  # Percentage or amount based on tip_method
    tip_amount = Column(
    ...
    participants = relationship(
        "SplitParticipant", back_populates="bill_split", cascade="all, delete-orphan"
    )
    ...
    def calculate_participant_share(self, participant_id: int) -> Decimal:
        ...
        if self.split_method == SplitMethod.EQUAL:
            active_participants = [
                p for p in self.participants if p.status != ParticipantStatus.DECLINED
            ]
...
class PaymentAllocation(Base, TimestampMixin):
    ...
    participant_id = Column(
        Integer, ForeignKey("split_participants.id"), nullable=False, index=True
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            ...
            "split_id",
            "participant_id",
            name="uq_payment_split_participant",
        ),
    )
...
class TipDistribution(Base, TimestampMixin):
    ...
    distribution_method = Column(
        String(50), nullable=False, default="pool"
    )  # pool, direct, percentage
