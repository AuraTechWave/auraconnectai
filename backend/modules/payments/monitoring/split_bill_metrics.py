...
split_bill_payment_total = Counter(
    'split_bill_payment_total',
    'Total split bill payments',
    ['status', 'split_method']
)

split_bill_cancelled_total = Counter(
    'split_bill_cancelled_total',
    'Total split bills cancelled',
    ['split_method']
)

split_bill_expired_total = Counter(
    'split_bill_expired_total',
    'Total split bills expired',
    ['split_method']
)
...
split_bill_payment_duration_seconds = Histogram(
    ...
    ['split_method'],
    buckets=(300, 600, 1800, 3600, 7200, 14400, 28800, 86400)  # 5m, 10m, 30m, 1h, 2h, 4h, 8h, 24h
)
...
split_bill_active_count = Gauge(
    'split_bill_active_count',
    'Current number of active split bills',
    ['split_method']
)

split_bill_pending_payments = Gauge(
    'split_bill_pending_payments',
    'Current number of pending participant payments'
)
...
tip_distribution_pending_count = Gauge(
    'tip_distribution_pending_count',
    'Current number of pending tip distributions'
)
...
class SplitBillMetrics:
    ...
    def record_split_payment(status: str, split_method: str):
        ...
        split_bill_payment_total.labels(
            status=status,
            split_method=split_method
        ).inc()
    ...
    def record_payment_duration(split_method: str, duration_seconds: float):
        ...
        split_bill_payment_duration_seconds.labels(
            split_method=split_method
...
async def update_split_bill_gauges(db):
    ...
    from ..models.split_bill_models import BillSplit, SplitParticipant, SplitStatus, ParticipantStatus, TipDistribution
    ...
    try:
        ...
        pending_payments = await db.execute(
            ...
            .where(
                and_(
                    SplitParticipant.status.in_([ParticipantStatus.PENDING, ParticipantStatus.ACCEPTED]),
        ...
        completion_stats = await db.execute(
            select(
                BillSplit.split_method,
                func.count(BillSplit.id).filter(BillSplit.status == SplitStatus.COMPLETED),
                func.count(BillSplit.id)
            )
        ...
        pending_tips = await db.execute(
            select(func.count(TipDistribution.id))
            .where(TipDistribution.is_distributed == False)
        )
