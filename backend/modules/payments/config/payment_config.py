...
class PaymentConfig(BaseSettings):
    ...
    PAYMENT_ENVIRONMENT: PaymentEnvironment = Field(
        default=PaymentEnvironment.DEVELOPMENT,
        description="Payment system environment"
    )
    ...
    WEBHOOK_QUEUE_NAME: str = Field(
        default="payment_webhooks",
        description="Queue name for webhook processing"
    )
    ...
    WEBHOOK_WORKER_CONCURRENCY: int = Field(
        default=10,
        description="Number of concurrent webhook workers"
    )
    ...
    GATEWAY_TIMEOUT_SECONDS: int = Field(
        default=30,
        description="Default gateway API timeout"
    )
    GATEWAY_CONNECT_TIMEOUT: int = Field(
        default=10,
        description="Gateway connection timeout"
    )
    
    # Action Expiration
    PAYMENT_ACTION_EXPIRY_MINUTES: int = Field(
        default=30,
        description="Minutes before payment actions expire"
    )
    ACTION_CHECK_INTERVAL_MINUTES: int = Field(
        default=5,
        description="Interval for checking expired actions"
    )
    
    # Security
    WEBHOOK_SIGNATURE_TOLERANCE_SECONDS: int = Field(
        default=300,
        description="Webhook signature time tolerance"
    )
    ...
    ENABLE_WEBHOOK_QUEUE: bool = Field(
        default=True,
        description="Enable webhook queue processing"
    )
    ENABLE_AUTO_RETRY: bool = Field(
        default=True,
        description="Enable automatic gateway retry"
    )
    ...
    ALERT_PAYMENT_FAILURE_THRESHOLD: float = Field(
        default=0.05,
        description="Payment failure rate threshold (5%)"
    )
    ALERT_GATEWAY_LATENCY_THRESHOLD: float = Field(
        default=5.0,
        description="Gateway latency threshold in seconds"
    )
    ALERT_WEBHOOK_BACKLOG_THRESHOLD: int = Field(
        default=100,
        description="Webhook backlog size threshold"
    )
