...
from ..models.payment_models import (
    Payment, Refund, PaymentWebhook, PaymentGateway,
...
class WebhookService:
        ...
        try:
            if event_type == 'payment_intent.succeeded':
                ...
                await self._update_payment_status(
                    db, PaymentGateway.STRIPE, payment_intent_id,
                    PaymentStatus.COMPLETED, webhook.id
                )
                ...
                await self._update_payment_status(
                    db, PaymentGateway.STRIPE, payment_intent_id,
                    PaymentStatus.FAILED, webhook.id,
                ...
                await self._update_payment_status(
                    db, PaymentGateway.STRIPE, payment_intent_id,
                    PaymentStatus.CANCELLED, webhook.id
                )
                ...
                if refunds:
                    ...
                    await self._update_refund_status(
                        db, PaymentGateway.STRIPE,
                        refund_data.get('id'),
                        RefundStatus.COMPLETED, webhook.id
                    )
                ...
                await self._update_payment_status(
                    db, PaymentGateway.STRIPE, payment_intent_id,
                    PaymentStatus.DISPUTED, webhook.id
                )
    ...
    async def _process_square_webhook(
        ...
        try:
            if event_type == 'payment.created' or event_type == 'payment.updated':
                ...
                if status == 'COMPLETED':
                    await self._update_payment_status(
                        db, PaymentGateway.SQUARE, payment_id,
                        PaymentStatus.COMPLETED, webhook.id
                    )
                elif status == 'FAILED':
                    await self._update_payment_status(
                ...
                if status == 'COMPLETED':
                    await self._update_refund_status(
                        db, PaymentGateway.SQUARE, refund_id,
                        RefundStatus.COMPLETED, webhook.id
                    )
                elif status == 'FAILED':
                    await self._update_refund_status(
                        db, PaymentGateway.SQUARE, refund_id,
                        RefundStatus.FAILED, webhook.id
                    )
    ...
    async def _process_paypal_webhook(
        ...
        try:
            if event_type == 'CHECKOUT.ORDER.APPROVED':
                ...
                await self._update_payment_status(
                    db, PaymentGateway.PAYPAL, order_id,
                    PaymentStatus.PROCESSING, webhook.id
                )
                ...
                await self._update_refund_status(
                    db, PaymentGateway.PAYPAL, refund_id,
                    RefundStatus.COMPLETED, webhook.id
                )
    ...
    def _extract_event_id(self, gateway: PaymentGateway, payload: Dict[str, Any]) -> Optional[str]:
    ...
    def _extract_event_type(self, gateway: PaymentGateway, payload: Dict[str, Any]) -> str:
