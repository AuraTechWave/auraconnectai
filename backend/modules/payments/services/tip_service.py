...
class TipService:
    ...
    def calculate_tip(
        self, subtotal: Decimal, tip_method: TipMethod, tip_value: Decimal, **kwargs
    ) -> Decimal:
        """
    ...
    def suggest_tip_amounts(
        self, subtotal: Decimal, default_percentages: Optional[List[int]] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def process_tip_distribution(
        self, db: AsyncSession, distribution_id: int, processed_by: int
    ) -> TipDistribution:
        """
    ...
    async def _get_eligible_staff(
        self, db: AsyncSession, distribution: TipDistribution
    ) -> List[Staff]:
        ...
        result = await db.execute(
            select(Staff).where(
                and_(Staff.is_active == True, Staff.receives_tips == True)
            )
        )
    ...
    async def _calculate_staff_distributions(
        ...
        if distribution.distribution_method == "pool":
            ...
            config = distribution.distribution_config or {}
            role_percentages = config.get(
                "role_percentages",
                {"server": 40, "bartender": 30, "busser": 15, "host": 15},
            )
            ...
            for role, percentage in role_percentages.items():
                if role in staff_by_role and percentage > 0:
                    role_total = (
                        distribution.tip_amount * Decimal(str(percentage)) / 100
                    )
    ...
    async def get_staff_tips_summary(
        ...
        return {
            ...
            "daily_breakdown": {
                str(date): float(amount) for date, amount in daily_totals.items()
            },
        }
