...
class SplitBillService:
    ...
    async def create_split(
        ...
        try:
            ...
            # Calculate tip
            tip_amount = self._calculate_tip_amount(subtotal, tip_method, tip_value)
            ...
            # Apply settings
            if settings:
                bill_split.allow_partial_payments = settings.get(
                    "allow_partial_payments", True
                )
                bill_split.require_all_acceptance = settings.get(
                    "require_all_acceptance", False
                )
                bill_split.auto_close_on_completion = settings.get(
        ...
        for participant_data in participants:
            ...
            if split_method == SplitMethod.EQUAL:
                ...
                if bill_split.subtotal > 0:
                    tip_share = bill_split.tip_amount * (
                        share_amount / bill_split.subtotal
                    )
                else:
                    tip_share = Decimal("0")
    ...
    def _validate_split_config(
        ...
        if split_method == SplitMethod.PERCENTAGE:
            ...
            if abs(total_amounts - total_amount) > Decimal("0.01"):
                raise ValueError(
                    f"Amounts must sum to total {total_amount}, got {total_amounts}"
                )
    ...
    async def get_split(
        self, db: AsyncSession, split_id: int, include_participants: bool = True
    ) -> Optional[BillSplit]:
        ...
        try:
            ...
            if split:
                SplitBillMetrics.record_participant_status_change(
                    status=status.value, split_method=split.split_method.value
                )
    ...
    async def _check_split_activation(self, db: AsyncSession, split_id: int):
        ...
        pending_count = sum(
            1 for p in participants if p.status == ParticipantStatus.PENDING
        )
    ...
    async def record_participant_payment(
        self, db: AsyncSession, participant_id: int, payment: Payment, amount: Decimal
    ) -> PaymentAllocation:
        ...
        try:
            ...
            if participant.paid_amount >= participant.total_amount:
                ...
                SplitBillMetrics.record_split_payment(
                    status="completed", split_method=split.split_method.value
                )
                ...
                if split.created_at and participant.paid_at:
                    ...
                    SplitBillMetrics.record_payment_duration(
                        split_method=split.split_method.value, duration_seconds=duration
                    )
    ...
    async def _create_tip_distribution(self, db: AsyncSession, split: BillSplit):
    ...
    async def cancel_split(
        self, db: AsyncSession, split_id: int, reason: Optional[str] = None
    ) -> BillSplit:
        """Cancel a bill split"""
    ...
    async def send_reminders(self, db: AsyncSession, split_id: int):
    ...
    async def _notify_status_change(
        self, db: AsyncSession, participant: SplitParticipant
    ):
    ...
    async def _notify_payment_received(
        self, db: AsyncSession, participant: SplitParticipant, amount: Decimal
    ):
    ...
    async def _notify_split_cancelled(self, db: AsyncSession, split: BillSplit):
        ...
        if participants_data:
            await split_bill_notification_queue.queue_split_cancelled(
                participants_data, split.organizer_name
            )
    ...
    async def _send_payment_reminder(self, participant: SplitParticipant):
        ...
        if participant.notify_via_email and participant.email:
            await split_bill_notification_queue.queue_payment_reminder(
                participant_data={"name": participant.name, "email": participant.email},
