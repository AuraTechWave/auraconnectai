...
class RefundService:
    ...
    async def create_refund_request(
        ...
        try:
            ...
            request = RefundRequest(
                ...
                customer_name=(
                    customer_info.get("name", order.customer_name)
                    if customer_info
                    else order.customer_name
                ),
                customer_email=(
                    customer_info.get("email", order.customer_email)
                    if customer_info
                    else order.customer_email
                ),
                customer_phone=(
                    customer_info.get("phone", order.customer_phone)
                    if customer_info
            ...
            self.audit_logger.log_action(
                ...
                details={
                    ...
                    "priority": priority,
                    "auto_approved": request.approval_status
            ...
            if request.approval_status == RefundApprovalStatus.AUTO_APPROVED:
                try:
                    await self.process_refund_request(
                        db, request.id, system_initiated=True
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to auto-process refund request {request.id}: {e}"
                    )
    ...
    async def _validate_refund_amount(
        self, db: AsyncSession, payment: Payment, requested_amount: Decimal
    ):
    ...
    async def _check_auto_approval(
        self, request: RefundRequest, policy: RefundPolicy
    ) -> bool:
    ...
    async def reject_refund_request(
        self, db: AsyncSession, request_id: int, rejector_id: int, reason: str
    ) -> RefundRequest:
    ...
    async def process_refund_request(
        self, db: AsyncSession, request_id: int, system_initiated: bool = False
    ...
    async def get_refund_statistics(
        ...
        ).group_by(RefundRequest.category, RefundRequest.approval_status)
        ...
        for row in rows:
            ...
            if category not in stats["by_category"]:
                stats["by_category"][category] = {"count": 0, "amount": Decimal("0")}

            if status not in stats["by_status"]:
                stats["by_status"][status] = {"count": 0, "amount": Decimal("0")}
