...
class SplitBillNotificationQueue:
    ...
    async def queue_participant_invitation(
        ...
        try:
            ...
            logger.info(
                f"Queued invitation for {participant_data.get('email')} - Job ID: {job.job_id}"
            )
            return job.job_id
    ...
    async def queue_bulk_invitations(
        self, participants: List[Dict[str, Any]], split_data: Dict[str, Any]
    ) -> List[str]:
    ...
    async def queue_payment_reminder(
        self, participant_data: Dict[str, Any], amount_due: float, access_link: str
    ) -> str:
    ...
    async def queue_split_cancelled(
        self, participants: List[Dict[str, Any]], organizer_name: str
    ) -> List[str]:
