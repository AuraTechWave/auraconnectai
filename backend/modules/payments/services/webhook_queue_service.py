...
class WebhookQueueService:
    ...
    def __init__(self):
        self.redis_settings = (
            RedisSettings.from_dsn(settings.redis_url) if settings.redis_url else None
        )
        self._pool = None
...
async def process_payment_webhook(
    ctx: dict, job_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        body = (
            job_data["body"].encode("utf-8")
            if isinstance(job_data["body"], str)
            else job_data["body"]
        )
...
async def retry_failed_webhooks(ctx: dict) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "error": str(e), "retried_count": retried_count}


# Worker configuration
WORKER_FUNCTIONS = [process_payment_webhook, retry_failed_webhooks]
