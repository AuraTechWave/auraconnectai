...
class PayPalGateway(PaymentGatewayInterface):
        ...
        try:
            ...
            order_data = {
                ...
                "purchase_units": [{
                    ...
                    } if self.config.get('payee_email') else None
                }],
                "payment_source": {
                    "paypal": {
                        "experience_context": {
                            ...
                            "landing_page": "LOGIN",
                            "user_action": "PAY_NOW",
                            "return_url": request.return_url or f"{self.config.get('base_url', '')}/payment/success",
            ...
            if request.customer_email:
                order_data["payment_source"]["paypal"]["email_address"] = request.customer_email
    ...
    async def capture_payment(self, payment_id: str, amount: Optional[Decimal] = None) -> PaymentResponse:
        ...
        try:
            ...
            return PaymentResponse(
                ...
                currency=result.get('purchase_units', [{}])[0].get('amount', {}).get('currency_code', 'USD'),
    ...
    async def get_payment(self, payment_id: str) -> PaymentResponse:
        ...
        try:
            result = await self._make_request("GET", f"/v2/checkout/orders/{payment_id}")
    ...
    async def create_refund(self, request: RefundRequest) -> RefundResponse:
        ...
        try:
            ...
            order = await self._make_request("GET", f"/v2/checkout/orders/{request.payment_id}")
    ...
    async def get_refund(self, refund_id: str) -> RefundResponse:
        """Get refund details"""
        try:
            result = await self._make_request("GET", f"/v2/payments/refunds/{refund_id}")
