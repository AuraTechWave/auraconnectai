...
class SquareGateway(PaymentGatewayInterface):
    ...
    def __init__(self, config: Dict[str, Any], test_mode: bool = True):
        ...
        self.client = Client(
            access_token=config.get("access_token"), environment=environment
        )
    ...
    async def create_payment(self, request: PaymentRequest) -> PaymentResponse:
        ...
        try:
            ...
            # Create money object
            amount_money = Money(amount=amount_cents, currency=request.currency)
            ...
            if result.is_error():
                ...
                error_message = (
                    ", ".join([e.detail for e in errors]) if errors else "Unknown error"
                )
        ...
        try:
            ...
            if processing_fee:
                ...
                fee_amount = self.parse_amount(
                    fee_money.get("amount", 0), fee_money.get("currency", "USD")
                )

                amount_money = payment.get("amount_money", {})
                total = self.parse_amount(
                    amount_money.get("amount", 0), amount_money.get("currency", "USD")
                )
    ...
    async def create_refund(self, request: RefundRequest) -> RefundResponse:
        ...
        try:
            ...
            if request.amount:
                amount_cents = self.format_amount(
                    request.amount, payment_amount.get("currency", "USD")
                )
            else:
                # Full refund
            ...
            refund_request = CreateRefundRequest(
                ...
                amount_money=Money(
                    amount=amount_cents, currency=payment_amount.get("currency", "USD")
                ),
    ...
    async def create_customer(self, request: CustomerRequest) -> CustomerResponse:
        ...
        try:
            ...
            return CustomerResponse(
                success=True, gateway_customer_id=customer.get("id")
            )
        ...
        try:
            ...
            update_request = {
                "given_name": (
                    ...
                    if request.name
                    else existing.get("given_name")
                ),
                "family_name": (
                    " ".join(request.name.split()[1:])
                    if request.name and len(request.name.split()) > 1
                    else existing.get("family_name")
                ),
            ...
            result = self.client.customers.update_customer(
                gateway_customer_id, update_request
            )
            ...
            return CustomerResponse(
                success=True, gateway_customer_id=gateway_customer_id
            )
    ...
    async def list_payment_methods(
        ...
        try:
            result = self.client.cards.list_cards(
                customer_id=customer_id, include_disabled=False
            )
    ...
    async def _get_or_create_customer(
        ...
        try:
            ...
            result = self.client.customers.search_customers(
                {"filter": {"reference_id": {"exact": str(request.customer_id)}}}
            )
            ...
            if response.success:
                ...
                get_result = self.client.customers.retrieve_customer(
                    response.gateway_customer_id
                )
