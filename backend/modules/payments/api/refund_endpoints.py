...
async def get_refund_request(
    ...
    # Check permissions
    if not current_user.is_staff:
        if hasattr(current_user, 'customer_id') and request.customer_id != current_user.customer_id:
...
async def upload_refund_evidence(
    ...
    for file in files:
        evidence_urls.append(f"https://storage.example.com/refunds/{request_id}/{file.filename}")
