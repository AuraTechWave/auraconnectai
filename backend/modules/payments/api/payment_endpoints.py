...
async def save_payment_method(
    ...
    if method_data.customer_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Cannot save payment methods for other customers"
        )
...
async def list_payment_methods(
    ...
    payment_methods = await payment_service.list_customer_payment_methods(
        db=db, customer_id=current_user.id, gateway=gateway, active_only=True
    )
...
async def delete_payment_method(
    ...
    if payment_method.customer_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Cannot delete payment methods for other customers"
        )
...
async def get_available_gateways(current_user: User = Depends(get_current_user)):
    """
    ...
    return [
        PaymentGatewayConfig(
            gateway=gateway, config=payment_service.get_public_gateway_config(gateway)
        )
...
async def handle_webhook(
    gateway: PaymentGateway, request: Request, db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        return PaymentWebhookResponse(
            status="accepted", message=f"Webhook queued for processing: {job_id}"
        )
        ...
        try:
            result = await webhook_service.process_webhook(
                db=db, gateway=gateway, headers=headers, body=body
            )
