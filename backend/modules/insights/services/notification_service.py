...
class InsightNotificationService:
    ...
    async def process_insight_notifications(
        ...
        for rule in rules:
            # Check rate limits
            if not self._check_rate_limits(rule.id, rule.max_per_hour, rule.max_per_day):
    ...
    async def _get_applicable_rules(
        ...
        for rule in all_rules:
            ...
            if rule.min_severity:
                ...
                if severity_order.get(insight.severity, 0) < severity_order.get(rule.min_severity, 0):
                    continue
            
            # Check impact score
            if rule.min_impact_score and (not insight.impact_score or insight.impact_score < rule.min_impact_score):
                continue
            
            # Check estimated value
            if rule.min_estimated_value and (not insight.estimated_value or insight.estimated_value < rule.min_estimated_value):
                continue
    ...
    async def _send_immediate_notifications(
        ...
        for i, result in enumerate(results):
            if not isinstance(result, Exception):
                self.sent_notifications.setdefault(rule.id, []).append(datetime.utcnow())
    ...
    async def _send_email_notification(
        ...
        try:
            ...
            logger.info(f"Email sent for insight {insight.id} to {len(recipients)} recipients")
        ...
        async with aiohttp.ClientSession() as session:
            for channel in target_channels:
                ...
                try:
                    ...
                    logger.info(f"Slack notification sent for insight {insight.id} to {channel}")
        ...
        async with aiohttp.ClientSession() as session:
            for webhook_url in webhooks:
                try:
                    ...
                    logger.info(f"Webhook sent for insight {insight.id} to {webhook_url}")
    ...
    async def _send_sms_notification(
        ...
        logger.info(f"SMS notification would be sent for insight {insight.id} to {len(phone_numbers)} numbers")
