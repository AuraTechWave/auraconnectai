...
class InsightThreadService:
    ...
    def create_or_update_thread(
        ...
        if thread:
            ...
            if insight.estimated_value:
                thread.total_value = float(thread.total_value or 0) + float(insight.estimated_value)
            ...
            thread = InsightThread(
                ...
                total_insights=1,
                total_value=float(insight.estimated_value) if insight.estimated_value else 0,
                is_active=True
            )
            self.db.add(thread)
    ...
    def get_thread(
        ...
        """Get thread by ID"""
        return self.db.query(InsightThread).filter(
            InsightThread.thread_id == thread_id,
    ...
    def _detect_recurrence_pattern(self, thread_id: str) -> bool:
        ...
        # Get recent insights
        recent_insights = self.db.query(Insight).filter(
            Insight.thread_id == thread_id
        ...
        for i in range(1, len(recent_insights)):
            interval = (recent_insights[i-1].created_at - recent_insights[i].created_at).days
    ...
    def _analyze_thread_patterns(self, insights: List[Insight]) -> Dict[str, Any]:
        ...
        value_trend = "increasing" if len(values) >= 2 and values[-1] > values[0] else "stable"
