...
class InsightsService:
    ...
    def create_insight(
        self, insight_data: InsightCreate, generated_by: str = "system"
    ) -> Insight:
        ...
        logger.info(
            f"Created insight {insight.id} for restaurant {insight.restaurant_id}"
        )
        return insight
    ...
    def get_insight(self, insight_id: int) -> Optional[Insight]:
        ...
        return (
            ...
            .options(joinedload(Insight.ratings), joinedload(Insight.actions))
            .first()
        )
    ...
    def update_insight(
        self, insight_id: int, update_data: InsightUpdate, user_id: int
    ) -> Optional[Insight]:
        """Update an insight"""
    ...
    def acknowledge_insight(self, insight_id: int, user_id: int) -> Optional[Insight]:
        ...
        return self.update_insight(
            insight_id, InsightUpdate(status=InsightStatus.ACKNOWLEDGED), user_id
        )

    def dismiss_insight(self, insight_id: int, user_id: int) -> Optional[Insight]:
        """Dismiss an insight"""
        return self.update_insight(
            insight_id, InsightUpdate(status=InsightStatus.DISMISSED), user_id
        )

    def log_action(
        self, insight_id: int, user_id: int, action_data: InsightActionCreate
    ) -> InsightAction:
    ...
    def get_insights_summary(
        self, restaurant_id: int, domain: Optional[InsightDomain] = None, days: int = 30
    ) -> InsightSummary:
        ...
        query = self.db.query(Insight).filter(
            Insight.restaurant_id == restaurant_id, Insight.created_at >= start_date
        )
        ...
        total_value = (
            self.db.query(func.sum(Insight.estimated_value))
            .filter(
        ...
        avg_impact = (
            self.db.query(func.avg(Insight.impact_score))
            .filter(
                Insight.restaurant_id == restaurant_id, Insight.created_at >= start_date
            )
            .scalar()
        ...
        for i in range(days):
            ...
            count = (
                self.db.query(func.count(Insight.id))
                .filter(
                    Insight.restaurant_id == restaurant_id,
            ...
            trend_data.append({"date": day.date().isoformat(), "count": count})
    ...
    def get_trend_analysis(
        ...
        for insight in insights:
            ...
            # Domain distribution
            domain_distribution[insight.domain.value] = (
                domain_distribution.get(insight.domain.value, 0) + 1
            )
    ...
    def get_impact_analysis(
        ...
        query = self.db.query(Insight).filter(Insight.restaurant_id == restaurant_id)
        ...
        total_value = sum(
            float(i.estimated_value) for i in insights if i.estimated_value
        )
        implemented_value = sum(
            float(i.estimated_value)
            for i in insights
        ...
        for insight in insights:
            if insight.estimated_value:
                ...
                if insight.status == InsightStatus.RESOLVED:
                    domain_impact[domain]["implemented_value"] += float(
        ...
        return {
            ...
            "implementation_rate": (
                round(implemented_value / total_value * 100, 2)
                if total_value > 0
                else 0
            ),
    ...
    async def export_insights(
        self, filters: InsightFilters, format: str, user_id: int
    ) -> str:
            ...
            export_data.append(
                {
                    ...
                    "acknowledged_at": (
                        insight.acknowledged_at.isoformat()
                        if insight.acknowledged_at
                        else ""
                    ),
                    "resolved_at": (
                        insight.resolved_at.isoformat() if insight.resolved_at else ""
                    ),
                }
            )
