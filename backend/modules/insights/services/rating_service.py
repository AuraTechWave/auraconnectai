...
from ..models.insight_models import Insight, InsightRating, InsightAction, InsightStatus
...
class InsightRatingService:
    ...
    def create_rating(
        self, insight_id: int, user_id: int, rating_data: InsightRatingCreate
    ) -> InsightRating:
        """Create or update a rating for an insight"""

        # Check if user already rated this insight
        existing_rating = (
            self.db.query(InsightRating)
            .filter(
                InsightRating.insight_id == insight_id, InsightRating.user_id == user_id
            )
            .first()
        )
    ...
    def get_insight_ratings(self, insight_id: int) -> Dict[str, Any]:
        ...
        ratings = (
            self.db.query(InsightRating)
            .filter(InsightRating.insight_id == insight_id)
            .options(joinedload(InsightRating.user))
            .all()
        )
    ...
    def _update_insight_status_from_ratings(self, insight_id: int):
        ...
        rating_counts = (
            self.db.query(
                InsightRating.rating, func.count(InsightRating.id).label("count")
            )
            .filter(InsightRating.insight_id == insight_id)
        ...
        if total_ratings >= 3:  # Minimum ratings threshold
            ...
            if irrelevant_count / total_ratings > 0.6:
                insight = (
                    self.db.query(Insight).filter(Insight.id == insight_id).first()
                )
