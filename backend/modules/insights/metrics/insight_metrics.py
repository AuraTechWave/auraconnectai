...
active_insights_count = Gauge(
    "active_insights_count", "Current number of active insights", ["severity", "domain"]
)
...
insight_thread_length = Summary(
    "insight_thread_length", "Number of insights in a thread", ["category"]
)
...
class InsightMetricsCollector:
    ...
    def record_insight_generated(
        ...
        if timer_id in self.generation_timers:
            ...
            insight_generation_duration.labels(
                generator=generator, domain=domain.value
            ).observe(duration)

        # Record impact score
        if impact_score is not None:
            insight_impact_score_distribution.labels(
                type=insight_type.value, domain=domain.value
    ...
    def record_insight_acknowledged(
        ...
        insights_acknowledged_total.labels(
            type=insight_type.value, severity=severity.value, domain=domain.value
        ).inc()

        time_to_acknowledge.labels(
            severity=severity.value, domain=domain.value
    ...
    def record_insight_resolved(
        ...
        time_to_resolve.labels(severity=severity.value, domain=domain.value).observe(
    ...
    def record_insight_rating(
        self, rating: str, insight_type: InsightType, domain: InsightDomain
    ):
        """Record insight rating"""

        insight_ratings_total.labels(
            rating=rating, type=insight_type.value, domain=domain.value
        ).inc()

    def record_notification_sent(self, channel: str, severity: InsightSeverity):
        """Record successful notification"""

        insight_notifications_sent_total.labels(
            channel=channel, severity=severity.value
        ).inc()

    def record_notification_failed(
        self, channel: str, severity: InsightSeverity, error_type: str
    ):
        """Record failed notification"""

        insight_notifications_failed_total.labels(
            channel=channel, severity=severity.value, error_type=error_type
        ).inc()

    def update_active_insights(self, counts_by_severity_domain: Dict[tuple, int]):
        ...
        for (severity, domain), count in counts_by_severity_domain.items():
            active_insights_count.labels(severity=severity, domain=domain).set(count)

    def update_oldest_unacknowledged(self, oldest_by_severity: Dict[str, float]):
        """Update oldest unacknowledged insight age"""

        for severity, age_hours in oldest_by_severity.items():
            unacknowledged_insights_age.labels(severity=severity).set(age_hours)

    def update_acceptance_rates(
        self, rates_by_type_domain: Dict[tuple, Dict[str, float]]
    ):
        ...
        for (insight_type, domain), rates in rates_by_type_domain.items():
            for time_window, rate in rates.items():
                insight_acceptance_rate.labels(
                    type=insight_type, domain=domain, time_window=time_window
                ).set(rate)

    def record_thread_length(self, category: str, length: int):
        """Record insight thread length"""

        insight_thread_length.labels(category=category).observe(length)
