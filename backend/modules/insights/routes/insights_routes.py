...
from ..schemas.insight_schemas import (
    ...
    InsightFilters, InsightBatchAction, InsightSummary,
    InsightRatingCreate, InsightRatingResponse,
    InsightActionCreate, InsightActionResponse,
    NotificationRuleCreate, NotificationRuleUpdate, NotificationRuleResponse,
    InsightThreadResponse, ThreadFilters
)
from ..models.insight_models import InsightType, InsightSeverity, InsightStatus, InsightDomain
...
async def create_insight(
    ...
    insight = service.create_insight(insight_data, generated_by=f"user_{current_user.id}")
...
async def list_insights(
    # Filters
    domain: Optional[InsightDomain] = Query(None, description="Filter by business domain"),
    ...
    # Sorting
    sort_by: str = Query("created_at", regex="^(created_at|severity|impact_score|estimated_value)$"),
...
async def update_insight(
    ...
    if update_data.status:
        valid_transitions = {
            InsightStatus.ACTIVE: [InsightStatus.ACKNOWLEDGED, InsightStatus.RESOLVED, InsightStatus.DISMISSED],
            InsightStatus.ACKNOWLEDGED: [InsightStatus.RESOLVED, InsightStatus.DISMISSED],
        ...
        if update_data.status not in valid_transitions.get(insight.status, []):
            raise APIValidationError(
                ...
                {"current_status": insight.status, "requested_status": update_data.status}
            )
...
async def update_notification_rule(
    ...
    notification_service = InsightNotificationService(db)
    rule = notification_service.update_rule(rule_id, update_data, current_user.restaurant_id)
