...
class SalesFilterRequest(BaseModel):
    ...
    # Entity filters
    staff_ids: Optional[List[int]] = Field(None, description="Filter by specific staff members")
    product_ids: Optional[List[int]] = Field(None, description="Filter by specific products/menu items")
    category_ids: Optional[List[int]] = Field(None, description="Filter by product categories")
    customer_ids: Optional[List[int]] = Field(None, description="Filter by specific customers")
    ...
    only_completed_orders: bool = Field(True, description="Only include completed orders")
    ...
    @validator('staff_ids', 'product_ids', 'category_ids', 'customer_ids')
    def validate_id_lists(cls, v):
        if v is not None:
            if not isinstance(v, list) or not all(isinstance(x, int) and x > 0 for x in v):
...
class SalesSummaryResponse(BaseModel):
    ...
    customer_retention_rate: Optional[Decimal] = Field(description="Customer retention percentage")
    
    # Growth metrics
    revenue_growth: Optional[Decimal] = Field(description="Revenue growth vs previous period")
    order_growth: Optional[Decimal] = Field(description="Order count growth vs previous period")
...
class DashboardMetricsResponse(BaseModel):
    ...
    previous_period: Dict[str, Any] = Field(description="Previous period for comparison")
...
class ReportTemplateRequest(BaseModel):
    ...
    description: Optional[str] = Field(max_length=1000, description="Template description")
    report_type: ReportType = Field(description="Type of report")
    
    # Configuration
    filters_config: Dict[str, Any] = Field(description="Default filter settings")
    columns_config: Optional[Dict[str, Any]] = Field(None, description="Column configuration")
    sorting_config: Optional[Dict[str, Any]] = Field(None, description="Sorting preferences")
    visualization_config: Optional[Dict[str, Any]] = Field(None, description="Chart settings")
...
class PaginatedSalesResponse(BaseModel):
    ...
    items: List[Union[SalesDetailResponse, StaffPerformanceResponse, ProductPerformanceResponse]]
    total: int
    page: int
    per_page: int
    total_pages: int
...
class SalesComparisonRequest(BaseModel):
    ...
    entity_ids: Optional[List[int]] = Field(None, description="Specific entities to compare")
    
    # Metrics to include
    metrics: List[str] = Field(default=["revenue", "orders", "customers"], description="Metrics to compare")
    
    @validator('compare_by')
    def validate_compare_by(cls, v):
        if v not in ['period', 'staff', 'product', 'category']:
            raise ValueError('compare_by must be one of: period, staff, product, category')
        return v
...
class SalesComparisonResponse(BaseModel):
    ...
    # Comparison results
    differences: Dict[str, Dict[str, Any]]  # Metric -> {absolute_change, percentage_change, etc.}
