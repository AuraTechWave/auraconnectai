...
class MetricSubscriptionRequest(BaseModel):
    ...
    update_interval: Optional[int] = Field(30, ge=1, le=300, description="Update interval in seconds")
...
class DashboardSubscriptionRequest(BaseModel):
    ...
    update_interval: Optional[int] = Field(30, ge=10, le=300, description="Update interval in seconds")
...
class CurrentDataRequest(BaseModel):
    ...
    metric_name: Optional[str] = Field(None, description="Specific metric name if requesting metric data")
    include_history: bool = Field(False, description="Include historical data")
    history_hours: Optional[int] = Field(24, ge=1, le=168, description="Hours of history to include")
...
class RealtimeDashboardResponse(BaseModel):
    ...
    critical_metrics: List[str] = Field(default_factory=list, description="Metrics requiring attention")
...
class WidgetConfiguration(BaseModel):
    ...
    widget_id: str
    widget_type: str = Field(..., description="Type of widget (chart, metric, table, etc.)")
    title: str
    position: Dict[str, int] = Field(..., description="Widget position (x, y, width, height)")
    refresh_interval: int = Field(30, ge=5, le=300, description="Refresh interval in seconds")
...
class WidgetDataResponse(BaseModel):
    ...
    data: Dict[str, Any] = Field(default_factory=dict)
    timestamp: datetime
    cache_status: str = Field("fresh", description="Cache status (fresh, cached, stale)")
