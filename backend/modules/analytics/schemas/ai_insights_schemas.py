...
class PeakTimeInsight(BaseModel):
    ...
    secondary_peak: Optional[TimePattern] = Field(None, description="Second busiest period")
    quiet_periods: List[TimePattern] = Field(default_factory=list, description="Low activity periods")
    weekly_pattern: Dict[str, List[TimePattern]] = Field(..., description="Peak times by day of week")
...
class ProductTrend(BaseModel):
    ...
    predicted_demand: Optional[int] = Field(None, description="Predicted units for next period")
    seasonality_factor: Optional[float] = Field(None, description="Seasonal influence (0-1)")


class ProductInsight(BaseModel):
    """AI insights for product performance"""
    insight_type: InsightType = Field(InsightType.PRODUCT_TREND)
    top_rising: List[ProductTrend] = Field(..., description="Products gaining popularity")
    top_falling: List[ProductTrend] = Field(..., description="Products losing popularity")
    stable_performers: List[ProductTrend] = Field(..., description="Consistently popular products")
    new_trending: List[ProductTrend] = Field(..., description="Newly introduced trending items")
...
class CustomerPattern(BaseModel):
    ...
    customer_segment: Optional[str] = Field(None, description="Customer segment affected")
...
class CustomerInsight(BaseModel):
    ...
    average_order_frequency: float = Field(..., description="Orders per customer per month")
    churn_risk_segments: List[Dict[str, Any]] = Field(..., description="Customer segments at risk")
...
class SeasonalityPattern(BaseModel):
    ...
    historical_accuracy: float = Field(..., ge=0, le=1, description="Historical prediction accuracy")
...
class InsightRequest(BaseModel):
    ...
    min_confidence: ConfidenceLevel = Field(ConfidenceLevel.MEDIUM, description="Minimum confidence level")
...
class InsightResponse(BaseModel):
    ...
    processing_time: float = Field(..., description="Time taken to generate insights (seconds)")
