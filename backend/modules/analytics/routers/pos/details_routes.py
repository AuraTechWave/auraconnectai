...
@router.post(
    "/provider/{provider_id}/details", response_model=POSProviderDetailsResponse
)
...
@router.post(
    "/terminal/{terminal_id}/details", response_model=POSTerminalDetailsResponse
)
async def get_terminal_analytics_details(
    ...
    try:
        ...
        terminal_details = await service.get_terminal_details(
            terminal_id=terminal_id, start_date=start_date, end_date=end_date
        )
