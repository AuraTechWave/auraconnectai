...
async def get_top_performers(
    limit: int = Query(
        5, ge=1, le=20, description="Number of top performers to return"
    ),
    db: Session = Depends(get_db),
    current_user: dict = Depends(
...
async def invalidate_cache(
    cache_pattern: Optional[str] = Query(
        None, description="Cache pattern to invalidate"
    ),
    db: Session = Depends(get_db),
    current_user: dict = Depends(
...
async def health_check():
    ...
    try:
        ...
        return {
            "status": "healthy",
            "services": {"realtime_metrics": "running", "websocket_manager": "running"},
            "metrics": {
...
async def handle_order_completed_event(
    order_data: Dict[str, Any], current_user: User = Depends(get_current_user)
):
    """
...
async def handle_staff_action_event(
    action_data: Dict[str, Any], current_user: User = Depends(get_current_user)
):
    """
