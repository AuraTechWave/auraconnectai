...
async def get_dashboard_metrics(
    current_date: Optional[date] = Query(
        None, description="Date for dashboard metrics"
    ),
...
async def generate_sales_summary(
    ...
    db: Session = Depends(get_db),
    current_user: User = Depends(
        require_analytics_permission(AnalyticsPermission.VIEW_SALES_REPORTS)
    ),
):
    """
...
@router.post(
    "/reports/staff-performance", response_model=List[StaffPerformanceResponse]
)
...
@router.post(
    "/reports/product-performance", response_model=List[ProductPerformanceResponse]
)
async def generate_product_performance_report(
    ...
    db: Session = Depends(get_db),
    current_user: User = Depends(
        require_analytics_permission(AnalyticsPermission.VIEW_PRODUCT_REPORTS)
    ),
):
    """
...
async def get_quick_stats(
    ...
    try:
        ...
        return {
            ...
            "growth": {
                "revenue_growth": (
                    float(summary.revenue_growth) if summary.revenue_growth else None
                ),
                "order_growth": (
                    float(summary.order_growth) if summary.order_growth else None
                ),
            },
        }
...
async def get_top_performers(
    ...
    entity_type: str = Query(
        "staff", pattern="^(staff|product|category)$", description="Entity type"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(
        require_analytics_permission(AnalyticsPermission.VIEW_SALES_REPORTS)
    ...
    try:
        ...
        filters = SalesFilterRequest(date_from=start_date, date_to=end_date)
            ...
            return [
                {
                    ...
                    "name": p.staff_name,
                    "metric_value": (
                        float(p.total_revenue_generated)
                        if metric == "revenue"
                        else p.total_orders_handled
                    ),
                    "rank": (
                        p.revenue_rank if metric == "revenue" else p.order_count_rank
                    ),
                }
                for p in performers
            ]
            ...
            return [
                {
                    ...
                    "rank": (
                        p.revenue_rank if metric == "revenue" else p.popularity_rank
                    ),
                }
                for p in performers
            ]
...
async def get_trends(
    metric: str = Query(
        "revenue", pattern="^(revenue|orders|customers)$", description="Trend metric"
    ),
    period_days: int = Query(30, ge=7, le=365, description="Period in days"),
    granularity: str = Query(
        "daily", pattern="^(hourly|daily|weekly)$", description="Data granularity"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(
        require_analytics_permission(AnalyticsPermission.VIEW_SALES_REPORTS)
...
async def export_to_csv(
    ...
    try:
        ...
        return await export_service.export_sales_report(
            request=request, format_type="csv", executed_by=current_user["id"]
        )
...
async def export_to_pdf(
    ...
    try:
        ...
        return await export_service.export_sales_report(
            request=request, format_type="pdf", executed_by=current_user["id"]
        )
...
async def export_to_excel(
    ...
    try:
        ...
        return await export_service.export_sales_report(
            request=request, format_type="xlsx", executed_by=current_user["id"]
        )
...
async def get_alert_rules(
    ...
    try:
        ...
        rules = alerting_service.get_alert_rules(
            include_inactive=include_inactive, created_by=current_user["id"]
        )
...
async def submit_large_report(
    filters: SalesFilterRequest,
    limit: int = Query(
        50000, ge=1000, le=100000, description="Maximum records to include"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(
        require_analytics_permission(AnalyticsPermission.VIEW_SALES_REPORTS)
    ...
    try:
        ...
        task_id = await submit_large_report_task(
            filters=filter_dict, user_id=current_user["id"], limit=limit
        )
...
async def get_task_status(task_id: str, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if task.created_by != current_user[
            "id"
        ] and not PermissionsService.has_permission(
...
async def get_user_tasks(
    ...
    try:
        tasks = async_processor.get_user_tasks(
            user_id=current_user["id"], status_filter=status_filter
        )
...
async def get_permissions_summary(current_user: User = Depends(get_current_user)):
    """
...
async def invalidate_widget_cache(
    widget_id: Optional[str] = Query(
        None, description="Specific widget ID to invalidate"
    ),
    current_user: User = Depends(
        require_analytics_permission(AnalyticsPermission.ADMIN_ANALYTICS)
    ),
...
async def get_menu_performance(
    ...
    # Entity filters
    menu_item_ids: Optional[List[int]] = Query(
        None, description="Filter by specific menu items"
    ),
    category_ids: Optional[List[int]] = Query(
        None, description="Filter by menu categories"
    ),
    # Pagination
    ...
    try:
        ...
        return service.generate_menu_performance_report(
