...
class POSAnalyticsSnapshot(Base, TimestampMixin):
    ...
    # POS dimensions
    provider_id = Column(
        Integer, ForeignKey("external_pos_providers.id"), nullable=True, index=True
    )
...
class POSProviderPerformance(Base, TimestampMixin):
    ...
    # Top issues
    top_error_types = Column(
        JSONB, nullable=False, default=list
    )  # [{type, count, percentage}]
    problematic_terminals = Column(
        JSONB, nullable=False, default=list
    )  # [{terminal_id, error_count}]
    ...
    __table_args__ = (
        UniqueConstraint(
            "provider_id", "measurement_date", name="uq_provider_performance_date"
        ),
...
class POSTerminalHealth(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "terminal_id", "provider_id", name="uq_terminal_health_terminal_provider"
        ),
...
class POSAnalyticsAlert(Base, TimestampMixin):
    ...
    )  # terminal_offline, high_error_rate, etc.
