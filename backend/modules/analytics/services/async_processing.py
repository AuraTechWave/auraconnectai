...
class AsyncTaskProcessor:
    ...
    async def submit_task(
        ...
        task = AsyncTask(
            ...
            metadata={"task_data": task_data, **(metadata or {})},
    ...
    def get_user_tasks(
        self, user_id: int, status_filter: Optional[TaskStatus] = None
    ) -> List[AsyncTask]:
    ...
    async def _execute_task(self, task_id: str):
        ...
        try:
            ...
            execution_task = asyncio.create_task(self._run_task_handler(handler, task))
    ...
    async def _run_task_handler(
        self, handler: Callable, task: AsyncTask
    ) -> Dict[str, Any]:
        ...
        if asyncio.iscoroutinefunction(handler):
            ...
            return await loop.run_in_executor(
                self.executor, lambda: handler(task_data, task)
            )
    ...
    async def _monitor_tasks(self):
        ...
        while self.running:
            try:
                ...
                logger.debug(
                    f"Queue status: {status['running_tasks']}/{status['max_workers']} workers busy"
                )
    ...
    async def _handle_large_report(
        self, task_data: Dict[str, Any], task: AsyncTask
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            report = service.generate_detailed_sales_report(
                filters=filters, page=1, per_page=task_data.get("limit", 10000)
            )
    ...
    async def _handle_calculate_trends(
        self, task_data: Dict[str, Any], task: AsyncTask
    ) -> Dict[str, Any]:
...
async def submit_export_task(
    export_format: str, data_query: Dict[str, Any], user_id: int
) -> str:
...
async def submit_trend_calculation_task(
    metrics: List[str], start_date: str, end_date: str, user_id: int
) -> str:
