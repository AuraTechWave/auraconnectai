...
from ..schemas.ai_assistant_schemas import (
    AnalyticsQuery, QueryIntent, QueryContext, ChatMessage,
    MessageRole, ChartType
)
...
class AIQueryProcessor:
    ...
    def _initialize_intent_patterns(self) -> Dict[QueryIntent, List[re.Pattern]]:
        ...
        return {
            QueryIntent.SALES_REPORT: [
                re.compile(r'\b(sales|revenue|income|earnings)\s*(report|summary|overview)?', re.I),
            ...
            QueryIntent.REVENUE_ANALYSIS: [
                re.compile(r'\b(revenue|income|earnings)\s*(analysis|trend|growth)', re.I),
                re.compile(r'analyze\s*(the)?\s*revenue', re.I),
                re.compile(r'revenue\s*(by|per|for)', re.I),
            ],
            QueryIntent.STAFF_PERFORMANCE: [
                re.compile(r'\b(staff|employee|team|worker)\s*(performance|productivity|efficiency)', re.I),
                re.compile(r'(best|top|worst|bottom)\s*(performing)?\s*(staff|employees)', re.I),
                re.compile(r'who\s*(sold|made|performed)', re.I),
            ],
            QueryIntent.PRODUCT_ANALYSIS: [
                re.compile(r'\b(product|item|menu)\s*(performance|analysis|popularity)', re.I),
    ...
    def _initialize_entity_extractors(self) -> Dict[str, Any]:
        ...
        return {
            'staff': re.compile(r'(?:staff|employee|worker)\s*(?:id\s*)?#?(\d+|\w+)', re.I),
            'product': re.compile(r'(?:product|item|menu)\s*(?:id\s*)?#?(\d+|[\w\s]+)', re.I),
            'category': re.compile(r'(?:category|type)\s*(?:of\s*)?([a-zA-Z\s]+)', re.I),
    ...
    def _initialize_metric_keywords(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'average_order_value': ['average order', 'aov', 'average sale', 'ticket size'],
    ...
    def _initialize_time_patterns(self) -> List[Tuple[re.Pattern, callable]]:
        ...
        return [
            ...
            (re.compile(r'last\s*(\d+)\s*days?', re.I),
             lambda m: (date.today() - timedelta(days=int(m.group(1))), date.today())),
            (re.compile(r'this\s*week', re.I),
             lambda: (date.today() - timedelta(days=date.today().weekday()), date.today())),
            (re.compile(r'last\s*week', re.I),
             lambda: (date.today() - timedelta(days=date.today().weekday() + 7),
            ...
            (re.compile(r'last\s*month', re.I),
             lambda: self._get_last_month_range()),
    ...
    def process_query(self, text: str, context: Optional[QueryContext] = None) -> AnalyticsQuery:
        """
        ...
        try:
            ...
            logger.info(f"Processed query: {query.intent} with confidence {query.confidence_score}")
            return query
    ...
    def _detect_intent(self, text: str, context: Optional[QueryContext] = None) -> Tuple[QueryIntent, float]:
        ...
        if context and context.conversation_history:
            ...
            last_intents = [msg.metadata.get('intent') for msg in context.conversation_history[-3:]
    ...
    def _build_filters(self, entities: Dict[str, Any], context: Optional[QueryContext] = None) -> Dict[str, Any]:
        """Build filters from extracted entities and context"""
        filters = {}
        
        # Add entity-based filters
        if 'staff' in entities:
            filters['staff_ids'] = [entities['staff']] if isinstance(entities['staff'], (str, int)) else entities['staff']
        
        if 'product' in entities:
            filters['product_ids'] = [entities['product']] if isinstance(entities['product'], (str, int)) else entities['product']
        
        if 'category' in entities:
            filters['category_names'] = [entities['category']] if isinstance(entities['category'], str) else entities['category']
    ...
    def _determine_grouping(self, text: str, intent: QueryIntent) -> Optional[List[str]]:
    ...
    def enhance_with_context(self, query: AnalyticsQuery, context: QueryContext) -> AnalyticsQuery:
        """Enhance query with conversation context"""
        # If query references previous results
        if any(word in query.original_text.lower() for word in ['that', 'those', 'same', 'previous']):
    ...
    def suggest_clarifications(self, query: AnalyticsQuery) -> List[str]:
        ...
        if query.intent == QueryIntent.STAFF_PERFORMANCE and not query.filters.get('staff_ids'):
            clarifications.append("Would you like to see all staff or specific team members?")
        
        if query.intent == QueryIntent.PRODUCT_ANALYSIS and not query.filters.get('product_ids'):
            clarifications.append("Are you interested in all products or specific categories?")
        
        # Check for missing metrics
        if not query.metrics:
            clarifications.append("What metrics would you like to see? (revenue, orders, growth, etc.)")
