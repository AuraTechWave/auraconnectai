...
from modules.analytics.schemas.pos_analytics_schemas import POSAlert, AlertSeverity
...
class POSAlertsService(POSAnalyticsBaseService):
    ...
    async def get_active_alerts(
        ...
        alerts = (
            query.order_by(
                POSAnalyticsAlert.severity.desc(), POSAnalyticsAlert.created_at.desc()
            )
            .limit(limit)
            .offset(offset)
            .all()
    ...
    async def acknowledge_alert(
        self, alert_id: str, acknowledged_by: int, notes: Optional[str] = None
    ) -> None:
    ...
    async def resolve_alert(
        self, alert_id: str, resolved_by: int, resolution_notes: str
    ) -> None:
        """Resolve an alert"""
    ...
    async def get_alert_history(
        ...
        query = (
            ...
            .filter(POSAnalyticsAlert.created_at >= since_date)
        )
        ...
        alerts = query.order_by(POSAnalyticsAlert.created_at.desc()).limit(limit).all()
        ...
        return [
            {
                ...
                "provider_id": alert.provider_id,
                "provider_name": (
                    alert.provider.provider_name if alert.provider else None
                ),
                "terminal_id": alert.terminal_id,
                "title": alert.title,
                "description": alert.description,
                ...
                "acknowledged": alert.acknowledged,
                "acknowledged_by": (
                    alert.acknowledger.name if alert.acknowledger else None
                ),
                "acknowledged_at": (
    ...
    async def create_alert(
        ...
        logger.info(f"Created alert {alert.alert_id}: {alert_type} - {severity.value}")
