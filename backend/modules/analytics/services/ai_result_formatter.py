...
from ..schemas.ai_assistant_schemas import (
    QueryResult, ChartData, TableData, DataPoint, ChartType,
    AnalyticsQuery, QueryIntent
)
...
class AIResultFormatter:
    ...
    def format_value(self, value: Any, format_type: str = 'number') -> str:
        ...
        try:
            ...
            rules = self.formatting_rules.get(format_type, self.formatting_rules['number'])
    ...
    def recommend_chart_type(self, data: List[DataPoint], query: AnalyticsQuery) -> ChartType:
        """
    ...
    def _is_date_label(self, label: str) -> bool:
        ...
        try:
            ...
            if ':' in label and any(label.endswith(suffix) for suffix in [':00', ':30', ':15', ':45']):
                return True
    ...
    def _summarize_table_data(self, table_data: TableData) -> Optional[str]:
        ...
        if table_data.total_rows:
            summaries.append(f"Showing {len(table_data.rows)} of {table_data.total_rows} total records")
        else:
    ...
    def create_comparison_summary(
        ...
        summary = f"{emoji} {metric_name.capitalize()} {direction} by {abs(change):.1f}%\n"
