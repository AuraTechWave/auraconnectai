...
from ..schemas.realtime_schemas import (
    OrderCompletedEvent, StaffActionEvent, SystemEvent, AlertSeverity
)
...
class RealtimeEventProcessor:
    ...
    async def process_event(self, event_type: EventType, event_data: Dict[str, Any], priority: bool = False):
    ...
    def _register_default_handlers(self):
        ...
        self.register_event_handler(EventType.ORDER_COMPLETED, self._handle_order_completed)
        ...
        # Alert handlers
        self.register_event_handler(EventType.ALERT_TRIGGERED, self._handle_alert_triggered)
    ...
    async def _process_single_event(self, event_record: Dict[str, Any]):
        ...
        try:
            ...
            for handler in handlers:
                task = asyncio.create_task(self._safe_handler_execution(handler, event_record))
            ...
            failed_handlers = sum(1 for result in results if isinstance(result, Exception))
            if failed_handlers > 0:
                logger.warning(f"Event {event_record['id']}: {failed_handlers}/{len(handlers)} handlers failed")
    ...
    async def _safe_handler_execution(self, handler: Callable, event_record: Dict[str, Any]):
    ...
    async def _process_aggregated_events(self):
        ...
        for event_type, events in self.aggregation_buffer.items():
            ...
            try:
                ...
                logger.debug(f"Processed {len(events)} aggregated events of type: {event_type}")
    ...
    async def _metrics_worker(self):
        ...
        while self.is_running:
            try:
                ...
                if (current_time - self.rate_limit_reset_time).total_seconds() >= self.rate_limit_window:
    ...
    async def _handle_order_completed(self, event_record: Dict[str, Any]):
        ...
        try:
            ...
            logger.debug(f"Processed order completed event: {order_data.get('order_id')}")
    ...
    async def _batch_update_order_metrics(self, events: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.debug(f"Batch updated order metrics for {len(staff_metrics)} staff members")
    ...
    async def _batch_update_staff_performance(self, events: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.debug(f"Batch updated staff performance for {len(staff_actions)} staff members")
