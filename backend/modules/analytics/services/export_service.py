...
from ..schemas.analytics_schemas import (
    SalesReportRequest, SalesDetailResponse, StaffPerformanceResponse,
    ProductPerformanceResponse, SalesSummaryResponse
)
...
try:
    ...
    logger.warning("OpenPyXL not available. Excel export functionality will be limited.")
...
class ExportService:
    ...
    async def export_sales_report(
        ...
        try:
            ...
            if request.report_type.value == "sales_detailed":
                ...
                summary_data = self.sales_service.generate_sales_summary(request.filters)
            ...
            if format_type.lower() == "csv":
                ...
                return await self._export_pdf(export_data, request.report_type.value, request)
            elif format_type.lower() == "xlsx":
                return await self._export_excel(export_data, request.report_type.value, request)
            else:
                raise HTTPException(
    ...
    async def _export_csv(
        self,
        data: List[Union[SalesDetailResponse, StaffPerformanceResponse, ProductPerformanceResponse, SalesSummaryResponse]],
        report_type: str
    ) -> StreamingResponse:
    ...
    async def _export_pdf(
        ...
        try:
            ...
            title = Paragraph(f"Sales Report - {report_type.replace('_', ' ').title()}", title_style)
            ...
            if not data:
                no_data = Paragraph("No data available for the selected criteria.", styles['Normal'])
    ...
    def _get_csv_headers(self, report_type: str) -> List[str]:
        ...
        if report_type == "sales_detailed":
            return [
                "Date", "Staff ID", "Staff Name", "Product ID", "Category",
                "Total Orders", "Total Revenue", "Items Sold", "Average Order Value",
                "Discounts", "Tax", "Net Revenue", "Customers"
            ]
        elif report_type == "staff_performance":
            return [
                "Staff ID", "Staff Name", "Orders Handled", "Revenue Generated",
                "Average Order Value", "Orders Per Hour", "Processing Time",
                "Revenue Rank", "Order Rank"
            ]
        elif report_type == "product_performance":
            return [
                "Product ID", "Product Name", "Category", "Quantity Sold",
                "Revenue Generated", "Average Price", "Order Frequency",
                "Revenue Share %", "Quantity Share %", "Popularity Rank"
            ]
        else:  # sales_summary
            return [
                "Period Start", "Period End", "Total Orders", "Total Revenue",
                "Items Sold", "Average Order Value", "Discounts", "Tax",
                "Net Revenue", "Customers", "Revenue Growth %", "Order Growth %"
            ]
    ...
    def _format_csv_row(self, item: Any, report_type: str) -> List[str]:
        ...
        if report_type == "sales_detailed":
            ...
            return [
                ...
                str(float(item.average_processing_time)) if item.average_processing_time else "",
    ...
    def _format_excel_row(self, item: Any, report_type: str) -> List[Any]:
        ...
        if report_type == "sales_detailed":
            ...
            return [
                ...
                float(item.average_processing_time) if item.average_processing_time else None,
                item.revenue_rank,
