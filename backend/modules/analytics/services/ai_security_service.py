...
class AISecurityService:
    ...
    def __init__(self):
        ...
        self.user_activity: Dict[int, Dict[str, deque]] = defaultdict(lambda: {
    ...
    def _initialize_blocked_patterns(self) -> List[re.Pattern]:
        ...
        return [
            ...
            re.compile(r'(union|select|insert|update|delete|drop|create|alter)\s+', re.I),
            re.compile(r'(;|--|\*|\/\*|\*\/)', re.I),
    ...
    def validate_request(self, request: ChatRequest, user_id: int) -> Tuple[bool, Optional[str]]:
        """
        ...
        for pattern in self.blocked_patterns:
            if pattern.search(request.message):
                logger.warning(f"Blocked pattern detected from user {user_id}: {pattern.pattern}")
        ...
        for pattern in self.sensitive_patterns:
            if pattern.search(request.message):
                ...
                return False, "Please do not include sensitive information in your queries"
    ...
    def check_rate_limit(self, user_id: int, limit_type: str) -> Tuple[bool, Optional[str]]:
        """
        ...
        if limit_type == 'messages':
            ...
            if len(recent_messages) > self.rate_limits['messages_per_minute']:
                return False, f"Rate limit exceeded: Max {self.rate_limits['messages_per_minute']} messages per minute"
        
        elif limit_type == 'queries':
            # Check queries per hour
            activity['queries'].append(now)
            recent_queries = [
                q for q in activity['queries']
                if now - q < timedelta(hours=1)
            ]
            
            if len(recent_queries) > self.rate_limits['queries_per_hour']:
                return False, f"Rate limit exceeded: Max {self.rate_limits['queries_per_hour']} queries per hour"
        
        elif limit_type == 'complex_queries':
            # Check complex queries per day
            activity['complex_queries'].append(now)
            recent_complex = [
                q for q in activity['complex_queries']
                if now - q < timedelta(days=1)
            ]
            
            if len(recent_complex) > self.rate_limits['complex_queries_per_day']:
                return False, f"Rate limit exceeded: Max {self.rate_limits['complex_queries_per_day']} complex queries per day"
    ...
    def calculate_query_complexity(self, query: AnalyticsQuery) -> float:
        ...
        if query.entities:
            entity_count = sum(
                ...
                for v in query.entities.values()
            )
    ...
    def is_query_allowed(self, query: AnalyticsQuery, user_id: int) -> Tuple[bool, Optional[str]]:
        """
    ...
    def log_security_event(self, user_id: int, event_type: str, details: Dict[str, any]):
        """
        ...
        """
        logger.warning(f"Security event - User: {user_id}, Type: {event_type}, Details: {json.dumps(details)}")
    ...
    def get_user_risk_score(self, user_id: int) -> float:
        ...
        recent_messages = [
            msg for msg in activity['messages']
            if now - msg < timedelta(minutes=5)
        ]
    ...
    def get_rate_limit_status(self, user_id: int) -> Dict[str, any]:
        ...
        return {
            'messages': {
                ...
                'remaining': max(0, self.rate_limits['messages_per_minute'] - recent_messages),
                'reset_in_seconds': 60
            },
            'queries': {
                'used': recent_queries,
                'limit': self.rate_limits['queries_per_hour'],
                'remaining': max(0, self.rate_limits['queries_per_hour'] - recent_queries),
                'reset_in_seconds': 3600
            },
            'complex_queries': {
                'used': recent_complex,
                'limit': self.rate_limits['complex_queries_per_day'],
                'remaining': max(0, self.rate_limits['complex_queries_per_day'] - recent_complex),
