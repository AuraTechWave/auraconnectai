...
class SalesReportService:
        ...
        try:
            ...
            if comparison_data:
                if comparison_data.total_revenue > 0:
                    revenue_growth = ((current_data.total_revenue - comparison_data.total_revenue) / 
                                    comparison_data.total_revenue * 100)
                if comparison_data.total_orders > 0:
                    order_growth = ((current_data.total_orders - comparison_data.total_orders) / 
                                  comparison_data.total_orders * 100)
            
            # Calculate customer retention rate
            customer_retention_rate = None
            if current_data.unique_customers > 0:
                customer_retention_rate = (current_data.returning_customers / 
                                         current_data.unique_customers * 100)
    ...
    def generate_product_performance_report(
        ...
        try:
            ...
            for result in results:
                ...
                product_performance.append(ProductPerformanceResponse(
    ...
    def get_dashboard_metrics(
        ...
        try:
            ...
            week_filters = SalesFilterRequest(
                date_from=week_ago,
                date_to=current_date
            )
            top_staff = self.generate_staff_performance_report(week_filters, per_page=5)
            top_products = self.generate_product_performance_report(week_filters, per_page=5)
            ...
            return DashboardMetricsResponse(
                ...
                top_products=[self._format_product_summary(product) for product in top_products],
    ...
    def _calculate_sales_metrics(self, filters: SalesFilterRequest) -> SalesCalculationResult:
        ...
        net_revenue = (metrics_query.total_revenue or 0) - (metrics_query.total_discounts or 0)
    ...
    def _build_snapshots_query(self, filters: SalesFilterRequest):
        ...
        # Apply filters
        if filters.date_from:
            query = query.filter(SalesAnalyticsSnapshot.snapshot_date >= filters.date_from)
        if filters.date_to:
            query = query.filter(SalesAnalyticsSnapshot.snapshot_date <= filters.date_to)
        
        if filters.staff_ids:
            query = query.filter(SalesAnalyticsSnapshot.staff_id.in_(filters.staff_ids))
        
        if filters.product_ids:
            query = query.filter(SalesAnalyticsSnapshot.product_id.in_(filters.product_ids))
        
        if filters.category_ids:
            query = query.filter(SalesAnalyticsSnapshot.category_id.in_(filters.category_ids))
    ...
    def _build_staff_performance_query(self, filters: SalesFilterRequest):
        ...
        query = self.db.query(
            ...
            func.rank().over(order_by=desc(staff_metrics.c.total_revenue)).label('revenue_rank'),
            func.rank().over(order_by=desc(staff_metrics.c.orders_handled)).label('order_count_rank'),
    ...
    def _build_product_performance_query(self, filters: SalesFilterRequest):
        ...
        if filters.date_from:
            product_metrics = product_metrics.filter(Order.created_at >= filters.date_from)
        if filters.date_to:
        ...
        query = self.db.query(
            ...
            func.rank().over(order_by=desc(product_metrics.c.quantity_sold)).label('popularity_rank'),
            func.rank().over(order_by=desc(product_metrics.c.revenue_generated)).label('revenue_rank')
    ...
    def _format_sales_detail(self, snapshot: SalesAnalyticsSnapshot) -> SalesDetailResponse:
    ...
    def _create_comparison_filters(self, filters: SalesFilterRequest) -> Optional[SalesFilterRequest]:
    ...
    def _calculate_growth_percentage(self, current: Union[int, Decimal], previous: Union[int, Decimal]) -> Decimal:
    ...
    def _calculate_page_summary(self, items: List[SalesDetailResponse], filters: SalesFilterRequest) -> SalesSummaryResponse:
    ...
    def _format_product_summary(self, product: ProductPerformanceResponse) -> Dict[str, Any]:
    ...
    def _get_trend_data(self, current_date: date) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        for i in range(30):
            ...
            revenue_trend.append({
                ...
                "value": float(day_data.total_revenue)
            })
            
            order_trend.append({
                "date": trend_date.isoformat(),
                "value": day_data.total_orders
            })
