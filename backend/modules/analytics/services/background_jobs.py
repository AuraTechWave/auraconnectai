...
class AnalyticsBackgroundJobs:
    ...
    async def _refresh_materialized_views(self):
        ...
        while self.running:
            try:
                ...
                db.execute(text("REFRESH MATERIALIZED VIEW CONCURRENTLY mv_top_performers;"))
                db.commit()
    ...
    async def _cleanup_old_data(self):
        ...
        while self.running:
            try:
                ...
                deleted_snapshots = db.query(SalesAnalyticsSnapshot).filter(
                    SalesAnalyticsSnapshot.snapshot_date < cutoff_date
                ).delete()
                
                # Clean up old metrics (keep 1 year)
                metrics_cutoff = datetime.now().date() - timedelta(days=365)
                
                deleted_metrics = db.query(SalesMetric).filter(
                    SalesMetric.metric_date < metrics_cutoff
                ).delete()
                
                db.commit()
                
                logger.info(f"Cleaned up {deleted_snapshots} snapshots and {deleted_metrics} metrics")
    ...
    async def _ensure_materialized_views_exist(self, db: Session):
        ...
        for view in materialized_views:
            try:
                ...
                """ if "staff_id" in view["query"] else f"""
                    CREATE UNIQUE INDEX IF NOT EXISTS {view['name']}_unique_idx 
                    ON {view['name']} (snapshot_date)
                """ if "snapshot_date" in view["query"] else f"""
                    CREATE UNIQUE INDEX IF NOT EXISTS {view['name']}_unique_idx 
    ...
    async def _generate_daily_snapshots(self, db: Session, start_date: date, end_date: date):
        ...
        while current_date <= end_date:
            try:
                ...
                existing = db.query(SalesAnalyticsSnapshot).filter(
                    and_(
                ...
                if daily_metrics and daily_metrics.total_orders > 0:
                    ...
                    snapshot = SalesAnalyticsSnapshot(
                        ...
                        average_order_value=Decimal(str(daily_metrics.average_order_value)),
                        total_discounts=Decimal(str(daily_metrics.total_discounts)),
                        total_tax=Decimal(str(daily_metrics.total_tax)),
                        net_revenue=Decimal(str(daily_metrics.total_revenue - daily_metrics.total_discounts)),
    ...
    async def _generate_weekly_snapshots(self, db: Session):
        ...
        for week_offset in range(4):
            ...
            weekly_metrics = db.query(
                ...
                func.sum(SalesAnalyticsSnapshot.total_items_sold).label('total_items_sold'),
                func.avg(SalesAnalyticsSnapshot.average_order_value).label('average_order_value'),
                func.sum(SalesAnalyticsSnapshot.total_discounts).label('total_discounts'),
                func.sum(SalesAnalyticsSnapshot.total_tax).label('total_tax'),
                func.sum(SalesAnalyticsSnapshot.unique_customers).label('unique_customers')
            ).filter(
                and_(
                    SalesAnalyticsSnapshot.snapshot_date >= week_start,
            ...
            if weekly_metrics and weekly_metrics.total_orders:
                weekly_snapshot = SalesAnalyticsSnapshot(
                    ...
                    net_revenue=weekly_metrics.total_revenue - weekly_metrics.total_discounts,
        ...
        for month_offset in range(6):
            if month_offset == 0:
                ...
                if month_start.month == 1:
                    target_month = month_start.replace(year=month_start.year - 1, month=12)
                else:
                    target_month = month_start.replace(month=month_start.month - month_offset)
            ...
            if monthly_metrics and monthly_metrics.total_orders:
                monthly_snapshot = SalesAnalyticsSnapshot(
                    ...
                    net_revenue=monthly_metrics.total_revenue - monthly_metrics.total_discounts,
    ...
    async def _evaluate_single_alert(self, db: Session, rule: AlertRule):
        ...
        try:
            ...
            current_value = self._get_metric_value(db, rule.metric_name, rule.evaluation_period)
    ...
    def _get_metric_value(self, db: Session, metric_name: str, period: str) -> Optional[float]:
        """Get current metric value"""
    ...
    def _get_comparison_value(self, db: Session, metric_name: str, comparison_period: str) -> Optional[float]:
    ...
    async def _trigger_alert(
        ...
        logger.warning(f"ALERT: {rule.name} - Current value: {current_value}, Threshold: {rule.threshold_value}")
