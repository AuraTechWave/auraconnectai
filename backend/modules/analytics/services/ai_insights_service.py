...
class AIInsightsService:
    ...
    async def generate_insights(
        ...
        if InsightType.PRODUCT_TREND in request.insight_types:
            summary.product_insights = await self._analyze_product_trends(start_date, end_date)
            summary.overall_recommendations.extend(
                summary.product_insights.recommendations[:2]
            )
        
        if InsightType.CUSTOMER_PATTERN in request.insight_types:
            summary.customer_insights = await self._analyze_customer_patterns(start_date, end_date)
            summary.overall_recommendations.extend(
    ...
    async def _analyze_peak_times(
        ...
        all_orders = [p.order_count for patterns in patterns_by_hour.values() for p in patterns]
        ...
        sorted_hours = sorted(hourly_averages.items(), key=lambda x: x[1].intensity, reverse=True)
        ...
        quiet_periods = [item[1] for item in sorted_hours[-quiet_threshold:]] if quiet_threshold > 0 else []
        ...
        for dow, hour_patterns in patterns_by_dow_hour.items():
            ...
            for hour, patterns in hour_patterns.items():
                if patterns:
                    ...
                    day_peaks.append(TimePattern(
                        hour=hour,
                        day_of_week=dow,
                        intensity=avg_orders / max_orders,
                        order_count=int(avg_orders),
                        revenue=Decimal(str(avg_revenue)),
                        customer_count=int(statistics.mean([p.customer_count for p in patterns]))
                    ))
        ...
        if lunch_hours and dinner_hours:
            ...
            if lunch_intensity > 0.7:
                recommendations.append("Strong lunch rush detected - ensure kitchen is fully prepared by 11:00")
            if dinner_intensity > 0.7:
                recommendations.append("Strong dinner rush detected - prepare for high volume from 17:00")
        
        confidence = ConfidenceLevel.HIGH if len(hourly_data) > 100 else ConfidenceLevel.MEDIUM
    ...
    async def _analyze_product_trends(
        ...
        current_period_data = self._get_product_metrics(
            start_date,
            end_date
        )
        ...
        previous_period_data = self._get_product_metrics(
            prev_start,
            prev_end
        )
        ...
        for product_id, current_metrics in current_period_data.items():
            prev_metrics = previous_period_data.get(product_id, {
                'quantity': 0,
                'revenue': Decimal('0'),
                'order_count': 0,
                'rank': None
            ...
            if prev_metrics['quantity'] > 0:
                ...
                # New product
                trend_direction = "rising" if current_metrics['quantity'] > 10 else "stable"
                trend_strength = 0.5
            
            # Simple demand prediction (could be enhanced with ML)
            if trend_direction == "rising":
                predicted_demand = int(current_metrics['quantity'] * (1 + trend_strength * 0.3))
            elif trend_direction == "falling":
                predicted_demand = int(current_metrics['quantity'] * (1 - trend_strength * 0.2))
            else:
        ...
        stable_performers = [t for t in product_trends if t.trend_direction == "stable" and t.current_rank <= 20][:10]
        new_trending = [t for t in product_trends if t.previous_rank is None and t.current_rank <= 50][:5]
        ...
        confidence = ConfidenceLevel.HIGH if len(product_trends) > 20 else ConfidenceLevel.MEDIUM
    ...
    async def _analyze_customer_patterns(
        ...
        avg_order_frequency = statistics.mean(order_frequencies) if order_frequencies else 0
        
        # Detect patterns
        if repeat_customer_rate < 0.3:
            patterns_detected.append(CustomerPattern(
        ...
        at_risk_count = sum(1 for c in customer_data if c.last_order.date() < recent_threshold)
        ...
        if 'vip' in clv_trends and clv_trends['vip'] > statistics.mean(list(clv_trends.values())) * 2:
            recommendations.append(
                "Focus on VIP customer retention - they generate significantly higher lifetime value"
            )
        
        confidence = ConfidenceLevel.HIGH if total_customers > 100 else ConfidenceLevel.MEDIUM
    ...
    async def _detect_anomalies(
        ...
        for day_data in daily_data:
            ...
            revenue_z_score = abs((day_revenue - revenue_mean) / revenue_stdev) if revenue_stdev > 0 else 0
            order_z_score = abs((day_orders - order_mean) / order_stdev) if order_stdev > 0 else 0
            
            if revenue_z_score > 2 or order_z_score > 2:
                anomaly_type = "revenue_spike" if day_revenue > revenue_mean else "revenue_drop"
                severity = "high" if revenue_z_score > 3 else "medium"
                
                deviation_percentage = ((day_revenue - revenue_mean) / revenue_mean) * 100
