...
class AlertingService:
    ...
    def get_alert_rules(
        self, include_inactive: bool = False, created_by: Optional[int] = None
    ) -> List[AlertRule]:
    ...
    def update_alert_rule(self, rule_id: int, updates: Dict[str, Any]) -> AlertRule:
        ...
        for rule in active_rules:
            try:
                ...
                results["evaluation_errors"].append(
                    {"rule_id": rule.id, "rule_name": rule.name, "error": str(e)}
                )
    ...
    async def test_alert_rule(
        self, rule: AlertRule, test_value: Optional[float] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if current_value is None:
                return {"success": False, "error": "Could not retrieve metric value"}
    ...
    async def get_alert_history(
        self, rule_id: Optional[int] = None, days_back: int = 30
    ) -> List[Dict[str, Any]]:
        ...
        rules = query.filter(AlertRule.last_triggered_at >= cutoff_date).all()
        ...
        try:
            ...
            if rule.metric_name == "daily_revenue":
                ...
                metric = (
                    ...
                    .order_by(desc(SalesMetric.created_at))
                    .first()
                )
                ...
                if metric:
                    return (
                        ...
                        else float(metric.value_integer or 0)
                    )
    ...
    def _evaluate_condition(
        ...
        if rule.condition_type == "above":
            ...
            change_percentage = (
                abs((current_value - comparison_value) / comparison_value) * 100
            )
            ...
            decrease_percentage = (
                (comparison_value - current_value) / comparison_value
            ) * 100
