...
class RealtimeMetricsService:
    ...
    async def get_top_performers(self, limit: int = 5) -> Dict[str, List[Dict[str, Any]]]:
    ...
    async def _metrics_update_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # Log metrics update
                logger.info(f"Dashboard metrics updated: Revenue: ${snapshot.revenue_today}, Orders: {snapshot.orders_today}")
    ...
    async def _generate_dashboard_snapshot(self) -> DashboardSnapshot:
        ...
        try:
            ...
            revenue_growth = self._calculate_growth_percentage(
                today_metrics.get("revenue", 0),
                yesterday_metrics.get("revenue", 0)
            )
            
            order_growth = self._calculate_growth_percentage(
                today_metrics.get("orders", 0),
                yesterday_metrics.get("orders", 0)
            )
            
            customer_growth = self._calculate_growth_percentage(
                today_metrics.get("customers", 0),
                yesterday_metrics.get("customers", 0)
            )
    ...
    async def _get_daily_metrics(self, db: Session, target_date: date) -> Dict[str, Any]:
        """Get metrics for a specific date"""
        
        # Try to get from snapshots first
        snapshot = db.query(SalesAnalyticsSnapshot).filter(
            and_(
    ...
    async def _calculate_top_performers(self, limit: int) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            top_staff_query = db.query(
                ...
                func.coalesce(func.sum(Order.total_amount), 0).label('revenue'),
                func.count(Order.id).label('orders')
            ).join(Order, Order.staff_id == StaffMember.id).filter(
                and_(
                    func.date(Order.created_at) >= week_start,
                    Order.status == 'completed'
                )
            ).group_by(StaffMember.id, StaffMember.name).order_by(
    ...
    async def _calculate_customers_metric(self, db: Session) -> RealtimeMetric:
        ...
        yesterday_customers = await self._get_daily_metrics(db, today - timedelta(days=1))
    ...
    async def _get_custom_metric(self, db: Session, metric_name: str) -> Optional[RealtimeMetric]:
        """Get custom metric from SalesMetric table"""
        try:
            metric = db.query(SalesMetric).filter(
                SalesMetric.metric_name == metric_name
            ).order_by(SalesMetric.created_at.desc()).first()
