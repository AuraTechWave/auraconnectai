    ...
    def detect_seasonality(self, data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            seasonality_strength = max(
                0, 1 - var_residual / (var_residual + var_seasonal)
            )
...
class ExponentialSmoothingModel(BaseForecastModel):
    ...
    def fit(self, data: pd.Series, **kwargs) -> None:
        ...
        try:
            ...
            logger.info(
                f"Exponential Smoothing fitted with seasonal_periods={self.seasonal_periods}"
            )
    ...
    def predict(self, steps: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        margin = (
            z_score
            * sigma
            * np.sqrt(1 + np.arange(1, steps + 1) / len(self.training_history))
        )
