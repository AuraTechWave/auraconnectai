...
class ForecastMonitoringService:
    ...
    async def track_forecast_accuracy(
        ...
        try:
            ...
            accuracy_metrics = self._calculate_accuracy_metrics(
                merged_df["predicted"], merged_df["actual"]
            )
            ...
            self._store_performance_metrics(
                entity_type, entity_id, accuracy_metrics, len(merged_df)
            )
            ...
            return ForecastComparison(
                ...
                actuals=[
                    {"timestamp": idx, "value": row["actual"]}
                    for idx, row in merged_df.iterrows()
                ],
    ...
    def _calculate_accuracy_metrics(
        ...
        if len(predicted) == 0:
            return {"mae": 0, "mape": 0, "rmse": 0, "bias": 0, "r_squared": 0}
    ...
    def _analyze_deviations(self, merged_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        analysis = {
            "max_over_prediction": {
                ...
                "date": (
                    deviations.idxmin().isoformat() if not deviations.empty else None
                ),
            },
            "max_under_prediction": {
                ...
                "date": (
                    deviations.idxmax().isoformat() if not deviations.empty else None
                ),
            },
            "systematic_bias": (
                "over_predicting" if deviations.mean() < 0 else "under_predicting"
            ),
            "volatility_ratio": float(
                merged_df["predicted"].std() / (merged_df["actual"].std() + 1e-10)
            ),
        }
        ...
        if "lower_bound" in merged_df.columns and "upper_bound" in merged_df.columns:
            within_bounds = (merged_df["actual"] >= merged_df["lower_bound"]) & (
    ...
    async def generate_accuracy_report(
        self, start_date: date, end_date: date, entity_type: Optional[str] = None
    ) -> ForecastAccuracyReport:
    ...
    def _aggregate_model_performances(
        self, performances: List[ForecastPerformance]
    ...
    def _get_product_accuracy_ranking(
        ...
        product_perfs = [
            p for p in performances if p.entity_type == "product" and p.entity_id
        ]
    ...
    def _generate_improvement_recommendations(
        self, performances: List[ForecastPerformance], overall_mape: float
    ) -> List[str]:
    ...
    async def detect_forecast_anomalies(
        self, entity_type: str, entity_id: Optional[int], recent_days: int = 7
        ...
        try:
            ...
            performances = (
                self.db.query(ForecastPerformance)
                .filter(
                    and_(
