...
class PermissionsService:
    ...
    def filter_data_by_permissions(
        user: Dict[str, Any], data: Dict[str, Any], data_type: str
    ) -> Dict[str, Any]:
        ...
        if AnalyticsPermission.VIEW_ALL_STAFF not in user_permissions:
            ...
            user_id = user.get("id")
            if (
                data_type == "staff_performance"
                and filtered_data.get("staff_id") != user_id
            ):
...
def require_analytics_role(role: AnalyticsRole):
    ...
    def role_dependency(current_user: Dict[str, Any]):
        ...
        if user_role != role:
            ...
            current_level = (
                role_hierarchy.index(user_role) if user_role in role_hierarchy else -1
            )
            required_level = (
                role_hierarchy.index(role) if role in role_hierarchy else 999
            )
