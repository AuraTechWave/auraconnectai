...
class AIChatService:
    ...
    async def process_message(
        self, request: ChatRequest, user_id: int, db: Session
    ) -> ChatResponse:
        """
        ...
        try:
            ...
            is_valid, error_message = self.security_service.validate_request(
                request, user_id
            )
            if not is_valid:
            ...
            # Process query
            query = self.query_processor.process_query(request.message, session.context)
            ...
            if clarifications and query.confidence_score < 0.7:
                return self._create_clarification_response(
                    session.session_id, query, clarifications
                )

            # Check if query is allowed (complexity and rate limits)
            is_allowed, limit_message = self.security_service.is_query_allowed(
                query, user_id
            )
            if not is_allowed:
            ...
            assistant_message = ChatMessage(
                ...
                type=(
                    MessageType.QUERY_RESULT if query_result.data else MessageType.TEXT
                ),
                metadata={
                    "query": query.dict(),
    ...
    def _get_or_create_session(
        self, session_id: Optional[str], user_id: int
    ) -> ChatSession:
        ...
        session = ChatSession(
            ...
            context=QueryContext(session_id=new_session_id, user_id=user_id),
        )
    ...
    async def _execute_query(
        self, query: AnalyticsQuery, user_id: int, db: Session
    ) -> QueryResult:
        ...
        try:
            ...
            executor = self.query_executors.get(
                query.intent, self._execute_general_question
            )
            ...
            logger.error(
                f"Error executing query for intent {query.intent}: {e}", exc_info=True
            )
            execution_time_ms = int(
                (datetime.now() - start_time).total_seconds() * 1000
            )
    ...
    async def _execute_sales_report(
        ...
        try:
            ...
            filters = SalesFilterRequest(
                date_from=(
                    query.time_range.get("start_date") if query.time_range else None
                ),
                ...
                category_ids=(
                    query.filters.get("category_ids") if query.filters else None
                ),
            )
            ...
            chart_data = ChartData(
                ...
                data=[
                    ...
                    DataPoint(
                        ...
                        value=float(summary.average_order_value),
                    ),
                ],
            ...
            if summary.revenue_growth:
                insights.append(
                    f"Revenue {'increased' if summary.revenue_growth > 0 else 'decreased'} by {abs(summary.revenue_growth):.1f}%"
                )
            if summary.order_growth:
                insights.append(
                    f"Order volume {'increased' if summary.order_growth > 0 else 'decreased'} by {abs(summary.order_growth):.1f}%"
                )
    ...
    async def _execute_revenue_analysis(
        ...
        try:
            ...
            granularity = (
                "hourly" if days_diff <= 1 else "daily" if days_diff <= 30 else "weekly"
            )
            ...
            chart_data = ChartData(
                ...
                data=[
                    DataPoint(
                        label=point.date.strftime(
                            "%Y-%m-%d" if granularity == "daily" else "%H:00"
                        ),
                ...
                rows=[
                    {
                        ...
                        "efficiency": (
                            float(staff.efficiency_score)
                            if staff.efficiency_score
                            else 0
                        ),
                    }
    ...
    async def _execute_product_analysis(
        ...
        try:
            ...
            service = SalesReportService(db)
            products = service.generate_product_performance_report(
                filters, page=1, per_page=10
            )
    ...
    async def _execute_trend_analysis(
        ...
        try:
            ...
            if stats["average_change"] > 0:
                insights.append(
                    f"{metric.capitalize()} is trending upward with {stats['average_change']:.1f}% average daily growth"
                )
            elif stats["average_change"] < 0:
                insights.append(
                    f"{metric.capitalize()} is trending downward with {abs(stats['average_change']):.1f}% average daily decline"
                )
    ...
    def _format_response_text(
        ...
        # Enhance with emojis
        formatted_text = self.result_formatter.enhance_with_emojis(
            formatted_text, query.intent
        )

        return formatted_text

    async def _generate_insights(
        self, query: AnalyticsQuery, result: QueryResult, db: Session
    ...
    def _generate_suggested_questions(
        self, query: AnalyticsQuery, result: QueryResult
    ) -> List[str]:
        ...
        suggestions = intent_suggestions.get(
            query.intent,
            [
    ...
    def _create_clarification_response(
        self, session_id: str, query: AnalyticsQuery, clarifications: List[str]
    ) -> ChatResponse:
    ...
    def _create_error_response(
        self, error_message: str, metadata: Optional[Dict[str, Any]] = None
    ) -> ChatResponse:
        ...
        return ChatResponse(message=message, requires_clarification=False)
    ...
    async def process_batch_messages(
        self, requests: List[ChatRequest], user_id: int, db: Session
    ) -> List[ChatResponse]:
        """
