...
class StockOptimizationService:
    ...
    async def optimize_stock_levels(
        self, request: StockOptimizationRequest
    ) -> StockOptimizationResult:
        """
        ...
        try:
            ...
            for product in products:
                try:
                    ...
                    demand_forecast = await self._get_product_demand_forecast(
                        product.id, request.lead_time_days
                    )

                    # Calculate optimal stock levels
                    recommendation = self._calculate_optimal_stock(
                        product, demand_forecast, request
                    )

                    recommendations.append(recommendation)
                    total_investment += recommendation.recommended_stock * Decimal(
            ...
            if (
                ...
                and total_investment > request.budget_constraint
            ):
                recommendations = self._apply_budget_constraint(
                    recommendations, request.budget_constraint
                )
    ...
    async def _get_product_demand_forecast(
        self, product_id: int, horizon_days: int
    ) -> Dict[str, Any]:
    ...
    def _calculate_optimal_stock(
        ...
        holding_cost = self.cost_calculator.calculate_holding_cost(
            product, recommended_stock, mean_demand
        )
        stockout_cost = self.cost_calculator.calculate_stockout_cost(
            product, stockout_risk, mean_demand
        )
    ...
    def _get_current_inventory(self, product_id: int) -> Dict[str, Any]:
        ...
        menu_inventory = (
            self.db.query(MenuItemInventory).filter_by(menu_item_id=product_id).first()
        )

        if menu_inventory:
            inventory = (
                self.db.query(Inventory)
                .filter_by(id=menu_inventory.inventory_id)
                .first()
            )
    ...
    def _apply_budget_constraint(
        self, recommendations: List[StockRecommendation], budget: Decimal
        ...
        for i, rec in enumerate(recommendations):
            ...
            value = 1 / (
                rec.expected_stockout_risk + 0.01
            )  # Higher value for lower risk
    ...
    def _calculate_overall_metrics(
        ...
        if not recommendations:
            return {"service_level": 0, "waste_reduction": 0, "total_products": 0}

        # Average service level
        avg_service_level = np.mean(
            [1 - rec.expected_stockout_risk for rec in recommendations]
        )
        ...
        if total_current > 0:
            ...
            if overstock_items:
                excess_stock = sum(
                    rec.current_stock - rec.recommended_stock for rec in overstock_items
                )
        ...
        return {
            ...
            "products_understocked": len(
                [r for r in recommendations if r.current_stock < r.reorder_point]
            ),
            "total_safety_stock": sum(r.safety_stock for r in recommendations),
            "avg_stockout_risk": np.mean(
                [r.expected_stockout_risk for r in recommendations]
            ),
        }
...
class InventoryCostCalculator:
    ...
    def calculate_holding_cost(
        self, product: MenuItem, stock_level: float, demand_rate: float
    ) -> Decimal:
    ...
    def calculate_stockout_cost(
        self, product: MenuItem, stockout_probability: float, demand_rate: float
    ) -> Decimal:
    ...
    def calculate_total_cost(
        ...
        holding_cost = self.calculate_holding_cost(
            product, stock_level, demand_params["mean"]
        )
        ...
        if stock_level >= reorder_point:
            ...
            z_score = (stock_level - demand_params["mean"]) / (
                demand_params["std"] + 1e-10
            )
            stockout_prob = stats.norm.cdf(z_score)

        stockout_cost = self.calculate_stockout_cost(
            product, stockout_prob, demand_params["mean"]
        )
