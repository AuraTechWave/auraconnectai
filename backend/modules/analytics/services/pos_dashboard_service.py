...
class POSDashboardService(POSAnalyticsBaseService):
    ...
    async def _generate_dashboard_data(
        ...
        # Get active alerts
        alerts, _ = await self.alerts_service.get_active_alerts(
            limit=20, offset=0
        )
    ...
    async def _get_provider_summaries(
        ...
        for row in results:
            ...
            sync_success_rate = (success_syncs / total_syncs * 100) if total_syncs > 0 else 0.0
            
            total_webhooks = row.total_webhooks or 0
            success_webhooks = row.successful_webhooks or 0
            webhook_success_rate = (success_webhooks / total_webhooks * 100) if total_webhooks > 0 else 0.0
