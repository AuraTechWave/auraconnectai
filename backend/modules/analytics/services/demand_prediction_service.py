...
class DemandPredictionService:
    ...
    async def forecast_demand(self, request: DemandForecastRequest) -> DemandForecast:
        """
        ...
        try:
            ...
            historical_data = self._get_historical_demand(
                request.entity_type, request.entity_id, request.time_granularity
            )
            ...
            if request.include_external_factors:
                time_series = self._apply_external_factors(
                    time_series, request.external_factors
                )

            # Select or create model
            model = self._get_or_create_model(
                request.entity_type, request.entity_id, request.model_type, time_series
            )
            ...
            prediction_points = self._create_prediction_points(
                predictions, lower_bounds, upper_bounds, request.time_granularity
            )

            # Detect patterns and generate insights
            seasonality_patterns = model.detect_seasonality(time_series)
            insights = self._generate_insights(
                time_series, predictions, seasonality_patterns
            )

            # Calculate forecast metadata
            metadata = self._create_forecast_metadata(
                model, time_series, predictions, seasonality_patterns
            )

            # Generate recommendations
            recommendations = self._generate_recommendations(
                predictions, time_series, request
            )

            # Get entity name
            entity_name = self._get_entity_name(request.entity_type, request.entity_id)
    ...
    def _get_historical_demand(
        self, entity_type: str, entity_id: Optional[int], granularity: TimeGranularity
    ) -> pd.DataFrame:
    ...
    def _apply_external_factors(
        self, time_series: pd.Series, external_factors: Optional[Dict[str, Any]]
    ) -> pd.Series:
        ...
        if "weather" in external_factors:
            weather_impact = self.external_factors.apply_weather_impact(
                time_series, external_factors["weather"]
            )
        ...
        if "events" in external_factors:
            event_impact = self.external_factors.apply_event_impact(
                time_series, external_factors["events"]
            )
        ...
        if "holidays" in external_factors:
            holiday_impact = self.external_factors.apply_holiday_impact(
                time_series, external_factors["holidays"]
            )
    ...
    def _get_or_create_model(
        ...
        if cached_model:
            logger.debug(
                f"Using cached model for {entity_type}:{entity_id}:{model_type_str}"
            )
        ...
        for i in range(len(predictions)):
            ...
            points.append(
                PredictionPoint(
                    ...
                    lower_bound=(
                        max(0, lower_bounds[i]) if lower_bounds is not None else None
                    ),
                    upper_bound=(
                        max(0, upper_bounds[i]) if upper_bounds is not None else None
                    ),
    ...
    def _generate_insights(
        ...
        if pred_mean > hist_mean * 1.1:
            insights.append(
                f"Demand is expected to increase by {((pred_mean/hist_mean - 1) * 100):.1f}% on average"
            )
        elif pred_mean < hist_mean * 0.9:
            insights.append(
                f"Demand is expected to decrease by {((1 - pred_mean/hist_mean) * 100):.1f}% on average"
            )
        else:
        ...
        if seasonality.get("seasonal"):
            ...
            if strength > 0.5:
                insights.append(
                    f"Strong seasonal pattern detected with {strength:.1%} strength"
                )
            elif strength > 0.2:
        ...
        if max_pred_value > hist_mean * 1.5:
            insights.append(
                f"Peak demand expected on day {max_pred_idx + 1} ({max_pred_value:.0f} units)"
            )
    ...
    def _create_forecast_metadata(
        ...
        model_type = model_type_map.get(model.__class__.__name__, ModelType.ENSEMBLE)
        ...
        if len(time_series) > 20:
            ...
            mape = (
                np.mean(np.abs((test_data.values - test_preds) / test_data.values))
                * 100
            )
...
class ExternalFactorsAnalyzer:
    ...
    def apply_weather_impact(
        self, time_series: pd.Series, weather_data: Dict[str, Any]
    ) -> pd.Series:
    ...
    def apply_event_impact(
        self, time_series: pd.Series, events: List[Dict[str, Any]]
    ) -> pd.Series:
    ...
    def apply_holiday_impact(
        self, time_series: pd.Series, holidays: List[Dict[str, Any]]
    ) -> pd.Series:
