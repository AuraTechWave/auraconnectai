...
class POSDetailsService(POSAnalyticsBaseService):
    ...
    async def get_provider_details(
        ...
        webhook_metrics = await self._get_webhook_metrics(provider_id, start_date, end_date)
        error_analysis = await self._get_error_analysis(provider_id, start_date, end_date) if include_errors else None
        performance_metrics = await self._get_performance_metrics(provider_id, start_date, end_date)
    ...
    async def _get_provider_terminals(
        self,
        provider_id: int
    ) -> List[Any]:
    ...
    def _calculate_rankings(
        ...
        for metric in metrics:
            if metric == "transactions":
                ...
                sorted_providers = sorted(
                    providers,
                    key=lambda p: p.transaction_success_rate,
                    reverse=True
                )
            else:
