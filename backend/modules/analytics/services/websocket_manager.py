...
from .realtime_metrics_service import DashboardSnapshot, RealtimeMetric, realtime_metrics_service
...
class WebSocketManager:
    ...
    async def broadcast_dashboard_update(self, snapshot: DashboardSnapshot):
        ...
        for client_id in self.dashboard_subscribers.copy():
            if self._client_has_permission(client_id, AnalyticsPermission.VIEW_DASHBOARD):
    ...
    async def broadcast_metric_update(self, metric: RealtimeMetric):
        ...
        for client_id in metric_subscribers.copy():
            if self._client_has_permission(client_id, AnalyticsPermission.VIEW_SALES_REPORTS):
                tasks.append(self._send_message(client_id, message))
        
        if tasks:
            await asyncio.gather(*tasks, return_exceptions=True)
            logger.debug(f"Metric update '{metric.metric_name}' sent to {len(tasks)} clients")
    ...
    async def broadcast_alert_notification(self, alert_data: Dict[str, Any]):
        ...
        for client_id in self.alert_subscribers.copy():
            if self._client_has_permission(client_id, AnalyticsPermission.CREATE_ALERTS):
                tasks.append(self._send_message(client_id, message))
        ...
        try:
            if subscription_type == "dashboard":
                if self._client_has_permission(client_id, AnalyticsPermission.VIEW_DASHBOARD):
                    ...
                    await self._send_error(client_id, "Insufficient permissions for dashboard subscription")
                    return
                ...
                if self._client_has_permission(client_id, AnalyticsPermission.VIEW_SALES_REPORTS):
                    ...
                    await self._send_error(client_id, "Insufficient permissions for metrics subscription")
                    return
                ...
                if self._client_has_permission(client_id, AnalyticsPermission.CREATE_ALERTS):
                    ...
                    await self._send_error(client_id, "Insufficient permissions for alerts subscription")
                    return
            
            else:
                await self._send_error(client_id, f"Unknown subscription type: {subscription_type}")
                return
    ...
    async def _handle_current_data_request(self, client_id: str, payload: Dict[str, Any]):
        ...
        try:
            if data_type == "dashboard":
                ...
                if metric_name and self._client_has_permission(client_id, AnalyticsPermission.VIEW_SALES_REPORTS):
                    ...
                    if metric:
                        ...
                        await self._send_error(client_id, f"Metric '{metric_name}' not found")
                else:
                    await self._send_error(client_id, "Invalid metric request or insufficient permissions")
    ...
    def _client_has_permission(self, client_id: str, permission: AnalyticsPermission) -> bool:
    ...
    async def _heartbeat_loop(self):
        ...
        while self.is_running:
            try:
                ...
                heartbeat_tasks = []
                for client_id in list(self.active_connections.keys()):
                    task = self._send_message(client_id, WebSocketMessage(
                        type=WebSocketMessageType.HEARTBEAT,
                        data={"status": "ping", "server_time": current_time.isoformat()},
    ...
    async def _cleanup_loop(self):
        ...
        while self.is_running:
            try:
                ...
                if empty_metrics:
                    logger.debug(f"Cleaned up {len(empty_metrics)} empty metric subscriptions")
