...
class POSTrendsService(POSAnalyticsBaseService):
    ...
    async def get_transaction_trends(
        ...
        # Generate cache key
        cache_key = (
            f"trends:transactions:{granularity}:{start_date.date()}:{end_date.date()}"
        )
        if provider_id:
    ...
    async def _generate_transaction_trends(
        ...
        if granularity == "hourly":
            query = query.group_by(
                POSAnalyticsSnapshot.snapshot_date, POSAnalyticsSnapshot.snapshot_hour
            ).order_by(
                POSAnalyticsSnapshot.snapshot_date, POSAnalyticsSnapshot.snapshot_hour
            )
        elif granularity == "daily":
            query = query.group_by(POSAnalyticsSnapshot.snapshot_date).order_by(
                POSAnalyticsSnapshot.snapshot_date
            )
        elif granularity == "weekly":
            # Group by week
            query = self.db.query(
                func.date_trunc("week", POSAnalyticsSnapshot.snapshot_date).label(
                    "week"
                ),
                func.sum(POSAnalyticsSnapshot.total_transactions).label("count"),
                func.sum(POSAnalyticsSnapshot.total_transaction_value).label("value"),
        ...
        for row in results:
            if granularity == "hourly":
                timestamp = datetime.combine(
                    row.snapshot_date, datetime.min.time()
                ) + timedelta(hours=row.snapshot_hour)
            elif granularity == "daily":
    ...
    async def get_performance_trends(
        ...
        # Cache key
        cache_key = (
            f"trends:{metric}:{granularity}:{start_date.date()}:{end_date.date()}"
        )
        if provider_id:
            cache_key += f":provider_{provider_id}"
    ...
    async def _get_success_rate_trends(
        ...
        for row in results:
            ...
            sync_rate = (
                (row.sync_successful / row.sync_total * 100)
                if row.sync_total > 0
                else 0.0
            )
            webhook_rate = (
                (row.webhook_successful / row.webhook_total * 100)
                if row.webhook_total > 0
                else 0.0
            )
    ...
    async def _get_error_rate_trends(
        ...
        for row in results:
            ...
            error_rate = (
                (row.errors / total_operations * 100) if total_operations > 0 else 0.0
            )
