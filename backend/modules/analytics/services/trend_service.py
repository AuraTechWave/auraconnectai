...
from ..models.analytics_models import (
    SalesAnalyticsSnapshot, SalesMetric, AggregationPeriod
)
...
class TrendService:
    ...
    def get_multi_metric_trend(
        ...
        try:
            ...
            query = self.db.query(
                ...
                func.avg(SalesAnalyticsSnapshot.average_order_value).label('avg_order_value'),
                ...
                and_(
                    ...
                    SalesAnalyticsSnapshot.period_type == self._get_period_type(granularity)
                )
            )
            ...
            for metric in metrics:
                ...
                for result in results:
                    ...
                    trend_points.append(TrendPoint(
                        date=result.period if isinstance(result.period, date) else result.period.date(),
                        value=float(value),
            ...
            if "staff" in comparison_entities:
                for staff_id in comparison_entities["staff"]:
                    ...
                    trend_data = self._get_entity_trend(
                        start_date, end_date, metric, granularity,
                        staff_ids=[staff_id]
                    )
    ...
    def get_trend_statistics(
        ...
        changes = [point.change_percentage for point in trend_points if point.change_percentage is not None]
        ...
        if n > 1:
            ...
            slope = (n * sum(i * v for i, v in zip(x_values, values)) - sum(x_values) * sum(values)) / \
    ...
    def _format_trend_points(self, results: List[Any], metric_type: str) -> List[TrendPoint]:
