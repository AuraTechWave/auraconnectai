...
class TestAnalyticsPerformance:
    ...
    def test_sales_summary_performance_large_dataset(
        self, db_session: Session, mock_large_dataset
    ):
        ...
        assert (
            execution_time < 2.0
        ), f"Sales summary took {execution_time:.2f}s, should be under 2s"
        assert result is not None
        assert result.total_orders > 0
        assert result.total_revenue > 0

    def test_detailed_report_performance_pagination(
        self, db_session: Session, mock_large_dataset
    ):
        ...
        filters = SalesFilterRequest(
            date_from=date.today() - timedelta(days=90), date_to=date.today()
        )
        ...
        for page_size in page_sizes:
            ...
            result = service.generate_detailed_sales_report(
                filters=filters, page=1, per_page=page_size
            )
            execution_time = time.time() - start_time
            ...
            max_time = 0.5 + (
                page_size / 10000
            )  # Allow 0.5s base + 0.1s per 1000 records
            assert (
                execution_time < max_time
            ), f"Page size {page_size} took {execution_time:.2f}s, should be under {max_time:.2f}s"
            assert len(result.items) <= page_size

    def test_trend_service_performance_optimization(
        self, db_session: Session, mock_large_dataset
    ):
        ...
        assert (
            execution_time < 1.0
        ), f"Revenue trend took {execution_time:.2f}s, should be under 1s"
        ...
        assert (
            execution_time < 1.5
        ), f"Multi-metric trend took {execution_time:.2f}s, should be under 1.5s"
        assert "revenue" in multi_trends
        assert "orders" in multi_trends

    def test_export_service_memory_efficiency(
        self, db_session: Session, mock_large_dataset
    ):
        ...
        with patch.object(export_service, "_get_report_data") as mock_get_data:
            ...
            assert (
                csv_time < 5.0
            ), f"CSV export took {csv_time:.2f}s, should be under 5s"
        ...
        async def run_async_test():
            ...
            for i in range(100):
                task_id = await processor.submit_task(
                    "test_task", {"data": f"test_{i}"}, priority=TaskPriority.NORMAL
                )
            ...
            assert (
                submission_time < 1.0
            ), f"Task submission took {submission_time:.2f}s, should be under 1s"
    ...
    def test_memory_usage_monitoring(self, db_session: Session, mock_large_dataset):
        ...
        for _ in range(5):
            filters = SalesFilterRequest(
                date_from=date.today() - timedelta(days=365), date_to=date.today()
            )

            result = service.generate_detailed_sales_report(
                filters=filters, page=1, per_page=5000
            )
    ...
    def test_database_connection_pooling(self, db_session: Session):
        ...
        assert (
            execution_time < 2.0
        ), f"Multiple service calls took {execution_time:.2f}s, should be under 2s"

    def test_query_optimization_with_indexes(
        self, db_session: Session, mock_large_dataset
    ):
        ...
        for filter_combo in filter_combinations:
            ...
            assert (
                execution_time < 1.0
            ), f"Query with {filter_combo} took {execution_time:.2f}s, should be under 1s"

    @pytest.mark.parametrize("concurrent_users", [5, 10, 25])
    def test_concurrent_user_performance(
        self, db_session: Session, mock_large_dataset, concurrent_users
    ):
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = [
                executor.submit(simulate_user_request) for _ in range(concurrent_users)
            ]
            results = [
        ...
        ), f"Average response time {avg_response_time:.2f}s too high with {concurrent_users} users"
        ...
        ), f"Concurrency benefit only {concurrency_benefit:.1f}x, should be at least 2x"
...
class TestAnalyticsStressTests:
    ...
    def test_extreme_date_ranges(self, db_session: Session):
        ...
        assert (
            execution_time < 10.0
        ), f"Extreme date range took {execution_time:.2f}s, should be under 10s"
        assert result is not None
    ...
    def test_maximum_pagination_limits(self, db_session: Session, mock_large_dataset):
        ...
        result = service.generate_detailed_sales_report(
            filters=filters, page=1, per_page=1000  # Max allowed in router
        )
    ...
    def test_database_timeout_handling(self, db_session: Session):
        ...
        with patch.object(db_session, "query") as mock_query:
            ...
            try:
                ...
                assert (
                    execution_time < 5.0
                ), "Should handle slow database within reasonable time"
