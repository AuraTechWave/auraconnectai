...
class TestAIQueryProcessor:
    ...
    def test_time_range_extraction(self, query_processor):
        ...
        result = query_processor.process_query(
            "Revenue between 2024-01-01 and 2024-01-31"
        )
        assert result.time_range is not None
    ...
    def test_query_enhancement_with_context(self, query_processor):
        ...
        query = query_processor.process_query(
            "How about for the same categories?", context
        )
...
class TestAIChatService:
    ...
    async def test_process_message_success(self, chat_service, mock_db):
        ...
        with (
            ...
            # Create request
            request = ChatRequest(
                message="Show me today's sales", session_id="test-session"
            )
    ...
    async def test_process_message_security_validation(self, chat_service, mock_db):
        ...
        with patch.object(
            ...
            request = ChatRequest(
                message="'; DROP TABLE users; --", session_id="test-session"
            )
    ...
    async def test_process_message_rate_limiting(self, chat_service, mock_db):
        ...
        with (
            ...
            request = ChatRequest(
                message="Show me complex forecast analysis", session_id="test-session"
            )
    ...
    async def test_session_management(self, chat_service):
        ...
        existing_session = chat_service._get_or_create_session(
            session.session_id, user_id=1
        )
    ...
    async def test_suggested_queries_generation(self, chat_service, mock_db):
        ...
        with patch(
            "backend.modules.analytics.services.ai_chat_service.realtime_metrics_service"
        ) as mock_service:
...
class TestAIResultFormatter:
    ...
    def test_chart_type_recommendation(self, formatter):
        ...
        query = AnalyticsQuery(
            original_text="Show trend", intent=QueryIntent.TREND_ANALYSIS
        )
    ...
    def test_summary_text_creation(self, formatter):
        ...
        query = AnalyticsQuery(
            original_text="Show revenue trend", intent=QueryIntent.REVENUE_ANALYSIS
        )
...
class TestWebSocketIntegration:
    ...
    async def test_websocket_message_format(self):
        ...
        message = WebSocketMessage(
            type="chat", data={"message": "Test message", "query_result": None}
        )
...
class TestEndToEndScenarios:
    ...
    async def test_sales_report_scenario(self):
        ...
        with (
            ...
            patch(
                "backend.modules.analytics.services.ai_chat_service.SalesReportService"
            ) as mock_sales,
        ):
    ...
    async def test_clarification_flow(self):
        ...
        with patch.object(
            chat_service.security_service, "validate_request", return_value=(True, None)
        ):
            # Ambiguous query
            ...
            with patch.object(
                chat_service.query_processor, "process_query"
            ) as mock_process:
                ...
                with patch.object(
                    chat_service.query_processor, "suggest_clarifications"
                ) as mock_clarify:
