...
class TestCachePerformance:
    ...
    async def test_concurrent_cache_access(
        self, widgets_service, sample_widget_configs
    ):
    ...
    def test_cache_thread_safety(self, widgets_service):
        ...
        for thread_id in range(num_threads):
            thread = threading.Thread(
                target=worker_thread, args=(thread_id, operations_per_thread)
            )
...
class TestCacheIntegrationBenchmarks:
    ...
    async def test_end_to_end_dashboard_performance(self):
        ...
        with (
            patch.object(
                widgets_service, "_process_metric_card", return_value=mock_data
            ),
            patch.object(
                widgets_service, "_process_line_chart", return_value=mock_data
            ),
