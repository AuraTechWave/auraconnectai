...
class TestAIInsightsService:
    ...
    @pytest.mark.asyncio
    async def test_generate_insights_with_cache(
        self, insights_service, sample_request, mock_db
    ):
    ...
    async def test_analyze_customer_patterns(self, insights_service, mock_db):
        ...
        mock_customer_data = [
            Mock(
                ...
                first_order=datetime(2025, 1, 1),
                last_order=datetime(2025, 1, 1),
            ),
            Mock(
                customer_id=2,
                order_count=5,
                ...
                first_order=datetime(2025, 1, 1),
                last_order=datetime(2025, 1, 15),
            ),
            Mock(
                customer_id=3,
                order_count=10,
                total_spent=Decimal("500"),
                first_order=datetime(2024, 12, 1),
                last_order=datetime(2025, 1, 20),
            ),
            Mock(
                customer_id=4,
                order_count=1,
                total_spent=Decimal("40"),
                first_order=datetime(2024, 11, 1),
                last_order=datetime(2024, 11, 1),
            ),  # At risk
        ]
    ...
    async def test_detect_anomalies(self, insights_service, mock_db):
        ...
        mock_daily_data = [
            ...
            Mock(
                date=date(2025, 1, 4), revenue=Decimal("3000"), order_count=60
            ),  # Anomaly - spike
            Mock(
                date=date(2025, 1, 5), revenue=Decimal("200"), order_count=4
            ),  # Anomaly - drop
    ...
    async def test_comprehensive_insights(
        self, insights_service, mock_db, sample_request
    ):
        ...
        with patch.object(insights_service, "_analyze_peak_times") as mock_peak:
            with patch.object(
                insights_service, "_analyze_product_trends"
            ) as mock_product:
                with patch.object(
                    insights_service, "_analyze_customer_patterns"
                ) as mock_customer:
                    with patch.object(
                        insights_service, "_detect_seasonality"
                    ) as mock_seasonality:
                        with patch.object(
                            insights_service, "_detect_anomalies"
                        ) as mock_anomalies:
                            with patch(
                                ...
                                assert (
                                    len(result.overall_recommendations) == 6
                                )  # 2 from each type
...
class TestAIInsightsRouter:
    ...
    async def test_generate_insights_endpoint(self, mock_current_user, mock_db):
        ...
        with patch(
            "backend.modules.staff.services.permission_service.has_permission"
        ) as mock_permission:
            with patch(
                "backend.modules.analytics.services.ai_insights_service.create_ai_insights_service"
            ) as mock_create:
                ...
                request = InsightRequest(
                    insight_types=[InsightType.PEAK_TIME], force_refresh=False
                )
