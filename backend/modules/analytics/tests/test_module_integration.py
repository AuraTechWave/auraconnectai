...
from ..integrations.module_hooks import (
    order_completed_hook, order_completed_sync, staff_action_hook, 
    customer_action_hook, payment_processed_hook, system_event_hook,
    invalidate_analytics_cache_hook, trigger_custom_alert_hook,
    get_analytics_status, force_dashboard_refresh
)
...
class TestModuleHooksIntegration:
    ...
    def mock_event_processor(self):
        ...
        with patch('backend.modules.analytics.integrations.module_hooks.event_processor') as mock:
    ...
    def mock_websocket_manager(self):
        ...
        with patch('backend.modules.analytics.integrations.module_hooks.websocket_manager') as mock:
    ...
    def mock_realtime_service(self):
        ...
        with patch('backend.modules.analytics.integrations.module_hooks.realtime_metrics_service') as mock:
    ...
    async def test_payment_processed_hook_integration(self, mock_event_processor):
        ...
        mock_event_processor.process_event.assert_called_once_with(
            mock_event_processor.EventType.PAYMENT_PROCESSED,
            {
                ...
                "status": status,
                "transaction_id": transaction_id,
                "processed_at": mock_event_processor.process_event.call_args[0][1]["processed_at"]
            },
    ...
    @pytest.mark.asyncio
    async def test_dashboard_refresh_hook_integration(self, mock_realtime_service, mock_websocket_manager):
        """Test dashboard refresh hook integration"""
        
        # Mock dashboard snapshot
        mock_snapshot = Mock()
        mock_realtime_service.get_current_dashboard_snapshot.return_value = mock_snapshot
        ...
        mock_websocket_manager.broadcast_dashboard_update.assert_called_once_with(mock_snapshot)
    
    def test_analytics_status_integration(self):
        """Test analytics status integration"""
        
        # Mock dependencies
        with patch('backend.modules.analytics.integrations.module_hooks.websocket_manager') as mock_ws, \
             patch('backend.modules.analytics.integrations.module_hooks.event_processor') as mock_ep:
...
class TestRealTimeFlowIntegration:
    ...
    async def test_order_to_dashboard_update_flow(self, mock_database):
        ...
        with patch('backend.core.database.get_db', return_value=iter([mock_database])), \
             patch('backend.modules.analytics.services.realtime_metrics_service.realtime_metrics_service') as mock_service, \
             patch('backend.modules.analytics.services.websocket_manager.websocket_manager') as mock_ws:
    ...
    async def test_concurrent_events_handling(self):
        ...
        with patch('backend.modules.analytics.integrations.module_hooks.event_processor') as mock_ep:
            ...
            # Order events
            for i in range(5):
                task = asyncio.create_task(order_completed_hook(
                    order_id=100 + i,
                    staff_id=1,
                    customer_id=None,
            ...
            # Staff events
            for i in range(3):
                task = asyncio.create_task(staff_action_hook(
                    staff_id=1,
                    action_type="order_processed",
            ...
            # Payment events
            for i in range(2):
                task = asyncio.create_task(payment_processed_hook(
                    order_id=300 + i,
                    payment_method="card",
    ...
    async def test_error_resilience_in_integration(self):
        ...
        with patch('backend.modules.analytics.integrations.module_hooks.websocket_manager') as mock_ws:
            mock_ws.broadcast_alert_notification.side_effect = Exception("Broadcast failed")
...
class TestCacheIntegration:
    ...
    async def test_cache_invalidation_propagation(self):
        ...
        with patch('backend.modules.analytics.services.realtime_metrics_service.realtime_metrics_service') as mock_service:
            mock_service.invalidate_cache = AsyncMock()
