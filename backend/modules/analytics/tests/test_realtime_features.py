...
class TestRealtimeMetricsService:
    ...
    async def test_daily_metrics_calculation(self, metrics_service, mock_db_session):
        ...
        with patch(
            "backend.core.database.get_db", return_value=iter([mock_db_session])
        ):
            metrics = await metrics_service._get_daily_metrics(
                mock_db_session, date.today()
            )
...
class TestWebSocketManager:
    ...
    async def test_client_connection(self, websocket_manager, mock_websocket):
        ...
        client_id = await websocket_manager.connect_client(
            websocket=mock_websocket, user_id=1, user_permissions=user_permissions
        )
...
class TestEventProcessor:
    ...
    def test_event_handler_registration(self, event_processor):
        ...
        # Unregister handler
        event_processor.unregister_event_handler(
            EventType.ORDER_COMPLETED, test_handler
        )

        # Verify handler is removed
        assert (
            test_handler
            not in event_processor.event_handlers[EventType.ORDER_COMPLETED]
        )
    ...
    async def test_event_processing(self, event_processor):
        ...
        success = await event_processor.process_event(
            EventType.ORDER_COMPLETED, test_event_data, priority=True
        )
    ...
    def test_rate_limiting(self, event_processor):
        ...
        event_processor.event_counts[EventType.ORDER_COMPLETED.value] = (
            event_processor.max_events_per_window
        )
...
class TestDashboardWidgetsService:
    ...
    def sample_widget_config(self):
        ...
        return WidgetConfiguration(
            ...
            config={"metric_name": "revenue_current", "format": "currency"},
        )
    ...
    async def test_metric_card_processing(self, widgets_service, sample_widget_config):
        ...
        with patch(
            "backend.modules.analytics.services.dashboard_widgets_service.realtime_metrics_service.get_realtime_metric",
            return_value=mock_metric,
        ):

            widget_data = await widgets_service._process_metric_card(
    ...
    async def test_widget_data_caching(self, widgets_service, sample_widget_config):
        ...
        # Mock the realtime metrics service
        mock_metric = RealtimeMetric(
            metric_name="revenue_current", value=1500.0, timestamp=datetime.now()
        )
    ...
    async def test_line_chart_processing(self, widgets_service):
        ...
        widget_config = WidgetConfiguration(
            ...
            config={"metric": "revenue", "hours_back": 12},
        )
        ...
        with patch(
            "backend.modules.analytics.services.dashboard_widgets_service.realtime_metrics_service.get_hourly_trends",
...
class TestRealtimeIntegration:
    ...
    def test_permission_integration(self):
        ...
        user_permissions = ["analytics:view_dashboard", "analytics:view_sales_reports"]
