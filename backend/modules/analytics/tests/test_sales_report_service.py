...
class TestSalesReportService:
    ...
    def test_generate_sales_summary_with_growth(
        self, 
        db_session, 
        sample_orders
    ):
        ...
        assert summary.revenue_growth is None or isinstance(summary.revenue_growth, Decimal)
    ...
    def test_generate_detailed_sales_report_sorting(
        ...
        if len(report_desc.items) > 1:
            assert (report_desc.items[0].total_revenue >= 
                   report_desc.items[1].total_revenue)
        
        if len(report_asc.items) > 1:
            assert (report_asc.items[0].total_revenue <= 
                   report_asc.items[1].total_revenue)
...
class TestSalesCalculationResult:
    ...
    def test_with_customer_breakdown(self):
        ...
        assert result.new_customers + result.returning_customers == result.unique_customers