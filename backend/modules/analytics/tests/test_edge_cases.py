...
class TestLargeDatasets:
    ...
    def test_top_performers_with_large_dataset(
        self, db_session: Session, large_dataset
    ):
        ...
        for i in range(len(performers) - 1):
            assert (
                performers[i].total_revenue_generated
                >= performers[i + 1].total_revenue_generated
            )
...
class TestExportEndpoints:
    ...
    def sample_report_data(self, db_session: Session):
        ...
        # Create staff
        staff = StaffMember(
            name="Test Staff", email="test@example.com", role="server", is_active=True
        )
        db_session.add(staff)
        db_session.flush()
    ...
    async def test_csv_export(self, db_session: Session, sample_report_data):
        ...
        response = await export_service.export_sales_report(
            request=request, format_type="csv", executed_by=sample_report_data.id
        )
    ...
    async def test_pdf_export(self, db_session: Session, sample_report_data):
        ...
        try:
            response = await export_service.export_sales_report(
                request=request, format_type="pdf", executed_by=sample_report_data.id
            )
    ...
    async def test_excel_export(self, db_session: Session, sample_report_data):
        ...
        try:
            response = await export_service.export_sales_report(
                request=request, format_type="xlsx", executed_by=sample_report_data.id
            )
...
class TestTrendEndpoints:
    ...
    def test_hourly_trend(self, db_session: Session, trend_data):
        ...
        trend_points = trend_service.get_revenue_trend(
            start_date=today, end_date=today, granularity="hourly"
        )
    ...
    def test_weekly_trend(self, db_session: Session, trend_data):
        ...
        trend_points = trend_service.get_revenue_trend(
            start_date=start_date, end_date=end_date, granularity="weekly"
        )
