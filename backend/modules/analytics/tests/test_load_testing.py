...
class TestAnalyticsLoadTesting:
    ...
    def test_concurrent_sales_report_generation(self, db_session: Session, load_test_data):
        ...
        for num_threads in concurrent_levels:
            ...
            with ThreadPoolExecutor(max_workers=num_threads) as executor:
                futures = [executor.submit(generate_report, i) for i in range(num_threads)]
            ...
            assert success_rate >= 95.0, f"Success rate {success_rate:.1f}% too low with {num_threads} threads"
            assert avg_response_time < 3.0, f"Average response time {avg_response_time:.3f}s too high with {num_threads} threads"
            
            if failed_requests:
                print("Failed request errors:", [r["error"] for r in failed_requests[:3]])

    def test_concurrent_detailed_report_generation(self, db_session: Session, load_test_data):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [executor.submit(generate_detailed_report, i) for i in range(num_threads)]
            results = [future.result() for future in as_completed(futures)]
        ...
        assert success_rate >= 90.0, f"Detailed report success rate {success_rate:.1f}% too low"
        assert avg_response_time < 5.0, f"Detailed report average response time {avg_response_time:.3f}s too high"
    ...
    def test_mixed_workload_simulation(self, db_session: Session, load_test_data):
        ...
        def mixed_operation(thread_id: int) -> Dict[str, Any]:
            ...
            try:
                if operation == "sales_summary":
                    ...
                    result = service.generate_detailed_sales_report(filters, page=1, per_page=100)
                elif operation == "staff_performance":
                    result = service.generate_staff_performance_report(filters, page=1, per_page=50)
                else:  # product_performance
                    result = service.generate_product_performance_report(filters, page=1, per_page=50)
        ...
        for op, stats in operation_stats.items():
            ...
            print(f"{op}: {total_ops} requests, {success_rate:.1f}% success, {avg_time:.3f}s avg")
            
            # Assert reasonable performance for each operation type
            assert success_rate >= 85.0, f"{op} success rate {success_rate:.1f}% too low"
    ...
    def test_async_task_processor_load(self):
        ...
        async def run_load_test():
            ...
            assert submission_time < 5.0, f"Task submission took {submission_time:.2f}s, should be under 5s"
            ...
            while completed_tasks < num_tasks and (time.time() - completion_start) < 60:
                ...
                completed_tasks = sum(
                    ...
                    if processor.get_task_status(task_id) and 
                    processor.get_task_status(task_id).status.value in ["completed", "failed"]
                )
            ...
            # Performance assertions
            assert completion_rate >= 95.0, f"Task completion rate {completion_rate:.1f}% too low"
            assert completion_time < 45.0, f"Task completion took {completion_time:.2f}s, should be under 45s"
    ...
    def test_database_connection_pool_stress(self, db_session: Session, load_test_data):
            ...
            try:
                for i in range(operations_per_thread):
                    ...
                    if i % 3 == 0:
                        ...
                        service.generate_staff_performance_report(filters, page=1, per_page=20)
                    else:
                        filters = SalesFilterRequest(
                        ...
                        service.generate_product_performance_report(filters, page=1, per_page=20)
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [executor.submit(database_intensive_operation, i) for i in range(num_threads)]
            results = [future.result() for future in as_completed(futures)]
        ...
        total_operations = sum(r.get("operations_completed", 0) for r in successful_threads)
        avg_thread_time = sum(r["execution_time"] for r in successful_threads) / len(successful_threads) if successful_threads else 0
        ...
        assert success_rate >= 85.0, f"Database stress test success rate {success_rate:.1f}% too low"
        assert avg_thread_time < 15.0, f"Average thread time {avg_thread_time:.2f}s too high under stress"
    ...
    def test_memory_usage_under_load(self, db_session: Session, load_test_data):
        ...
        def sustained_load():
            ...
            for i in range(50):  # 50 operations
                ...
                if i % 3 == 0:
                    ...
                    service.generate_detailed_sales_report(filters, page=1, per_page=100)
                else:
                    service.generate_staff_performance_report(filters, page=1, per_page=50)
        ...
        assert peak_memory_increase < 100, f"Peak memory increase {peak_memory_increase:.1f} MB too high"
        ...
        assert memory_recovery > peak_memory_increase * 0.5, "Poor memory recovery after GC"