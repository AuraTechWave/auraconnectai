...
class TestWebSocketEndToEnd:
    ...
    async def test_dashboard_update_broadcast(self, websocket_manager, mock_websocket):
        ...
        snapshot = DashboardSnapshot(
            ...
            hourly_trends=[
                {"hour": "2024-01-01 09:00", "revenue": 100, "orders": 3, "customers": 3},
                {"hour": "2024-01-01 10:00", "revenue": 150, "orders": 5, "customers": 4}
            ],
            active_alerts=1,
    ...
    @pytest.mark.asyncio
    async def test_alert_notification_broadcast(self, websocket_manager, mock_websocket):
    ...
    async def test_permission_based_message_filtering(self, websocket_manager):
        ...
        admin_client_id = await websocket_manager.connect_client(
            ...
            user_id=1,
            user_permissions=["analytics:view_dashboard", "analytics:admin", "analytics:receive_alerts"]
        )
...
class TestRealTimeEventFlow:
    ...
    async def test_order_to_websocket_flow(self):
        ...
        with patch('backend.modules.analytics.integrations.module_hooks.realtime_metrics_service') as mock_service, \
             patch('backend.modules.analytics.integrations.module_hooks.websocket_manager', websocket_manager), \
             patch('backend.modules.analytics.integrations.module_hooks.event_processor') as mock_processor:
            ...
            mock_service.get_current_dashboard_snapshot = AsyncMock(return_value=mock_snapshot)
    ...
    async def test_concurrent_client_handling(self):
        ...
        for mock_ws in mock_websockets:
            dashboard_updates = sum(
                1 for msg_str in mock_ws.messages_sent
...
class TestWebSocketErrorHandling:
    ...
    async def test_invalid_message_handling(self):
        ...
        try:
            await websocket_manager.handle_client_message(
                client_id, 
                json.dumps({"type": "subscribe"})  # Missing data field
            )
            # Should not raise exception
            assert True
        except Exception:
    ...
    async def test_client_disconnection_during_broadcast(self):
        ...
        mock_websocket.send_text = AsyncMock(side_effect=ConnectionResetError("Client disconnected"))
