...
class TestAIInsightsIntegration:
    ...
    def sample_data(self, test_db):
        ...
        category = Category(id=1, name="Main Dishes", description="Main course items")
    ...
    async def test_time_range_filtering(self, test_db, sample_data):
        ...
        assert (
            result_30days.peak_times.confidence.value
            >= result_7days.peak_times.confidence.value
        )
    ...
    async def test_product_trend_detection(self, test_db, sample_data):
        ...
        rising_product_names = [
            p.product_name for p in result.product_insights.top_rising
        ]
