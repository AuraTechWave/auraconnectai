...
def sample_category(db_session: Session):
    ...
    category = Category(name="Beverages", description="Hot and cold beverages")
...
@pytest.fixture
def sample_orders(
    db_session: Session, sample_staff_member, sample_customer, sample_category
):
...
def sample_report_template(db_session: Session, sample_staff_member):
    ...
    template = ReportTemplate(
        ...
        columns_config={"columns": ["date", "revenue", "orders", "avg_order_value"]},
        sorting_config={"sort_by": "date", "sort_order": "desc"},
...
@pytest.fixture
def sample_report_execution(
    db_session: Session, sample_report_template, sample_staff_member
):
...
def mock_limited_user():
    ...
    return {
        ...
        "is_admin": False,
        "analytics_permissions": ["analytics:view_dashboard"],
    }
