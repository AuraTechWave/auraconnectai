...
class TaxFilingAutomationService:
            ...
            for nexus in nexus_list:
                try:
                    ...
                    if self._is_filing_due(nexus, frequency):
                        ...
                        task_id = await self._schedule_filing_task(nexus, tenant_id)
    ...
    async def generate_automated_filing(
        self, nexus_id: int, tenant_id: Optional[int] = None, user_id: str = "system"
    ) -> TaxFiling:
        ...
        try:
            ...
            transaction_data = await self._collect_transaction_data(
                nexus.jurisdiction_id, period_start, period_end, tenant_id
            )
            ...
            filing = (
                self.db.query(TaxFiling)
                .filter(TaxFiling.id == filing_response.id)
                .first()
            )
    ...
    async def auto_submit_ready_filings(
        self, tenant_id: Optional[int] = None, dry_run: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for filing in ready_filings:
                try:
                    ...
                    if not self._is_auto_submit_enabled(
                        filing.jurisdiction_id, tenant_id
                    ):
                        continue
                    ...
                    if not dry_run:
                        ...
                        if submission_result["success"]:
                            ...
                            self.compliance_service.submit_filing(
                                filing.id, submit_data, "system", tenant_id
                            )
                    ...
                    errors.append({"filing_id": filing.id, "error": str(e)})
    ...
    async def reconcile_tax_accounts(
        ...
        try:
            ...
            for filing in filings:
                ...
                discrepancies = self._calculate_discrepancies(filing, actual_data)
    ...
    async def generate_estimated_payments(
        self, tenant_id: Optional[int] = None, tax_year: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for jurisdiction in jurisdictions:
                ...
                if jurisdiction.filing_frequency == "quarterly":
                    ...
                    for month in range(1, 13):
                        due_date = date(
                            tax_year, month, jurisdiction.filing_due_day or 15
                        )
            ...
            return {
                ...
                "payment_schedule": sorted(
                    payment_schedule, key=lambda x: x["due_date"]
                ),
            }
    ...
    # Helper methods
    def _is_filing_due(self, nexus: TaxNexus, frequency: AutomationFrequency) -> bool:
        ...
        if frequency == AutomationFrequency.DAILY:
            ...
            return (
                nexus.next_filing_date <= week_ahead if nexus.next_filing_date else True
            )
    ...
    async def _schedule_filing_task(
        self, nexus: TaxNexus, tenant_id: Optional[int]
    ) -> Optional[str]:
        ...
        try:
            ...
            task = asyncio.create_task(
                self.generate_automated_filing(nexus.id, tenant_id, "system")
            )
    ...
    def _calculate_next_filing_date(self, nexus: TaxNexus) -> date:
        ...
        if nexus.filing_frequency == "monthly":
            ...
            return date(
                year, next_quarter_month, nexus.jurisdiction.filing_due_day or 20
            )
    ...
    async def _validate_filing_data(self, filing: TaxFiling) -> Dict[str, Any]:
        ...
        line_items_total = sum(item.tax_amount for item in filing.line_items)
    ...
    def _is_auto_submit_enabled(
        self, jurisdiction_id: int, tenant_id: Optional[int]
    ) -> bool:
    ...
    async def _submit_to_tax_authority(
        self, filing: TaxFiling, submit_data: TaxFilingSubmit
    ) -> Dict[str, Any]:
    ...
    def _calculate_discrepancies(
        self, filing: TaxFiling, actual_data: Dict[str, Decimal]
    ) -> Dict[str, Any]:
        ...
        for filing_field, data_field in fields_to_compare:
            ...
            if discrepancy != Decimal("0"):
                discrepancies[filing_field] = {
                    ...
                    "percentage": float(
                        (discrepancy / filed_amount * 100) if filed_amount > 0 else 0
                    ),
                }
    ...
    def _determine_reconciliation_action(self, discrepancies: Dict[str, Any]) -> str:
    ...
    def _calculate_previous_year_tax(
        self, jurisdiction_id: int, year: int, tenant_id: Optional[int]
    ) -> Decimal:
        ...
        filings = (
            ...
            .filter(
                ...
                TaxFiling.status.in_(
                    [FilingStatus.SUBMITTED, FilingStatus.ACCEPTED, FilingStatus.PAID]
                ),
            )
            .all()
        )
    ...
    def _get_estimated_payment_due_date(self, year: int, quarter: int) -> date:
