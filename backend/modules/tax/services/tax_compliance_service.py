...
class TaxComplianceService:
        ...
        try:
            ...
            for item_data in filing_data.line_items:
                line_item = TaxFilingLineItem(
                    filing_id=filing.id, **item_data.model_dump()
                )
    ...
    def update_filing(
        ...
        try:
            ...
            if filing.status not in [FilingStatus.DRAFT, FilingStatus.READY]:
                raise ValueError(f"Cannot update filing with status {filing.status}")
            ...
            for field, value in update_data.model_dump(exclude_unset=True).items():
                if hasattr(filing, field) and getattr(filing, field) != value:
                    changes[field] = {"before": getattr(filing, field), "after": value}
    ...
    def get_filing(
        self, filing_id: int, tenant_id: Optional[int] = None
    ) -> TaxFilingResponse:
    ...
    def list_filings(
        ...
        filings = (
            query.order_by(TaxFiling.due_date.desc()).limit(limit).offset(offset).all()
        )
    ...
    def get_compliance_dashboard(
        self, tenant_id: Optional[int] = None, as_of_date: Optional[date] = None
    ) -> TaxComplianceDashboard:
        ...
        recent = (
            ...
            .order_by(TaxFiling.filed_date.desc())
            .limit(10)
            .all()
        )
        ...
        compliant_jurisdictions = sum(
            1 for s in compliance_statuses if s.current_period_status == "compliant"
        )
        at_risk_jurisdictions = sum(
            1 for s in compliance_statuses if s.risk_level in ["high", "critical"]
        )
        ...
        overall_risk_level = self._calculate_overall_risk_level(compliance_statuses)
    ...
    def generate_report(
        ...
        try:
            ...
            if report_request.template_id:
                template = (
                    self.db.query(TaxReportTemplate)
                    .filter(
            ...
            if report_request.output_format != "json":
                download_url = self._generate_report_file(
                    report_id, report_request.output_format, summary, details, trends
                )
            ...
            return TaxReportResponse(
                ...
                expires_at=(
                    datetime.utcnow() + timedelta(days=7) if download_url else None
                ),
            )
    ...
    def _calculate_filing_totals(self, filing: TaxFiling):
        ...
        # Sum line items
        line_items = (
            self.db.query(TaxFilingLineItem)
            .filter(TaxFilingLineItem.filing_id == filing.id)
            .all()
        )
        ...
        # Calculate total due
        filing.total_due = filing.tax_due + filing.penalties + filing.interest
    ...
    def _analyze_jurisdiction_compliance(
        self, jurisdiction: TaxJurisdiction, as_of_date: date, tenant_id: Optional[int]
    ) -> TaxComplianceStatus:
        ...
        total_paid = sum(f.total_due for f in filings if f.status == FilingStatus.PAID)
        ...
        current_filing = next(
            (f for f in filings if f.period_start >= current_period_start), None
        )
    ...
    def _estimate_penalties(self, filing: TaxFiling, as_of_date: date) -> Decimal:
    ...
    def _calculate_overall_risk_level(
        self, compliance_statuses: List[TaxComplianceStatus]
    ) -> str:
        ...
        critical_count = sum(
            1 for s in compliance_statuses if s.risk_level == "critical"
        )
    ...
    def _generate_compliance_recommendations(
        ...
        critical_deadlines = [d for d in upcoming_deadlines if d["days_until_due"] <= 7]
        ...
        high_risk_jurisdictions = [
            s for s in compliance_statuses if s.risk_level in ["high", "critical"]
        ]
    ...
    def _generate_report_summary(self, filings: List[TaxFiling]) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_paid": sum(
                f.total_due for f in filings if f.status == FilingStatus.PAID
            ),
        }

    def _generate_report_details(
        self, filings: List[TaxFiling], request: TaxReportRequest
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_report_trends(
        self, filings: List[TaxFiling], tenant_id: Optional[int]
    ) -> Dict[str, Any]:
        """Generate trend analysis"""
        # Group filings by month
        monthly_data = defaultdict(
            lambda: {"count": 0, "tax_due": Decimal("0"), "total_due": Decimal("0")}
        )
    ...
    def _calculate_year_over_year(self, filings: List[TaxFiling]) -> Dict[str, Any]:
        """Calculate year-over-year trends"""
        current_year = date.today().year
        last_year = current_year - 1

        current_year_filings = [f for f in filings if f.period_end.year == current_year]

        last_year_filings = [f for f in filings if f.period_end.year == last_year]
    ...
    def _calculate_compliance_trends(self, filings: List[TaxFiling]) -> Dict[str, Any]:
        """Calculate compliance trends"""
        on_time_count = sum(
            1 for f in filings if f.filed_date and f.filed_date.date() <= f.due_date
        )
        ...
        pending_count = sum(
            1 for f in filings if f.status in [FilingStatus.DRAFT, FilingStatus.READY]
        )
