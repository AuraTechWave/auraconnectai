...
class TaxProviderInterface(ABC):
    ...
    async def calculate_tax(
        self, request: EnhancedTaxCalculationRequest
    ) -> EnhancedTaxCalculationResponse:
    ...
    async def get_tax_rates(
        self, location: TaxCalculationLocation, tax_date: date
    ) -> List[Dict[str, Any]]:
        """Get tax rates for a location"""
        pass

    @abstractmethod
    async def file_return(self, filing_data: Dict[str, Any]) -> Dict[str, Any]:
        """File a tax return"""
        pass
...
class AvalaraTaxProvider(TaxProviderInterface):
    ...
    async def calculate_tax(
        ...
        try:
            ...
            avatax_request = {
                ...
                "customerCode": (
                    str(request.customer_id) if request.customer_id else "GUEST"
                ),
            ...
            response = await self.client.post(
                "/api/v2/transactions/create", json=avatax_request
            )
    ...
    async def validate_address(self, address: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "standardized": (
                    result.get("validatedAddresses", [{}])[0]
                    if result.get("validatedAddresses")
                    else None
                ),
    ...
    def _convert_avatax_response(
        self, avatax_response: Dict[str, Any], request: EnhancedTaxCalculationRequest
    ) -> EnhancedTaxCalculationResponse:
        ...
        for line in avatax_response.get("lines", []):
            ...
            line_results.append(
                TaxCalculationResult(
                    ...
                    effective_rate=(
                        Decimal(str(line["tax"]))
                        / Decimal(str(line["lineAmount"]))
                        * 100
                        if Decimal(str(line["lineAmount"])) > 0
                        else Decimal("0")
                    ),
                )
            )
...
class TaxJarProvider(TaxProviderInterface):
    ...
    async def calculate_tax(
        ...
        try:
            ...
            taxjar_request = {
                ...
                "amount": float(
                    sum(item.amount * item.quantity for item in request.line_items)
                ),
                "shipping": (
                    float(request.shipping_amount) if request.shipping_amount else 0
                ),
                "line_items": [],
            }
            ...
            # Call TaxJar API
            response = await self.client.post("/taxes", json=taxjar_request)
    ...
    async def validate_address(self, address: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "valid": True,
                "standardized": (
                    result["addresses"][0] if result.get("addresses") else None
                ),
                "messages": [],
            }
    ...
    def _convert_taxjar_response(
        self, taxjar_response: Dict[str, Any], request: EnhancedTaxCalculationRequest
    ) -> EnhancedTaxCalculationResponse:
        ...
        if taxjar_response.get("state_tax_collectable", 0) > 0:
            tax_details.append(
                {
                    "jurisdiction_name": taxjar_response.get("jurisdictions", {}).get(
                        "state", "State"
                    ),
                    "jurisdiction_type": "state",
                    "tax_type": "sales",
                    "rate": Decimal(str(taxjar_response.get("state_tax_rate", 0)))
                    * 100,
                    "tax_amount": Decimal(
                        str(taxjar_response["state_tax_collectable"])
                    ),
                }
            )
            ...
            tax_details.append(
                {
                    ...
                    "jurisdiction_type": "county",
                    "tax_type": "sales",
                    "rate": Decimal(str(taxjar_response.get("county_tax_rate", 0)))
                    * 100,
                    "tax_amount": Decimal(
                        str(taxjar_response["county_tax_collectable"])
                    ),
                }
            )
            ...
            tax_details.append(
                {
                    ...
                    "jurisdiction_type": "special",
                    "tax_type": "sales",
                    "rate": Decimal(str(taxjar_response.get("special_tax_rate", 0)))
                    * 100,
                    "tax_amount": Decimal(
                        str(taxjar_response["special_district_tax_collectable"])
                    ),
                }
            )
        ...
        for item in request.line_items:
            ...
            item_proportion = (
                item_amount / total_amount if total_amount > 0 else Decimal("0")
            )
            ...
            for detail in tax_details:
                ...
                item_tax_details.append({**detail, "tax_amount": item_tax})
            ...
            line_results.append(
                TaxCalculationResult(
                    ...
                    effective_rate=(
                        item_total_tax / item_amount * 100
                        if item_amount > 0
                        else Decimal("0")
                    ),
                )
...
class TaxIntegrationService:
    ...
    async def calculate_tax(
        self, request: EnhancedTaxCalculationRequest, provider: Optional[str] = None
    ) -> EnhancedTaxCalculationResponse:
        ...
        if provider_name == "internal":
            ...
            raise NotImplementedError(
                "Use TaxCalculationEngine for internal calculations"
            )
    ...
    async def validate_address(
        self, address: Dict[str, str], provider: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def sync_tax_rates(
        self, provider: str, locations: List[TaxCalculationLocation]
    ) -> Dict[str, Any]:
