...
class TaxJurisdictionBase(BaseModel):
    ...
    filing_frequency: Optional[str] = Field(None, pattern="^(monthly|quarterly|annually)$")
...
class TaxRateBase(BaseModel):
    ...
    calculation_method: str = Field(default="percentage", pattern="^(percentage|flat|tiered)$")
    
    @field_validator('max_amount')
    def validate_max_amount(cls, v, values):
        if v is not None and 'min_amount' in values.data and values.data['min_amount'] is not None:
...
class TaxNexusBase(BaseModel):
    ...
    nexus_type: str = Field(..., pattern="^(physical|economic|affiliate|click_through)$")
    ...
    threshold_period: Optional[str] = Field(None, pattern="^(annual|quarterly|monthly)$")
