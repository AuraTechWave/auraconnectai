...
from ..schemas import (
    TaxJurisdictionCreate, TaxJurisdictionUpdate, TaxJurisdictionResponse,
    TaxRateCreate, TaxRateUpdate, TaxRateResponse,
    TaxNexusCreate, TaxNexusUpdate, TaxNexusResponse
)
...
async def create_jurisdiction(
    ...
    try:
        # Check for duplicate
        existing = db.query(TaxJurisdiction).filter(
            TaxJurisdiction.code == jurisdiction_data.code,
            TaxJurisdiction.jurisdiction_type == jurisdiction_data.jurisdiction_type,
...
async def list_jurisdictions(
    ...
    query = db.query(TaxJurisdiction).filter(
        TaxJurisdiction.tenant_id == tenant_id
    )
...
async def list_jurisdiction_rates(
    ...
    if effective_date:
        query = query.filter(
            ...
            db.or_(
                TaxRate.expiry_date.is_(None),
                TaxRate.expiry_date >= effective_date
            )
        )
...
async def create_nexus(
    ...
    existing = db.query(TaxNexus).filter(
...
async def list_all_nexus(
    ...
    query = db.query(TaxNexus).filter(
        TaxNexus.tenant_id == tenant_id
    )
...
async def update_nexus(
    ...
    """Update tax nexus"""
    nexus = db.query(TaxNexus).filter(
    ...
    if not nexus:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Nexus {nexus_id} not found"
        )
...
async def import_jurisdictions(
    ...
    return {
        "message": "Jurisdiction import not yet implemented",
        "file_url": file_url
    }
