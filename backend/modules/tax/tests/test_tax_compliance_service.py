...
class TestTaxComplianceService:
    ...
    def test_submit_filing(self, test_db, sample_jurisdiction):
        ...
        service.update_filing(
            filing.id, TaxFilingUpdate(status=FilingStatus.READY), "test_user", 1
        )

        # Submit filing
        submit_data = TaxFilingSubmit(
            prepared_by="John Doe", reviewed_by="Jane Smith", approved_by="Bob Johnson"
        )
    ...
    def test_duplicate_filing_prevention(self, test_db, sample_jurisdiction):
        ...
        # Mark as submitted
        service.update_filing(
            filing1.id, TaxFilingUpdate(status=FilingStatus.SUBMITTED), "test_user", 1
        )
