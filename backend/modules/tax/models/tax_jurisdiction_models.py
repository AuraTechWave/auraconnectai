...
class TaxJurisdiction(Base, TimestampMixin, TenantMixin):
    ...
    parent_jurisdiction_id = Column(
        Integer, ForeignKey("tax_jurisdictions.id"), nullable=True
    )
    ...
    tax_rates = relationship(
        "TaxRate", back_populates="jurisdiction", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "jurisdiction_type",
            "tenant_id",
            name="uq_jurisdiction_code_type_tenant",
        ),
...
class TaxRate(Base, TimestampMixin, TenantMixin):
    ...
    jurisdiction_id = Column(
        Integer, ForeignKey("tax_jurisdictions.id"), nullable=False
    )

    # Tax type and category
    tax_type = Column(
        String(50), nullable=False
    )  # sales, income, payroll, property, excise
    tax_subtype = Column(
        String(50), nullable=True
    )  # e.g., state_income, federal_income
    tax_category = Column(
        String(100), nullable=True
    )  # e.g., prepared_food, general_merchandise
...
class TaxRuleConfiguration(Base, TimestampMixin, TenantMixin):
    ...
    )  # exemption, holiday, threshold, nexus
...
class TaxExemptionCertificate(Base, TimestampMixin, TenantMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "certificate_number", "tenant_id", name="uq_certificate_number_tenant"
        ),
...
class TaxNexus(Base, TimestampMixin, TenantMixin):
    ...
    nexus_type = Column(
        String(50), nullable=False
    )  # physical, economic, affiliate, click_through
