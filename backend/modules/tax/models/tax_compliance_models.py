...
class TaxFiling(Base, TimestampMixin, TenantMixin):
    ...
    line_items = relationship(
        "TaxFilingLineItem", back_populates="filing", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_filing_jurisdiction_period",
            "jurisdiction_id",
            "period_start",
            "period_end",
        ),
        UniqueConstraint(
            "internal_reference", "tenant_id", name="uq_filing_reference_tenant"
        ),
    )
...
class TaxFilingLineItem(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "taxable_amount >= 0", name="check_taxable_amount_non_negative"
        ),
...
class TaxRemittance(Base, TimestampMixin, TenantMixin):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "payment_reference", "tenant_id", name="uq_payment_reference_tenant"
        ),
    )
...
class TaxAuditLog(Base, TimestampMixin, TenantMixin):
    ...
    )  # calculation, filing, payment, adjustment
    ...
    )  # order, filing, payment, certificate
...
class TaxReportTemplate(Base, TimestampMixin, TenantMixin):
    ...
    report_type = Column(
        String(100), nullable=False
    )  # filing_form, summary_report, detail_report
    filing_type = Column(
        Enum(
            FilingType,
            values_callable=lambda obj: [e.value for e in obj],
