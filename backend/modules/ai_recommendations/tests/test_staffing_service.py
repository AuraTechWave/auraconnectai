...
class TestStaffingRecommendationService:
    ...
    @pytest.mark.asyncio
    async def test_generate_staffing_recommendations(
        self, service, mock_db, sample_request
    ):
        ...
        with patch.object(service, "_generate_demand_forecasts") as mock_forecasts:
            mock_forecasts.return_value = {
                date(2025, 2, 1): [
                    Mock(
                        predicted_orders=30,
                        predicted_revenue=450,
                        predicted_customers=25,
                    )
                    for _ in range(24)
                ]
            }
            ...
            with patch.object(service, "_identify_staffing_patterns") as mock_patterns:
                ...
                with patch("backend.core.cache.cache_service.get") as mock_cache_get:
                    ...
                    with patch(
                        ...
                        with patch.object(
                            service, "_generate_daily_recommendation"
                        ) as mock_daily:
                            ...
                            result = await service.generate_staffing_recommendations(
                                sample_request
                            )
    ...
    def test_apply_forecast_adjustments(self, service):
        ...
        base_forecast = {"avg_orders": 40, "avg_revenue": 600, "avg_customers": 35}

        # Regular day
        forecast = service._apply_forecast_adjustments(
            base_forecast, date(2025, 2, 3), 12  # Monday
        )
        ...
        holiday_forecast = service._apply_forecast_adjustments(
            base_forecast, date(2025, 7, 4), 12  # July 4th
        )
    ...
    def test_assess_staffing_level(self, service):
        ...
        # Optimal staffing
        gap_optimal = {StaffRole.CHEF: 0, StaffRole.SERVER: 1, StaffRole.DISHWASHER: 0}
        assert service._assess_staffing_level(gap_optimal) == StaffingLevel.OPTIMAL

        # Understaffed
        gap_under = {StaffRole.CHEF: 2, StaffRole.SERVER: 3, StaffRole.DISHWASHER: 1}
        ...
        assert (
            service._assess_staffing_level(gap_severe)
            == StaffingLevel.SEVERELY_UNDERSTAFFED
        )

        # Overstaffed
        gap_over = {StaffRole.CHEF: -1, StaffRole.SERVER: -2, StaffRole.DISHWASHER: -1}
    ...
    def test_calculate_labor_cost(self, service):
        ...
        requirements = [
            StaffRequirement(
                role=StaffRole.CHEF, min_required=1, optimal=2, max_useful=3
            ),
            StaffRequirement(
                role=StaffRole.SERVER, min_required=2, optimal=4, max_useful=6
            ),
            StaffRequirement(
                role=StaffRole.DISHWASHER, min_required=1, optimal=1, max_useful=2
            ),
        ]
    ...
    async def test_identify_staffing_patterns(self, service, mock_db):
        ...
        with patch.object(
            service.insights_service, "generate_insights"
        ) as mock_insights:
            mock_insights_result = Mock()
            mock_insights_result.peak_times = Mock(
                primary_peak=Mock(hour=12), secondary_peak=Mock(hour=18)
            )
...
class TestStaffingSchemaValidation:
    ...
    def test_staffing_optimization_request_validation(self):
        ...
        with pytest.raises(ValueError):
            StaffingOptimizationRequest(
                start_date=date(2025, 2, 7), end_date=date(2025, 2, 1)
            )

        # Invalid date range (too long)
        with pytest.raises(ValueError):
            StaffingOptimizationRequest(
                start_date=date(2025, 1, 1), end_date=date(2025, 5, 1)  # > 90 days
            )
