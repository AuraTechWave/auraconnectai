...
from ..schemas.pricing_schemas import (
    ...
    DemandLevel, PriceElasticity, MenuItemPricingContext
)
from ..services.pricing_recommendation_service import (
    PricingRecommendationService, create_pricing_recommendation_service
)
...
class TestPricingRecommendationService:
    ...
    @pytest.mark.asyncio
    async def test_generate_bulk_recommendations(self, service, mock_db, sample_request, sample_menu_item):
            ...
            with patch.object(service, '_build_pricing_context') as mock_context:
                ...
                with patch('backend.core.cache.cache_service.get') as mock_cache_get:
                    ...
                    with patch('backend.core.cache.cache_service.set') as mock_cache_set:
                        result = await service.generate_bulk_recommendations(sample_request)
    ...
    def test_calculate_sales_trend(self, service, mock_db):
        ...
        # Mock recent sales
        mock_db.query.return_value.join.return_value.filter.return_value.scalar.side_effect = [50, 40]
    ...
    def test_estimate_price_elasticity(self, service):
        ...
        assert service._estimate_price_elasticity(low_price_item, Mock()) == PriceElasticity.ELASTIC
        
        # High price item
        high_price_item = Mock(price=35.99)
        assert service._estimate_price_elasticity(high_price_item, Mock()) == PriceElasticity.INELASTIC
        
        # Medium price item
        medium_price_item = Mock(price=18.99)
        assert service._estimate_price_elasticity(medium_price_item, Mock()) == PriceElasticity.UNIT_ELASTIC
    ...
    def test_round_price(self, service):
        """Test price rounding"""
        assert service._round_price(Decimal("10.12"), Decimal("0.05")) == Decimal("10.10")
        assert service._round_price(Decimal("10.13"), Decimal("0.05")) == Decimal("10.15")
        assert service._round_price(Decimal("10.17"), Decimal("0.05")) == Decimal("10.15")
        assert service._round_price(Decimal("10.18"), Decimal("0.05")) == Decimal("10.20")
        
        assert service._round_price(Decimal("10.24"), Decimal("0.25")) == Decimal("10.25")
        assert service._round_price(Decimal("10.12"), Decimal("0.25")) == Decimal("10.00")
        assert service._round_price(Decimal("10.37"), Decimal("0.25")) == Decimal("10.25")
        assert service._round_price(Decimal("10.38"), Decimal("0.25")) == Decimal("10.50")
...
class TestPricingSchemaValidation:
    ...
    def test_bulk_pricing_request_validation(self):
        ...
        with pytest.raises(ValueError):
            BulkPricingRequest(
                menu_item_ids=[1],
                round_to_nearest=Decimal("0.07")  # Invalid
            )
