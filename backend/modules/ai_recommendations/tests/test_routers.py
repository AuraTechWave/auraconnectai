...
class TestPricingRouter:
    ...
    async def test_generate_pricing_recommendations_success(self, mock_user, mock_db):
        ...
        with patch('backend.modules.ai_recommendations.services.pricing_recommendation_service.create_pricing_recommendation_service') as mock_create:
    ...
    async def test_generate_pricing_recommendations_error(self, mock_user, mock_db):
        ...
        with patch('backend.modules.ai_recommendations.services.pricing_recommendation_service.create_pricing_recommendation_service') as mock_create:
            mock_service = AsyncMock()
            mock_service.generate_bulk_recommendations.side_effect = Exception("Database error")
            mock_create.return_value = mock_service
            ...
            assert "Failed to generate pricing recommendations" in str(exc_info.value.detail)
...
class TestStaffingRouter:
    ...
    async def test_optimize_staffing_success(self, mock_user, mock_db):
        ...
        with patch('backend.modules.ai_recommendations.services.staffing_recommendation_service.create_staffing_recommendation_service') as mock_create:
            mock_service = AsyncMock()
            ...
            mock_service.generate_staffing_recommendations.assert_called_once_with(request)
    ...
    async def test_labor_cost_analysis(self, mock_user, mock_db):
        ...
        with patch('backend.modules.ai_recommendations.services.staffing_recommendation_service.create_staffing_recommendation_service') as mock_create:
            ...
            mock_recommendations = Mock(
                ...
                daily_recommendations=[
                    Mock(
                        ...
                        staffing_level=Mock(value="optimal" if i < 5 else "understaffed")
                    ) for i in range(7)
                ]
            )
            mock_service.generate_staffing_recommendations.return_value = mock_recommendations
            mock_create.return_value = mock_service
            ...
            assert len(result["optimization_opportunities"]) > 0  # Should identify understaffing
    ...
    def test_pricing_endpoints_require_permissions(self):
        ...
        for route in router.routes:
            if hasattr(route, 'endpoint'):
                if route.path in ["/apply/{recommendation_id}", "/rollback/{rollback_token}"]:
                    # These should require admin permissions
                    dependencies = route.dependencies if hasattr(route, 'dependencies') else []
                    assert any('require_analytics_permission' in str(dep) for dep in dependencies)
    ...
    def test_date_range_validation(self):
        ...
        with pytest.raises(ValueError):
            StaffingOptimizationRequest(
                ...
                end_date=date(2025, 6, 1)  # > 90 days
            )
