...
class TestAIRecommendationsIntegration:
            ...
            for order_num in range(num_orders):
                ...
                for item_idx in range(1, 3):
                    order_item = OrderItem(
                        ...
                        total_price=items[(order_num + item_idx) % len(items)].price * (1 + (order_num % 3))
                    )
    ...
    async def test_pricing_recommendations_with_real_data(self, test_db, sample_data):
        ...
        for rec in result.recommendations:
            ...
            assert rec.strategy_used in [PricingStrategy.DYNAMIC, PricingStrategy.COST_PLUS]
    ...
    async def test_pricing_impact_calculation(self, test_db, sample_data):
        ...
        if result.recommendations:
            ...
            # Check bounds
            assert rec.min_recommended_price <= rec.recommended_price <= rec.max_recommended_price
    ...
    async def test_demand_forecast_accuracy(self, test_db, sample_data):
        ...
        for forecast in forecasts[tomorrow]:
            ...
            assert forecast.orders_lower_bound <= forecast.predicted_orders <= forecast.orders_upper_bound
...
class TestPerformanceAndScalability:
    ...
    async def test_large_dataset_performance(self, test_db):
        ...
        print(f"Processed {result.total_items_analyzed} items in {execution_time:.2f} seconds")