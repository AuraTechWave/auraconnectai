...
class MenuItemPricingContext(BaseModel):
    ...
    avg_daily_sales: float = Field(
        ..., ge=0, description="Average daily sales quantity"
    )
    ...
    # Market context
    competitor_prices: Optional[List[Decimal]] = Field(
        None, description="Competitor prices for similar items"
    )
    local_market_index: float = Field(
        default=1.0, gt=0, description="Local market price index"
    )

    # Demand indicators
    current_demand: DemandLevel = Field(default=DemandLevel.NORMAL)
    seasonal_factor: float = Field(
        default=1.0, gt=0, description="Seasonal demand multiplier"
    )
    event_factor: float = Field(
        default=1.0, gt=0, description="Special event multiplier"
    )
...
class BulkPricingRequest(BaseModel):
    ...
    menu_item_ids: Optional[List[int]] = Field(
        None, description="Specific items to price"
    )

    # Optimization parameters
    optimization_goal: PriceOptimizationGoal = Field(
        default=PriceOptimizationGoal.MAXIMIZE_PROFIT
    )
    ...
    maintain_price_relationships: bool = Field(
        default=True, description="Maintain relative pricing between items"
    )
    round_to_nearest: Decimal = Field(
        default=Decimal("0.05"), description="Round prices to nearest value"
    )
