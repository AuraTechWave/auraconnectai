...
class VertexAIProvider(ModelProviderInterface):
    ...
    async def _generate_palm(self, request: ModelRequest, model_name: str) -> str:
        ...
        response = await loop.run_in_executor(
            None, lambda: model.predict(request.prompt, **parameters)
        )

        return response.text

    async def _generate_gemini(self, request: ModelRequest, model_name: str) -> str:
        ...
        if request.messages:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: chat.send_message(
                    request.messages[-1]["content"], generation_config=generation_config
                ),
            )
        else:
            # Single prompt
    ...
    async def generate_stream(
        ...
        if model_type == "gemini":
            ...
            response_stream = await loop.run_in_executor(
                None,
                lambda: model.generate_content(
                    request.prompt, generation_config=generation_config, stream=True
                ),
            )
    ...
    async def create_embeddings(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            batch_embeddings = await loop.run_in_executor(
                None, lambda: model.get_embeddings(batch)
            )
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            models = await loop.run_in_executor(None, lambda: aiplatform.Model.list())
            return True
        except Exception as e:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if health_info["status"] == "healthy":
            try:
                ...
                health_info["details"]["models_available"] = list(
                    self.model_types.keys()
                )
            except Exception as e:
