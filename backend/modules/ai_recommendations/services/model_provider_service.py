...
class ModelProviderRegistry:
    ...
    def register_provider(
        self, name: str, provider_class: Type[ModelProviderInterface]
    ):
        """Register a new provider type"""
        self._providers[name] = provider_class
        logger.info(f"Registered model provider: {name}")

    def configure_provider(self, name: str, config: ModelProviderConfig):
...
class ModelProviderService:
    ...
    def configure_from_settings(self, settings: Dict[str, Any]):
        ...
        for provider_name, provider_settings in providers_config.items():
            ...
            config = ModelProviderConfig(
                ...
                max_requests_per_minute=provider_settings.get(
                ...
                supports_function_calling=provider_settings.get(
    ...
    async def generate(
        ...
        if use_fallback:
            providers_to_try.extend(
                [p for p in self._fallback_providers if p not in providers_to_try]
            )
        ...
        for provider_name in providers_to_try:
            try:
                ...
                ai_model_metrics.track_request_start(
                    request_id, provider_name, domain, endpoint
                )
        ...
        raise ModelProviderException(f"All providers failed. Last error: {last_error}")
        ...
        for provider_name in capable_providers:
            try:
                return await self.generate(
                    request, provider_name=provider_name, use_fallback=False
                )
            except Exception as e:
                if not use_fallback:
                    raise
    ...
    async def streaming_generate(
        ...
        try:
            ...
            # Track success
            ai_model_metrics.track_request_end(
                request_id, provider_name, domain, endpoint, success=True
            )
    ...
    async def moderate_content(
        self, text: str, provider_name: Optional[str] = None
    ) -> Dict[str, Any]:
