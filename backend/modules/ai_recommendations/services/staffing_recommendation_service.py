...
class StaffingRecommendationService:
    ...
    async def generate_staffing_recommendations(
        self, request: StaffingOptimizationRequest
    ) -> StaffingRecommendationSet:
        ...
        patterns = await self._identify_staffing_patterns(
            request.start_date - timedelta(days=90), request.start_date
        )
        ...
        result = self._build_recommendation_set(
            daily_recommendations, patterns, request
        )
    ...
    async def _generate_demand_forecasts(
        ...
        while current_date <= end_date:
            ...
            for hour in range(24):
                ...
                hist_avg = self._get_historical_average(historical_data, dow, hour)

                # Apply trend and seasonality
                forecast = self._apply_forecast_adjustments(
                    hist_avg, current_date, hour
                )
    ...
    def _get_historical_average(
        self, historical_data: List[Dict[str, Any]], day_of_week: int, hour: int
    ) -> Dict[str, float]:
        ...
        if not matching_data:
            return {"avg_orders": 0, "avg_revenue": 0, "avg_customers": 0}

        return {
            "avg_orders": statistics.mean([d["order_count"] for d in matching_data]),
            "avg_revenue": statistics.mean(
                [float(d["revenue"]) for d in matching_data]
            ),
            "avg_customers": statistics.mean(
                [d["customer_count"] for d in matching_data]
            ),
        }

    def _apply_forecast_adjustments(
        self, base_forecast: Dict[str, float], forecast_date: date, hour: int
    ) -> DemandForecast:
    ...
    async def _identify_staffing_patterns(
        ...
        weekday_pattern = await self._analyze_pattern(
            start_date, end_date, [0, 1, 2, 3, 4], "Weekday Standard"  # Monday-Friday
        )
        if weekday_pattern:
        ...
        weekend_pattern = await self._analyze_pattern(
            start_date, end_date, [5, 6], "Weekend Standard"  # Saturday-Sunday
        )
        if weekend_pattern:
    ...
    async def _analyze_pattern(
        ...
        for hour in range(24):
            ...
            is_peak = (
                insights.peak_times.primary_peak
                and insights.peak_times.primary_peak.hour == hour
            ) or (
                insights.peak_times.secondary_peak
                and insights.peak_times.secondary_peak.hour == hour
            )
        ...
        # Calculate totals
        total_hours = sum(
            sum(counts.values()) for counts in hourly_requirements.values()
        )
    ...
    def _generate_shift_templates(
        self, hourly_requirements: Dict[int, Dict[StaffRole, int]]
    ) -> List[Dict[str, Any]]:
    ...
    async def _generate_daily_recommendation(
        ...
        staffing_gap = self._calculate_staffing_gap(
            staff_requirements, current_scheduled
        )
        ...
        cost_per_order = (
            labor_cost / total_predicted_orders
            if total_predicted_orders > 0
            else Decimal("0")
        )

        total_predicted_revenue = sum(f.predicted_revenue for f in demand_forecasts)
        labor_percentage = (
            float(labor_cost / total_predicted_revenue * 100)
        ...
        peak_hours = [
            f.hour
            for f in sorted(
                demand_forecasts, key=lambda x: x.predicted_orders, reverse=True
            )[:3]
        ]

        # Generate flexibility notes
        flexibility_notes = self._generate_flexibility_notes(
            staff_requirements, demand_forecasts
        )
    ...
    def _calculate_staff_requirements(
        ...
        peak_demand = (
            max(f.predicted_orders for f in demand_forecasts) if demand_forecasts else 0
        )
        ...
        for role in StaffRole:
            ...
            # Base requirement
            base_required = max(
                self.minimum_staff.get(role, 0), int(peak_demand / productivity)
            )
    ...
    def _get_current_schedule(self, schedule_date: date) -> Dict[StaffRole, int]:
        ...
        shifts = (
            self.db.query(Role.name, func.count(Shift.id).label("count"))
            .join(StaffMember, Shift.staff_id == StaffMember.id)
            .join(Role, StaffMember.role_id == Role.id)
            .filter(func.date(Shift.date) == schedule_date)
            .group_by(Role.name)
            .all()
        )
    ...
    def _calculate_staffing_gap(
        self, requirements: List[StaffRequirement], current: Dict[StaffRole, int]
    ) -> Dict[StaffRole, int]:
    ...
    def _assess_staffing_level(self, gap: Dict[StaffRole, int]) -> StaffingLevel:
        """Assess overall staffing level"""

        total_gap = sum(gap.values())
        critical_gaps = sum(
            1 for role, g in gap.items() if g > 0 and role in self.minimum_staff
        )
    ...
    def _calculate_labor_cost(self, requirements: List[StaffRequirement]) -> Decimal:
    ...
    def _identify_priority_roles(self, gap: Dict[StaffRole, int]) -> List[StaffRole]:
    ...
    def _generate_flexibility_notes(
        self, requirements: List[StaffRequirement], forecasts: List[DemandForecast]
    ) -> List[str]:
        ...
        if len(low_confidence) > len(forecasts) * 0.3:
            notes.append(
                "Forecast confidence is lower than usual - maintain scheduling flexibility"
            )
    ...
    def _build_recommendation_set(
        ...
        total_cost = sum(rec.estimated_labor_cost for rec in daily_recommendations)
        ...
        implementation_priority = self._build_implementation_priorities(
            daily_recommendations
        )
    ...
    def _identify_understaffing_risks(
        self, recommendations: List[ShiftRecommendation]
    ) -> List[Dict[str, Any]]:
    ...
    def _identify_overstaffing_risks(
        ...
        overstaffed = [
            rec
            for rec in recommendations
            if rec.staffing_level
            in [StaffingLevel.OVERSTAFFED, StaffingLevel.SEVERELY_OVERSTAFFED]
        ]
    ...
    def _generate_alerts(self, recommendations: List[ShiftRecommendation]) -> List[str]:
        """Generate alerts for critical issues"""

        alerts = []

        # Check labor percentage
        high_labor = [rec for rec in recommendations if rec.labor_percentage > 35]
    ...
    def _generate_warnings(
        ...
        if recommendations:
            avg_gap = statistics.mean(
                [sum(rec.staffing_gap.values()) for rec in recommendations]
            )
...
# Service factory
def create_staffing_recommendation_service(
    db: Session,
