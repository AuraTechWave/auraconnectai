...
from ..schemas.pricing_schemas import (
    ...
    PricingRecommendationSet, PricingStrategy, PriceOptimizationGoal,
    DemandLevel, PriceElasticity
)
...
class PricingRecommendationService:
    ...
    async def _generate_item_recommendation(
        ...
        # Estimate impact
        impacts = self._estimate_price_change_impact(
        ...
        return PricingRecommendation(
            ...
            price_change_percentage=float(
                (recommended_price - context.current_price) / context.current_price * 100
            ),
    ...
    def _calculate_recommended_price(
        ...
        if strategy == PricingStrategy.DEMAND_BASED:
            ...
            return context.current_price, {"primary": "No change recommended", "details": []}
    ...
    def _competition_based_pricing(
        ...
        if not context.competitor_prices:
            return context.current_price, {"primary": "No competitor data", "details": []}
    ...
    def _apply_pricing_constraints(
        ...
        max_increase = current_price * (1 + Decimal(str(request.max_price_increase_percent / 100)))
        max_decrease = current_price * (1 - Decimal(str(request.max_price_decrease_percent / 100)))
    ...
    def _estimate_price_change_impact(
        ...
        price_change_pct = float((new_price - context.current_price) / context.current_price)
        ...
        margin = float((context.current_price - context.base_cost) / context.current_price)
        new_margin = float((new_price - context.base_cost) / new_price)
        
        profit_impact_pct = (
            (new_margin * (1 + demand_change_pct)) /
            margin - 1
        ) if margin > 0 else 0
    ...
    def _assess_pricing_risks(
        ...
        if price_change_pct > 0 and context.price_elasticity in [
            PriceElasticity.ELASTIC, PriceElasticity.HIGHLY_ELASTIC
        ]:
            risks.append("Price increase on elastic item may significantly reduce demand")
        
        if context.competitor_prices and new_price > max(context.competitor_prices) * Decimal("1.1"):
        ...
        if context.customer_rating and context.customer_rating < 4.0 and price_change_pct > 0:
    ...
    def _get_factors_considered(
        ...
        if context.competitor_prices:
            factors.append(f"Competitor prices: {len(context.competitor_prices)} data points")
    ...
    def _generate_implementation_notes(
        ...
        price_change = abs(float((new_price - context.current_price) / context.current_price))
        
        if price_change > 0.10:
            notes.append("Consider phased implementation over 2-3 weeks")
        
        if context.price_elasticity in [PriceElasticity.ELASTIC, PriceElasticity.HIGHLY_ELASTIC]:
    ...
    def _build_recommendation_set(
        ...
        for rec in recommendations:
            strategies_used[rec.strategy_used] = strategies_used.get(rec.strategy_used, 0) + 1
        ...
        return PricingRecommendationSet(
            ...
            total_recommendations=len([r for r in recommendations if r.recommended_price != r.current_price]),
    ...
    def _build_implementation_phases(
        ...
        ][:10]  # Limit to 10 items per phase
        ...
        if phase1:
            phases.append({
                ...
                "expected_impact": sum(r.expected_profit_impact for r in phase1) / len(phase1),
        ...
        phase2 = [
            r for r in sorted_recs
            if r not in phase1 and r.confidence_score > 0.5
        ][:10]
        ...
        if phase2:
            phases.append({
                ...
                "expected_impact": sum(r.expected_profit_impact for r in phase2) / len(phase2),
    ...
    def _generate_warnings(
        ...
        large_changes = [
            ...
            if abs(r.price_change_percentage) > 15
        ]
        if large_changes:
            warnings.append(
    ...
    def _generate_notes(
        ...
        notes.append(f"Recommended {increases} price increases and {decreases} price decreases")
