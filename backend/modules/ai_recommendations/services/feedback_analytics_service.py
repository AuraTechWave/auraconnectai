...
class FeedbackAnalyticsService:
    ...
    async def _get_model_breakdown(
        ...
        query = (
            select(
                ...
                func.sum(case((SuggestionFeedback.rating >= 4, 1), else_=0)).label(
                    "positive_count"
                ),
            )
            .join(SuggestionFeedback)
            .where(
                and_(
                    SuggestionFeedback.created_at >= start_date,
                    SuggestionFeedback.created_at <= end_date,
        ...
        return [
            {
                ...
                "positive_feedback_rate": (
                    row.positive_count / row.feedback_count
    ...
    async def _get_domain_breakdown(
        ...
        query = (
            select(
                ...
                func.count(SuggestionFeedback.id).label("feedback_count"),
                func.avg(SuggestionFeedback.rating).label("avg_rating"),
                func.count(func.distinct(SuggestionFeedback.user_id)).label(
                    "unique_users"
                ),
            )
            .join(SuggestionFeedback)
    ...
    async def get_model_performance(
        ...
        query = (
            ...
            .having(func.count(AISuggestion.id) >= min_requests)
        )
        ...
        for row in result:
            ...
            feedback_rate = (
                row.feedback_count / row.total_requests if row.total_requests > 0 else 0
            )
    ...
    async def get_domain_insights(
        self, db: AsyncSession, domain: str, start_date: datetime, end_date: datetime
        ...
        suggestions_per_user = (
            total_suggestions / unique_users if unique_users > 0 else 0
        )
        ...
        avg_rating = (
            sum(f.rating for f in all_feedback) / len(all_feedback)
            if all_feedback
            else 0
        )
        ...
        sentiment = {
            "positive": (
                len([f for f in all_feedback if f.rating >= 4]) / len(all_feedback)
                if all_feedback
                else 0
            ),
            "neutral": (
                len([f for f in all_feedback if f.rating == 3]) / len(all_feedback)
                if all_feedback
                else 0
            ),
            "negative": (
                len([f for f in all_feedback if f.rating <= 2]) / len(all_feedback)
                if all_feedback
                else 0
            ),
        }
        ...
        return DomainInsightsResponse(
            ...
            model_performance=[
                m for m in model_performance if m.model_type in models_used
            ],
    ...
    async def get_feedback_trends(
        ...
        summary_stats = {
            ...
            "avg_feedback_per_period": (
                sum(t["feedback_count"] for t in trends) / len(trends) if trends else 0
            ),
                    ...
                    changes.append(
                        {
                            ...
                            "change_percentage": (
                                rating_change / prev["average_rating"]
                            )
                            * 100,
                        }
                    )
    ...
    async def get_improvement_recommendations(
        self, db: AsyncSession, min_feedback_count: int = 50
            ...
            recommendations.append(
                ImprovementRecommendation(
                    ...
                    supporting_data={
                        ...
                        "negative_feedback_rate": summary.feedback_by_type.get(
                            "negative", 0
                        )
            ...
            recommendations.append(
                ImprovementRecommendation(
                    ...
                    supporting_data={
                        ...
                        "feedback_per_user": (
                            summary.total_feedback_count / summary.unique_users
    ...
    async def configure_alerts(
        self, db: AsyncSession, user_id: int, config: Dict[str, Any]
    ) -> Dict[str, Any]:
