...
class AIModelMetrics:
    ...
    def track_request_start(
        self, request_id: str, model_type: str, domain: str, endpoint: str
    ):
        """Track the start of a model request"""
        self.request_timestamps[request_id] = datetime.utcnow()

        ai_model_requests_total.labels(
            model_type=model_type, domain=domain, endpoint=endpoint
        ).inc()

        ai_model_active_requests.labels(model_type=model_type, domain=domain).inc()
    ...
    def track_request_end(
        ...
        ai_model_active_requests.labels(model_type=model_type, domain=domain).dec()
        ...
        if success:
            ...
            if confidence_score is not None:
                ai_model_confidence_score.labels(
                    model_type=model_type, domain=domain
            ...
            if cache_key not in self.success_cache:
                self.success_cache[cache_key] = []
            self.success_cache[cache_key].append(
                {"timestamp": datetime.utcnow(), "success": False}
            )
    ...
    def track_feedback(
        ...
        if value_impact is not None:
            ai_suggestion_value_impact.labels(
                model_type=model_type, domain=domain, metric_type="revenue_impact"
    ...
    def _update_success_rate_gauges(self, model_type: str, domain: str):
        ...
        for window_name, window_delta in time_windows.items():
            ...
            recent_requests = [
                r for r in self.success_cache[cache_key] if r["timestamp"] > cutoff_time
            ]
            ...
            if recent_requests:
                ...
                ai_model_success_rate.labels(
                    model_type=model_type, domain=domain, time_window=window_name
                ).set(success_rate)

        # Clean up old entries
        week_ago = now - timedelta(days=7)
        self.success_cache[cache_key] = [
            r for r in self.success_cache[cache_key] if r["timestamp"] > week_ago
        ]
    ...
    def _update_feedback_gauges(self, model_type: str, domain: str):
        ...
        for window_name, window_delta in time_windows.items():
            ...
            recent_feedback = [
                f
                for f in self.feedback_cache[cache_key]
        ...
        week_ago = now - timedelta(days=7)
        self.feedback_cache[cache_key] = [
            f for f in self.feedback_cache[cache_key] if f["timestamp"] > week_ago
        ]
