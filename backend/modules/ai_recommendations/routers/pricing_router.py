...
from ..schemas.pricing_schemas import (
    BulkPricingRequest, PricingRecommendationSet,
    PriceTestingConfig, PriceTestingResult
)
from ..services.pricing_recommendation_service import create_pricing_recommendation_service

logger = logging.getLogger(__name__)

router = APIRouter(
    prefix="/ai-recommendations/pricing",
    tags=["AI Pricing Recommendations"]
)
...
async def get_category_pricing_recommendations(
    category_id: int,
    max_price_change: float = Query(20.0, ge=0, le=50, description="Maximum price change percentage"),
...
async def configure_price_test(
    ...
    return {
        ...
        "end_date": datetime.now().date().replace(day=datetime.now().day + config.test_duration_days)
    }
...
async def get_competitive_pricing_insights(
    include_recommendations: bool = Query(True, description="Include pricing recommendations"),
    current_user: dict = Depends(require_analytics_permission(AnalyticsPermission.VIEW_FINANCIAL)),
    db: Session = Depends(get_db)
):
    """
...
async def apply_pricing_recommendation(
    ...
    effective_date: Optional[date] = Query(None, description="When to apply the price change"),
