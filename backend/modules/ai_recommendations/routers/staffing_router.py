...
from ..schemas.staffing_schemas import (
    ...
    LaborCostAnalysis, DayOfWeek
)
from ..services.staffing_recommendation_service import create_staffing_recommendation_service

logger = logging.getLogger(__name__)

router = APIRouter(
    prefix="/ai-recommendations/staffing",
    tags=["AI Staffing Recommendations"]
)
...
async def get_weekly_staffing_summary(
    start_date: Optional[date] = Query(None, description="Start of week (defaults to current week)"),
    current_user: dict = Depends(require_analytics_permission(AnalyticsPermission.VIEW_STAFF_REPORTS)),
    db: Session = Depends(get_db)
):
    """
    ...
    for rec in recommendations.daily_recommendations:
        weekly_summary["daily_summary"].append({
            ...
            "total_staff_needed": sum(req.optimal for req in rec.staff_requirements),
...
async def get_staffing_patterns(
    pattern_type: Optional[str] = Query(None, description="Filter by pattern type (weekday/weekend)"),
    current_user: dict = Depends(require_analytics_permission(AnalyticsPermission.VIEW_STAFF_REPORTS)),
    db: Session = Depends(get_db)
):
    """
    ...
    request = StaffingOptimizationRequest(
        start_date=end_date,  # Just one day to get patterns
        end_date=end_date
    )
    ...
    if pattern_type:
        patterns = [p for p in patterns if pattern_type.lower() in p.pattern_name.lower()]
        ...
        "patterns": [
            {
                ...
                "estimated_daily_cost": float(p.average_hourly_cost) * p.total_labor_hours,
...
async def get_labor_cost_analysis(
    ...
    current_user: dict = Depends(require_analytics_permission(AnalyticsPermission.VIEW_FINANCIAL)),
    db: Session = Depends(get_db)
):
    """
    ...
    analysis = {
        ...
        "recommended_staffing": {
            "total_hours": recommendations.total_recommended_hours,
            "total_cost": str(recommendations.total_estimated_cost),
            "average_daily_cost": str(recommendations.total_estimated_cost / total_days),
    ...
    if recommendations.average_labor_percentage > 30:
        analysis["optimization_opportunities"].append({
            ...
            "target": 28.0,
            "potential_savings": str(recommendations.total_estimated_cost * 0.067)  # 2% reduction
        })
    
    understaffed_days = sum(
        1 for rec in recommendations.daily_recommendations
...
async def get_demand_forecast(
    ...
    # Generate forecast
    forecasts = await service._generate_demand_forecasts(
        target_date,
        target_date
    )
    ...
    return {
        ...
        "daily_totals": {
            "predicted_orders": sum(f.predicted_orders for f in forecasts.get(target_date, [])),
            "predicted_revenue": str(sum(f.predicted_revenue for f in forecasts.get(target_date, []))),
            "predicted_customers": sum(f.predicted_customers for f in forecasts.get(target_date, []))
        }
    }
...
async def generate_schedule(
    week_start: date,
    auto_assign_staff: bool = Query(False, description="Automatically assign available staff"),
    respect_preferences: bool = Query(True, description="Respect staff scheduling preferences"),
    current_user: dict = Depends(require_analytics_permission(AnalyticsPermission.MANAGE_ALERTS)),
    db: Session = Depends(get_db)
):
    """
    ...
    return {
        ...
        "status": "draft",
        "summary": {
            "total_shifts": len(recommendations.daily_recommendations) * 15,  # Avg shifts per day
            "total_hours": recommendations.total_recommended_hours,
        ...
        ] if auto_assign_staff else ["Manual assignment required"],
        "next_steps": [
