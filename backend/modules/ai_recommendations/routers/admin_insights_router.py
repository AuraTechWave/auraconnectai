...
async def compare_models(
    ...
    metric: str = Query("average_rating", pattern="^(average_rating|success_rate|response_time|confidence_score)$"),
    days: int = Query(30, ge=1, le=90),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
    ...
    if len(model_types) < 2:
        raise HTTPException(
            status_code=400,
            detail="At least 2 model types required for comparison"
        )
