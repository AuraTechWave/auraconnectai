...
class TaxCalculator:
    ...
    async def _calculate_tax_approximation(
        ...
        try:
            ...
            tax_service_result = await self.tax_service.calculate_taxes(tax_service_request)
    ...
    def calculate_medicare_additional_tax(
        ...
        if ytd_earnings + gross_pay > threshold:
            if ytd_earnings >= threshold:
                ...
                additional_medicare = (gross_pay * additional_medicare_rate).quantize(Decimal('0.01'))
            else:
                # Only portion above threshold
                excess_amount = (ytd_earnings + gross_pay) - threshold
                additional_medicare = (excess_amount * additional_medicare_rate).quantize(Decimal('0.01'))
        else:
    ...
    def batch_calculate_taxes(
        self, 
        payroll_data: Dict[int, Dict[str, any]]
