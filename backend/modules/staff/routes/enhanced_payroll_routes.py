...
@router.post(
    "/run", response_model=PayrollRunResponse, status_code=status.HTTP_202_ACCEPTED
)
async def run_payroll(
    ...
    try:
        ...
        config_service.update_job_progress(job_id=job_id, status="processing")
...
async def get_payroll_run_status(
    ...
    if not job_status:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Payroll job not found"
        )
    ...
    return PayrollBatchStatus(
        ...
        error_summary=[
            ...
            for error in (
                job_status.get("error_details", {}).get("processing_errors", [])[:5]
            )
        ],
    )
...
async def get_staff_payroll(
    staff_id: int,
    limit: int = Query(
        10, ge=1, le=100, description="Number of payroll records to return"
    ),
    ...
    try:
        ...
        payment_history = await payroll_service.get_employee_payment_history(
            staff_id=staff_id, limit=limit, tenant_id=tenant_id
        )
            ...
            payroll_summaries.append(
                {
                    ...
                    "total_hours": (payment["regular_hours"] or 0)
                    + (payment["overtime_hours"] or 0),
...
async def get_staff_payroll_detail(
    ...
    try:
        ...
        return StaffPayrollDetail(
            ...
            total_hours=payroll_response.breakdown.hours_worked
            + payroll_response.breakdown.overtime_hours,
            # Pay rates
            base_hourly_rate=payroll_response.breakdown.hourly_rate,
            overtime_rate=payroll_response.breakdown.overtime_rate,
            # Earnings
            regular_pay=payroll_response.breakdown.hours_worked
            * payroll_response.breakdown.hourly_rate,
            overtime_pay=payroll_response.breakdown.overtime_hours
            * payroll_response.breakdown.overtime_rate,
            gross_pay=payroll_response.gross_pay,
            # Tax deductions
            federal_tax=payroll_response.breakdown.tax_deductions
            * 0.6,  # Estimated breakdown
            ...
            # Benefit deductions
            health_insurance=payroll_response.breakdown.other_deductions
            * 0.6,  # Estimated breakdown
            ...
            rule_info_list.append(
                {
                    ...
                    or f"{rule.tax_type} tax for {rule.jurisdiction}",
...
async def get_payroll_statistics(
    ...
    try:
        ...
        return PayrollStatsResponse(
            ...
            average_gross_pay=(
                summary["total_gross_pay"] / summary["total_employees"]
...
async def export_payroll_data(
    ...
    try:
        ...
        background_tasks.add_task(process_payroll_export, export_id, request)

        return PayrollExportResponse(
            export_id=export_id, status="processing", created_at=datetime.utcnow()
        )
