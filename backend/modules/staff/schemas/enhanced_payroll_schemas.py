...
class PayrollRunResponse(BaseModel):
    ...
    successful_count: int = Field(..., description="Number of successfully processed staff")
    ...
    created_at: datetime = Field(..., description="Timestamp when payroll run was created")
...
class StaffPayrollDetail(BaseModel):
    ...
    double_time_hours: Decimal = Field(Decimal('0.00'), description="Double-time hours worked")
    ...
    double_time_pay: Decimal = Field(Decimal('0.00'), description="Double-time pay amount")
    ...
    unemployment: Decimal = Field(Decimal('0.00'), description="Unemployment tax deduction")
    total_tax_deductions: Decimal = Field(..., description="Total tax deductions")
    
    # Benefit deductions
    health_insurance: Decimal = Field(Decimal('0.00'), description="Health insurance deduction")
    dental_insurance: Decimal = Field(Decimal('0.00'), description="Dental insurance deduction")
    retirement_contribution: Decimal = Field(Decimal('0.00'), description="Retirement contribution")
    parking_fee: Decimal = Field(Decimal('0.00'), description="Parking fee deduction")
    total_benefit_deductions: Decimal = Field(..., description="Total benefit deductions")
    
    # Other deductions
    garnishments: Decimal = Field(Decimal('0.00'), description="Garnishment deductions")
    loan_repayments: Decimal = Field(Decimal('0.00'), description="Loan repayment deductions")
    ...
    payment_id: Optional[int] = Field(None, description="Associated EmployeePayment record ID")
...
class TaxRuleInfo(BaseModel):
    ...
    rate: Decimal = Field(..., description="Tax rate (as decimal, e.g., 0.0825 for 8.25%)")
    description: str = Field(..., description="Human-readable description of the rule")
    effective_date: Optional[date] = Field(None, description="Date when rule becomes effective")
...
class PayrollStatsResponse(BaseModel):
    ...
    total_tax_deductions: Decimal = Field(..., description="Total tax deductions")
    total_benefit_deductions: Decimal = Field(..., description="Total benefit deductions")
    average_hours_per_employee: Decimal = Field(..., description="Average hours per employee")
    average_gross_pay: Decimal = Field(..., description="Average gross pay per employee")
