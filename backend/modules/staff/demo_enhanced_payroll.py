            ...
            logs.append(
                {
                    ...
                    "check_out": datetime.combine(
                        work_date, datetime.strptime("17:36", "%H:%M").time()
                    ),  # 9.6 hours/day
                }
            )
...
class PayrollDemo:
    ...
    def demonstrate_hours_calculation(self):
        ...
        for staff in self.db.staff_members:
            ...
            staff_logs = [
                log for log in self.db.attendance_logs if log["staff_id"] == staff_id
            ]
    ...
    def demonstrate_earnings_calculation(self):
        ...
        for staff in self.db.staff_members:
            ...
            total_hours = sum(
                Decimal(
                    str((log["check_out"] - log["check_in"]).total_seconds() / 3600)
                )
                for log in staff_logs
            )
            ...
            print(
                f"  Regular Pay: {regular_hours:.2f} hrs × ${base_rate:.2f} = ${regular_pay:.2f}"
            )
            print(
                f"  Overtime Pay: {overtime_hours:.2f} hrs × ${overtime_rate:.2f} = ${overtime_pay:.2f}"
            )
    ...
    def demonstrate_tax_calculation(self):
        ...
        for staff in self.db.staff_members:
            ...
            staff_logs = [
                log for log in self.db.attendance_logs if log["staff_id"] == staff["id"]
            ]
            total_hours = sum(
                Decimal(
                    str((log["check_out"] - log["check_in"]).total_seconds() / 3600)
            ...
            regular_hours = min(total_hours, Decimal("40.00"))
            overtime_hours = max(Decimal("0.00"), total_hours - Decimal("40.00"))
            gross_pay = regular_hours * staff["hourly_rate"] + overtime_hours * staff[
                "hourly_rate"
            ] * Decimal("1.5")
    ...
    def demonstrate_comprehensive_payroll(self):
        ...
        for staff in self.db.staff_members:
            ...
            overtime_hours = max(Decimal("0.00"), total_hours - Decimal("40.00"))
            gross_pay = (
                regular_hours * base_rate + overtime_hours * base_rate * Decimal("1.5")
            )
            ...
            print(
                f"  Overtime Hours: {overtime_hours:.2f} @ ${base_rate * Decimal('1.5'):.2f}/hr"
            )
            print(f"  Total Hours: {total_hours:.2f}")
