...
class EnhancedShift(Base):
    ...
    staff_member = relationship(
        "StaffMember", foreign_keys=[staff_id], back_populates="scheduled_shifts"
    )
    ...
    swap_requests_from = relationship(
        "ShiftSwap", foreign_keys="ShiftSwap.from_shift_id", back_populates="from_shift"
    )
    swap_requests_to = relationship(
        "ShiftSwap", foreign_keys="ShiftSwap.to_shift_id", back_populates="to_shift"
    )
...
class ShiftTemplate(Base):
    ...
    requirements = relationship(
        "ShiftRequirement", back_populates="template", cascade="all, delete-orphan"
    )
...
class ShiftSwap(Base):
    ...
    secondary_approver = relationship(
        "StaffMember", foreign_keys=[secondary_approver_id]
    )
    from_shift = relationship(
        "EnhancedShift",
        foreign_keys=[from_shift_id],
        back_populates="swap_requests_from",
    )
    to_shift = relationship(
        "EnhancedShift", foreign_keys=[to_shift_id], back_populates="swap_requests_to"
    )
...
class SwapApprovalRule(Base):
    ...
    max_hours_difference = Column(
        Float
    )  # Max difference in shift hours for auto-approval
    ...
    restricted_shifts = Column(
        JSON, default=list
    )  # Shift types that can't be auto-approved
