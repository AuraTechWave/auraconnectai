...
from ..schemas.schedule_schemas import (
    SchedulePreviewResponse, ScheduleCreateRequest, ScheduleUpdateRequest,
    PaginatedPreviewResponse, ScheduleResponse
)
...
class ScheduleService:
    ...
    async def generate_preview(
        ...
        query = select(Schedule).where(
            ...
            selectinload(Schedule.staff).selectinload(Staff.role)
        )
        ...
        if filters.get("department_id"):
            query = query.join(Staff).join(StaffRole).where(
    ...
    async def send_schedule_notifications(
        ...
        return await schedule_notification_service.send_schedule_published_notifications(
    ...
    async def create_shift(
        ...
        staff_result = await db.execute(
            select(Staff).where(Staff.id == shift_data.staff_id)
            .options(selectinload(Staff.role))
        )
