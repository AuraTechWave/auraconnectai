...
from ...payroll.models.payroll_configuration import (
...
class ConfigManager:
    ...
    def get_config_with_cache_key(
        self, location: str = "default"
    ) -> tuple[PayrollConfig, str]:
        """
    ...
    def _load_configuration(self, location: str) -> PayrollConfig:
        ...
        config.daily_overtime_threshold = Decimal(
            os.getenv(
                "PAYROLL_DAILY_OT_THRESHOLD", str(config.daily_overtime_threshold)
            )
        )
        config.weekly_overtime_threshold = Decimal(
            os.getenv(
                "PAYROLL_WEEKLY_OT_THRESHOLD", str(config.weekly_overtime_threshold)
            )
        )
        ...
        config.double_time_threshold = Decimal(
            os.getenv(
                "PAYROLL_DOUBLE_TIME_THRESHOLD", str(config.double_time_threshold)
            )
        )
        config.double_time_multiplier = Decimal(
            os.getenv(
                "PAYROLL_DOUBLE_TIME_MULTIPLIER", str(config.double_time_multiplier)
            )
        )

        # Load database configurations
        db_configs = (
            self.db.query(PayrollConfiguration)
            .filter(
    ...
    def _apply_db_config(self, config: PayrollConfig, db_config: PayrollConfiguration):
        ...
        if db_config.config_type == PayrollConfigurationType.OVERTIME_RULES:
            if db_config.config_key == "daily_overtime_threshold":
                config.daily_overtime_threshold = Decimal(
                    str(config_value.get("threshold", config.daily_overtime_threshold))
                )
            elif db_config.config_key == "weekly_overtime_threshold":
                config.weekly_overtime_threshold = Decimal(
                    str(config_value.get("threshold", config.weekly_overtime_threshold))
                )
            elif db_config.config_key == "overtime_multiplier":
                config.overtime_multiplier = Decimal(
                    str(config_value.get("multiplier", config.overtime_multiplier))
                )
            elif db_config.config_key == "double_time_threshold":
                config.double_time_threshold = Decimal(
                    str(config_value.get("threshold", config.double_time_threshold))
                )
            elif db_config.config_key == "double_time_multiplier":
                config.double_time_multiplier = Decimal(
                    str(config_value.get("multiplier", config.double_time_multiplier))
                )

        elif db_config.config_type == PayrollConfigurationType.BENEFIT_PRORATION:
            if db_config.config_key == "monthly_to_biweekly_factor":
                config.monthly_to_biweekly_factor = Decimal(
                    str(config_value.get("factor", config.monthly_to_biweekly_factor))
                )
            elif db_config.config_key == "monthly_to_weekly_factor":
                config.monthly_to_weekly_factor = Decimal(
                    str(config_value.get("factor", config.monthly_to_weekly_factor))
                )

        elif db_config.config_type == PayrollConfigurationType.TAX_SETTINGS:
            if db_config.config_key == "social_security_wage_cap":
                config.social_security_wage_cap = Decimal(
                    str(config_value.get("cap", config.social_security_wage_cap))
                )
            elif db_config.config_key == "medicare_additional_threshold":
                config.medicare_additional_threshold_single = Decimal(
                    str(
                        config_value.get(
                            "single", config.medicare_additional_threshold_single
                        )
                    )
                )
                config.medicare_additional_threshold_joint = Decimal(
                    str(
                        config_value.get(
                            "joint", config.medicare_additional_threshold_joint
                        )
                    )
                )
    ...
    def validate_overtime_rules(self, rules: Dict[str, Any]) -> List[str]:
        ...
        if not isinstance(double_time_threshold, (int, float, Decimal)):
            ...
            errors.append(
                "Double time threshold must be greater than daily overtime threshold"
            )
    ...
    def get_overtime_rules_with_cache_key(
        self, location: str = "default"
    ...
    def update_overtime_rules(
        self, rules: Dict[str, Any], location: str = "default", description: str = ""
    ) -> List[str]:
        """
    ...
    def get_benefit_proration_factors(
        self, location: str = "default"
    ) -> Dict[str, Decimal]:
    ...
    def get_tax_caps_and_thresholds(
        self, location: str = "default"
    ) -> Dict[str, Decimal]:
