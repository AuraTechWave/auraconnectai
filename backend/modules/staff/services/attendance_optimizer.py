...
class AttendanceOptimizer:
    ...
    def get_daily_hours_aggregated(
        ...
        try:
            ...
            if (end_date - start_date).days > 365:
                logger.warning(
                    f"Large date range requested: {start_date} to {end_date} for staff {staff_id}"
                )
            ...
            query = (
                self.db.query(
                    ...
                    func.sum(
                        func.extract(
                            "epoch", AttendanceLog.check_out - AttendanceLog.check_in
                        )
                        / 3600
                    ).label("total_hours"),
                ...
                .group_by(cast(AttendanceLog.check_in, Date))
                .order_by(cast(AttendanceLog.check_in, Date))
            )
            ...
            daily_summaries = [
                DailyHoursSummary(
                    ...
                    total_hours=Decimal(str(result.total_hours or 0)).quantize(
            ...
            logger.debug(
                f"Retrieved {len(daily_summaries)} daily summaries for staff {staff_id}"
            )
    ...
    def calculate_overtime_efficiently(
        ...
        total_overtime = (
            total_daily_overtime + total_weekly_overtime - total_double_time
        )
        ...
        return {
            "regular_hours": total_regular.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            ),
            "overtime_hours": total_overtime.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            ),
            "double_time_hours": total_double_time.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            ),
            "total_hours": weekly_total.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            ),
        }
    ...
    def get_attendance_statistics(
        ...
        """
        # Get daily summaries
        daily_summaries = self.get_daily_hours_aggregated(
        ...
        overtime_hours = sum(
            max(Decimal("0.00"), hours - Decimal("8.0")) for hours in hours_per_day
        )
        ...
        return {
            ...
            "average_hours_per_day": (
                (total_hours / total_days).quantize(
                    Decimal("0.01"), rounding=ROUND_HALF_UP
                )
                if total_days > 0
                else Decimal("0.00")
            ),
            "max_hours_in_day": max_hours.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            ),
            "min_hours_in_day": min_hours.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            ),
            "total_shifts": total_shifts,
            "average_shifts_per_day": (
                (Decimal(str(total_shifts)) / total_days).quantize(
                    Decimal("0.01"), rounding=ROUND_HALF_UP
                )
                if total_days > 0
                else Decimal("0.00")
            ...
            "total_overtime_hours": overtime_hours.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            ),
        }
    ...
    def batch_calculate_hours_for_staff(
        ...
        query = (
            ...
            .group_by(AttendanceLog.staff_id, cast(AttendanceLog.check_in, Date))
        )
