...
class JobTracker:
    ...
    def get_job_status(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        job_data = {
            ...
            "updated_at": (
                job_record.updated_at.isoformat() if job_record.updated_at else None
            ),
            "finished_at": (
                job_record.finished_at.isoformat() if job_record.finished_at else None
            ),
            "estimated_completion": (
                job_record.estimated_completion.isoformat()
                if job_record.estimated_completion
                else None
            ),
    ...
    def _cache_job_in_redis(self, job_id: str, job_record: PayrollJobTracking):
        ...
        try:
            ...
            self.redis.setex(
                f"job:{job_id}", self.redis_ttl, json.dumps(cache_data, default=str)
            )
        except Exception:
