...
from ..schemas.scheduling_schemas import ScheduleConflict, StaffingAnalytics
...
class SchedulingService:
    ...
    def check_availability(
            ...
            .filter(
                and_(
                    ...
                    StaffAvailability.status.in_(
                        [AvailabilityStatus.AVAILABLE, AvailabilityStatus.PREFERRED]
                    ),
                    or_(
        ...
        if recurring_availability:
            ...
            if (
                shift_start_time < recurring_availability.start_time
                or shift_end_time > recurring_availability.end_time
            ):
    ...
    def detect_conflicts(
        ...
        weekly_hours = (
            self.db.query(
                func.sum(
                    func.extract(
                        "epoch", EnhancedShift.end_time - EnhancedShift.start_time
                    )
                    / 3600
                )
            )
    ...
    def calculate_labor_cost(
        ...
        # Calculate cost
        cost = (regular_hours * hourly_rate) + (
                    ...
                    and_(
                        ...
                        or_(
                            ...
                            and_(
                                ...
                                ShiftTemplate.recurrence_days.contains(
                                    [day_of_week.value]
                                ),
                            ),
                        ),
                ...
                for i in range(shifts_to_create):
                    ...
                    if auto_assign:
                        ...
                        available_staff = self._find_available_staff(
                            shift_start, shift_end, template.role_id, location_id
                        )
                        if available_staff:
    ...
    def _find_available_staff(
        ...
        query = self.db.query(StaffMember).filter(StaffMember.status == "active")
    ...
    def validate_swap_request(
        ...
        from_shift = (
            self.db.query(EnhancedShift)
            .filter(EnhancedShift.id == from_shift_id)
            .first()
        )
        ...
        if to_shift_id:
            to_shift = (
                self.db.query(EnhancedShift)
                .filter(EnhancedShift.id == to_shift_id)
                .first()
            )
        ...
        if to_staff_id:
            to_staff = (
                self.db.query(StaffMember).filter(StaffMember.id == to_staff_id).first()
            )
            ...
            available, reason = self.check_availability(
                to_staff_id, from_shift.start_time, from_shift.end_time
            )
    ...
    def approve_shift_swap(
        self, swap_id: int, approved_by_id: int, notes: Optional[str] = None
    ) -> ShiftSwap:
        ...
        valid, reason = self.validate_swap_request(
            swap.from_shift_id, swap.to_shift_id, swap.to_staff_id
        )
    ...
    def get_staffing_analytics(
        self, start_date: date, end_date: date, location_id: int
        ...
        while current_date <= end_date:
            ...
            # Calculate metrics
            scheduled_staff = len(
                set(shift.staff_id for shift in shifts if shift.staff_id)
            )
            ...
            analytics.append(
                StaffingAnalytics(
                    ...
                    coverage_percentage=(
                        (scheduled_staff / required_staff * 100)
                        if required_staff > 0
                        else 100
                    ),
    ...
    def generate_demand_aware_schedule(
        ...
        while current_date <= end_date:
            ...
            hourly_demand = self._estimate_peak_orders(
                current_date, location_id, demand_lookback_days
            )
            hourly_role_requirements = self._map_orders_to_role_requirements(
                hourly_demand, location_id
            )
            ...
            for start_t, end_t in shift_blocks:
                ...
                for hour in range(shift_start_hour, shift_end_hour):
                    ...
                    if actual_hour in hourly_role_requirements:
                        for role_id, required in hourly_role_requirements[
                            actual_hour
                        ].items():
                            if role_id not in peak_requirements:
                                peak_requirements[role_id] = 0
                            peak_requirements[role_id] = max(
                                peak_requirements[role_id], required
                            )

                # Create shifts based on peak requirements
                for role_id, required in peak_requirements.items():
                    required_with_buffer = max(
                        0, int(round(required * (1 + buffer_percentage / 100.0)))
                    )
                    for _ in range(required_with_buffer):
                        ...
                        assigned_shifts_by_staff[staff_member.id].append(
    ...
    def _create_shifts_for_role_on_date(
        ...
        shift_periods = self._group_hours_into_shifts(
            active_hours, min_shift_hours, max_shift_hours
        )
        ...
        for start_hour, end_hour in shift_periods:
            ...
            # Apply buffer
            required_with_buffer = max(
                0, int(round(peak_required * (1 + buffer_percentage / 100.0)))
            )
    ...
    def _group_hours_into_shifts(
        self, active_hours: List[int], min_shift_hours: int, max_shift_hours: int
    ) -> List[Tuple[int, int]]:
    ...
    def _estimate_peak_orders(
        self, target_date: date, location_id: int, demand_lookback_days: int
    ) -> int:
        ...
        rows = (
            ...
            .join(StaffMember, Order.staff_id == StaffMember.id)
            .filter(
    ...
    def _map_orders_to_role_requirements(
        self, peak_orders: int, location_id: int
    ) -> Dict[int, int]:
        ...
        for staff in candidates:
            ...
            if staff.id in assigned_shifts_by_staff:
                generation_conflicts = self._check_generation_conflicts(
                    start_time,
                    end_time,
                    assigned_shifts_by_staff[staff.id],
                    min_hours_between_shifts,
                )
    ...
    def _check_generation_conflicts(
        ...
        for shift_start, shift_end in assigned_shifts:
            # Check for overlap
            if (
                shift_start <= start_time < shift_end
                or shift_start < end_time <= shift_end
                or (start_time <= shift_start and end_time >= shift_end)
            ):
                return True
    ...
    def validate_break_duration(
        self, shift_duration_minutes: int, break_duration: int
    ) -> bool:
    ...
    def validate_shift_assignment(
        ...
        if staff and staff.max_hours_per_week:
            ...
            if current_hours + shift_hours > staff.max_hours_per_week:
                return (
                    False,
                    f"Would exceed maximum weekly hours ({staff.max_hours_per_week})",
                )
        ...
        if template_id:
            ...
            if template:
                ...
                if (
                    ...
                    or shift_end_time != template.end_time
                ):
        ...
        shifts = (
            ...
            .filter(
                and_(
                    ...
                    EnhancedShift.status.in_(
                        [ShiftStatus.SCHEDULED, ShiftStatus.PUBLISHED]
                    ),
                )
            )
            .all()
    ...
    def get_shifts_by_date_range(
        self, restaurant_id: int, start_date: date, end_date: date
    ) -> List[EnhancedShift]:
        ...
        return (
            ...
            .order_by(EnhancedShift.date, EnhancedShift.start_time)
            .all()
        )

    def get_staff_shifts_by_location(
        self, staff_id: int, location_id: int, start_date: date, end_date: date
    ) -> List[EnhancedShift]:
    ...
    def cache_analytics(
        self, location_id: int, start_date: date, end_date: date, data: Dict
    ):
    ...
    def publish_schedule(
        self, location_id: int, start_date: date, end_date: date, notify: bool = True
    ) -> Dict[str, Any]:
        ...
        return {"published_count": published_count, "publication_id": publication.id}
