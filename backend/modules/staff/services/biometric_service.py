...
AUDIT_LOG_RETENTION_DAYS = int(
    os.getenv("AUDIT_LOG_RETENTION_DAYS", str(config.DATA_RETENTION_DAYS * 7))
)  # default 7x general retention
...
class BiometricService:
    ...
    def enroll_fingerprint(
        self, staff_id: int, fingerprint_data: str, device_id: str
    ) -> Tuple[bool, str]:
        """Enroll a staff member's fingerprint with enhanced security"""
        try:
            # Verify staff exists
            staff = (
                self.db.query(StaffMember).filter(StaffMember.id == staff_id).first()
            )
            ...
            biometric = (
                self.db.query(StaffBiometric)
                .filter(StaffBiometric.staff_id == staff_id)
                .first()
            )

            if not biometric:
                biometric = StaffBiometric(
                    staff_id=staff_id, created_at=datetime.utcnow()
                )
            ...
            fingerprint_hash = self._hash_biometric_with_salt(
                fingerprint_data, fingerprint_salt
            )
            ...
            biometric.fingerprint_hash_prefix = fingerprint_hash[
                :8
            ]  # Store prefix for faster lookup
    ...
    def enroll_face(
        self, staff_id: int, face_data: str, device_id: str
    ) -> Tuple[bool, str]:
    ...
    def verify_biometric(
        ...
        try:
            ...
            test_hash = self._hash_biometric_with_salt(
                biometric_data, ""
            )  # Temporary hash for prefix
            ...
            for biometric in biometrics:
                ...
                try:
                    ...
                    logger.warning(
                        f"Legacy biometric data found for staff_id: {biometric.staff_id}"
                    )
                    continue
    ...
    def delete_biometric_data(
        self, staff_id: int, data_type: str = "all"
    ) -> Tuple[bool, str]:
    ...
    def export_biometric_data(self, staff_id: int) -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            export_data = {
                ...
                "biometric_data": {
                    ...
                    "fingerprint_enrolled_at": (
                        biometric.fingerprint_enrolled_at.isoformat()
                        if biometric.fingerprint_enrolled_at
                        else None
                    ),
                    "face_enrolled": biometric.is_face_enabled,
                    "face_enrolled_at": (
                        biometric.face_enrolled_at.isoformat()
                        if biometric.face_enrolled_at
                        else None
                    ),
                    "pin_enabled": biometric.is_pin_enabled,
                    "pin_updated_at": (
                        biometric.pin_updated_at.isoformat()
                        if biometric.pin_updated_at
                        else None
                    ),
                },
            ...
            self._audit_log(
                "biometric_export",
                {"staff_id": staff_id, "exported_at": datetime.utcnow().isoformat()},
            )
    ...
    def cleanup_expired_data(self) -> Tuple[int, int]:
        ...
        try:
            ...
            expired_biometrics = (
                self.db.query(StaffBiometric)
                .filter(StaffBiometric.updated_at < cutoff_date)
                .all()
            )
