...
class EnhancedPayrollEngine:
    ...
    def get_staff_pay_policy(self, staff_id: int) -> StaffPayPolicy:
        ...
        try:
            ...
            db_policy = (
                self.db.query(StaffPayPolicyModel)
                .filter(
            ...
            if db_policy:
                return StaffPayPolicy(
                    ...
                    regular_hours_threshold=Decimal(
                        str(db_policy.regular_hours_threshold)
                    ),
                    ...
                    retirement_contribution=Decimal(
                        str(db_policy.retirement_contribution or 0)
                    ),
                    parking_fee=Decimal(str(db_policy.parking_fee or 0)),
                )
        except Exception as e:
            logger.warning(
                f"Could not load pay policy from database for staff {staff_id}: {e}"
            )
    ...
    def calculate_hours_for_period(
        ...
        while current_day < end_date:
            ...
            current_day = datetime.combine(
                current_day + datetime.timedelta(days=1), datetime.min.time()
            ).date()
        ...
        return HoursBreakdown(
            regular_hours=regular_hours.quantize(
    ...
    def calculate_earnings(
        self, hours: HoursBreakdown, policy: StaffPayPolicy
    ) -> EarningsBreakdown:
        """
        ...
        double_time_pay = hours.double_time_hours * (
            policy.base_hourly_rate * Decimal("2.0")
        )
        ...
        return EarningsBreakdown(
            ...
            double_time_pay=double_time_pay.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            ),
            ...
            commission=Decimal(
                "0.00"
            ),  # Would be calculated for commission-based roles
        )
    ...
    def apply_benefit_deductions(
        ...
        proration_factor = self.config_service.get_benefit_proration_factor(
            location=policy.location, tenant_id=tenant_id
        )

        deductions.health_insurance = (
            policy.health_insurance * proration_factor
        ).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        deductions.dental_insurance = (
            policy.dental_insurance * proration_factor
        ).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        deductions.retirement_contribution = (
            policy.retirement_contribution * proration_factor
        ).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    ...
    async def compute_comprehensive_payroll(
        ...
        hours = self.calculate_hours_for_period(
        ...
        total_deductions = self.apply_benefit_deductions(
            tax_deductions, policy, tenant_id
        )
    ...
    async def create_employee_payment_record(
        self, payroll_calculation: Dict, tenant_id: Optional[int] = None
    ) -> EmployeePayment:
        """
        ...
        payment = EmployeePayment(
            ...
            other_deductions_amount=(
                deductions.dental_insurance
                + deductions.parking_fee
                + deductions.garnishments
                + deductions.loan_repayments
            ),
            tenant_id=tenant_id,
