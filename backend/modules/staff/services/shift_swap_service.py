...
from ..enums.scheduling_enums import ShiftStatus, SwapStatus, AvailabilityStatus
...
class ShiftSwapService:
    ...
    def check_auto_approval_eligibility(
        ...
        rules = (
            ...
            .order_by(SwapApprovalRule.priority.desc())
            .all()
        )
        ...
        # Check each rule
        for rule in rules:
            eligible, reason = self._check_rule(
                swap, from_shift, to_shift, to_staff, rule
            )
            if not eligible:
                continue
    ...
    def _check_rule(
        ...
        hours_until_shift = (
            shift_datetime - datetime.now(timezone.utc)
        ).total_seconds() / 3600
        if hours_until_shift < rule.min_advance_notice_hours:
            return (
                False,
                f"Insufficient advance notice (requires {rule.min_advance_notice_hours} hours)",
            )

        if (
            rule.max_advance_notice_hours
            and hours_until_shift > rule.max_advance_notice_hours
        ):
            return (
                False,
                f"Too far in advance (max {rule.max_advance_notice_hours} hours)",
            )

        # Check blackout dates
        # from_shift.date is a datetime object, extract the date part
        shift_date = (
            from_shift.date.date()
            if hasattr(from_shift.date, "date")
            else from_shift.date
        )
        ...
        requester_tenure_days = (
            datetime.now(timezone.utc)
            - requester.created_at.replace(tzinfo=timezone.utc)
        ).days
        ...
        if to_shift:
            ...
            if rule.max_hours_difference:
                from_hours = (
                    from_shift.end_time - from_shift.start_time
                ).total_seconds() / 3600
                to_hours = (
                    to_shift.end_time - to_shift.start_time
                ).total_seconds() / 3600
                if abs(from_hours - to_hours) > rule.max_hours_difference:
                    return (
                        False,
                        f"Shift hours difference exceeds limit ({rule.max_hours_difference} hours)",
                    )
            ...
            if (
                rule.same_location_required
                and from_shift.location_id != to_shift.location_id
            ):
        ...
        if (
            from_shift.shift_type
            and from_shift.shift_type.value in rule.restricted_shifts
        ):
            return (
                False,
                f"Shift type '{from_shift.shift_type.value}' cannot be auto-approved",
            )

        # All checks passed
        return True, "All criteria met"

    def process_swap_request(self, swap_id: int, restaurant_id: int) -> ShiftSwap:
        ...
        try:
            ...
            logger.info(
                f"Processing swap request {swap_id} for restaurant {restaurant_id}"
            )
            ...
            if eligible:
                ...
                swap.response_deadline = datetime.now(timezone.utc) + timedelta(
                    hours=deadline_hours
                )
                logger.debug(
                    f"Set response deadline for swap {swap_id} to {deadline_hours} hours ({urgency} urgency)"
                )
    ...
    def approve_swap(
        self, swap_id: int, approver_id: int, notes: Optional[str] = None
    ) -> ShiftSwap:
    ...
    def reject_swap(self, swap_id: int, approver_id: int, reason: str) -> ShiftSwap:
    ...
    def cancel_swap(self, swap_id: int, user_id: int) -> ShiftSwap:
    ...
    def get_pending_swaps_for_approval(
        self, manager_id: int, restaurant_id: int
    ) -> List[ShiftSwap]:
        ...
        swaps = (
            self.db.query(ShiftSwap)
            .join(EnhancedShift, ShiftSwap.from_shift_id == EnhancedShift.id)
            .filter(
                and_(
