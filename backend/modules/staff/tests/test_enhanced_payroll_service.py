...
class TestEnhancedPayrollService:
    ...
    def payroll_service(self, mock_db):
        ...
        with patch(
            "modules.staff.services.enhanced_payroll_service.EnhancedPayrollEngine"
        ):
    ...
    def sample_payroll_calculation(self):
        ...
        return {
            ...
            "hours_breakdown": HoursBreakdown(
                regular_hours=Decimal("40.00"), overtime_hours=Decimal("8.00")
            ),
    ...
    async def test_process_payroll_for_staff_new_record(
        ...
        with patch(
            "modules.staff.services.enhanced_payroll_service.Payroll",
            return_value=sample_payroll_record,
        ):
    ...
    async def test_process_payroll_batch_success(self, payroll_service):
        ...
        payroll_service.process_payroll_for_staff = AsyncMock(
    ...
    async def test_get_payroll_summary_with_data(self, payroll_service, mock_db):
        ...
        summary = await payroll_service.get_payroll_summary(
            pay_period_start=date(2024, 1, 15), pay_period_end=date(2024, 1, 29)
        )
        ...
        assert summary["average_hours_per_employee"] == Decimal(
            "40.00"
        )  # (45 + 35) / 2
    ...
    def test_create_payroll_breakdown(
        self, payroll_service, sample_payroll_calculation
    ):
        """Test creating PayrollBreakdown from payroll calculation."""
        breakdown = payroll_service._create_payroll_breakdown(
            sample_payroll_calculation
        )
    ...
    async def test_get_employee_payment_history(self, payroll_service, mock_db):
        ...
        history = await payroll_service.get_employee_payment_history(
            staff_id=1, limit=2
        )
    ...
    @pytest.mark.asyncio
    async def test_get_employee_payment_history_with_tenant_filter(
        self, payroll_service, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_recalculate_payroll_no_existing_record(
        self, payroll_service, mock_db
    ):
...
class TestPayrollServiceErrorHandling:
    ...
    async def test_get_payroll_summary_with_none_values(self, payroll_service, mock_db):
        ...
        assert summary["total_benefit_deductions"] == Decimal(
            "55.00"
    ...
    def test_create_payroll_breakdown_with_minimal_data(self, payroll_service):
        ...
        minimal_calc = {
            "hours_breakdown": HoursBreakdown(
                regular_hours=Decimal("20.00"), overtime_hours=Decimal("0.00")
            ),
            "earnings_breakdown": EarningsBreakdown(
                regular_pay=Decimal("300.00"), overtime_pay=Decimal("0.00")
            ),
            "deductions_breakdown": DeductionsBreakdown(federal_tax=Decimal("60.00")),
            "policy": Mock(
