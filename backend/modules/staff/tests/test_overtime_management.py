...
from modules.staff.services.attendance_optimizer import (
    AttendanceOptimizer,
    DailyHoursSummary,
)
...
class TestConfigManager:
    ...
    def test_validate_overtime_rules_invalid_daily_threshold(self):
        ...
        assert len(errors) > 0
        assert any(
            "Daily overtime threshold should be between 4 and 12 hours" in error
            for error in errors
        )
    ...
    def test_validate_overtime_rules_invalid_multiplier(self):
        ...
        assert len(errors) > 0
        assert any(
            "Overtime multiplier should be between 1.0 and 3.0" in error
            for error in errors
        )
...
class TestAttendanceOptimizer:
    ...
    def test_calculate_overtime_efficiently_double_time(self):
        ...
        assert result["double_time_hours"] == Decimal(
            "2.00"
        )  # 2 hours double time (12-14)
    ...
    def test_batch_calculate_hours_passes_config_parameters(self):
        ...
        mock_results = [
            Mock(
                staff_id=1, work_date=date(2024, 1, 1), total_hours=14.0
            ),  # 14 hours - should trigger double time
...
class TestHoursCalculator:
    ...
    def test_calculate_hours_for_period_optimized(self):
        ...
        with patch.object(
            calculator.optimizer, "get_daily_hours_aggregated"
        ) as mock_get_daily:
            with patch.object(
                calculator.optimizer, "calculate_overtime_efficiently"
            ) as mock_calc_ot:
                with patch.object(
                    calculator.config_manager, "get_overtime_rules"
                ) as mock_get_rules:
                    # Setup mocks
                    mock_get_daily.return_value = [
                        DailyHoursSummary(
                    ...
                    result = calculator.calculate_hours_for_period(
                        1, date(2024, 1, 1), date(2024, 1, 1)
                    )
    ...
    def test_batch_calculate_hours(self):
        ...
        # Mock the optimizer
        with patch.object(
            calculator.optimizer, "batch_calculate_hours_for_staff"
        ) as mock_batch:
            ...
            result = calculator.batch_calculate_hours(
                [1, 2], date(2024, 1, 1), date(2024, 1, 7)
            )
