...
class TestConflictDetection:
    ...
    def test_detect_overlapping_shifts(self, scheduling_service, mock_db):
        ...
        conflicts = scheduling_service.check_shift_conflicts(
            new_shift.staff_id, new_shift.date, new_shift.start_time, new_shift.end_time
        )
...
class TestAutoScheduling:
    ...
    def test_auto_schedule_respects_availability(
        self, scheduling_service, mock_db, sample_staff
    ):
        ...
        mock_db.query.return_value.filter.return_value.all.return_value = sample_staff[
            :1
        ]
    ...
    def test_auto_schedule_respects_max_hours(
        self, scheduling_service, mock_db, sample_staff
    ):
        ...
        current_hours = scheduling_service._calculate_weekly_hours(
            staff.id, date(2024, 2, 12)
        )

        # Should not exceed max hours (40)
        assert current_hours <= staff.max_hours_per_week

    def test_auto_schedule_avoids_conflicts(
        self, scheduling_service, mock_db, sample_staff
    ):
        ...
        result = scheduling_service.auto_schedule(
            1, date(2024, 2, 15), date(2024, 2, 15)
        )
...
class TestShiftSwapWorkflow:
    ...
    def test_create_swap_request(self, scheduling_service, mock_db):
        ...
        request = scheduling_service.create_swap_request(
            shift_id=1, requester_id=1, target_staff_id=2, reason="Personal emergency"
        )
...
class TestPublishingWorkflow:
    ...
    def test_publish_schedule(self, scheduling_service, mock_db):
        ...
        unpublished_shifts = [
            ScheduledShift(
                id=1, staff_id=1, date=date(2024, 2, 15), status=ShiftStatus.SCHEDULED
            ),
            ScheduledShift(
                id=2, staff_id=2, date=date(2024, 2, 16), status=ShiftStatus.SCHEDULED
            ),
        ]
        ...
        result = scheduling_service.publish_schedule(
            1, date(2024, 2, 15), date(2024, 2, 16)
        )

        assert result["published_count"] == 2
        assert all(
            shift.status == ShiftStatus.PUBLISHED for shift in unpublished_shifts
        )
        assert mock_db.commit.called

    @patch("backend.modules.staff.services.scheduling_service.send_notification")
    def test_publish_schedule_sends_notifications(
        self, mock_send_notification, scheduling_service, mock_db
    ):
        ...
        result = scheduling_service.publish_schedule(
            1, date(2024, 2, 15), date(2024, 2, 15), notify=True
        )
...
class TestValidation:
    ...
    def test_validate_shift_times(self, scheduling_service):
        ...
        # Valid shift
        assert (
            scheduling_service.validate_shift_times(
                datetime(2024, 2, 15, 8, 0), datetime(2024, 2, 15, 16, 0)
            )
            is True
        )
        ...
        assert (
            scheduling_service.validate_shift_times(
                datetime(2024, 2, 15, 16, 0), datetime(2024, 2, 15, 8, 0)
            )
            is False
        )
        ...
        assert (
            scheduling_service.validate_shift_times(
                datetime(2024, 2, 15, 8, 0), datetime(2024, 2, 15, 21, 0)
            )
            is False
        )
    ...
    def test_validate_availability(self, scheduling_service):
        ...
        # Valid availability
        assert scheduling_service.validate_availability(time(8, 0), time(16, 0)) is True

        # Invalid: end before start
        assert (
            scheduling_service.validate_availability(time(16, 0), time(8, 0)) is False
        )
