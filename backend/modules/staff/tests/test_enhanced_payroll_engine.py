...
class TestEnhancedPayrollEngine:
    ...
    def payroll_engine(self, mock_db):
        ...
        with (
            patch("modules.staff.services.enhanced_payroll_engine.PayrollTaxEngine"),
    ...
    def sample_attendance_logs(self):
        ...
        for i in range(5):
            ...
            check_in = datetime.combine(
                work_date, datetime.strptime("09:00", "%H:%M").time()
            )
            check_out = datetime.combine(
                work_date, datetime.strptime("17:00", "%H:%M").time()
            )

            log = AttendanceLog(
                id=i + 1, staff_id=1, check_in=check_in, check_out=check_out
            )
            logs.append(log)
    ...
    def overtime_attendance_logs(self):
        ...
        for i in range(5):
            ...
            check_out = datetime.combine(
                work_date, datetime.strptime("18:00", "%H:%M").time()
            )
    ...
    def test_get_staff_pay_policy_default_rate(
        self, payroll_engine, mock_db, sample_staff_member
    ):
    ...
    def test_calculate_hours_regular_time_only(
        self, payroll_engine, mock_db, sample_attendance_logs
    ):
    ...
    def test_calculate_hours_with_overtime(
        self, payroll_engine, mock_db, overtime_attendance_logs
    ):
        """Test calculating hours with overtime."""
        mock_db.query.return_value.filter.return_value.all.return_value = (
            overtime_attendance_logs
        )
    ...
    def test_calculate_earnings_regular_hours_only(self, payroll_engine):
        ...
        hours = HoursBreakdown(
            regular_hours=Decimal("40.00"), overtime_hours=Decimal("0.00")
        )
    ...
    def test_calculate_earnings_with_overtime(self, payroll_engine):
        ...
        policy = StaffPayPolicy(
            base_hourly_rate=Decimal("15.00"), overtime_multiplier=Decimal("1.5")
        )
        hours = HoursBreakdown(
            regular_hours=Decimal("40.00"), overtime_hours=Decimal("10.00")
        )
    ...
    async def test_calculate_tax_deductions(self, payroll_engine):
        ...
        payroll_engine.tax_service.calculate_and_save_taxes = AsyncMock(
            return_value=mock_tax_response
        )
    ...
    def test_apply_benefit_deductions(self, payroll_engine):
        ...
        assert updated_deductions.retirement_contribution == Decimal(
            "23.00"
        )  # 50 * 0.46
        assert updated_deductions.parking_fee == Decimal("6.90")  # 15 * 0.46

    @pytest.mark.asyncio
    async def test_compute_comprehensive_payroll(
        self, payroll_engine, mock_db, sample_staff_member, sample_attendance_logs
    ):
    ...
    async def test_create_employee_payment_record(self, payroll_engine, mock_db):
        ...
        payroll_calc = {
            ...
            "hours_breakdown": HoursBreakdown(
                regular_hours=Decimal("40.00"), overtime_hours=Decimal("5.00")
            ),
        ...
        with patch(
            "modules.staff.services.enhanced_payroll_engine.EmployeePayment",
...
class TestPayrollEngineEdgeCases:
    ...
    def test_fractional_hours_precision(self, payroll_engine):
        ...
        hours = HoursBreakdown(
            regular_hours=Decimal("37.25"), overtime_hours=Decimal("2.75")
        )
