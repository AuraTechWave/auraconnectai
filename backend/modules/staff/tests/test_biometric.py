...
class TestBiometricService:
    ...
    def test_fingerprint_enrollment(self, test_db, test_staff):
        ...
        biometric = test_db.query(StaffBiometric).filter(
            StaffBiometric.staff_id == 1
        ).first()
    ...
    def test_pin_setup(self, test_db, test_staff):
        ...
        success, message = service.setup_pin(
            staff_id=1,
            pin="1234"
        )
    ...
    def test_biometric_verification_and_checkin(self, test_db, test_staff):
        ...
        attendance = test_db.query(AttendanceLog).filter(
            AttendanceLog.staff_id == 1
        ).first()
    ...
    def test_biometric_checkout(self, test_db, test_staff):
        ...
        fingerprint_data = "test_fingerprint_data"
        service.enroll_fingerprint(1, fingerprint_data, "TEST-001")
        service.verify_biometric(fingerprint_data, CheckInMethod.FINGERPRINT, "TEST-001")
    ...
    def test_salt_uniqueness(self, test_db, test_staff):
        ...
        bio1 = test_db.query(StaffBiometric).filter(
            StaffBiometric.staff_id == 1
        ).first()
        bio2 = test_db.query(StaffBiometric).filter(
            StaffBiometric.staff_id == 2
        ).first()
