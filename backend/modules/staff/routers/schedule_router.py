...
async def get_schedule_preview(
    ...
    preview_data = await schedule_service.generate_preview(
        db, current_user.restaurant_id, start_date, end_date, filters
    )
...
async def publish_schedule(
    ...
    await schedule_cache_service.invalidate_restaurant_cache(current_user.restaurant_id)
...
async def create_shift(
    ...
    shift = await schedule_service.create_shift(
        db, current_user.restaurant_id, shift_data, current_user.id
    )
...
async def update_shift(
    ...
    shift = await schedule_service.update_shift(
        db, current_user.restaurant_id, shift_id, update_data, current_user.id
    )
...
async def delete_shift(
    ...
    Requires permission: schedule.manage
    """
    await schedule_service.delete_shift(db, current_user.restaurant_id, shift_id)
...
async def warm_schedule_cache(
    ...
    return {"success": True, "message": f"Cache warmed for week starting {week_start}"}
...
async def get_cache_statistics(
    ...
    Requires permission: schedule.manage
    """
    stats = await schedule_cache_service.get_cache_stats(current_user.restaurant_id)
...
async def clear_schedule_cache(
    ...
    return {"success": True, "message": "Schedule cache cleared successfully"}


@router.post("/reminders/send")
@require_permission("schedule.manage")
async def send_shift_reminders(
    hours_before: int = Query(
    ...
    result = await schedule_notification_service.send_shift_reminders(
        db, current_user.restaurant_id, hours_before
    )
