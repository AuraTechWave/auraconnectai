...
class PasswordValidationModel(BaseModel):
    ...
    email: Optional[str] = Field(
        None, description="User's email for personal info check"
    )
...
def log_security_event(
    ...
    logger.info(
        f"Security event logged: {event_type}, user_id={user_id}, success={success}"
    )
    return audit_log
...
async def validate_password_strength(
    request: PasswordValidationModel, db: Session = Depends(get_db)
    ...
    if user:
        ...
        if reset_token:
            ...
            email_sent = email_service.send_password_reset_email(
                email=request.email, reset_token=reset_token, expires_in_minutes=30
            )
...
async def confirm_password_reset(
    ...
    if user.hashed_password:
        current_algorithm_info = password_security.get_algorithm_info(
    ...
    email_service.send_password_changed_notification(
        email=user.email, ip_address=ip_address, user_agent=user_agent
    )
...
async def change_password(
    ...
    if password_security.verify_password(
        request.new_password, current_user.hashed_password
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    # Send password changed notification
    email_service.send_password_changed_notification(
        email=current_user.email, ip_address=ip_address, user_agent=user_agent
    )
...
async def generate_secure_password(
    length: int = 16, current_user: User = Depends(get_current_user)
):
    """
    ...
    validation_result = password_security.validate_password(
        generated_password, current_user.email
    )
