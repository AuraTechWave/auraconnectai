...
def get_coupon_analytics(
    ...
    try:
        ...
        report = service.generate_coupon_analytics_report(
            start_date=start_date, end_date=end_date, promotion_id=promotion_id
        )
        return report
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to generate coupon analytics: {str(e)}"
        )
...
def get_referral_analytics(
    ...
    try:
        ...
        report = service.generate_referral_analytics_report(
            start_date=start_date, end_date=end_date, program_id=program_id
        )
        return report
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to generate referral analytics: {str(e)}"
        )
...
def get_daily_analytics_aggregates(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate daily aggregates: {str(e)}"
        )
...
def get_comparison_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate comparison report: {str(e)}"
        )
...
def get_scheduled_report(
    ...
    try:
        ...
        report = service.generate_scheduled_report(
            report_type=report_type, frequency=frequency, recipients=recipients
        )
        return report
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to generate scheduled report: {str(e)}"
        )
    ...
    try:
        ...
        if report_type == "performance":
            report_data = analytics_service.generate_promotion_performance_report(
                start_date=start_date, end_date=end_date, promotion_ids=promotion_ids
            )
...
def get_metrics_summary(
    ...
    try:
        ...
        week_promotions = (
            ...
            .filter(PromotionUsage.created_at >= week_ago)
            .count()
        )

        week_coupons = (
            db.query(CouponUsage).filter(CouponUsage.created_at >= week_ago).count()
        )

        week_referrals = (
            db.query(CustomerReferral)
            .filter(CustomerReferral.created_at >= week_ago)
            .count()
        )

        month_revenue = (
            db.query(func.sum(PromotionUsage.final_order_amount))
            .filter(PromotionUsage.created_at >= month_ago)
            .scalar()
            or 0
        )

        month_discounts = (
            db.query(func.sum(PromotionUsage.discount_amount))
            .filter(PromotionUsage.created_at >= month_ago)
            .scalar()
            or 0
        )
        ...
        return {
            ...
            "last_30_days": {
                ...
                "roi_percentage": round(
                    (
                        ((month_revenue - month_discounts) / month_discounts * 100)
                        if month_discounts > 0
                        else 0
                    ),
                    2,
                ),
    ...
    try:
        ...
        for data in daily_data:
            ...
            if data.total_discount and data.total_discount > 0:
                roi = (
                    (data.total_revenue - data.total_discount) / data.total_discount
                ) * 100
        ...
        return {
            ...
            "summary": {
                ...
                "average_daily_usage": (
                    round(sum(t["usage_count"] for t in trends) / len(trends), 2)
                    if trends
                    else 0
                ),
            },
    ...
    try:
        ...
        for segment_name, segment_data in segments.items():
            if segment_data["count"] > 0:
                segment_data["avg_savings"] = round(
                    segment_data["total_savings"] / segment_data["count"], 2
                )
                segment_data["avg_spent"] = round(
                    segment_data["total_spent"] / segment_data["count"], 2
                )
                segment_data["avg_promotions"] = round(
                    segment_data["avg_promotions"] / segment_data["count"], 2
                )
            else:
        ...
        return {
            ...
            "segment_distribution": {
                f"{segment}_percentage": (
                    round((data["count"] / total_customers * 100), 1)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get customer segment analytics: {str(e)}",
        )
