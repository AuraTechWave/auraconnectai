...
def create_ab_test(
    ...
    try:
        if len(variant_promotions) == 0:
            raise HTTPException(
                status_code=400, detail="At least one variant promotion is required"
            )

        if len(variant_promotions) > 5:
            raise HTTPException(
                status_code=400, detail="Maximum 5 variant promotions allowed"
            )
...
def start_ab_test(
    test_id: str, db: Session = Depends(get_db), current_user=Depends(require_admin)
):
...
def stop_ab_test(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to stop A/B test: {str(e)}"
        )
...
def assign_user_to_variant(
    ...
    try:
        if not customer_id and not session_id:
            raise HTTPException(
                status_code=400,
                detail="Either customer_id or session_id must be provided",
            )

        service = ABTestingService(db)
        assignment = service.assign_user_to_variant(
            test_id=test_id, customer_id=customer_id, session_id=session_id
        )
        return assignment
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def get_ab_test_results(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get A/B test results: {str(e)}"
        )
...
def get_active_ab_tests(
    ...
    try:
        ...
        return {"total_active_tests": len(active_tests), "active_tests": active_tests}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get active A/B tests: {str(e)}"
        )
...
def get_all_ab_tests(
    status_filter: Optional[str] = Query(
        None, pattern="^(draft|active|stopped|winner|loser)$"
    ),
    ...
    try:
        ...
        query = db.query(Promotion).filter(Promotion.metadata["ab_test"].isnot(None))
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get A/B tests: {str(e)}"
        )
...
def get_ab_test_summary(
    ...
    try:
        ...
        total_conversions = sum(
            v["metrics"]["conversions"] for v in results["variant_results"]
        )
        total_revenue = sum(
            v["metrics"]["total_revenue"] for v in results["variant_results"]
        )

        control_variant = next(
            (v for v in results["variant_results"] if v["variant_type"] == "control"),
            None,
        )
        ...
        summary = {
            ...
            "control_conversion_rate": (
                control_variant["metrics"]["conversion_rate"] if control_variant else 0
            ),
            "winner": best_variant,
            "statistical_significance": results["statistical_analysis"].get(
                "overall_significance", "unknown"
            ),
            "created_at": (
                results.get("started_at")
                or results["variant_results"][0].get("created_at")
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get A/B test summary: {str(e)}"
        )
...
def extend_ab_test(
    ...
    try:
        ...
        if test_status != "active":
            raise HTTPException(
                status_code=400, detail=f"Cannot extend test with status: {test_status}"
            )

        # Extend end date for all test promotions
        for promotion in test_promotions:
            if promotion.end_date:
                promotion.end_date = promotion.end_date + timedelta(
        ...
        return {
            ...
            "new_end_date": (
                test_promotions[0].end_date.isoformat()
                if test_promotions[0].end_date
                else None
            ),
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to extend A/B test: {str(e)}"
        )
...
def get_ab_test_config(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get A/B test config: {str(e)}"
        )
...
def validate_ab_test_config(
    ...
    try:
        ...
        return {"valid": False, "message": str(e), "validated_config": test_config}
    except Exception as e:
        raise HTTPException(
