...
def create_bulk_coupons(
    ...
    try:
        if bulk_data.quantity > 10000:
            raise HTTPException(
                status_code=400, detail="Maximum 10,000 coupons per batch"
            )
...
def get_batch_coupons(
    batch_id: str, db: Session = Depends(get_db), current_user=Depends(require_admin)
):
...
def get_coupon_analytics(
    ...
    try:
        ...
        analytics = service.get_coupon_analytics(
            promotion_id=promotion_id, start_date=start_date, end_date=end_date
        )
...
def cleanup_expired_coupons(
    ...
    try:
        ...
        return {"message": "Expired coupons cleaned up", "expired_count": expired_count}
    except Exception as e:
...
def get_coupon_by_code(
    coupon_code: str, db: Session = Depends(get_db), current_user=Depends(require_admin)
):
