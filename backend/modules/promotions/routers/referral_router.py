...
from ..schemas.promotion_schemas import (
    ...
    ReferralProgram as ReferralProgramSchema,
    CustomerReferralCreate,
    CustomerReferral as CustomerReferralSchema,
)
...
def get_referral_programs(
    ...
    try:
        ...
        if active_only:
            now = datetime.utcnow()
            query = query.filter(
                ReferralProgram.is_active == True, ReferralProgram.start_date <= now
            ).filter(
                (ReferralProgram.end_date.is_(None)) | (ReferralProgram.end_date > now)
            )
...
def get_referral_program(
    ...
    try:
        ...
        program = (
            db.query(ReferralProgram).filter(ReferralProgram.id == program_id).first()
        )
...
def create_referral(
    ...
    try:
        ...
        if (
            ...
            and current_user.id != referral_data.referrer_id
            and not hasattr(current_user, "is_admin")
        ):
            raise HTTPException(
                status_code=403, detail="Can only create referrals for yourself"
            )

        service = ReferralService(db)
        referral = service.create_referral(
            referral_data=referral_data, program_id=program_id
        )
        return referral
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@router.get(
    "/customer/{customer_id}/as-referrer", response_model=List[CustomerReferralSchema]
)
def get_customer_referrals_as_referrer(
    ...
    try:
        ...
        referrals = service.get_customer_referrals(
            customer_id=customer_id, as_referrer=True, status=status
        )
        return referrals
    except HTTPException:
        raise
...
@router.get(
    "/customer/{customer_id}/as-referee", response_model=List[CustomerReferralSchema]
)
def get_customer_referrals_as_referee(
    ...
    try:
        ...
        service = ReferralService(db)
        referrals = service.get_customer_referrals(
            customer_id=customer_id, as_referrer=False, status=status
        )
        return referrals
    except HTTPException:
        raise
...
def get_my_referrals(
    ...
    try:
        ...
        service = ReferralService(db)
        referrals = service.get_customer_referrals(
            customer_id=current_user.id, as_referrer=as_referrer, status=status
        )
        return referrals
    except HTTPException:
        raise
...
def process_referral_signup(
    ...
    try:
        ...
        updated_referrals = service.process_referral_signup(
            referee_email=referee_email, referee_id=referee_id
        )
...
def process_referral_completion(
    order_id: int, db: Session = Depends(get_db), current_user=Depends(require_admin)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to process referral completion"
        )


@router.post("/{referral_id}/issue-rewards")
def issue_referral_rewards(
    referral_id: int, db: Session = Depends(get_db), current_user=Depends(require_admin)
):
    ...
    try:
        ...
        return {"message": "Referral rewards processed", "results": results}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
def get_referral_analytics(
    ...
    try:
        ...
        analytics = service.get_referral_analytics(
            program_id=program_id, start_date=start_date, end_date=end_date
        )
...
def expire_old_referrals(
    ...
    try:
        ...
        return {"message": "Old referrals expired", "expired_count": expired_count}
    except Exception as e:
...
def generate_referral_code(
    ...
    try:
        ...
        service = ReferralService(db)
        code = service.generate_referral_code(customer_id=customer_id, length=length)
