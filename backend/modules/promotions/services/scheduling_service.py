...
class PromotionSchedulingService:
    ...
    async def process_scheduled_promotions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            promotions_to_activate = self.db.query(Promotion).filter(
                Promotion.status == PromotionStatus.SCHEDULED,
            ...
            for promotion in promotions_to_activate:
                try:
                    ...
                    if scheduling.get('schedule_options', {}).get('auto_activate', True):
                        promotion.status = PromotionStatus.ACTIVE
            ...
            promotions_to_deactivate = self.db.query(Promotion).filter(
                Promotion.status == PromotionStatus.ACTIVE,
                Promotion.end_date <= now
            ).all()
            ...
            for promotion in promotions_to_deactivate:
                try:
                    ...
                    if scheduling.get('schedule_options', {}).get('auto_deactivate', True):
                        ...
                        if await self._handle_recurrence(promotion):
                            results['scheduled'].append({
                                'id': promotion.id,
                                'name': promotion.name,
                                'next_occurrence': promotion.metadata['scheduling']['next_occurrence']
                            })
                        
                        logger.info(f"Deactivated expired promotion: {promotion.name}")
                except Exception as e:
                    logger.error(f"Error deactivating promotion {promotion.id}: {str(e)}")
                    results['errors'].append({
                        'promotion_id': promotion.id,
                        'error': str(e)
                    })
            ...
            logger.info(f"Processed scheduled promotions: {len(results['activated'])} activated, "
    ...
    async def _handle_recurrence(self, promotion: Promotion) -> bool:
        ...
        if not schedule_options.get('recurrence_pattern') or schedule_options['recurrence_pattern'] == 'none':
            return False
        ...
        if max_occurrences and occurrences_count >= max_occurrences:
            logger.info(f"Promotion {promotion.id} reached max occurrences ({max_occurrences})")
            return False
    ...
    def create_time_based_promotion(
        ...
        try:
            ...
            logger.info(f"Created time-based promotion: {promotion.name} with {len(time_rules)} rules")
            return promotion
    ...
    async def process_time_based_promotions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            promotions_with_rules = self.db.query(Promotion).join(PromotionRule).filter(
                PromotionRule.rule_type == 'time_based',
                PromotionRule.is_active == True
            ).distinct().all()
            ...
            for promotion in promotions_with_rules:
                try:
                    ...
                    should_be_active, reason = self.evaluate_time_based_rules(promotion.id)
                    ...
                    elif not should_be_active and promotion.status == PromotionStatus.ACTIVE:
    ...
    def create_automated_promotion_sequence(
        ...
        try:
            ...
            logger.info(f"Created promotion sequence '{sequence_name}' with {len(created_promotions)} promotions")
        ...
        try:
            ...
            for promotion in promotions:
                ...
                scheduled_info = {
                    ...
                    'is_recurring': bool(scheduling.get('schedule_options', {}).get('recurrence_pattern')),
    ...
    def cancel_scheduled_promotion(
        self,
        promotion_id: int,
        cancel_future_occurrences: bool = True
    ) -> bool:
        ...
        try:
            ...
            if promotion.status not in [PromotionStatus.SCHEDULED, PromotionStatus.DRAFT]:
                raise ValueError(f"Cannot cancel promotion with status {promotion.status}")
