...
class CustomerIntegrationService:
    ...
    def process_customer_signup(
        ...
        try:
            ...
            if referral_code:
                try:
                    referral_results = self.referral_service.process_referral_signup(
                        referee_email=customer.email, referee_id=customer.id
                    )
                    ...
                    if referral_results:
                        ...
                        logger.info(
                            f"Processed referral signup for customer {customer.id} "
            ...
            logger.error(
                f"Error processing customer signup for customer {customer.id}: {str(e)}"
            )
            return {
                "customer_id": customer.id,
                "error": str(e),
    ...
    def process_customer_first_order(
        self, customer: Customer, order_amount: float
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(
                f"Error processing first order for customer {customer.id}: {str(e)}"
            )
            self.db.rollback()
            return {"error": str(e)}
        ...
        try:
            ...
            for promotion in welcome_promotions_query:
                try:
                    ...
                    logger.error(
                        f"Error creating welcome coupon for promotion {promotion.id}: {str(e)}"
                    )
                    continue

            if welcome_promotions:
                logger.info(
                    f"Created {len(welcome_promotions)} welcome promotions for customer {customer.id}"
                )
            ...
            logger.error(
                f"Error creating welcome promotions for customer {customer.id}: {str(e)}"
            )
            return []

    def process_customer_tier_change(
        self, customer: Customer, old_tier: str, new_tier: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            results = {
                "customer_id": customer.id,
                "tier_change": {"old_tier": old_tier, "new_tier": new_tier},
                "new_promotions": [],
                "tier_benefits": [],
            }
            ...
            logger.info(
                f"Processed tier change for customer {customer.id}: "
            ...
            logger.error(
                f"Error processing tier change for customer {customer.id}: {str(e)}"
            )
            return {"error": str(e)}
    ...
    def get_customer_promotion_history(
        ...
        try:
            ...
            coupon_usages = (
                ...
                .filter(CouponUsage.customer_id == customer_id)
            ...
            referrals_made = self.referral_service.get_customer_referrals(
                customer_id=customer_id, as_referrer=True
            )

            referrals_received = self.referral_service.get_customer_referrals(
                customer_id=customer_id, as_referrer=False
            )
            ...
            return {
                ...
                "referrals_summary": {
                    ...
                    "completed": len(
                        [r for r in referrals_made if r.status == "completed"]
                    ),
                    "rewarded": len(
                        [r for r in referrals_made if r.status == "rewarded"]
                    ),
                },
            }
            ...
            logger.error(
                f"Error getting promotion history for customer {customer_id}: {str(e)}"
            )
            return {"error": str(e)}
