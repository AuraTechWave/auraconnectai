...
from ..models.promotion_models import (
    Promotion, PromotionStatus, PromotionUsage, PromotionAnalytics
)
...
class ABTestingService:
    ...
    def create_ab_test(
        ...
        try:
            ...
            for idx, variant_data in enumerate(variant_promotions):
                ...
                traffic_percentage = test_config.get('variant_traffic_percentages', [])[idx] if idx < len(test_config.get('variant_traffic_percentages', [])) else (50 / len(variant_promotions))
    ...
    def _validate_ab_test_config(self, test_config: Dict[str, Any], variant_count: int):
        ...
        if variant_traffic_percentages:
            if len(variant_traffic_percentages) != variant_count:
                raise ValueError("Number of variant traffic percentages must match number of variants")
        ...
        if 'minimum_sample_size' in test_config and test_config['minimum_sample_size'] < 10:
    ...
    def assign_user_to_variant(
        ...
        try:
            # Get test promotions
            test_promotions = self.db.query(Promotion).filter(
            ...
            control_promotion = next((p for p in test_promotions if p.metadata['ab_test']['variant_type'] == 'control'), None)
            if not control_promotion or control_promotion.metadata['ab_test']['test_status'] != 'active':
            ...
            sorted_promotions = sorted(test_promotions, key=lambda p: (
            ...
            assignment_data = {
                ...
                'assigned_variant': assigned_promotion.metadata['ab_test']['variant_id'],
    ...
    def start_ab_test(self, test_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for promotion in test_promotions:
                ...
                promotion.metadata['ab_test']['started_at'] = datetime.utcnow().isoformat()
            
            self.db.commit()
            
            logger.info(f"Started A/B test {test_id} with {len(test_promotions)} promotions")
    ...
    def stop_ab_test(self, test_id: str, winning_variant: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Stopped A/B test {test_id}, winner: {winning_variant or 'none'}")
        ...
        try:
            ...
            for promotion in test_promotions:
                ...
                usage_stats = self.db.query(
                    ...
                    func.avg(PromotionUsage.final_order_amount).label('avg_order_value')
                ).filter(
            ...
            variant_results.sort(key=lambda x: (x['variant_type'] != 'control', x['variant_id']))
            
            # Determine statistical significance and winner
            significance_result = self._calculate_statistical_significance(variant_results)
            
            return {
                'test_id': test_id,
                'test_name': test_name,
                'test_config': test_config,
                'test_status': control_promotion.metadata['ab_test'].get('test_status', 'unknown'),
    ...
    def _calculate_statistical_significance(self, variant_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for variant in variant_results:
                ...
                improvement = ((variant_conversion_rate - control_conversion_rate) / control_conversion_rate * 100) if control_conversion_rate > 0 else 0
                
                # Simple confidence level estimation (this is very simplified)
                sample_size_factor = min(control_impressions, variant_impressions) / 1000
            ...
            return {
                ...
                'overall_significance': 'significant' if any(r['significance'] == 'significant' for r in significance_results) else 'weak'
            }
    ...
    def _determine_winner(self, variant_results: List[Dict[str, Any]], significance_result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for variant in variant_results:
                ...
                if variant['variant_type'] == 'control':
                    ...
                    for comparison in significance_result.get('variant_comparisons', []):
                        if comparison['variant_id'] == variant_id and comparison['significance'] == 'significant':
                            is_significant = True
                            break
            ...
            if best_variant:
                return {
                    ...
                    'improvement_over_control': 0 if best_variant['variant_type'] == 'control' else round(
                        ((best_conversion_rate - next(v['metrics']['conversion_rate'] for v in variant_results if v['variant_type'] == 'control')) / 
                         next(v['metrics']['conversion_rate'] for v in variant_results if v['variant_type'] == 'control') * 100), 2
                    ) if next(v['metrics']['conversion_rate'] for v in variant_results if v['variant_type'] == 'control') > 0 else 0
                }
    ...
    def get_active_ab_tests(self) -> List[Dict[str, Any]]:
        ...
        try:
            active_tests = self.db.query(Promotion).filter(
                Promotion.metadata['ab_test']['test_status'].astext == 'active'
            ).all()
            ...
            for promotion in active_tests:
                ...
                tests_by_id[test_id]['promotions'].append({
                    ...
                    'traffic_percentage': promotion.metadata['ab_test']['traffic_percentage']
                })
