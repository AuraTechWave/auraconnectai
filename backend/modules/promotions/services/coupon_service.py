...
from ..models.promotion_models import (
    Coupon, CouponUsage, Promotion, CouponType, PromotionStatus
)
...
class CouponService:
    ...
    def generate_coupon_code(
        ...
        raise ValueError(f"Could not generate unique coupon code after {max_attempts} attempts")
    
    def create_coupon(self, coupon_data: CouponCreate, generated_by: Optional[int] = None) -> Coupon:
        ...
        try:
            ...
            if not promotion:
                raise ValueError(f"Promotion {coupon_data.promotion_id} not found or cancelled")
            ...
            logger.info(f"Created coupon: {code} for promotion {coupon_data.promotion_id}")
            return coupon
    ...
    def create_bulk_coupons(
        ...
        try:
            ...
            if not promotion:
                raise ValueError(f"Promotion {bulk_data.promotion_id} not found or cancelled")
            ...
            created_coupons = self.db.query(Coupon).filter(
    ...
    def validate_coupon(self, validation_request: CouponValidationRequest) -> CouponValidationResponse:
        ...
        try:
            ...
            coupon = self.db.query(Coupon).options(
                joinedload(Coupon.promotion)
            ).filter(
            ...
            if not coupon:
                return CouponValidationResponse(
                    is_valid=False,
                    error_message="Coupon code not found or inactive"
                )
            ...
            if coupon.customer_id and validation_request.customer_id != coupon.customer_id:
                return CouponValidationResponse(
                    is_valid=False,
                    coupon=coupon,
            ...
            if validation_request.customer_id:
                customer_usage_count = self.db.query(CouponUsage).filter(
                    CouponUsage.coupon_id == coupon.id,
                    CouponUsage.customer_id == validation_request.customer_id
                ).count()
                
                promotion_usage_count = self.db.query(CouponUsage).join(Coupon).filter(
                    Coupon.promotion_id == coupon.promotion_id,
                    CouponUsage.customer_id == validation_request.customer_id
                ).count()
                
                if (coupon.promotion.max_uses_per_customer and 
                    promotion_usage_count >= coupon.promotion.max_uses_per_customer):
                    return CouponValidationResponse(
                        is_valid=False,
                        coupon=coupon,
            ...
            if promotion.min_order_amount and validation_request.order_total < promotion.min_order_amount:
                return CouponValidationResponse(
                    is_valid=False,
                    coupon=coupon,
            ...
            return CouponValidationResponse(
                is_valid=False,
                error_message="Error validating coupon code"
            )
    ...
    def use_coupon(
        ...
        try:
            ...
            logger.info(f"Coupon {coupon_code} used for order {order_id}, discount: ${discount_amount:.2f}")
            return usage
    ...
    def get_customer_coupons(
        ...
        query = self.db.query(Coupon).options(
            joinedload(Coupon.promotion)
        ).filter(
            or_(
    ...
    def get_coupon_usage_history(
        ...
        query = self.db.query(CouponUsage).options(
            ...
            joinedload(CouponUsage.customer)
        )
    ...
    def deactivate_coupon(self, coupon_id: int, reason: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"Deactivated coupon {coupon.code}, reason: {reason or 'Manual'}")
            return True
    ...
    def get_batch_coupons(self, batch_id: str) -> List[Coupon]:
        ...
        return self.db.query(Coupon).filter(
            Coupon.batch_id == batch_id
        ).order_by(Coupon.created_at).all()
    ...
    def get_coupon_analytics(
        ...
        try:
            ...
            return {
                ...
                "average_discount": round(total_discount / total_usage, 2) if total_usage > 0 else 0
            }
    ...
    def cleanup_expired_coupons(self) -> int:
        ...
        try:
            ...
            expired_count = self.db.query(Coupon).filter(
                Coupon.is_active == True,
                Coupon.valid_until < now
