...
from ..models.promotion_models import (
    Promotion, PromotionUsage, PromotionType, PromotionStatus, DiscountTarget
)
...
class DiscountService:
    ...
    def calculate_order_discounts(
        ...
        try:
            ...
            if request.coupon_codes:
                ...
                for coupon_code in request.coupon_codes:
                    ...
                    if validation_result.is_valid and validation_result.coupon:
                        valid_coupon_promotions.append(validation_result.coupon.promotion)
                    else:
            ...
            return DiscountCalculationResponse(
                ...
                applicable_promotions=[self._promotion_to_dict(p) for p in result.applicable_promotions],
        ...
        if customer:
            ...
            query = query.filter(
                or_(
                    ...
                    Promotion.max_uses_per_customer > func.coalesce(
    ...
    def _sort_promotions_for_application(self, promotions: List[Promotion]) -> List[Promotion]:
        ...
        if non_stackable:
            ...
            non_stackable_discount = self._estimate_promotion_discount(best_non_stackable)
    ...
    def _estimate_promotion_discount(self, promotion: Promotion) -> float:
        ...
        if promotion.promotion_type == PromotionType.PERCENTAGE_DISCOUNT:
            ...
            return promotion.max_discount_amount or (promotion.discount_value * 10)  # Rough estimate
        elif promotion.promotion_type == PromotionType.FIXED_DISCOUNT:
    ...
    def _calculate_bogo_discount(
        ...
        get_discount_percent = conditions.get("get_discount_percent", 100)  # 100% = free
    ...
    def _calculate_bundle_discount(
        ...
        for required_item in required_items:
            ...
            order_item = next((item for item in order_items if item.get("item_id") == item_id), None)
    ...
    def validate_promotion_stackability(
        ...
        try:
            ...
            if non_stackable and len(promotions) > 1:
                errors.append("Cannot stack non-stackable promotion with other promotions")
