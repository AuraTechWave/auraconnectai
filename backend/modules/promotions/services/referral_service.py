...
from ..models.promotion_models import (
    ReferralProgram, CustomerReferral, ReferralStatus
)
...
class ReferralService:
    ...
    def generate_referral_code(self, customer_id: int, length: int = 8) -> str:
        ...
        for attempt in range(max_attempts):
            ...
            # Check uniqueness
            existing = self.db.query(CustomerReferral).filter(
                CustomerReferral.referral_code == code
            ).first()
            
            if not existing:
                return code
        
        raise ValueError(f"Could not generate unique referral code after {max_attempts} attempts")
    ...
    def create_referral(
        ...
        try:
            ...
            referrer = self.db.query(Customer).filter(
                Customer.id == referral_data.referrer_id
            ).first()
            ...
            if program.max_referrals_per_customer:
                existing_referrals = self.db.query(CustomerReferral).filter(
            ...
            existing_referral = self.db.query(CustomerReferral).filter(
                ...
                CustomerReferral.status.in_([ReferralStatus.PENDING, ReferralStatus.COMPLETED])
            ).first()
            
            if existing_referral:
                raise ValueError("This email has already been referred by this customer")
            
            # Check if referee email belongs to existing customer who was already referred
            referee_customer = self.db.query(Customer).filter(
                Customer.email == referral_data.referee_email
            ).first()
            
            if referee_customer:
                existing_referee_referral = self.db.query(CustomerReferral).filter(
                    CustomerReferral.program_id == program.id,
                    CustomerReferral.referee_id == referee_customer.id,
                    CustomerReferral.status.in_([ReferralStatus.COMPLETED, ReferralStatus.REWARDED])
                ).first()
                
                if existing_referee_referral:
                    raise ValueError("This customer has already been referred in this program")
            
            # Generate referral code
            referral_code = self.generate_referral_code(referral_data.referrer_id)
            
            # Calculate expiration date
            expires_at = datetime.utcnow() + timedelta(days=program.referral_validity_days)
    ...
    def process_referral_signup(self, referee_email: str, referee_id: int) -> List[CustomerReferral]:
        """
        ...
        try:
            ...
            pending_referrals = self.db.query(CustomerReferral).filter(
            ...
            logger.info(f"Updated {len(updated_referrals)} referrals for new customer {referee_id}")
    ...
    def process_referral_completion(self, order_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            referrals_to_complete = self.db.query(CustomerReferral).options(
            ...
            for referral in referrals_to_complete:
                ...
                if program.min_referee_order_amount and order.total < program.min_referee_order_amount:
                    continue
            ...
            logger.info(f"Completed {len(processed_referrals)} referrals for order {order_id}")
    ...
    def issue_referral_rewards(self, referral_id: int) -> Dict[str, Any]:
        ...
        """
        try:
            referral = self.db.query(CustomerReferral).options(
                joinedload(CustomerReferral.program),
    ...
    def _issue_reward(
        ...
        try:
            if reward_type == "points":
                ...
                logger.info(f"Cash reward of ${reward_value} issued to customer {customer.id}: {reason}")
                return True
    ...
    def get_referral_by_code(self, referral_code: str) -> Optional[CustomerReferral]:
        ...
        return self.db.query(CustomerReferral).options(
            joinedload(CustomerReferral.program),
            joinedload(CustomerReferral.referrer)
        ).filter(
            CustomerReferral.referral_code == referral_code.upper()
        ).first()
    ...
    def get_referral_analytics(
        ...
        try:
            ...
            total_referrer_rewards = query.filter(
                CustomerReferral.referrer_rewarded == True
            ).with_entities(
                func.sum(CustomerReferral.referrer_reward_amount)
            ).scalar() or 0.0
            
            total_referee_rewards = query.filter(
                CustomerReferral.referee_rewarded == True
            ).with_entities(
                func.sum(CustomerReferral.referee_reward_amount)
            ).scalar() or 0.0
            
            # Average time to completion
            completed_query = query.filter(
                CustomerReferral.completed_at.isnot(None)
            )
            ...
            return {
                ...
                "total_rewards_issued": round(total_referrer_rewards + total_referee_rewards, 2),
    ...
    def expire_old_referrals(self) -> int:
        ...
        try:
            ...
            expired_count = self.db.query(CustomerReferral).filter(
                CustomerReferral.status == ReferralStatus.PENDING,
