...
from ..schemas.promotion_schemas import DiscountCalculationRequest, DiscountCalculationResponse
...
class OrderPromotionService:
    ...
    def calculate_order_discounts(
        ...
        try:
            ...
            logger.info(f"Calculated discounts for order {order.id}: "
                       f"${result.total_discount:.2f} off ${result.original_total:.2f}")
            
            return result
            
        except Exception as e:
            logger.error(f"Error calculating order discounts for order {order.id}: {str(e)}")
            raise
    ...
    def apply_discounts_to_order(
        ...
        try:
            ...
            self.calculation_service.recalculate_with_discount(
                order=order,
                discount_amount=discount_response.total_discount
            )
            
            self.db.commit()
            
            logger.info(f"Applied discounts to order {order.id}: "
    ...
    def process_order_completion(self, order: Order) -> Dict[str, Any]:
        ...
        try:
            ...
            if order.customer_id:
                referral_results = self.referral_service.process_referral_completion(order.id)
                ...
                for referral in referral_results:
                    try:
                        ...
                        logger.error(f"Error issuing referral rewards for "
                                   f"referral {referral['referral_id']}: {str(e)}")
            
            # Update promotion analytics
            self._update_promotion_analytics(order)
            
            logger.info(f"Processed order completion for order {order.id}: "
                       f"{len(results['referrals_processed'])} referrals processed")
            
            return results
            
        except Exception as e:
            logger.error(f"Error processing order completion for order {order.id}: {str(e)}")
            return {"error": str(e)}
    ...
    def validate_promotion_eligibility(
        ...
        try:
            ...
            if promotion.min_order_amount and order_total < promotion.min_order_amount:
                return False, f"Order total must be at least ${promotion.min_order_amount:.2f}"
        ...
        try:
            ...
            for promotion in active_promotions:
                ...
                if is_eligible:
                    ...
                    discount_amount = self.discount_service.calculate_promotion_discount(
                        promotion=promotion,
                        order_total=order_total,
                        order_items=order_items,
                        customer_id=order.customer_id
            ...
            applicable_promotions.sort(key=lambda x: x["potential_discount"], reverse=True)
            
            return applicable_promotions
            
        except Exception as e:
            logger.error(f"Error getting applicable promotions for order {order.id}: {str(e)}")
            return []
    ...
    def _update_promotion_analytics(self, order: Order):
        ...
        try:
            ...
            usages = self.db.query(PromotionUsage).filter(
    ...
    def get_order_promotion_summary(self, order_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error getting order promotion summary for order {order_id}: {str(e)}")
            return {"error": str(e)}