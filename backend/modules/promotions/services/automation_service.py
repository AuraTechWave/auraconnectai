...
class PromotionAutomationService:
    ...
    def create_automated_promotion(
        ...
        try:
            ...
            rule = PromotionRule(
                ...
                action_value={'duration_hours': automation_options.get('duration_hours', 24)},
                is_active=True
            )
            self.db.add(rule)
            ...
            logger.info(f"Created automated promotion: {name} with trigger {trigger_type}")
            return promotion
    ...
    async def process_triggers(self, trigger_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for rule in rules:
                try:
                    ...
                    if not handler:
                        logger.warning(f"No handler for trigger type: {rule.condition_type}")
                        continue
    ...
    async def _activate_triggered_promotion(
        ...
        try:
            ...
            if last_triggered:
                cooldown_hours = automation.get('automation_options', {}).get('cooldown_hours', 0)
                if cooldown_hours > 0:
                    last_triggered_dt = datetime.fromisoformat(last_triggered)
                    if datetime.utcnow() - last_triggered_dt < timedelta(hours=cooldown_hours):
    ...
    def _handle_customer_lifecycle_trigger(
        ...
        event_type = conditions.get('event_type')  # signup, first_purchase, birthday, etc.
        
        if event_type == 'signup':
            # Check for new signups in the last hour
            hour_ago = datetime.utcnow() - timedelta(hours=1)
            new_customers = self.db.query(Customer).filter(
                Customer.created_at >= hour_ago
            ).count()
    ...
    def _handle_purchase_behavior_trigger(
        ...
        if behavior_type == 'high_value_purchase':
            ...
            high_value_orders = self.db.query(Order).filter(
            ...
            category_orders = self.db.query(Order).filter(
                Order.created_at >= hour_ago,
    ...
    def get_automation_performance(
        ...
        try:
            ...
            automated_promotions = self.db.query(Promotion).filter(
                Promotion.metadata['automation'].isnot(None)
            ).all()
            ...
            for promotion in automated_promotions:
                ...
                revenue = self.db.query(func.sum(PromotionUsage.final_order_amount)).filter(
                ...
                metrics['performance_by_trigger'][trigger_type]['total_triggers'] += trigger_count
                metrics['performance_by_trigger'][trigger_type]['total_revenue'] += float(revenue)
