...
class IdempotentDiscountService:
    ...
    def check_existing_discount_application(
        ...
        if promotion_ids:
            existing_promo_usage = self.db.query(PromotionUsage).filter(
        ...
        if coupon_codes:
            ...
            if coupon_ids:
                existing_coupon_usage = self.db.query(CouponUsage).filter(
        ...
        existing_applications["has_applications"] = (
            len(existing_applications["promotions"]) > 0 or 
            len(existing_applications["coupons"]) > 0
        )
    ...
    def apply_discount_idempotent(
        ...
        try:
            ...
            if cached_result and not force_reapply:
                logger.info(f"Returning cached discount application result for key: {idempotency_key}")
                ...
                for promotion_id in promotion_ids:
                    try:
                        promo_result = self._apply_single_promotion(
                            order, order_items, customer_id, promotion_id, idempotency_key
                        )
                        
                        if promo_result["success"]:
                            application_results["promotions_applied"].append(promo_result)
                            application_results["total_discount"] += promo_result["discount_amount"]
                        else:
                ...
                for coupon_code in coupon_codes:
                    try:
                        coupon_result = self._apply_single_coupon(
                            order, order_items, customer_id, coupon_code, idempotency_key
                        )
                        
                        if coupon_result["success"]:
                            application_results["coupons_applied"].append(coupon_result)
                            application_results["total_discount"] += coupon_result["discount_amount"]
                        else:
            ...
            self._invalidate_related_caches(order_id, customer_id, promotion_ids, coupon_codes)
            
            logger.info(f"Successfully applied discounts to order {order_id}. Total discount: {application_results['total_discount']}")
    ...
    def _apply_single_promotion(
        ...
        existing_usage = self.db.query(PromotionUsage).filter(
    ...
    def _apply_single_coupon(
        ...
        existing_usage = self.db.query(CouponUsage).filter(
    ...
    def _invalidate_related_caches(
        ...
        order_cache_patterns = [
            f"order:{order_id}:*",
            f"order_discount:{order_id}:*"
        ]
        ...
        try:
            ...
            for usage in promotion_usages:
                promotion = self.db.query(Promotion).filter(
                    Promotion.id == usage.promotion_id
                ).first()
            ...
            order = self.db.query(Order).filter(Order.id == order_id).first()
            if order:
                order.discount_amount = max(0.0, (order.discount_amount or 0.0) - total_rollback_amount)
                order.final_amount = order.total_amount - order.discount_amount
                order.updated_at = datetime.utcnow()
            ...
            logger.info(f"Successfully rolled back discount application for order {order_id}")
            return result
    ...
    def get_application_status(self, idempotency_key: str) -> Dict[str, Any]:
        ...
        if promotion_usages or coupon_usages:
            ...
            total_discount = sum(float(u.discount_amount) for u in promotion_usages + coupon_usages)
