...
class AnalyticsTaskService:
    ...
    async def run_daily_analytics_aggregation(self, target_date: datetime = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            storage_results = await self._store_daily_aggregates(db, aggregates, target_date)
            
            logger.info(f"Completed daily analytics aggregation for {target_date.date()}")
    ...
    async def _store_daily_aggregates(
        ...
        try:
            ...
            for promo_agg in aggregates.get("promotion_aggregates", []):
                ...
                existing = db.query(PromotionAnalytics).filter(
                ...
                if existing:
                    ...
                    existing.conversion_rate = promo_agg["usage_count"] / max(promo_agg.get("impressions", 1), 1) * 100
                    existing.average_order_value = promo_agg["total_revenue"] / max(promo_agg["usage_count"], 1)
                    
                    if promo_agg["total_discount"] > 0:
                        existing.return_on_investment = ((promo_agg["total_revenue"] - promo_agg["total_discount"]) / promo_agg["total_discount"]) * 100
                    ...
                    analytics_record = PromotionAnalytics(
                        ...
                        conversion_rate=promo_agg["usage_count"] / max(promo_agg.get("impressions", 1), 1) * 100,
                        average_order_value=promo_agg["total_revenue"] / max(promo_agg["usage_count"], 1),
                        customer_acquisition_cost=0.0,  # Would need marketing spend data
                        return_on_investment=((promo_agg["total_revenue"] - promo_agg["total_discount"]) / promo_agg["total_discount"]) * 100 if promo_agg["total_discount"] > 0 else 0
                    )
            ...
            return {
                ...
                "referral_aggregates_stored": bool(aggregates.get("referral_aggregates"))
            }
    ...
    async def cleanup_old_analytics_data(self, retention_days: int = 365) -> Dict[str, Any]:
        ...
        try:
            ...
            deleted_count = db.query(PromotionAnalytics).filter(
                PromotionAnalytics.date < cutoff_date.date()
            ).delete()
    ...
    async def update_promotion_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            from ..models.promotion_models import Promotion, PromotionUsage, PromotionStatus
            ...
            for promotion in active_promotions:
                ...
                if usage_data and usage_data.total_usage:
                    ...
                    if promotion.impressions > 0:
                        promotion.conversion_rate = (usage_data.total_usage / promotion.impressions) * 100
    ...
    async def detect_promotion_anomalies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_report = analytics_service.generate_promotion_performance_report(
            ...
            for promo in performance_report["promotion_details"]:
                ...
                conversion_rate = promo["engagement_metrics"]["conversion_rate_percentage"]
                if conversion_rate < 1.0 and promo["engagement_metrics"]["impressions"] > 100:
                    anomalies.append({
                ...
                if (promo["usage_metrics"]["total_usage"] == 0 and 
                    promo["engagement_metrics"]["impressions"] > 50):
                    anomalies.append({
                        "promotion_id": promo["promotion_id"],
                        "promotion_name": promo["promotion_name"],
