...
from ..models.promotion_models import (
    Promotion, PromotionUsage, PromotionStatus, PromotionType, PromotionAnalytics
)
...
class PromotionService:
    ...
    def update_promotion(
        ...
        try:
            ...
            if promotion.status in [PromotionStatus.CANCELLED, PromotionStatus.ENDED]:
                raise ValueError(f"Cannot update promotion with status {promotion.status}")
    ...
    def search_promotions(
        ...
        try:
            ...
            if params.promotion_type:
                query = query.filter(Promotion.promotion_type.in_(params.promotion_type))
            ...
            if params.requires_coupon is not None:
                query = query.filter(Promotion.requires_coupon == params.requires_coupon)
            ...
            if params.min_discount_value is not None:
                query = query.filter(Promotion.discount_value >= params.min_discount_value)
            
            if params.max_discount_value is not None:
                query = query.filter(Promotion.discount_value <= params.max_discount_value)
            
            if params.target_customer_segment:
                query = query.filter(
                    Promotion.target_customer_segments.contains([params.target_customer_segment])
                )
    ...
    def pause_promotion(self, promotion_id: int) -> bool:
        ...
        try:
            ...
            if promotion.status != PromotionStatus.ACTIVE:
                raise ValueError(f"Cannot pause promotion with status {promotion.status}")
    ...
    def record_promotion_usage(
        ...
        try:
            ...
            logger.info(f"Recorded promotion usage: {promotion_id} for order {order_id}")
            return usage
        ...
        if customer_id:
            ...
            query = query.outerjoin(
                ...
                or_(
                    ...
                    Promotion.max_uses_per_customer > func.coalesce(customer_usage_subquery.c.usage_count, 0)
                )
            )
        
        return query.order_by(Promotion.priority.desc(), Promotion.created_at.desc()).all()
    ...
    def update_promotion_status_by_schedule(self) -> Dict[str, int]:
        ...
        try:
            ...
            activated = self.db.query(Promotion).filter(
                Promotion.status == PromotionStatus.SCHEDULED,
                Promotion.start_date <= now
            ).update({
            ...
            expired = self.db.query(Promotion).filter(
                Promotion.status == PromotionStatus.ACTIVE,
                Promotion.end_date < now
            ).update({
            ...
            logger.info(f"Updated promotion statuses: {activated} activated, {expired} expired")
    ...
    def create_ab_test(self, ab_config: ABTestConfig) -> List[Promotion]:
        ...
        try:
            ...
            logger.info(f"Created A/B test: {ab_config.test_name} with {len(created_promotions)} variants")
    ...
    def get_promotion_analytics_summary(
        ...
        try:
            ...
            if start_date:
                usage_query = usage_query.filter(PromotionUsage.created_at >= start_date)
            ...
            # Conversion rate
            conversion_rate = (promotion.conversions / promotion.impressions * 100) if promotion.impressions > 0 else 0
            ...
            promotion_cost = total_discount  # Simplified - actual cost might include other factors
            roi = ((total_revenue - promotion_cost) / promotion_cost * 100) if promotion_cost > 0 else 0
            ...
            return {
                ...
                "period": {
                    "start_date": start_date.isoformat() if start_date else promotion.start_date.isoformat(),
                    "end_date": end_date.isoformat() if end_date else promotion.end_date.isoformat()
                },
                "usage_metrics": {
                    "total_usage": total_usage,
                    "unique_customers": unique_customers,
                    "usage_percentage": (total_usage / promotion.max_uses_total * 100) if promotion.max_uses_total else 0
                },
                "financial_metrics": {
                ...
                "engagement_metrics": {
                    ...
                    "click_through_rate": round((promotion.clicks / promotion.impressions * 100) if promotion.impressions > 0 else 0, 2)
                }
            }
