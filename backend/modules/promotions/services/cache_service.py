...
class PromotionCacheService:
    ...
    def cache_promotion_eligibility(
        ...
        value = {"is_eligible": is_eligible, "reason": reason, "cached_at": datetime.utcnow().isoformat()}
        return self.set(key, value, ttl)
    
    def get_promotion_eligibility(self, promotion_id: int, customer_id: int) -> Optional[Dict]:
    ...
    def get_discount_calculation(self, promotion_id: int, order_hash: str) -> Optional[Dict]:
    ...
    def get_coupon_validation(self, coupon_code: str, customer_id: int) -> Optional[Dict]:
    ...
    def get_analytics_report(self, report_type: str, parameters_hash: str) -> Optional[Dict]:
    ...
    def invalidate_coupon_cache(self, coupon_code: str) -> None:
        ...
        deleted_count = self.delete_pattern(pattern)
        if deleted_count > 0:
            logger.info(f"Invalidated {deleted_count} cache entries for coupon: {coupon_code}")
    ...
    def check_rate_limit(
        ...
        try:
            ...
            return {"allowed": True, "remaining": limit, "reset_time": None, "error": str(e)}
...
def cached(
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            cache_key = cache._generate_cache_key(key_prefix, func_name, *args, **kwargs)
