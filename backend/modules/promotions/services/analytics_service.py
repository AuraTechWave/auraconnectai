...
class PromotionAnalyticsService:
    ...
    def generate_promotion_performance_report(
        ...
        try:
            ...
            if total_discounts_given > 0:
                roi_percentage = (
                    (total_revenue_generated - total_discounts_given)
                    / total_discounts_given
                ) * 100

            # Top performing promotions
            top_promotions = sorted(
                promotion_metrics, key=lambda x: x["roi_percentage"], reverse=True
            )[:5]
            ...
            engagement_metrics = self._calculate_customer_engagement(
                start_date, end_date
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "active_promotions": len(
                        [p for p in promotions if p.status == PromotionStatus.ACTIVE]
                    ),
                    ...
                    "average_discount_per_order": round(
                        (
                            total_discounts_given / total_orders_affected
    ...
    def _calculate_promotion_metrics(
        self, promotion: Promotion, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "time_metrics": {
                ...
                "days_remaining": (
                    max(0, (promotion.end_date - datetime.utcnow()).days)
    ...
    def _analyze_promotion_types(self, promotion_metrics: List[Dict]) -> Dict[str, Any]:
        ...
        for metrics in promotion_metrics:
            ...
            stats["total_discount"] += metrics["financial_metrics"][
        ...
        for promo_type, stats in type_stats.items():
            if stats["total_discount"] > 0:
                stats["avg_roi"] = (
                    (stats["total_revenue"] - stats["total_discount"])
                    / stats["total_discount"]
                ) * 100
            stats["avg_usage_per_promotion"] = stats["total_usage"] / stats["count"]
            stats["avg_discount_per_promotion"] = (
                stats["total_discount"] / stats["count"]
            )
    ...
    def _calculate_customer_engagement(
        ...
        customer_usage = (
            self.db.query(
                ...
                func.count(func.distinct(PromotionUsage.promotion_id)).label(
                    "unique_promotions"
                ),
            )
            .filter(
            ...
            .group_by(PromotionUsage.customer_id)
            .all()
        )
        ...
        segments = {
            ...
            "medium_users": len(
                [cu for cu in customer_usage if 3 <= cu.usage_count <= 5]
            ),
        ...
        return {
            ...
            "engagement_distribution": {
                "light_users_percentage": round(
                    (segments["light_users"] / total_customers) * 100, 1
                ),
                "medium_users_percentage": round(
                    (segments["medium_users"] / total_customers) * 100, 1
                ),
                "heavy_users_percentage": round(
                    (segments["heavy_users"] / total_customers) * 100, 1
                ),
            },
        }
    ...
    def generate_coupon_analytics_report(
        ...
        try:
            ...
            unique_customers = len(
                set(usage.customer_id for usage in coupon_usages if usage.customer_id)
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_discount_per_coupon": round(
                        (
                            total_discount / total_coupons_used
                            if total_coupons_used > 0
                            else 0
                        ),
                        2,
                    ),
        ...
        try:
            ...
            for status in ReferralStatus:
                status_counts[status.value] = len(
                    [r for r in referrals if r.status == status]
                )

            # Calculate completion rate
            total_referrals = len(referrals)
            completed_referrals = status_counts.get("completed", 0) + status_counts.get(
                "rewarded", 0
            )
            completion_rate = (
            ...
            rewarded_referrals = [
                r for r in referrals if r.status == ReferralStatus.REWARDED
            ]
            total_rewards = sum(
            ...
            top_referrers = (
                ...
                .group_by(CustomerReferral.referrer_id)
                .order_by(desc("successful_referrals"))
                .limit(10)
                .all()
            )
            ...
            program_performance = (
                ...
                .join(CustomerReferral)
                .filter(
                    CustomerReferral.created_at >= start_date,
                    CustomerReferral.created_at <= end_date,
                )
                .group_by(ReferralProgram.id, ReferralProgram.name)
                .all()
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "unique_referees": len(
                        set(r.referee_id for r in referrals if r.referee_id)
                    ),
                },
                ...
                "top_referrers": [
                    {
                        ...
                        "success_rate": round(
                            (tr.successful_referrals / tr.referral_count * 100), 2
                        ),
                    }
                ...
                "program_performance": [
                    {
                        ...
                        "success_rate": (
                            round(
                                (pp.successful_referrals / pp.total_referrals * 100), 2
                            )
                            if pp.total_referrals > 0
                            else 0
                        ),
                    }
    ...
    def generate_daily_analytics_aggregates(
        ...
        try:
            ...
            coupon_usage = (
                self.db.query(
                    ...
                    func.sum(CouponUsage.discount_amount).label("total_discount"),
                    func.count(func.distinct(CouponUsage.customer_id)).label(
                        "unique_customers"
                    ),
                )
                .filter(
            ...
            referral_stats = (
                self.db.query(
                    func.count(CustomerReferral.id).label("total_referrals"),
                    func.count(
                        case([(CustomerReferral.status == ReferralStatus.COMPLETED, 1)])
                    ).label("completed_referrals"),
                    func.count(
                        case([(CustomerReferral.status == ReferralStatus.REWARDED, 1)])
            ...
            logger.error(
                f"Error generating daily analytics for {target_date}: {str(e)}"
            )
            raise
