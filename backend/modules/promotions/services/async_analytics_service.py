...
class AsyncAnalyticsService:
    ...
    async def _get_async_engine(self):
        ...
        if not self._async_engine:
            ...
            database_url = getattr(
                settings, "DATABASE_URL", "postgresql://localhost/auraconnect"
            )
            async_url = database_url.replace("postgresql://", "postgresql+asyncpg://")

            self._async_engine = create_async_engine(
                async_url, echo=False, pool_size=5, max_overflow=10, pool_pre_ping=True
            )
    ...
    async def generate_daily_analytics_batch(
        self, start_date: datetime, end_date: datetime, batch_size: int = 1000
    ) -> Dict[str, Any]:
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                errors.append({"date": target_date.isoformat(), "error": str(result)})
            else:
                successful_results.append(
                    {"date": target_date.isoformat(), "result": result}
                )
    ...
    async def generate_daily_analytics_async(
        ...
        try:
            async with await self._get_async_session() as session:
                ...
                result = await session.execute(
                    promotion_query, {"target_date": target_date.date()}
                )
                ...
                coupon_result = await session.execute(
                    coupon_query, {"target_date": target_date.date()}
                )
                ...
                referral_result = await session.execute(
                    referral_query, {"target_date": target_date.date()}
                )
                    ...
                    "promotion_aggregates": [
                        {
                            ...
                            "conversion_rate": (
                                row.usage_count / max(row.impressions or 1, 1)
                            )
                            * 100,
                        }
            ...
            logger.error(
                f"Error generating async daily analytics for {target_date}: {e}"
            )
            raise
    ...
    async def generate_real_time_metrics(self) -> Dict[str, Any]:
        ...
        try:
            async with await self._get_async_session() as session:
                ...
                result = await session.execute(
                    metrics_query, {"current_hour": current_hour}
                )
    ...
    async def stream_analytics_updates(
        self, callback: Callable[[Dict[str, Any]], None], interval_seconds: int = 60
    ):
    ...
    async def batch_update_promotion_metrics(
        self, promotion_ids: List[int], batch_size: int = 100
    ) -> Dict[str, Any]:
        ...
        tasks = [self._update_promotion_batch_metrics(batch) for batch in batches]
    ...
    async def _update_promotion_batch_metrics(
        ...
        try:
            async with await self._get_async_session() as session:
                ...
                result = await session.execute(
                    update_query, {"promotion_ids": promotion_ids}
                )
    ...
    async def generate_analytics_summary_async(
        ...
        try:
            ...
            promotion_summary, coupon_summary, referral_summary, customer_summary = (
                results
            )
    ...
    async def _get_promotion_summary_async(
        ...
        async with await self._get_async_session() as session:
            ...
            return {
                ...
                "unique_customers": row.unique_customers,
                "avg_discount_per_order": float(row.total_discount)
                / max(row.total_usage, 1),
                "roi_percentage": (
                    (float(row.total_revenue) - float(row.total_discount))
                    / max(float(row.total_discount), 1)
                )
                * 100,
            }
    ...
    async def _get_referral_summary_async(
        ...
        async with await self._get_async_session() as session:
            ...
            return {
                ...
                "completion_rate": (
                    row.completed_referrals / max(row.total_referrals, 1)
                )
                * 100,
            }
    ...
    async def _get_customer_summary_async(
        ...
        async with await self._get_async_session() as session:
            ...
            return {
                ...
                "interactions_per_customer": (row.total_interactions or 0)
                / max(row.total_customers_engaged or 1, 1),
            }
    ...
    async def cleanup_old_analytics_async(
        ...
        try:
            ...
            async with await self._get_async_session() as session:
                ...
                result = await session.execute(
                    delete_query, {"cutoff_date": cutoff_date.date()}
                )
