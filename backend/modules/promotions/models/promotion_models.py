...
class Promotion(Base, TimestampMixin):
    ...
    status = Column(
        String(20), nullable=False, default=PromotionStatus.DRAFT, index=True
    )
    ...
    coupons = relationship(
        "Coupon", back_populates="promotion", cascade="all, delete-orphan"
    )
...
class Coupon(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    promotion_id = Column(
        Integer, ForeignKey("promotions.id"), nullable=False, index=True
    )
...
class CustomerReferral(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    program_id = Column(
        Integer, ForeignKey("referral_programs.id"), nullable=False, index=True
    )
    ...
    referrer = relationship(
        "Customer", foreign_keys=[referrer_id], back_populates="referrals_made"
    )
    referee = relationship(
        "Customer", foreign_keys=[referee_id], back_populates="referrals_received"
    )
...
class PromotionRule(Base, TimestampMixin):
    ...
    )  # customer_segment, item_quantity, etc.
    ...
    )  # e.g., customer_lifecycle, purchase_behavior
    condition_value = Column(
        JSONB, nullable=True
    )  # JSON structure defining the condition specifics
    action_type = Column(
        String(50), nullable=True
    )  # e.g., activate_promotion, deactivate_promotion
    action_value = Column(
        JSONB, nullable=True
    )  # JSON detailing action parameters (duration_hours, status, etc.)
