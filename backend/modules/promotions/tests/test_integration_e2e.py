...
class TestPromotionSystemE2E:
    ...
    def test_complete_promotion_lifecycle(self, test_app):
        ...
        response = client.post(
            "/api/v1/promotions/", json=promotion_data, headers=headers
        )
        ...
        response = client.post(
            f"/api/v1/promotions/{promotion_id}/activate", headers=headers
        )
        ...
        response = client.post(
            f"/api/v1/coupons/bulk/{promotion_id}", json=coupon_data, headers=headers
        )
        assert response.status_code == 201
        ...
        discount_data = {"order_items": order_items, "customer_id": customer.id}
        ...
        apply_data = {"order_items": order_items, "coupon_code": test_coupon_code}
        ...
        response = client.get(
            f"/api/v1/promotions/{promotion_id}/statistics", headers=headers
        )
        assert response.status_code == 200
        stats = response.json()
    ...
    def test_referral_program_complete_flow(self, test_app):
        ...
        response = client.post(
            "/api/v1/referrals/programs/", json=program_data, headers=headers
        )
        assert response.status_code == 201
        ...
        response = client.get(
            f"/api/v1/referrals/validate/{referral_code}", headers=headers
        )
        assert response.status_code == 200
        assert response.json()["is_valid"] is True
        ...
        response = client.post(
            "/api/v1/referrals/process", json=referral_process_data, headers=headers
        )
        assert response.status_code == 200
        ...
        completion_data = {"referral_code": referral_code, "order_id": order.id}

        response = client.post(
            "/api/v1/referrals/complete", json=completion_data, headers=headers
        )
        assert response.status_code == 200

        # Step 6: Verify rewards were issued
        response = client.get(
            f"/api/v1/referrals/customer/{referrer.id}/summary", headers=headers
        )
        assert response.status_code == 200
    ...
    def test_ab_testing_complete_flow(self, test_app):
        ...
        response = client.post(
            "/api/v1/ab-testing/create", json=ab_test_data, headers=headers
        )
        assert response.status_code == 200
        ...
        assert (
            first_assignment["assigned_variant"]
            == second_assignment["assigned_variant"]
        )
    ...
    def test_automation_triggers_flow(self, test_app):
        ...
        automation_data = {
            ...
            "trigger_conditions": {"event_type": "birthday"},
            "promotion_config": {
                "name": "Happy Birthday",
            ...
            "automation_options": {"duration_hours": 48, "max_triggers": 1},
        }

        response = client.post(
            "/api/v1/automation/create", json=automation_data, headers=headers
        )
        assert response.status_code == 200
    ...
    def test_bulk_operations_performance(self, test_app):
        ...
        bulk_update_data = {"promotion_ids": promotion_ids, "status": "active"}

        response = client.post(
            "/api/v1/promotions/bulk-update-status",
    ...
    def test_concurrent_usage_handling(self, test_app):
        ...
        for i, order in enumerate(orders):
            try:
                ...
                apply_data = {"order_items": order_items, "coupon_code": coupon_code}
