...
from modules.promotions.schemas.promotion_schemas import (
    PromotionCreate,
    PromotionUpdate,
)
...
class TestPromotionService:
    ...
    def test_update_promotion(self, promotion_service, sample_promotion):
        ...
        update_data = PromotionUpdate(name="Updated Promotion", discount_value=20.0)
    ...
    def test_activate_promotion(self, promotion_service, db_session, promotion_factory):
        ...
        promotion = promotion_factory.create_percentage_promotion(
            db_session, status=PromotionStatus.DRAFT
        )
    ...
    def test_activate_promotion_invalid_status(
        self, promotion_service, db_session, promotion_factory
    ):
        """Test activating a promotion with invalid status"""
        promotion = promotion_factory.create_percentage_promotion(
            db_session, status=PromotionStatus.ENDED
        )
    ...
    def test_get_active_promotions(
        ...
        active_promo1 = promotion_factory.create_percentage_promotion(
            db_session, name="Active 1", status=PromotionStatus.ACTIVE
        )
        active_promo2 = promotion_factory.create_percentage_promotion(
            db_session, name="Active 2", status=PromotionStatus.ACTIVE
        )
        promotion_factory.create_percentage_promotion(
            db_session, name="Draft", status=PromotionStatus.DRAFT
        )
        promotion_factory.create_percentage_promotion(
            db_session, name="Paused", status=PromotionStatus.PAUSED
        )
    ...
    def test_get_promotions_by_type(
        self, promotion_service, db_session, promotion_factory
    ):
    ...
    def test_check_promotion_eligibility(
        self, promotion_service, sample_promotion, sample_customer
    ):
    ...
    def test_check_promotion_eligibility_inactive(
        self, promotion_service, sample_promotion, sample_customer
    ):
    ...
    def test_check_promotion_eligibility_expired(
        self, promotion_service, sample_promotion, sample_customer
    ):
    ...
    def test_check_promotion_eligibility_max_uses_reached(
        self, promotion_service, sample_promotion, sample_customer
    ):
    ...
    def test_increment_promotion_usage(self, promotion_service, sample_promotion):
        ...
        updated_promotion = promotion_service.increment_promotion_usage(
            sample_promotion.id
        )

        assert updated_promotion.current_uses == initial_uses + 1

    def test_get_promotion_statistics(
        self, promotion_service, sample_promotion, db_session
    ):
    ...
    def test_bulk_update_promotion_status(
        self, promotion_service, db_session, promotion_factory
    ):
        ...
        promo3 = promotion_factory.create_percentage_promotion(
            db_session, status=PromotionStatus.ACTIVE
        )
