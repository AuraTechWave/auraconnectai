...
class TestCouponService:
    ...
    def test_generate_coupon_code_with_prefix_suffix(self, coupon_service):
        ...
        code = coupon_service.generate_coupon_code(
            length=6, prefix="SALE", suffix="2024"
        )
    ...
    def test_generate_coupon_code_allow_ambiguous(self, coupon_service):
        ...
        codes = [
            coupon_service.generate_coupon_code(exclude_ambiguous=False)
            for _ in range(100)
        ]
    ...
    def test_create_coupon_duplicate_code(
        self, coupon_service, sample_promotion, sample_coupon
    ):
        """Test creating a coupon with duplicate code fails"""
        coupon_data = {"code": sample_coupon.code, "max_uses": 5}  # Use existing code
    ...
    def test_create_bulk_coupons(self, coupon_service, sample_promotion, db_session):
        ...
        coupons = coupon_service.create_bulk_coupons(
            promotion_id=sample_promotion.id, count=10, coupon_config=coupon_config
        )
    ...
    def test_create_bulk_coupons_large_batch(self, coupon_service, sample_promotion):
        ...
        coupons = coupon_service.create_bulk_coupons(
            promotion_id=sample_promotion.id, count=100, coupon_config=coupon_config
        )
    ...
    def test_validate_coupon_code_valid(
        self, coupon_service, sample_coupon, sample_customer
    ):
    ...
    def test_validate_coupon_code_inactive(
        self, coupon_service, sample_coupon, sample_customer
    ):
    ...
    def test_validate_coupon_code_expired(
        self, coupon_service, sample_coupon, sample_customer
    ):
    ...
    def test_validate_coupon_code_max_uses_reached(
        self, coupon_service, sample_coupon, sample_customer
    ):
    ...
    def test_use_coupon(
        self, coupon_service, sample_coupon, sample_customer, sample_order
    ):
        """Test using a coupon"""
    ...
    def test_use_coupon_invalid_code(
        self, coupon_service, sample_customer, sample_order
    ):
    ...
    def test_get_coupons_by_promotion(
        self, coupon_service, sample_promotion, db_session
    ):
    ...
    def test_get_coupon_usage_statistics(
        self, coupon_service, sample_coupon, db_session
    ):
    ...
    def test_bulk_deactivate_coupons(
        self, coupon_service, db_session, sample_promotion
    ):
        """Test bulk deactivating coupons"""
        # Create multiple active coupons
        coupons = coupon_service.create_bulk_coupons(
            promotion_id=sample_promotion.id, count=5, coupon_config={"max_uses": 10}
        )
