...
class TestPromotionPerformance:
    ...
    def test_bulk_discount_calculation_performance(self, perf_db_session):
        ...
        print(
            f"Bulk discount calculation time: {calculation_time:.3f}s for {len(promotion_ids)} promotions"
        )
    ...
    def test_coupon_generation_performance(self, perf_db_session):
        ...
        for batch_size in batch_sizes:
            ...
            assert generation_time < (
                batch_size / 100
            )  # Should generate ~100 coupons per second minimum
            ...
            print(
                f"Generated {batch_size} coupons in {generation_time:.3f}s ({batch_size/generation_time:.1f} coupons/sec)"
            )
    ...
    def test_promotion_eligibility_check_performance(self, perf_db_session):
        ...
        print(
            f"Checked eligibility for {len(promotions)} promotions in {check_time:.3f}s"
        )
    ...
    def test_analytics_aggregation_performance(self, perf_db_session):
        ...
        for i in range(1000):  # 1000 usage records
            usage = PromotionUsage(
                ...
                created_at=datetime.utcnow()
                - timedelta(hours=i % 168),  # Spread over a week
            )
    ...
    def test_concurrent_coupon_validation_performance(self, perf_db_session):
        ...
        promotion = promotions[0]
        coupons = coupon_service.create_bulk_coupons(
            promotion_id=promotion.id, count=100, coupon_config={"max_uses": 10}
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            futures = [
                executor.submit(validate_coupon, coupon.code) for coupon in coupons
            ]
        ...
        assert (
            concurrent_time < 5.0
        )  # Should handle 100 concurrent validations within 5 seconds
    ...
    def test_database_query_optimization(self, perf_db_session):
        ...
        complex_query = (
            ...
            .order_by(Promotion.priority.desc(), Promotion.discount_value.desc())
            .limit(20)
        )
    ...
    def test_memory_usage_bulk_operations(self, perf_db_session):
        ...
        print(
            f"Memory usage for {large_batch_size} coupons: {memory_increase:.1f}MB increase"
        )
    ...
    async def test_async_operations_performance(self, perf_db_session):
        ...
        assert (
            async_time < 1.0
        )  # Should complete much faster than sequential (0.5s vs 50*0.01=0.5s)
        assert all(results)
