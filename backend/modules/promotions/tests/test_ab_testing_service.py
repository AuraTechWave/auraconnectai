...
class TestABTestingService:
    ...
    def test_create_ab_test(self, ab_testing_service, db_session):
        ...
        assert (
            result["control_promotion"].metadata["ab_test"]["variant_type"] == "control"
        )
        assert len(result["variant_promotions"]) == 2

        # Check traffic percentages
        assert (
            result["control_promotion"].metadata["ab_test"]["traffic_percentage"] == 40
        )
        assert (
            result["variant_promotions"][0].metadata["ab_test"]["traffic_percentage"]
            == 30
        )
        assert (
            result["variant_promotions"][1].metadata["ab_test"]["traffic_percentage"]
            == 30
        )
    ...
    def test_assign_user_to_variant(self, ab_testing_service, db_session):
        ...
        control_count = sum(
            1 for a in assignments if a["assigned_variant"] == "control"
        )
        variant_count = sum(
            1 for a in assignments if a["assigned_variant"] == "variant_1"
        )
    ...
    def test_assign_user_to_variant_inactive_test(self, ab_testing_service, db_session):
        ...
        with pytest.raises(ValueError, match="A/B test .* is not active"):
            ab_testing_service.assign_user_to_variant(test_id=test_id, customer_id=1)
    ...
    def test_stop_ab_test(self, ab_testing_service, db_session):
        ...
        stop_result = ab_testing_service.stop_ab_test(
            test_id, winning_variant="variant_1"
        )
