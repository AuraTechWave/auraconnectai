...
class TestDiscountCalculationService:
    ...
    def test_calculate_percentage_discount(
        self, discount_service, db_session, promotion_factory
    ):
    ...
    def test_calculate_fixed_amount_discount(
        ...
        promotion = promotion_factory.create_fixed_amount_promotion(
            db_session, discount_value=25.0, status=PromotionStatus.ACTIVE  # $25 off
        )
    ...
    def test_calculate_fixed_amount_discount_exceeds_total(
        self, discount_service, db_session, promotion_factory
    ):
        """Test fixed amount discount when it exceeds order total"""
        promotion = promotion_factory.create_fixed_amount_promotion(
            db_session, discount_value=50.0, status=PromotionStatus.ACTIVE  # $50 off
        )
    ...
    def test_calculate_bogo_discount(
        self, discount_service, db_session, promotion_factory
    ):
    ...
    def test_calculate_discount_minimum_order_not_met(
        self, discount_service, db_session, promotion_factory
    ):
    ...
    def test_calculate_discount_minimum_order_met(
        self, discount_service, db_session, promotion_factory
    ):
    ...
    def test_calculate_discount_inactive_promotion(
        self, discount_service, db_session, promotion_factory
    ):
        """Test discount calculation for inactive promotion"""
        promotion = promotion_factory.create_percentage_promotion(
            db_session, discount_value=20.0, status=PromotionStatus.DRAFT  # Not active
        )
    ...
    def test_calculate_discount_nonexistent_promotion(self, discount_service):
        ...
        with pytest.raises(ValueError, match="Promotion .* not found"):
            discount_service.calculate_discount(99999, order_items, customer_id=1)
    ...
    def test_apply_discount_to_order(
        ...
        promotion = promotion_factory.create_percentage_promotion(
            db_session, discount_value=10.0, status=PromotionStatus.ACTIVE
        )
    ...
    def test_calculate_multiple_promotions(
        self, discount_service, db_session, promotion_factory
    ):
    ...
    def test_calculate_multiple_promotions_with_stacking_rules(
        self, discount_service, db_session, promotion_factory
    ):
    ...
    def test_get_best_promotion_for_order(
        self, discount_service, db_session, promotion_factory
    ):
    ...
    def test_validate_discount_constraints(
        self, discount_service, db_session, promotion_factory
    ):
    ...
    def test_calculate_tiered_discount(self, discount_service, db_session):
        ...
        for order_amount, expected_discount in test_cases:
            ...
            ), f"Order amount {order_amount}: expected {expected_discount}, got {discount_amount}"
