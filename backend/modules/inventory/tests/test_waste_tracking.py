...
class TestWasteTracking:
    ...
    def test_create_valid_waste_event(
        self, client: TestClient, sample_inventory_item: Inventory, auth_headers: dict
    ):
    ...
    def test_create_waste_event_with_other_reason(
        self, client: TestClient, sample_inventory_item: Inventory, auth_headers: dict
    ):
        ...
        assert (
            data["custom_reason"]
            == "Power outage caused temperature excursion beyond safe limits"
        )

    def test_create_waste_event_invalid_quantity(
        self, client: TestClient, sample_inventory_item: Inventory, auth_headers: dict
    ):
    ...
    def test_create_waste_event_exceeds_available_quantity(
        self, client: TestClient, sample_inventory_item: Inventory, auth_headers: dict
    ):
        ...
        assert (
            "Cannot waste 150.0 lbs. Only 100.0 lbs available"
            in response.json()["detail"]
        )

    def test_create_waste_event_nonexistent_item(
        ...
        response = client.post(
            "/api/v1/inventory/99999/waste", json=waste_data, headers=auth_headers
        )

        assert response.status_code == 404
        assert "Inventory item with ID 99999 not found" in response.json()["detail"]

    def test_create_waste_event_mismatched_inventory_id(
        self, client: TestClient, sample_inventory_item: Inventory, auth_headers: dict
    ):
        ...
        assert response.status_code == 400
        assert (
            "Inventory ID in request body doesn't match path parameter"
            in response.json()["detail"]
        )

    def test_create_waste_event_other_reason_missing_custom(
        self, client: TestClient, sample_inventory_item: Inventory, auth_headers: dict
    ):
        ...
        assert response.status_code == 422
        assert "Custom reason is required when waste_reason is OTHER" in str(
            response.json()["detail"]
        )

    def test_get_waste_history(
        self,
        client: TestClient,
        sample_inventory_item: Inventory,
        auth_headers: dict,
        db: Session,
    ):
        ...
        for i in range(3):
            waste_data = {
                ...
                "quantity": 2.0,
                "waste_reason": (
                    WasteReason.EXPIRED.value
                    if i % 2 == 0
                    else WasteReason.DAMAGED.value
                ),
        ...
        # Get waste history
        response = client.get(
            f"/api/v1/inventory/{sample_inventory_item.id}/waste", headers=auth_headers
        )
    ...
    def test_get_waste_history_with_filters(
        self, client: TestClient, sample_inventory_item: Inventory, auth_headers: dict
    ):
    ...
    def test_get_waste_history_nonexistent_item(
        self, client: TestClient, auth_headers: dict
    ):
        """Test 404 error when getting history for non-existent item"""
        response = client.get("/api/v1/inventory/99999/waste", headers=auth_headers)

        assert response.status_code == 404
        assert "Inventory item with ID 99999 not found" in response.json()["detail"]

    def test_waste_event_updates_inventory_quantity(
        self,
        client: TestClient,
        sample_inventory_item: Inventory,
        auth_headers: dict,
    ...
    def test_waste_event_creates_audit_trail(
        self,
        client: TestClient,
        sample_inventory_item: Inventory,
        auth_headers: dict,
    ...
    def test_concurrent_waste_events(
        self, client: TestClient, sample_inventory_item: Inventory, auth_headers: dict
    ):
    ...
    def test_waste_event_pagination(
        self, client: TestClient, sample_inventory_item: Inventory, auth_headers: dict
    ):
