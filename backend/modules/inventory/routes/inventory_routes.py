...
async def get_inventory_items(
    ...
    low_stock_only: Optional[bool] = Query(
        None, description="Show only low stock items"
    ),
    active_only: Optional[bool] = Query(True, description="Show only active items"),
    limit: int = Query(50, ge=1, le=500, description="Items per page"),
    offset: int = Query(0, ge=0, description="Items to skip"),
    sort_by: str = Query(
        "item_name", pattern=r"^(item_name|quantity|threshold|category|created_at)$"
    ),
...
async def update_inventory_item(
    ...
    return inventory_service.update_inventory_item(
        inventory_id, item_data.dict(exclude_unset=True), current_user.id
    )
...
async def get_inventory_adjustments(
    ...
    adjustments = (
        inventory_service.db.query(
            inventory_service.db.query(InventoryAdjustment.__class__)
        )
        .filter(
    ...
    for adjustment_data in bulk_request.adjustments:
        try:
            adjustment = inventory_service.adjust_inventory_quantity(
                ...
                reference_id=adjustment_data.reference_id,
                notes=(
                    f"{bulk_request.notes}\n{adjustment_data.notes}"
                    if bulk_request.notes
                    else adjustment_data.notes
                ),
            )
...
async def get_waste_history(
    ...
    start_date: Optional[date] = Query(
        None, description="Start date for waste history"
    ),
    end_date: Optional[date] = Query(None, description="End date for waste history"),
    waste_reason: Optional[WasteReason] = Query(
        None, description="Filter by waste reason"
    ),
    limit: int = Query(50, ge=1, le=500),
    offset: int = Query(0, ge=0),
    inventory_service: InventoryService = Depends(get_inventory_service),
...
async def get_inventory_alerts(
    ...
    total = query.count()
    alerts = (
        query.order_by(InventoryAlert.priority.desc(), InventoryAlert.created_at.asc())
        .offset(offset)
        .limit(limit)
        .all()
    )
    ...
    return AlertResponse(items=alerts, total=total, page=page, size=limit, pages=pages)
...
async def get_inventory_usage(
    ...
    return (
        query.order_by(InventoryUsageLog.order_date.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
    ...
    for inventory_id in bulk_data.inventory_ids:
        try:
            item = inventory_service.update_inventory_item(
                inventory_id, update_dict, current_user.id
            )
...
async def get_waste_summary_report(
    ...
    # Sort by cost
    sorted_items = sorted(
        waste_summary.values(), key=lambda x: x["total_cost"], reverse=True
    )
    ...
    return {
        ...
        "summary": {
            ...
            "total_waste_incidents": sum(
                item["waste_count"] for item in waste_summary.values()
            ),
        },
        "by_item": sorted_items[:20],  # Top 20 items by cost
        "by_reason": list(reason_summary.values()),
        "recommendations": _generate_waste_recommendations(
            sorted_items, reason_summary
        ),
    }
...
def _generate_waste_recommendations(items_summary: list, reasons_summary: dict) -> list:
    ...
    if (
        "expired" in reasons_summary
        and reasons_summary["expired"]["total_incidents"] > 10
    ):
        recommendations.append(
            {
                "priority": "high",
    ...
    for log in usage_logs:
        if log.inventory_id not in usage_summary:
            usage_summary[log.inventory_id] = {
                "inventory_item": (
                    log.inventory_item.item_name
                    if hasattr(log, "inventory_item")
                    else f"Item {log.inventory_id}"
                ),
                "total_usage": 0,
                "total_cost": 0,
                "usage_count": 0,
...
async def get_recipe_cost_analysis(
    ...
    try:
        ...
        logger.error(
            f"Error calculating cost for recipe {recipe_id}: {exc}", exc_info=True
        )
