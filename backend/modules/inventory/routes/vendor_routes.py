...
async def get_vendors(
    ...
    if search:
        ...
        vendors = [v for v in vendors if search_term in v.name.lower() or 
                  (v.contact_person and search_term in v.contact_person.lower())]
...
@router.post("/{vendor_id}/purchase-orders", response_model=PurchaseOrder, status_code=status.HTTP_201_CREATED)
...
@router.get("/{vendor_id}/purchase-orders/{po_id}", response_model=PurchaseOrderWithItems)
...
@router.post("/{vendor_id}/purchase-orders/{po_id}/items", response_model=PurchaseOrderItem)
...
@router.post("/{vendor_id}/purchase-orders/{po_id}/receive", response_model=PurchaseOrder)
async def receive_purchase_order(
    ...
    for received_item in received_items:
        po_item = inventory_service.db.query(PurchaseOrderItem).filter(
        ...
        if po_item:
            ...
            po_item.status = "received" if po_item.quantity_received >= po_item.quantity_ordered else "partial"
    ...
    # Update PO status
    all_items = inventory_service.db.query(PurchaseOrderItem).filter(
        PurchaseOrderItem.purchase_order_id == po_id
    ).all()
...
async def get_vendor_analytics(
    ...
    total_amount = po_query.with_entities(func.sum(PurchaseOrder.total_amount)).scalar() or 0
    
    # On-time delivery rate
    completed_orders = po_query.filter(PurchaseOrder.status == "received").all()
    on_time_deliveries = len([
        po for po in completed_orders 
        if po.actual_delivery_date and po.expected_delivery_date and 
    ...
    # Item performance
    items_supplied = inventory_service.db.query(func.count(func.distinct(
        inventory_service.db.query(PurchaseOrderItem.inventory_id).join(PurchaseOrder).filter(
            PurchaseOrder.vendor_id == vendor_id
...
async def rate_vendor(
    ...
    if notes:
        vendor.notes = f"{vendor.notes}\n\nRating Update: {rating}/5 - {notes}" if vendor.notes else f"Rating: {rating}/5 - {notes}"
