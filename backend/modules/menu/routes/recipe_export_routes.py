...
router = APIRouter(
    prefix="/api/v1/menu/recipes/export",
    tags=["recipe-export"]
)
...
async def export_compliance_report(
    ...
    max_items: int = Query(5000, ge=100, le=10000, description="Maximum items to export"),
    ...
    **Access Control:** Requires admin or manager role.
    ...
    if current_user.get("role") not in ["admin", "manager"]:
        raise HTTPException(
            status_code=403,
            detail="Insufficient permissions to export compliance data"
        )
...
async def export_recipe_details(
    ...
    **Access Control:** Requires authentication.
    ...
    if len(recipe_ids) > 100:
        raise HTTPException(
            status_code=400,
            detail="Maximum 100 recipes can be exported at once"
        )
...
async def get_export_summary(
    ...
    items_with_recipes = db.query(MenuItem).join(Recipe).filter(
        MenuItem.is_active == True,
        MenuItem.deleted_at.is_(None),
    ...
    # Get categories
    categories = db.query(MenuItem.category, func.count(MenuItem.id)).filter(
        MenuItem.is_active == True,
        MenuItem.deleted_at.is_(None)
    ...
    return {
        "summary": {
            ...
            "compliance_percentage": (items_with_recipes / total_menu_items * 100) if total_menu_items > 0 else 0
        },
        "categories": [
        ...
        "export_info": {
            ...
            "estimated_json_size_mb": round(total_menu_items * 0.005, 2),  # ~5KB per item
            "estimated_csv_size_mb": round(total_menu_items * 0.002, 2)   # ~2KB per item
        }
    }