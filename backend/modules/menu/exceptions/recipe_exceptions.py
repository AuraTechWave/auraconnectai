...
class RecipeComplianceError(RecipeException):
    ...
    def __init__(
        ...
        details = {"compliance_level": compliance_level or "error"}
        if menu_items:
    ...
    def __init__(
        ...
        super().__init__(
            status_code=(
                ...
                if size and max_size and size > max_size
...
RECIPE_VALIDATION_RULES = {
    ...
    "RVR004": ValidationRule(
        ...
        lambda recipe: all(
            ing.get("quantity", 0) > 0 for ing in recipe.get("ingredients", [])
        ),
    ),
...
def validate_recipe_data(
    recipe_data: Dict[str, Any], context: Dict[str, Any] = None
) -> List[RecipeValidationError]:
    """
