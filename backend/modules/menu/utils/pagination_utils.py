...
class CursorPaginator:
    ...
    def paginate(
        ...
        query = query.order_by(
            desc(order_field) if cursor_info and cursor_info.direction == "prev" else asc(order_field),
            asc(unique_field)
        )
    ...
    def paginate(
        ...
        if offset >= self.offset_threshold:
            ...
            if offset_item:
                ...
                return self.cursor_paginator.paginate(
                    cursor_info.encode(), page_size
                )
