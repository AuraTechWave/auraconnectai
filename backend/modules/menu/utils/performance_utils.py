...
def _execute_with_timing(
    ...
    **kwargs,
    ...
    endpoint_thresholds = PerformanceThresholds.ENDPOINTS.get(func.__name__, {})
    ...
    try:
        ...
        if elapsed_ms >= error_ms:
            ...
            logger.debug(
                f"Operation completed: {op_name} took {elapsed_ms:.2f}ms", extra=context
            )
...
class ParallelExecutor:
    ...
    def parallel_map(
        self, func: Callable[[T], Any], items: List[T], timeout: Optional[float] = None
        ...
        # Submit all tasks
        future_to_index = {
            self._executor.submit(func, item): i for i, item in enumerate(items)
        }
    ...
    def parallel_batch_process(
        ...
        # Submit batch tasks
        future_to_batch = {
            self._executor.submit(func, batch): batch for batch in batches
        }
        ...
        for future in as_completed(future_to_batch):
            try:
                ...
                logger.error(f"Batch processing failed for {len(batch)} items: {e}")
                results.append(e)
