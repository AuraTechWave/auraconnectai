...
def bulk_recalculate_costs_async(
    ...
    try:
            ...
            for recipe in batch:
                try:
                    ...
                    self.cache_service.set(
                        "cost_analysis", cost_analysis.dict(), recipe.id
                    )
                    ...
                    results["failed"] += 1
                    results["errors"].append({"recipe_id": recipe.id, "error": str(e)})
        ...
        results["errors"].append({"error": f"Bulk processing error: {str(e)}"})
    ...
    if not recipe_ids:  # Only cache if processing all recipes
        self.cache_service.set(
            "bulk_cost_calculation", results, "all", ttl=1800  # 30 minutes
        )
...
def schedule_cost_recalculation(
    recipe_ids: Optional[List[int]] = None, delay_seconds: int = 0, priority: int = 5
) -> str:
    """
    ...
    task = bulk_recalculate_costs_async.apply_async(
        args=[recipe_ids], countdown=delay_seconds, priority=priority
    )
