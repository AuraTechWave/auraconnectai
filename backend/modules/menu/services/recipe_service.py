...
from ..schemas.recipe_schemas import (
    ...
    RecipeSubRecipeCreate, RecipeSearchParams,
    RecipeCostAnalysis, RecipeValidation,
...
class RecipeService:
    ...
    def create_recipe(self, recipe_data: RecipeCreate, user_id: int) -> Recipe:
        ...
        menu_item = self.db.query(MenuItem).filter(
            MenuItem.id == recipe_data.menu_item_id
        ).first()
    ...
    def update_recipe(self, recipe_id: int, recipe_data: RecipeUpdate, user_id: int) -> Recipe:
        ...
        if changes:
            ...
            self._create_history_entry(
                recipe, 
                "updated", 
                f"Updated fields: {', '.join(changes[:5])}" + ("..." if len(changes) > 5 else ""),
                user_id
            )
    ...
    def calculate_recipe_cost(self, recipe_id: int) -> RecipeCostAnalysis:
        ...
        if food_cost_percentage > 35:
            suggestions.append("Food cost percentage is high. Consider reviewing portion sizes or ingredient choices.")
        
        # Find most expensive ingredients
        expensive_ingredients = sorted(ingredient_costs, key=lambda x: x['total_cost'], reverse=True)[:3]
    ...
    def validate_recipe(self, recipe_id: int) -> RecipeValidation:
        ...
        for ingredient in recipe.ingredients:
            if not ingredient.inventory_item.is_active:
                ...
                errors.append(f"Ingredient '{ingredient.inventory_item.item_name}' is not active")
            elif ingredient.inventory_item.quantity <= 0:
                warnings.append(f"Ingredient '{ingredient.inventory_item.item_name}' is out of stock")
        ...
        if recipe.food_cost_percentage:
            if recipe.food_cost_percentage > 35:
                ...
                warnings.append(f"Food cost percentage ({recipe.food_cost_percentage:.1f}%) exceeds target of 35%")
    ...
    def get_recipe_compliance_report(self) -> RecipeComplianceReport:
        ...
        compliance_percentage = (items_with_recipes / total_items * 100) if total_items > 0 else 0
        
        # Calculate percentages by category
        for category in compliance_by_category.values():
            if category["total"] > 0:
                category["compliance_percentage"] = (category["with_recipes"] / category["total"]) * 100
            else:
    ...
    def clone_recipe(self, clone_request: RecipeCloneRequest, user_id: int) -> Recipe:
        ...
        target_item = self.db.query(MenuItem).filter(
            MenuItem.id == clone_request.target_menu_item_id
        ).first()
        ...
        for ingredient in source_recipe.ingredients:
            new_ingredient = RecipeIngredient(
                ...
                total_cost=ingredient.total_cost * adjust_factor if ingredient.total_cost else None,
                created_by=user_id
            )
    ...
    def _create_history_entry(
        ...
        for ing in recipe.ingredients:
            ingredients_snapshot.append({
                ...
                "inventory_name": ing.inventory_item.item_name if ing.inventory_item else None,
