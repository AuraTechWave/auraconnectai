...
class OptimizedRecipeService(RecipeService):
    ...
    @timing_logger(
        "calculate_recipe_cost", warning_threshold_ms=300, error_threshold_ms=1000
    )
    def calculate_recipe_cost(
        ...
        for sub_recipe_link in recipe.sub_recipes:
            ...
            sub_cost_analysis = self.calculate_recipe_cost(
                sub_recipe_link.sub_recipe_id, use_cache=True
            )
    ...
    @timing_logger(
        "compliance_report", warning_threshold_ms=500, error_threshold_ms=2000
    )
    def get_recipe_compliance_report(
        ...
        if use_cache:
            cached_result = self.cache_service.get(
                "compliance_report", cache_key_suffix
            )
            if cached_result:
        ...
        menu_items = (
            query.offset(offset)
            .limit(page_size)
            .options(joinedload(MenuItem.recipe))
            .all()
        )
        ...
        compliance_percentage = (
            (items_with_recipes / len(menu_items) * 100) if menu_items else 0
        )
    ...
    @timing_logger(
        "bulk_cost_recalculation", warning_threshold_ms=5000, error_threshold_ms=30000
    )
    async def recalculate_all_recipe_costs_optimized(
        self, user_id: int, batch_size: int = 100, use_background_task: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if use_background_task:
            ...
            task = bulk_recalculate_costs_async.delay(
                recipe_ids=None, user_id=user_id, batch_size=batch_size
            )
            ...
            with ParallelExecutor(max_workers=4, chunk_size=batch_size) as executor:
                ...
                def calculate_cost_for_recipe(recipe):
                    try:
                        ...
                        logger.error(
                            f"Error calculating cost for recipe {recipe.id}: {str(e)}"
                        )
                        return False
    ...
    def warm_cache(self, recipe_ids: Optional[List[int]] = None):
        ...
        with ParallelExecutor(max_workers=4, chunk_size=25) as executor:
            ...
            results = executor.parallel_map(
                warm_single_recipe, recipe_ids, timeout=60  # 1 minute timeout
            )
