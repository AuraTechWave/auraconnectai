...
class RecipeExportService:
    ...
    def __init__(self, db: Session, recipe_service: Optional[OptimizedRecipeService] = None):
        self.db = db
    ...
    def export_compliance_report(
        ...
        query = self.db.query(MenuItem).options(
            joinedload(MenuItem.recipe).joinedload(Recipe.ingredients).joinedload(RecipeIngredient.inventory_item)
        ).filter(
            ...
            def process_menu_item(item):
                ...
                if item.recipe:
                    ...
                    item_data.update({
                        ...
                        "recipe_created_at": recipe.created_at.isoformat() if recipe.created_at else None,
                        "recipe_updated_at": recipe.updated_at.isoformat() if recipe.updated_at else None
                    })
                    ...
                    for ing in recipe.ingredients:
                        ingredients.append({
                            "ingredient_name": ing.inventory_item.name if ing.inventory_item else "Unknown",
                    ...
                    if include_costs:
                        try:
                            cost_analysis = self.recipe_service.calculate_recipe_cost(recipe.id, use_cache=True)
                            item_data.update({
                            ...
                            logger.error(f"Cost calculation failed for recipe {recipe.id}: {e}")
        ...
        export = {
            ...
            "summary": {
                "compliance": {
                    ...
                    "draft_recipes": len(draft_recipes),
                    "inactive_recipes": len(inactive_recipes),
                    "compliance_percentage": ((len(data) - len(missing_recipes)) / len(data) * 100) if data else 0
                },
                "cost_analysis": {
        ...
        for item in data:
            ...
            if "ingredients" in item and item["ingredients"]:
                ...
                if len(item["ingredients"]) > 5:
                    ingredients_summary += f"... and {len(item['ingredients']) - 5} more"
                ...
                "ingredients": [
                    {
                        ...
                        } if ing.inventory_item else None,
                        "quantity": ing.quantity,
                        "unit": ing.unit,
                        "notes": ing.notes,
            ...
            if recipe_data:
                ...
                for recipe in recipe_data:
                    csv_data.append({
                        "recipe_id": recipe["id"],
                        "recipe_name": recipe["name"],
                        "menu_item": recipe["menu_item"]["name"] if recipe["menu_item"] else None,
                        "status": recipe["status"],
                        "ingredient_count": len(recipe["ingredients"]),
                        "total_cost": recipe.get("cost_analysis", {}).get("total_cost"),
                        "cost_per_serving": recipe.get("cost_analysis", {}).get("cost_per_serving")
                    })
