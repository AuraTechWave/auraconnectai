...
def update_recipe_sub_recipes(
    ...
    existing_sub_recipes = (
        ...
        .filter(RecipeSubRecipe.parent_recipe_id == recipe_id)
        .all()
    )
    ...
    for sub_data in sub_recipes:
        # Get the sub-recipe
        sub_recipe = (
            db.query(Recipe).filter(Recipe.id == sub_data.sub_recipe_id).first()
        )

        # Create the link
        sub_recipe_link = RecipeSubRecipe(
            parent_recipe_id=recipe_id, created_by=user_id, **sub_data.dict()
        )
    ...
    # Get ingredient cost
    ingredient_cost = sum(
        ing.total_cost or 0 for ing in recipe.ingredients if ing.is_active
    )
    ...
    if recipe_service_instance and hasattr(
        ...
        change_summary = (
            "; ".join(change_parts) if change_parts else "Sub-recipes updated"
        )

        recipe_service_instance._create_history_entry(
            recipe, "sub_recipes_updated", change_summary, user_id
        )
...
def add_single_sub_recipe(
    db: Session, recipe_id: int, sub_recipe_data: RecipeSubRecipeCreate, user_id: int
) -> RecipeSubRecipe:
    """
    ...
    if not parent_recipe:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Parent recipe not found"
        )

    # Check sub-recipe exists
    sub_recipe = (
        db.query(Recipe)
        .filter(Recipe.id == sub_recipe_data.sub_recipe_id, Recipe.deleted_at.is_(None))
        .first()
    )
    ...
    try:
        validator.validate_no_circular_reference(
            recipe_id, sub_recipe_data.sub_recipe_id
        )
    except CircularDependencyError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    ...
    # Create the link
    sub_recipe_link = RecipeSubRecipe(
        parent_recipe_id=recipe_id, created_by=user_id, **sub_recipe_data.dict()
    )
    db.add(sub_recipe_link)
...
def remove_sub_recipe(
    db: Session, recipe_id: int, sub_recipe_id: int, user_id: int
) -> None:
    """
    ...
    if not link:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Sub-recipe link not found"
        )
