    ...
    def delete_pattern(self, pattern: str) -> int:
        ...
        try:
            if self.redis:
                ...
                cursor = 0
                while True:
                    cursor, keys = self.redis.scan(cursor, match=f"recipe_cache:{pattern}*", count=100)
                    if keys:
                ...
                keys_to_delete = [k for k in self._local_cache.keys() if k.startswith(f"recipe_cache:{pattern}")]
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            if self.redis:
                ...
                stats["hit_rate"] = info.get("keyspace_hits", 0) / max(1, info.get("keyspace_hits", 0) + info.get("keyspace_misses", 0))
            else:
                stats["entries"] = len(self._local_cache)
                # Estimate memory usage for local cache
                stats["memory_usage"] = sum(len(str(k)) + len(str(v)) for k, (v, _) in self._local_cache.items())
        except Exception as e:
...
def get_recipe_cache_service(redis_client: Optional[Redis] = None) -> RecipeCacheService:
