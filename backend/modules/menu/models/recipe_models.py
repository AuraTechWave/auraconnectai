...
from sqlalchemy import (Column, Integer, String, ForeignKey, DateTime,
                        Float, Text, Boolean, JSON, Enum as SQLEnum, UniqueConstraint)
...
class Recipe(Base, TimestampMixin):
    ...
    complexity = Column(SQLEnum(RecipeComplexity), nullable=False, default=RecipeComplexity.SIMPLE)
    ...
    quality_standards = Column(JSON, nullable=True)  # Temperature, texture, presentation standards
    ...
    sub_recipes = relationship("RecipeSubRecipe", foreign_keys="RecipeSubRecipe.parent_recipe_id", back_populates="parent_recipe")
    parent_recipes = relationship("RecipeSubRecipe", foreign_keys="RecipeSubRecipe.sub_recipe_id", back_populates="sub_recipe")
    history = relationship("RecipeHistory", back_populates="recipe", order_by="RecipeHistory.created_at.desc()")
...
class RecipeSubRecipe(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint('parent_recipe_id', 'sub_recipe_id', name='_parent_sub_recipe_uc'),
    )
    ...
    # Relationships
    parent_recipe = relationship("Recipe", foreign_keys=[parent_recipe_id], back_populates="sub_recipes")
    sub_recipe = relationship("Recipe", foreign_keys=[sub_recipe_id], back_populates="parent_recipes")
...
class RecipeHistory(Base, TimestampMixin):
    ...
    # What changed
    change_type = Column(String(50), nullable=False)  # created, updated, ingredients_changed, etc.
    change_summary = Column(Text, nullable=False)
    
    # Snapshot of recipe data
    recipe_snapshot = Column(JSON, nullable=False)  # Complete recipe data at this version
...
class RecipeNutrition(Base, TimestampMixin):
    ...
    # Calculation method
    calculation_method = Column(String(50), nullable=True)  # manual, calculated, estimated
