...
class TestRecipeCircularDependencies:
    ...
    def test_hierarchy_validation(self, db: Session, setup_test_data):
        ...
        level2_recipe = service.create_recipe(
            RecipeCreate(
                ...
                sub_recipes=[{
                    'sub_recipe_id': level1_recipe.id,
                    'quantity': 1.5
                }]
            ),
            data['staff'].id
        ...
        dish = service.create_recipe(
            RecipeCreate(
                ...
                sub_recipes=[
                    ...
                    {'sub_recipe_id': sauce.id, 'quantity': 0.2}  # Also uses sauce directly
                ]
            ),
            data['staff'].id
        )
        ...
        with pytest.raises(HTTPException) as exc_info:
            service.update_recipe_sub_recipes(
                ...
                [
                    ...
                    RecipeSubRecipeCreate(sub_recipe_id=recipes[1].id, quantity=2.0)  # Duplicate
                ],
                data['staff'].id
            )
    ...
    def test_cost_recalculation_with_circular_prevention(self, db: Session, setup_test_data):
